...
export class ServerManager {
  ...
  constructor(configPresenter: IConfigPresenter, ports: McpHostPorts) {
  ...
  async testNpmRegistrySpeed(useCache: boolean = true): Promise<string> {
    ...
    const proxyUrl = this.ports.proxy.getProxyUrl();
    const proxyOptions = (() => {
      if (!proxyUrl) return {};
    ...
    const successfulResults = results.filter((result) => result.success).sort((a, b) => a.time - b.time);
  ...
  async startServer(name: string): Promise<void> {
    ...
    try {
      ...
      this.ports.events.broadcast(MCP_EVENTS.CLIENT_LIST_UPDATED, undefined);
    }
  }
