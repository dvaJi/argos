...
export class ToolManager {
  ...
  constructor(configPresenter: IConfigPresenter, serverManager: ServerManager, ports: McpHostPorts) {
    this.configPresenter = configPresenter;
    this.serverManager = serverManager;
    this.ports = ports;
    this.unsubClientList = ports.events.subscribe(MCP_EVENTS.CLIENT_LIST_UPDATED, this.handleServerListUpdate);
    this.unsubConfig = ports.events.subscribe(MCP_EVENTS.CONFIG_CHANGED, this.handleConfigChange);
  }
    ...
    for (const client of clients) {
      try {
        ...
        if (!this.isPluginOwnedClient(client)) {
          ...
          this.ports.events.broadcastError(NOTIFICATION_EVENTS.SHOW_ERROR, {
  ...
  private async resolveAcpSessionContext(conversationId?: string): Promise<{
    ...
    try {
      const session = (await this.ports.services.getSession?.(sessionId)) as {
        agentId?: string;
        providerId?: string;
        projectDir?: string;
      } | null | undefined;
  ...
  private checkToolPermission(
    ...
    const pluginPolicy = this.ports.services.getPluginToolPolicy?.(serverName, originalToolName) as
      | string
      | undefined;
  ...
  async preCheckToolPermission(
    ...
    const pluginPolicy = this.ports.services.getPluginToolPolicy?.(toolServerName, originalName) as
      | string
      | undefined;
  ...
  async callTool(
    ...
    try {
      ...
      // Trigger event
      this.ports.events.broadcast(MCP_EVENTS.TOOL_CALL_RESULT, response);
