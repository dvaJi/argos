...
// Schema definitions
const RagflowKnowledgeSearchArgsSchema = zod.object({
  ...
  scoreThreshold: zod.number().optional().default(0.2).describe("Similarity threshold (0-1, default 0.2)"),
  keyword: zod.boolean().optional().default(false).describe("Whether to enable keyword matching (default false)"),
  highlight: zod.boolean().optional().default(false).describe("Whether to highlight matched text (default false)"),
});
      ...
      const tools = this.configs
        ...
        .map((config, index) => {
          ...
          return {
            ...
            inputSchema: zod.toJSONSchema(RagflowKnowledgeSearchArgsSchema, { unrepresentable: "any" }),
            annotations: {
