...
const StartDeepResearchArgsSchema = zod.object({
  question: zod.string().describe("The research question or topic to start the deep research with."),
});

// SingleWebSearchArgsSchema: Parameters for a single web search.
const SingleWebSearchArgsSchema = zod.object({
  session_id: zod.string().describe("The research session ID returned by start_deep_research."),
  query: zod.string().describe("The single search query to execute."),
  max_results: zod
    .number()
    .min(5)
    .max(15)
...
const RequestResearchDataArgsSchema = zod.object({
  session_id: zod.string().describe("The research session ID."),
  iteration: zod
    .number()
...
const SubmitReflectionResultsArgsSchema = zod.object({
  session_id: zod.string().describe("The research session ID."),
  iteration: zod.number().describe("The iteration count for this reflection."),
  needs_more_research: zod.boolean().describe("Whether the LLM believes more research is needed after analysis."),
  missing_information: zod
    .array(zod.string())
    .describe("List of missing information identified by the LLM, if more research is needed."),
  quality_assessment: zod.string().describe("The LLM's quality assessment of the current research results."),
  suggested_queries: zod
    .array(zod.string())
...
const GenerateFinalAnswerArgsSchema = zod.object({
  session_id: zod.string().describe("The research session ID returned by start_deep_research."),
  documentation_prompt: zod.string().optional().describe("Custom documentation generation prompt."),
});
        ...
        tools: [
          {
            ...
            inputSchema: zod.toJSONSchema(StartDeepResearchArgsSchema, { unrepresentable: "any" }),
            annotations: {
          ...
          },
          {
            ...
            inputSchema: zod.toJSONSchema(SingleWebSearchArgsSchema, { unrepresentable: "any" }),
            annotations: {
          ...
          },
          {
            ...
            inputSchema: zod.toJSONSchema(RequestResearchDataArgsSchema, { unrepresentable: "any" }),
            annotations: {
          ...
          },
          {
            ...
            inputSchema: zod.toJSONSchema(SubmitReflectionResultsArgsSchema, { unrepresentable: "any" }),
            annotations: {
          ...
          },
          {
            ...
            inputSchema: zod.toJSONSchema(GenerateFinalAnswerArgsSchema, { unrepresentable: "any" }),
            annotations: {
...
`,
  ...
  private async handleGenerateFinalAnswer(args: unknown) {
    ...
    const locale = this.deps?.getLanguage?.() || "zh-CN"; // Get user language setting
