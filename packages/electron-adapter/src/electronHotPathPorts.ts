...
export function createElectronHotPathPorts(deps: {
  ...
  return {
    ...
    windowEventPort: {
      publish: <T extends DeepchatEventName>(name: T, payload: DeepchatEventPayload<T>) => {
        ...
        deps.eventBusInstance.sendToRenderer(DEEPCHAT_EVENT_CHANNEL, SendTarget.ALL_WINDOWS, envelope);
      },
    },
  };
}
