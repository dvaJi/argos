...
export const TELEGRAM_REMOTE_DEFAULT_AGENT_ID = "argos";
...
export const buildModelMenuChoiceCallbackData = (token: string, providerIndex: number, modelIndex: number): string =>
  `${TELEGRAM_MODEL_MENU_CALLBACK_PREFIX}:${token}:m:${providerIndex}:${modelIndex}`;
...
export const parseModelMenuCallbackData = (data: string): TelegramModelMenuCallback | null => {
  ...
  if (action === "m" && parts[3] !== undefined && parts[4] !== undefined) {
    ...
    if (Number.isInteger(providerIndex) && providerIndex >= 0 && Number.isInteger(modelIndex) && modelIndex >= 0) {
      return {
        action: "model",
        token,
        providerIndex,
...
export const buildPendingInteractionOptionCallbackData = (token: string, optionIndex: number): string =>
  `${TELEGRAM_INTERACTION_CALLBACK_PREFIX}:${token}:o:${optionIndex}`;

export const parsePendingInteractionCallbackData = (data: string): TelegramPendingInteractionCallback | null => {
  const parts = data.trim().split(":");
...
const hasAnyOwn = (value: Record<string, unknown>, keys: string[]): boolean => keys.some((key) => hasOwn(value, key));
...
const extractLegacyDiscordConfig = (input: unknown): LegacyDiscordRemoteConfig | null => {
  ...
  const record = input as Record<string, unknown>;
  if (!hasAnyOwn(record, ["botToken", "pairedChannelIds", "lastFatalError"]) && !hasBindingPrefix(record, "discord:")) {
    return null;
  }
...
const normalizeStringList = (input: Array<string | number> | undefined): string[] =>
  Array.from(new Set((input ?? []).map((value) => String(value ?? "").trim()).filter(Boolean))).sort((left, right) =>
  ...
  return {
    telegram: {
      ...
      pairing: {
        ...
        expiresAt: typeof telegram.pairing?.expiresAt === "number" ? telegram.pairing.expiresAt : null,
        failedAttempts:
          typeof telegram.pairing?.failedAttempts === "number" && telegram.pairing.failedAttempts >= 0
    ...
    discord: {
      ...
      pairing: {
        ...
        expiresAt: typeof discord.pairing?.expiresAt === "number" ? discord.pairing.expiresAt : null,
        failedAttempts:
...
export const buildTelegramBindingMeta = (chatId: number, messageThreadId: number): RemoteEndpointBindingMeta => {
...
export const deriveTelegramBindingMeta = (endpointKey: string): RemoteEndpointBindingMeta | null => {
...
export const parseWeixinIlinkEndpointKey = (endpointKey: string): { accountId: string; userId: string } | null => {
...
export const buildWeixinIlinkBindingMeta = (params: { userId: string }): RemoteEndpointBindingMeta => ({
...
export const deriveWeixinIlinkBindingMeta = (endpointKey: string): RemoteEndpointBindingMeta | null => {
...
export const normalizeTelegramSettingsInput = (input: TelegramRemoteSettings): TelegramRemoteSettings => ({
...
export const normalizeFeishuSettingsInput = (input: FeishuRemoteSettings): FeishuRemoteSettings => ({
...
export const normalizeDiscordSettingsInput = (input: DiscordRemoteSettings): DiscordRemoteSettings => ({
  botToken: input.botToken?.trim() ?? "",
  remoteEnabled: Boolean(input.remoteEnabled),
...
export const normalizeWeixinIlinkSettingsInput = (input: WeixinIlinkRemoteSettings): WeixinIlinkRemoteSettings => ({
  remoteEnabled: Boolean(input.remoteEnabled),
...
export const buildTelegramPairingSnapshot = (settings: TelegramRemoteRuntimeConfig): TelegramPairingSnapshot => ({
  pairCode: settings.pairing.code,
  pairCodeExpiresAt: settings.pairing.expiresAt,
  allowedUserIds: [...settings.allowlist],
});

export const buildFeishuPairingSnapshot = (settings: FeishuRemoteRuntimeConfig): FeishuPairingSnapshot => ({
  pairCode: settings.pairing.code,
  pairCodeExpiresAt: settings.pairing.expiresAt,
  pairedUserOpenIds: [...settings.pairedUserOpenIds],
});

export const buildQQBotPairingSnapshot = (settings: QQBotRemoteRuntimeConfig): QQBotPairingSnapshot => ({
  pairCode: settings.pairing.code,
  pairCodeExpiresAt: settings.pairing.expiresAt,
...
export const buildDiscordPairingSnapshot = (settings: DiscordRemoteRuntimeConfig): DiscordPairingSnapshot => ({
  pairCode: settings.pairing.code,
  pairCodeExpiresAt: settings.pairing.expiresAt,
