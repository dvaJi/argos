// Use the global WebSocket (Bun + Node ≥ 22 ship a standard WebSocket global).
// Avoids the `undici` dependency so the runtime is portable across hosts.
...
const isFatalDiscordError = (error: unknown): boolean =>
  error instanceof DiscordApiRequestError && error.status >= 400 && error.status < 500 && error.status !== 429;

const normalizeGatewayUrl = (url: string): string => (url.includes("?") ? url : `${url}?v=10&encoding=json`);
...
export class DiscordGatewaySession {
  ...
  private async run(signal?: AbortSignal): Promise<void> {
    ...
    while (!this.stopRequested && !signal?.aborted) {
      try {
        ...
        const delayMs =
          DISCORD_GATEWAY_RECONNECT_BACKOFF_MS[Math.min(attempt, DISCORD_GATEWAY_RECONNECT_BACKOFF_MS.length - 1)] ??
          30_000;
        attempt += 1;
        this.emitStatus({
          state: "backoff",
  ...
  private async connectOnce(signal?: AbortSignal): Promise<void> {
    const gatewayUrl = normalizeGatewayUrl(this.resumeGatewayUrl?.trim() || (await this.deps.client.getGatewayUrl()));
        ...
        void (async () => {
          try {
            ...
            if (payload.t === "READY") {
              ...
              const applicationId = ready.application?.id === undefined ? "" : String(ready.application.id).trim();
              if (applicationId) {
