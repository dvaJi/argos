import { REMOTE_PAIR_CODE_MAX_FAILURES, type TelegramInboundEvent, type TelegramInboundMessage } from "../types";
...
export class RemoteAuthGuard {
  ...
  ensureAuthorized(event: TelegramInboundEvent): RemoteAuthResult {
    ...
    return {
      ok: false,
      message: "This Telegram account is not paired. Use /pair <code> from Argos Remote settings.",
    };
  }
  ...
  pair(message: TelegramInboundMessage, rawCode: string): string {
    ...
    if (pairing.code !== normalizedCode) {
      const result = this.bindingStore.recordPairCodeFailure("telegram", REMOTE_PAIR_CODE_MAX_FAILURES);
      if (result.exhausted) {
