...
export class QQBotAuthGuard {
  ...
  ensureAuthorized(message: QQBotInboundMessage): QQBotAuthResult {
    ...
    if (message.chatType === "c2c") {
      ...
      return {
        ok: false,
        message: "This QQ account is not paired. Open Remote settings and use the current /pair code.",
      };
    }
    ...
    return {
      ok: false,
      message:
        "This QQ group is not authorized. Generate a /pair code in Argos Remote settings, then send /pair <code> in this group.",
    };
  }
