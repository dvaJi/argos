...
const REMOTE_ASSET_ROOT = ".argos/remote-assets";
const REMOTE_GENERATED_ASSET_ROOT = "remote-assets";

/**
 * Data directory injected by the host at runtime construction (replaces
 * `app.getPath("userData")`). Module-level because the generated-image cache
 * helpers below are free functions; the runtime is a singleton per host.
 */
let RUNTIME_DATA_DIR = "";
export function setRuntimeDataDir(dataDir: string): void {
...
const isInvalidPathSegment = (value: string): boolean => value.length === 0 || value === "." || value === "..";
...
const sanitizeFileName = (value: string | null | undefined, fallback: string, mimeType?: string | null): string => {
...
const channelFromEndpointKey = (endpointKey: string): string => endpointKey.split(":", 1)[0]?.trim() || "remote";
...
const resolveCachedImagePath = (source: string): string => {
  const cacheDir = path.join(RUNTIME_DATA_DIR, "images");
...
const resolveGeneratedImageContent = async (
  ...
  if (normalizedSource.startsWith(IMGCACHE_URL_PREFIX)) {
    ...
    return {
      ...
      mimeType: normalizeImageMimeType(fallbackMimeType) ?? inferImageMimeTypeFromPath(imagePath) ?? "image/png",
    };
  }
...
const buildPromotedPreviewKey = (toolCallId: string | undefined, previewId: string | undefined): string | null => {
  const normalizedPreviewId = previewId?.trim();
  if (!normalizedPreviewId) {
    return null;
  }
  const normalizedToolCallId = toolCallId?.trim();
  return normalizedToolCallId ? `${normalizedToolCallId}:${normalizedPreviewId}` : normalizedPreviewId;
};
    ...
    blocks
      ...
      .map((candidateBlock) =>
        buildPromotedPreviewKey(
          typeof candidateBlock.extra?.toolCallId === "string" ? candidateBlock.extra.toolCallId : undefined,
      ...
      const candidate = createRemoteImageAssetCandidate({
        ...
        filenameBase:
          typeof block.extra?.toolImagePreviewSource === "string"
            ? [sanitizePathSegment(block.extra.toolImagePreviewSource, "tool-result-image"), blockIndex + 1].join("-")
...
const hasAttachmentDownloadSource = (attachment: RemoteInputAttachment): boolean =>
  Boolean(
    !attachment.failedDownload && (attachment.data?.trim() || attachment.url?.trim() || attachment.encryptedMedia),
  );

const buildCdnDownloadUrl = (encryptedQueryParam: string, cdnBaseUrl: string): string =>
  `${cdnBaseUrl.replace(/\/+$/, "")}/download?encrypted_query_param=${encodeURIComponent(encryptedQueryParam)}`;

const parseAes128Key = (value: string, encoding: "auto" | "hex" | undefined, label: string): Buffer => {
  if (encoding === "hex") {
...
export class RemoteConversationRunner {
  ...
  async createNewSession(
    ...
    const agentType = await this.deps.configPort.getAgentType(agentId);
    ...
    const session = await this.deps.sessionPort.createDetachedSession({
  ...
  async getCurrentSession(endpointKey: string): Promise<SessionWithState | null> {
    ...
    const session = await this.deps.sessionPort.getSession(binding.sessionId);
    if (!session) {
  ...
  async listSessions(endpointKey: string): Promise<SessionWithState[]> {
    ...
    const sessions = await this.deps.sessionPort.getSessionList({
      agentId,
    });
  ...
  async useSessionByIndex(
    ...
    const session = await this.deps.sessionPort.getSession(sessionId);
    if (!session) {
  ...
  async listAvailableModelProviders(): Promise<TelegramModelProviderOption[]> {
    const enabledProviders = this.deps.configPort.getEnabledProviders();
    const enabledModelGroups = await this.deps.configPort.getAllEnabledModels();
    const providerNameById = new Map(enabledProviders.map((provider) => [provider.id, provider.name]));
  ...
  async setSessionModel(endpointKey: string, providerId: string, modelId: string): Promise<SessionWithState> {
    const session = await this.getCurrentSession(endpointKey);
    if (!session) {
      throw new Error("No bound session. Send a message, /new, or /use first.");
    }

    return await this.deps.sessionPort.setSessionModel(session.id, providerId, modelId);
  }

  async listAvailableAgents(): Promise<TelegramAgentOption[]> {
    const agents = await this.deps.configPort.listAgents();
    return agents
  ...
  async setChannelDefaultAgent(
    ...
    if (matched.type === "acp") {
      ...
      if (!channelDefaultWorkdir) {
        throw new Error(
          "Cannot switch to ACP agent: this channel has no default workdir set. Configure the channel default workdir in Argos first.",
        );
      }
    }
  ...
  async sendInput(
    ...
    const beforeMessages = await this.deps.sessionPort.getMessages(session.id);
    const lastOrderSeq = beforeMessages.at(-1)?.orderSeq ?? 0;
    const previousActiveEventId = this.deps.generationPort.getActiveGeneration(session.id)?.eventId ?? null;
    ...
    await this.deps.sessionPort.sendMessage(session.id, messageInput);
  ...
  async respondToPendingInteraction(
    ...
    const result = await this.deps.sessionPort.respondToolInteraction(
      session.id,
  ...
  async stop(endpointKey: string): Promise<boolean> {
    ...
    const activeEventId =
      ...
      this.deps.generationPort.getActiveGeneration(session.id)?.eventId ??
      null;
    ...
    const stopped = await this.deps.generationPort.cancelGenerationByEventId(session.id, activeEventId);
    if (stopped) {
      this.bindingStore.clearActiveEvent(endpointKey);
    }
    return stopped;
  ...
  async open(endpointKey: string): Promise<RemoteOpenSessionResult> {
    ...
    // The `/open` command focuses a chat window — a desktop-only UX nicety. In
    // headless/daemon mode (no `onOpenEndpoint` hook) it returns `windowNotFound`.
    // The desktop proxy installs the hook to resolve the session + focus a window.
    if (!this.deps.onOpenEndpoint) {
      return { status: "windowNotFound" };
    }
    const result = await this.deps.onOpenEndpoint(endpointKey);
    if (result.status !== "ok") {
      // Non-ok statuses (noSession/windowNotFound) carry no session payload.
      return { status: result.status } as RemoteOpenSessionResult;
    }
  ...
  async getStatus(endpointKey: string): Promise<RemoteRunnerStatus> {
    ...
    return {
      session,
      activeEventId,
      isGenerating: !pendingInteraction && (Boolean(activeEventId) || session.status === "generating"),
      pendingInteraction: pendingInteraction ? this.stripPendingInteractionDetails(pendingInteraction) : null,
    };
  }
  ...
  async isSessionModelLocked(session: Pick<SessionWithState, "agentId">): Promise<boolean> {
    return (await this.deps.configPort.getAgentType(session.agentId)) === "acp";
  }
  ...
  private getGlobalDefaultWorkdir(): string | null {
    const projectDir = this.deps.configPort.getDefaultProjectPath();
  ...
  private async resolveDefaultWorkdirForAgent(endpointKey: string, agentId: string): Promise<string | null> {
    const channelDefaultWorkdir = this.getChannelDefaultWorkdir(endpointKey);
    if (channelDefaultWorkdir) {
      return channelDefaultWorkdir;
    }

    if ((await this.deps.configPort.getAgentType(agentId)) === "acp") {
      return null;
    }
  ...
  private async resolveGeneratedImageAssetRoot(
    ...
    return path.join(this.deps.dataDir, REMOTE_GENERATED_ASSET_ROOT);
  }
  ...
  private async prepareRemoteAttachments(
    ...
    const sourceMessageId = sanitizePathSegment(input.sourceMessageId || attachments[0]?.id, `message-${Date.now()}`);
  ...
  private async readAttachmentData(attachment: RemoteInputAttachment): Promise<Buffer> {
    ...
    if (!response.ok) {
      throw new Error(`Failed to download remote attachment: ${response.status} ${response.statusText}`);
    }
  ...
  private async readEncryptedAttachmentData(attachment: RemoteInputAttachment): Promise<Buffer> {
    ...
    const url =
      fullUrl || (encryptedQueryParam && cdnBaseUrl ? buildCdnDownloadUrl(encryptedQueryParam, cdnBaseUrl) : "");
    ...
    return decryptAes128Ecb(content, parseAes128Key(aesKey, media.aesKeyEncoding, attachment.filename));
  }
  ...
  private async prepareMessageFile(
    ...
    try {
      if (this.deps.filePort) {
        const preparedFile = await this.deps.filePort.prepareFile(filePath, mediaType);
        return {
          ...preparedFile,
  ...
  private async getConversationSnapshot(
    ...
    const activeGeneration = this.deps.generationPort.getActiveGeneration(sessionId);
    const trackedMessage = await this.resolveTrackedAssistantMessage(sessionId, tracking, activeGeneration);
    if (trackedMessage) {
    ...
    const generatedImages = await this.persistGeneratedImages(endpointKey, session, trackedMessage.id, blocks);
    ...
    const renderBlocks = await buildRemoteRenderableBlocks({
      ...
      loadSearchResults: async (messageId, searchId) => await this.loadSearchResults(messageId, searchId),
    });
    const fullText = buildRemoteFullText(renderBlocks);
    const pendingInteraction = collectPendingInteraction(trackedMessage.id, trackedMessage.orderSeq, blocks);
    ...
    const finalText = buildRemoteFinalText(blocks, {
      ...
      fallbackErrorText: trackedMessage.status === "error" ? "The conversation ended with an error." : undefined,
      fallbackNoResponseText: REMOTE_NO_RESPONSE_TEXT,
    });
    const resolvedFinalText = generatedImages.length > 0 && finalText === REMOTE_NO_RESPONSE_TEXT ? "" : finalText;
    const completed =
      Boolean(pendingInteraction) ||
      (trackedMessage.status !== "pending" && (!activeGeneration || activeGeneration.eventId !== trackedMessage.id));
  ...
  private async loadSearchResults(messageId: string, searchId?: string): Promise<SearchResult[]> {
    if (typeof this.deps.sessionPort.getSearchResults !== "function") {
      return [];
    }

    try {
      return await this.deps.sessionPort.getSearchResults(messageId, searchId);
    } catch (error) {
  ...
  private async persistGeneratedImages(
    ...
    for (const [candidateIndex, candidate] of imageCandidates.entries()) {
      try {
        ...
        const filenameBase = sanitizePathSegment(candidate.filenameBase, `generated-${candidateIndex + 1}`);
  ...
  private async waitForAssistantMessage(
    ...
    while (Date.now() < deadline) {
      ...
      if (activeGeneration?.eventId && activeGeneration.eventId !== options?.ignoreMessageId) {
        const message = await this.deps.sessionPort.getMessage(activeGeneration.eventId);
        if (message?.role === "assistant") {
          return message;
        }
      }

      const fallback = await this.findLatestAssistantMessageAfter(sessionId, afterOrderSeq, options?.ignoreMessageId);
      if (fallback) {
        return fallback;
      }
  ...
  private async resolveTrackedAssistantMessage(
    ...
    for (const messageId of candidateIds) {
      ...
      const message = await this.deps.sessionPort.getMessage(messageId);
      if (message?.role === "assistant") {
        return message;
      }
    }

    return await this.findLatestAssistantMessageAfter(sessionId, tracking.afterOrderSeq, tracking.ignoreMessageId);
  }
  ...
  private async findLatestAssistantMessageAfter(
    ...
    const messages = await this.deps.sessionPort.getMessages(sessionId);
    const assistants = messages.filter(
      (message) => message.role === "assistant" && message.orderSeq > afterOrderSeq && message.id !== ignoreMessageId,
    );
  ...
  private async getCurrentPendingInteractionDetails(sessionId: string): Promise<PendingInteractionDetails | null> {
    const messages = await this.deps.sessionPort.getMessages(sessionId);
  ...
  private stripPendingInteractionDetails(interaction: PendingInteractionDetails): RemotePendingInteraction {
    const { messageOrderSeq: _messageOrderSeq, ...rest } = interaction;
    return rest;
  }
}
