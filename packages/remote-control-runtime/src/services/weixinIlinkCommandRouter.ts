...
export class WeixinIlinkCommandRouter {
  ...
  private describeInteractionResponse(
    ...
    if (interaction.type === "permission") {
      return response.kind === "permission" && response.granted ? "Permission granted." : "Permission denied.";
    }

    return response.kind === "question_option" ? `Selected option: ${response.optionLabel}` : "Answer received.";
  }
  ...
  private formatPendingCommandBlockedMessage(interaction: RemotePendingInteraction): string {
    return ["Finish the current pending interaction first.", this.formatPendingInteraction(interaction)].join("\n\n");
  }
  ...
  private formatHelpMessage(): string {
    return ["Available commands:", ...COMMANDS.map((item) => `/${item.command} - ${item.description}`)].join("\n");
  }
