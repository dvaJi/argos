...
const readHeaderValue = (headers: unknown, name: string): string | undefined => {
  ...
  if (typeof (headers as { get?: unknown }).get === "function") {
    return ((headers as { get: (key: string) => string | null }).get(name) ?? "").trim() || undefined;
  }
...
export const chunkFeishuText = (text: string, limit: number = FEISHU_OUTBOUND_TEXT_LIMIT): string[] => {
...
export class FeishuClient {
  ...
  async startMessageStream(params: { onMessage: (event: FeishuRawMessageEvent) => Promise<void> }): Promise<void> {
    this.stop();
  ...
  async downloadMessageResource(params: { messageId: string; fileKey: string; type: "image" | "file" }): Promise<{
    data: string;
    mediaType?: string;
  }> {
