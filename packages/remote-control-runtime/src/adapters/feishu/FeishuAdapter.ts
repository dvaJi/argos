...
export class FeishuAdapter extends ChannelAdapter {
  ...
  async sendImage(chatId: string, imagePath: string, opts?: SendMessageOptions): Promise<string> {
    ...
    const messageId = await this.client.sendImage(this.parseTransportTarget(chatId, opts?.replyToMessageId), imagePath);
    if (!messageId) {
  ...
  private handleStatusChange(snapshot: FeishuRuntimeStatusSnapshot): void {
    ...
    this.setStatus({
      connected: snapshot.state === "starting" || snapshot.state === "running" || snapshot.state === "backoff",
  ...
  private parseTransportTarget(chatId: string, replyToMessageId?: number | string): FeishuTransportTarget {
    ...
    return {
      ...
      replyToMessageId:
        replyToMessageId === undefined || replyToMessageId === null ? null : String(replyToMessageId).trim() || null,
    };
  }
}
