...
} from "../process/acpProcessManager";
...
import { convertMcpConfigToAcpFormat } from "../protocol/mcpConfigConverter";
import { filterMcpServersByTransportSupport } from "../protocol/mcpTransportFilter";
import type * as schema from "@agentclientprotocol/sdk";
import type { LifecyclePort } from "../host/ports";
import {
...
export class AcpSessionManager {
  ...
  private async createSession(
    ...
    const session = await this.initializeSession(handle, conversationId, agent, workdir, hooks).catch(async (error) => {
      const initError = error;
      try {
        await this.processManager.unbindProcess(agent.id, conversationId);
      } catch (cleanupError) {
        console.warn("[ACP] Failed to unbind process after session initialization error:", cleanupError);
      }
      throw initError;
    });
    const detachListeners = session.detachHandlers ?? this.attachSessionHooks(agent.id, session.sessionId, hooks);
    ...
    let configState = session.configState ?? handle.configState ?? createEmptyAcpConfigState("legacy");
    const legacyModeState = getLegacyModeState(configState);
    const availableModes = session.availableModes ?? legacyModeState?.availableModes ?? handle.availableModes;
    // Prefer handle.currentModeId (which may contain preferredMode from warmup) over session default
    let currentModeId = handle.currentModeId ?? session.currentModeId ?? legacyModeState?.currentModeId;
    ...
    if (
      ...
      try {
        ...
        if (modeOption?.type === "select") {
          configState = updateAcpConfigStateValue(configState, modeOption.id, currentModeId) ?? configState;
          handle.configState = configState;
        }
        console.info(
  ...
  private attachSessionHooks(agentId: string, sessionId: string, hooks: SessionHooks): Array<() => void> {
    const detachUpdate = this.processManager.registerSessionListener(agentId, sessionId, hooks.onSessionUpdate);
    const detachPermission = this.processManager.registerPermissionResolver(agentId, sessionId, hooks.onPermission);
  ...
  private async initializeSession(
    ...
    try {
      ...
      const persistedSession = await this.sessionPersistence.getSessionData(conversationId, agent.id);
      ...
      if (!currentModeId || (availableModes && !availableModes.some((m) => m.id === currentModeId))) {
