...
/**
 * Structural interface for the MCP config access AcpConfHelper needs.
 * Hosts inject their own MCP config helper (desktop `McpConfHelper` or a daemon
 * equivalent); only `getMcpServers()` is required.
 */
export interface AcpMcpConfigLike {
...
const normalizeRegistryState = (agentId: string, value: unknown, defaults?: Partial<AcpAgentState>): AcpAgentState => {
  const record = value && typeof value === "object" && !Array.isArray(value) ? (value as Record<string, unknown>) : {};

  return {
    agentId,
    enabled: typeof record.enabled === "boolean" ? record.enabled : Boolean(defaults?.enabled ?? false),
...
const normalizeManualAgent = (
  ...
  return {
    ...
    enabled: typeof agent.enabled === "boolean" ? agent.enabled : Boolean(defaults?.enabled ?? true),
...
const getActiveLegacyProfile = (agent: AcpBuiltinAgent): AcpAgentProfile | null => {
  return agent.profiles.find((profile) => profile.id === agent.activeProfileId) ?? agent.profiles[0] ?? null;
};

export class AcpConfHelper {
  private store: StoreLike<InternalStore & Record<string, unknown>>;
  private readonly mcpConfHelper: AcpMcpConfigLike | undefined;

  constructor(options?: { mcpConfHelper?: AcpMcpConfigLike; storeFactory?: StoreFactory }) {
    this.mcpConfHelper = options?.mcpConfHelper;
    if (options?.storeFactory) {
      this.store = options.storeFactory<InternalStore>({
        name: "acp_agents",
        defaults: {
          enabled: false,
      ...
      const data = { ...defaults } as InternalStore & Record<string, unknown>;
      this.store = {
  ...
  updateManualAgent(agentId: string, updates: Partial<Omit<AcpManualAgent, "id" | "source">>): AcpManualAgent | null {
    const manualAgents = this.getManualAgents();
  ...
  getInstallStates(): Record<string, AcpAgentInstallState> {
    ...
    const entries = Object.entries(raw)
      .map(([agentId, value]) => [resolveAcpAgentAlias(agentId), normalizeInstallState(value)] as const)
  ...
  async setAgentMcpSelections(agentId: string, _isBuiltin: boolean, mcpIds: string[]): Promise<void> {
    void agentId;
  ...
  private migrateStore(): void {
    ...
    const pushSelections = (...groups: Array<unknown>) => {
      sharedMcpSelections.splice(0, sharedMcpSelections.length, ...mergeMcpSelections(sharedMcpSelections, ...groups));
    };

    const rawRegistryStates = this.store.get("registryStates");
    if (rawRegistryStates && typeof rawRegistryStates === "object" && !Array.isArray(rawRegistryStates)) {
    ...
    this.store.set("manualAgents", Array.from(new Map(manualAgents.map((agent) => [agent.id, agent])).values()));
    this.store.set("installStates", this.getInstallStates());
    this.store.set("sharedMcpSelections", mergeMcpSelections(rawSharedMcpSelections, sharedMcpSelections));
    this.store.set("version", ACP_STORE_VERSION);
