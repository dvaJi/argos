import type { IConfigPresenter, ISQLitePresenter } from "@argos/shared/presenter";
...
export class AcpRegistryMigrationService {
  ...
  async runIfNeeded(): Promise<boolean> {
    const currentVersion = this.configPresenter.getSetting<number>("acpRegistryMigrationVersion") ?? 0;
  ...
  async compensateEnabledRegistryAgentInstalls(): Promise<void> {
    ...
    for (const agent of agents) {
      ...
      try {
        ...
        console.warn(`[ACP] Failed to compensate install state for enabled registry agent ${agent.id}:`, error);
      }
    }
  }
