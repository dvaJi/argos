...
type AcpRegistryServiceOptions = {
  ...
  /** Host user-data dir (replaces `app.getPath("userData")`). */
  userDataDir: () => string;
  /** Host app bundle root, optional (replaces `app.getAppPath()`). */
  appPath?: () => string;
  /** SVG sanitization (replaces the desktop `SVGSanitizer`). */
  sanitizeSvg: (svg: string) => string | null;
};
...
const normalizeEnv = (value: unknown): Record<string, string> | undefined => {
  ...
  const entries = Object.entries(value)
    .map(([key, envValue]) => [key.trim(), typeof envValue === "string" ? envValue : String(envValue)])
...
export class AcpRegistryService {
  ...
  private readonly sanitizeSvg: (svg: string) => string | null;
  ...
  constructor(options: AcpRegistryServiceOptions) {
    ...
    this.sanitizeSvg = options.sanitizeSvg;
    this.appPath = options.appPath;
    const userDataPath = options.userDataDir();
  ...
  private getBuiltInResourceRoots(): string[] {
    const roots: string[] = [];
    if (this.appPath) {
      roots.push(path.join(this.appPath(), ...ACP_REGISTRY_RESOURCE_DIR));
    }
    roots.push(path.join(process.cwd(), ...ACP_REGISTRY_RESOURCE_DIR));
    return roots;
  }
  ...
  private async syncIconCache(manifest: RegistryManifest): Promise<void> {
    ...
    const cacheableAgents = manifest.agents.filter((agent) => agent.icon && isAcpRegistryIconUrl(agent.icon.trim()));
