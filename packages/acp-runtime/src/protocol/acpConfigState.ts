...
import type { AcpConfigOption, AcpConfigOptionValue, AcpConfigState } from "@argos/shared/presenter";
...
export const createEmptyAcpConfigState = (source: AcpConfigState["source"] = "legacy"): AcpConfigState => ({
  source,
  options: [],
});
...
export const hasAcpConfigStateData = (state: AcpConfigState | null | undefined): state is AcpConfigState =>
...
export const getAcpConfigOptionLabel = (option: AcpConfigOption | null | undefined): string | null => {
  if (!option) {
    return null;
  }
...
export const getLegacyModeState = (
  ...
  | undefined => {
  const option = state?.options.find((entry) => entry.id === LEGACY_MODE_CONFIG_ID || entry.category === "mode");
