...
> {
  getOrCreateSession(conversationId: string, agent: TAgentDescriptor, options: TSessionOptions): Promise<TSession>;
...
export interface AgentProcessManager<THandle extends AgentProcessHandle = AgentProcessHandle, TDescriptor = string> {
...
export type AgentPermissionResult = { outcome: "cancelled" } | { outcome: "selected"; optionId: string };
