...
const ACP_WORKSPACE_EVENTS = {
  ...
  SESSION_COMMANDS_READY: "acp-workspace:session-commands-ready",
  SESSION_CONFIG_OPTIONS_READY: "acp-workspace:session-config-options-ready",
} as const;
...
export class AcpProcessManager implements AgentProcessManager<AcpProcessHandle, AcpAgentConfig> {
  ...
  private readonly resolveLaunchSpec: (agentId: string, workdir?: string) => Promise<AcpResolvedLaunchSpec>;
  ...
  registerSessionWorkdir(sessionId: string, workdir: string, conversationId?: string): void {
    ...
    this.fsHandlers.set(sessionId, new AcpFsHandler({ workspaceRoot: workdir, fsHelpers: this.ports.fs }));
  }
  ...
  private getFsHandler(sessionId: string): AcpFsHandler {
    ...
    if (!handler) {
      ...
      const fallbackHandler = new AcpFsHandler({ workspaceRoot: fallbackWorkdir, fsHelpers: this.ports.fs });
  ...
  private resolveCwdInsideWorkdir(workdir: string, cwd: string): string | null {
    ...
    const resolvedCwd = path.isAbsolute(cwd) ? path.resolve(cwd) : path.resolve(resolvedWorkdir, cwd);
  ...
  private getFallbackWorkdir(): string {
    const tempDir = path.join(this.ports.paths.tempDir(), "argos-acp", "sessions");
    try {
      fs.mkdirSync(tempDir, { recursive: true });
    } catch (error) {
      console.warn("[ACP] Failed to create fallback workdir, defaulting to system temp:", error);
      return this.ports.paths.tempDir();
    }
    return tempDir;
  }
    ...
    if (existingWarmup && this.isHandleAlive(existingWarmup)) {
      ...
      if (modeOption?.type === "select") {
        existingWarmup.configState =
          updateAcpConfigStateValue(existingWarmup.configState, modeOption.id, modeId) ?? existingWarmup.configState;
  ...
  getProcess(agentId: string): AcpProcessHandle | null {
    const warmupHandle = Array.from(this.handles.values()).find((handle) => handle.agentId === agentId);
  ...
  updateBoundProcessMode(conversationId: string, modeId: string): boolean {
    ...
    if (modeOption?.type === "select") {
      handle.configState = updateAcpConfigStateValue(handle.configState, modeOption.id, modeId) ?? handle.configState;
  ...
  bindProcess(agentId: string, conversationId: string, workdir?: string): void {
    ...
    const handle =
      warmupHandles.find(([, candidate]) => candidate.workdir === resolvedWorkdir)?.[1] ?? warmupHandles[0]?.[1];
    if (!handle) {
      console.warn(`[ACP] No warmup handle to bind for agent ${agentId}`);
      return;
    }
    if (handle.state !== "warmup") {
      console.warn(`[ACP] Cannot bind handle in state "${handle.state}" for agent ${agentId}, expected warmup`);
      return;
    }
  ...
  registerSessionListener(agentId: string, sessionId: string, handler: SessionNotificationHandler): () => void {
  ...
  registerPermissionResolver(agentId: string, sessionId: string, resolver: PermissionResolver): () => void {
    if (this.permissionResolvers.has(sessionId)) {
      console.warn(`[ACP] Overwriting existing permission resolver for session "${sessionId}" (agent ${agentId})`);
    }
  ...
  private async spawnProcessOnce(
    ...
    if (child.killed) {
      throw new Error(`[ACP] Agent process ${agent.id} exited before initialization (PID: ${child.pid})`);
    }
    ...
    try {
      ...
      const connectionClosedPromise = connection.closed.then(() => {
        if (!initializationSettled) {
          throw new Error(`[ACP] Protocol stream closed before initialization completed for agent ${agent.id}`);
        }
        return new Promise<never>(() => {});
      });
      const timeoutPromise = new Promise<never>((_, reject) => {
        timeoutHandle = setTimeout(() => {
          reject(new Error(`[ACP] Connection initialization timeout after ${timeoutMs}ms for agent ${agent.id}`));
        }, timeoutMs);
      });
      ...
      if (initAvailableModes) {
        console.info(`[ACP] Available modes: ${JSON.stringify(initAvailableModes.map((m) => m.id) ?? [])}`);
    ...
    if (!this.isHandleAlive(handle)) {
      ...
      this.clearSessionsForAgent(agent.id);
      throw new Error(`[ACP] Agent process ${agent.id} exited before becoming ready (PID: ${child.pid})`);
    }
    this.debugLog.append(agent.id, {
      kind: "lifecycle",
  ...
  private findNpxCacheRepairTarget(error: unknown): NpxCacheRepairTarget | null {
    ...
    const packageJsonPathPattern = /((?:[A-Za-z]:)?[\\/][^'"\r\n]*[\\/]_npx[\\/][^\\/ "'\r\n]+[\\/]package\.json)/i;
  ...
  private async spawnAgentProcess(
    ...
    const useBundledRuntime = launchSpec.distributionType === "npx" || launchSpec.distributionType === "uvx";
    const expandedCommand = this.ports.runtime.expandPath(launchSpec.command);
    ...
    if (!processedCommand || processedCommand.trim().length === 0) {
      throw new Error(`[ACP] Invalid processed command for agent ${agent.id}: "${agent.command}" -> empty`);
    }
    ...
    try {
      ...
      console.warn(`[ACP] Failed to get shell environment variables for agent ${agent.id}, using fallback:`, error);
    }
    ...
    if (useBundledRuntime) {
      const withRuntimePaths = this.ports.runtime.buildSpawnEnv(env);
      Object.assign(env, withRuntimePaths);

      env.ACP_IDE = "argos";
      env.ARGOS_ACP_AGENT_ID = agent.id;
    ...
    setPathEntriesOnEnv(
      env,
      [getPathEntriesFromEnv(userEnvOverride), getPathEntriesFromEnv(launchSpec.env), getPathEntriesFromEnv(env)],
      {
  ...
  private getProtocolRequestMap(store: Map<string, Map<JsonRpcId, string>>, agentId: string): Map<JsonRpcId, string> {
  ...
  private summarizeProtocolMessage(
    ...
    if (directMethod && id !== undefined) {
      kind = "request";
      const store = direction === "out" ? this.protocolRequestsToAgent : this.protocolRequestsFromAgent;
      this.getProtocolRequestMap(store, agentId).set(id, directMethod);
    } else if (directMethod) {
      kind = "notification";
    } else if (id !== undefined) {
      kind = "response";
      const store = direction === "in" ? this.protocolRequestsToAgent : this.protocolRequestsFromAgent;
  ...
  private logProtocolMessage(agentId: string, direction: ProtocolDirection, message: unknown): void {
    const summary = this.summarizeProtocolMessage(agentId, direction, message);
    const route = direction === "out" ? "client->agent" : "agent->client";
    const isImportant =
      Boolean(summary.error) || (summary.method ? IMPORTANT_PROTOCOL_METHODS.has(summary.method) : false);
  ...
  private deliverSessionUpdate(entry: SessionListenerEntry, notification: schema.SessionNotification): void {
  ...
  private bufferSessionUpdate(notification: schema.SessionNotification): void {
    ...
    console.warn(`[ACP] Buffered session update for unbound session "${sessionId}" (${next.length} pending)`);
  }
  ...
  private async dispatchPermissionRequest(
    ...
    if (!entry) {
      console.warn(`[ACP] Missing permission resolver for session "${params.sessionId}", returning cancelled`);
  ...
  private notifyModesReady(handle: AcpProcessHandle, conversationId?: string): void {
    ...
    this.ports.events.broadcast(ACP_WORKSPACE_EVENTS.SESSION_MODES_READY, {
  ...
  private notifyConfigOptionsReady(handle: AcpProcessHandle, conversationId?: string): void {
    ...
    this.ports.events.broadcast(ACP_WORKSPACE_EVENTS.SESSION_CONFIG_OPTIONS_READY, {
      conversationId: conversationId ?? handle.boundConversationId,
      agentId: handle.agentId,
      workdir: handle.workdir,
      configState,
    });
    this.ports.events.publish("sessions.acp.configOptions.ready", {
  ...
  private getScopedHandle(agentId: string, workdir?: string): AcpProcessHandle | undefined {
    const aliveHandles = this.getHandlesByAgent(agentId).filter((handle) => this.isHandleAlive(handle));
  ...
  private getRestartCount(agentId: string): number {
    return this.getHandlesByAgent(agentId).reduce((max, handle) => Math.max(max, handle.restarts ?? 0), 0);
  }
  ...
  private findReusableHandle(agentId: string, workdir: string): AcpProcessHandle | undefined {
    const candidates = this.getHandlesByAgent(agentId).filter(
      (handle) => handle.workdir === workdir && handle.state === "warmup" && this.isHandleAlive(handle),
    );
    return candidates[0];
  }
  ...
  private applyPreferredMode(handle: AcpProcessHandle, preferredModeId?: string): void {
    ...
    if (modeOption?.type === "select") {
      handle.configState =
        updateAcpConfigStateValue(handle.configState, modeOption.id, preferredModeId) ?? handle.configState;
  ...
  private killChild(child: ChildProcessWithoutNullStreams, reason?: string): void {
    ...
    if (!child.killed) {
      try {
        ...
        console.warn(`[ACP] Failed to kill agent process${pid ? ` ${pid}` : ""} (${reason ?? "unknown"}):`, error);
      }
    }
  }
  ...
  private isHandleAlive(handle: AcpProcessHandle): boolean {
    return !handle.child.killed && handle.child.exitCode === null && handle.child.signalCode === null;
  }
}
