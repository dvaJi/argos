...
import type { AcpFsHelpers } from "../host/ports";
import { AcpPathGuard } from "../config/acpPathGuard";
...
export interface FsHandlerOptions {
  ...
  /** Host-injected binary-file read gating (replaces desktop binaryReadGuard). */
  fsHelpers: AcpFsHelpers;
}
...
export class AcpFsHandler {
  ...
  private readonly fsHelpers: AcpFsHelpers;
  ...
  constructor(options: FsHandlerOptions) {
    ...
    this.fsHelpers = options.fsHelpers;
  }
  ...
  async readTextFile(params: schema.ReadTextFileRequest): Promise<schema.ReadTextFileResponse> {
    try {
      ...
      const { reject, mimeType } = await this.fsHelpers.shouldRejectAcpTextRead(filePath);
      if (reject) {
        throw RequestError.invalidParams(
          { path: params.path, mimeType },
          this.fsHelpers.buildBinaryReadGuidance(filePath, mimeType, "acp"),
        );
      }
