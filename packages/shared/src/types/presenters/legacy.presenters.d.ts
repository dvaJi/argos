...
export interface IYoBrowserPresenter {
  ...
  loadUrl(sessionId: string, url: string, timeoutMs?: number, hostWindowId?: number): Promise<YoBrowserStatus>;
...
export interface IWindowPresenter {
  createAppWindow(options?: { initialRoute?: string; x?: number; y?: number }): Promise<number | null>;
  ...
  sendToDefaultWindow(channel: string, switchToTarget?: boolean, ...args: unknown[]): Promise<boolean>;
...
export interface ITabPresenter {
  ...
  captureTabArea(tabId: number, rect: { x: number; y: number; width: number; height: number }): Promise<string | null>;
...
export interface ISQLitePresenter {
  ...
  getConversationList(page: number, pageSize: number): Promise<{ total: number; list: CONVERSATION[] }>;
  ...
  addMessageAttachment(messageId: string, attachmentType: string, attachmentData: string): Promise<void>;
  ...
  getAcpSessionByAgentAndSessionId(agentId: string, sessionId: string): Promise<AcpSessionEntity | null>;
  upsertAcpSession(conversationId: string, agentId: string, data: AcpSessionUpsertPayload): Promise<void>;
  updateAcpSessionId(conversationId: string, agentId: string, sessionId: string | null): Promise<void>;
  updateAcpWorkdir(conversationId: string, agentId: string, workdir: string | null): Promise<void>;
  updateAcpSessionStatus(conversationId: string, agentId: string, status: AgentSessionLifecycleStatus): Promise<void>;
...
export interface IConfigPresenter {
  ...
  addManualAcpAgent(agent: Omit<AcpManualAgent, "id" | "source"> & { id?: string }): Promise<AcpManualAgent>;
  ...
  getArgosAgentConfig(agentId: string): Promise<ArgosAgentConfig | null>;
  resolveArgosAgentConfig(agentId: string): Promise<ArgosAgentConfig>;
  agentSupportsCapability?(agentId: string, capability: "vision"): Promise<boolean>;
  createArgosAgent(input: CreateArgosAgentInput): Promise<Agent>;
  updateArgosAgent(agentId: string, updates: UpdateArgosAgentInput): Promise<Agent | null>;
  deleteArgosAgent(agentId: string): Promise<boolean>;
...
export type LLM_PROVIDER_BASE = Omit<LLM_PROVIDER, "models" | "customModels" | "enabledModels" | "disabledModels"> & {
  models?: MODEL_META[];
  customModels?: MODEL_META[];
  enabledModels?: string[];
  disabledModels?: string[];
...
export interface ILlmProviderPresenter {
  ...
  addCustomModel(providerId: string, model: Omit<MODEL_META, "providerId" | "isCustom" | "group">): Promise<MODEL_META>;
  removeCustomModel(providerId: string, modelId: string): Promise<boolean>;
  updateCustomModel(providerId: string, modelId: string, updates: Partial<MODEL_META>): Promise<boolean>;
...
export interface IThreadPresenter {
  ...
  updateConversationSettings(conversationId: string, settings: Partial<CONVERSATION_SETTINGS>): Promise<void>;
  ...
  // Message operations
  getMessages(conversationId: string, page: number, pageSize: number): Promise<{ total: number; list: MESSAGE[] }>;
  getMessageThread(
    conversationId: string,
    page: number,
    pageSize: number,
...
export interface IFilePresenter {
  ...
  copyImage(file: { source: string; mimeType?: string; suggestedName?: string }): Promise<{ copied: boolean }>;
