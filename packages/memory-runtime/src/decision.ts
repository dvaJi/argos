...
const DECISION_KINDS: ReadonlySet<string> = new Set(["ADD", "UPDATE", "SUPERSEDE", "NOOP", "CHALLENGE"]);
const TARGETED_KINDS: ReadonlySet<MemoryDecisionKind> = new Set(["UPDATE", "SUPERSEDE", "NOOP", "CHALLENGE"]);
...
export function buildDecisionPrompt(candidate: NormalizedMemoryCandidate, neighbors: DecisionNeighbor[]): string {
  const neighborList = neighbors.map((neighbor, index) => `[${index}] ${truncate(neighbor.content)}`).join("\n");
  return [
...
export function parseDecision(raw: string, neighborCount: number): MemoryDecision {
  ...
  const mergedContent =
    typeof obj.mergedContent === "string" && obj.mergedContent.trim() ? obj.mergedContent.trim() : null;
