...
export async function fetchModelScopeMcpServers(
  ...
  if (response.status === 500 || !response.ok) {
    ...
    throw new Error(`ModelScope MCP sync failed: ${response.status} ${response.statusText} - ${errorText}`);
  }
...
export function convertModelScopeMcpServerToConfig(mcpServer: ModelScopeMcpServer): Record<string, unknown> {
  ...
  return {
    ...
    env: {},
    descriptions:
      mcpServer.locales?.zh?.description || mcpServer.description || `ModelScope MCP Server: ${displayName}`,
    icons: randomEmoji,
    autoApprove: ["all"],
    enabled: false,
    disable: false,
