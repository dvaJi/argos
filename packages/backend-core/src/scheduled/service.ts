...
const RECENT_DRIFT_TOLERANCE_MS = 60 * 1000;
...
export interface NotificationPort {
  showNotification(input: { id: string; title: string; body: string }): Promise<void>;
}

export interface WindowPort {
  sendToWindow(windowId: number, channel: string, ...args: unknown[]): void;
  focusMainWindow(): void;
  mainWindow: { id: number; isDestroyed(): boolean } | null;
}

export interface ScheduledTasksConfigPort {
  getScheduledTasksConfig(): ScheduledTasksSettings;
  setScheduledTasksConfig(settings: ScheduledTasksSettings): ScheduledTasksSettings;
  getNotificationsEnabled(): boolean;
}

export interface ScheduledTasksServiceDeps {
  configPresenter: ScheduledTasksConfigPort;
  notificationPresenter: NotificationPort;
  windowPresenter: WindowPort;
...
export class ScheduledTasksService {
  ...
  private async fireAndPersist(task: ScheduledTask): Promise<void> {
    try {
      ...
      console.error("[ScheduledTasks] Dispatch failed:", error);
    } finally {
      this.markFired(task);
  ...
  private async runPromptDraft(
    ...
    if (target && !target.isDestroyed()) {
      this.windowPresenter.sendToWindow(target.id, "deeplink:start", {
        msg: action.message,
      ...
      console.warn("[ScheduledTasks] No main window available for prompt draft action");
    }
  ...
  private async runPromptAutoSend(
    ...
    if (!this.sessionCreator) {
      console.warn("[ScheduledTasks] sessionCreator is not wired; falling back to draft mode");
      await this.runPromptDraft(taskId, action);
      return;
    }
    ...
    try {
      ...
      console.error("[ScheduledTasks] Failed to create session for task:", error);
      await this.runPromptDraft(taskId, action);
    }
  }
}
