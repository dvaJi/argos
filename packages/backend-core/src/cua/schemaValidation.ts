...
function cloneBoundedJson(value: unknown, label: string, limits: CloneLimits): unknown {
  ...
  const visit = (current: unknown, depth: number, path: string, complexity: CloneState): unknown => {
    ...
    if (Array.isArray(current)) {
      ...
      const cloned = current.map((entry, index) =>
        entry === undefined ? null : visit(entry, depth + 1, `${path}/${index}`, complexity),
      );
      seen.delete(current);
      return cloned;
    }
...
export function assertBoundedMcpJson(value: unknown, label: string, maxBytes: number): void {
  cloneBoundedJson(value, label, { maxBytes });
}
...
export function validateAndCloneJsonSchema(value: unknown, label: string): Record<string, unknown> {
  ...
  const cloned = cloneBoundedJson(value, label, { maxBytes: MAX_SCHEMA_BYTES }) as Record<string, unknown>;
