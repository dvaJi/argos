...
export class ChatService {
  ...
  async steerActiveTurn(sessionId: string, content: string | SendMessageInput): Promise<{ accepted: true }> {
    const session = await this.deps.scheduler.timeout({
      task: this.deps.sessionRepository.get(sessionId),
      ms: CHAT_LOOKUP_TIMEOUT_MS,
  ...
  async stopStream(input: { sessionId?: string; requestId?: string }): Promise<{ stopped: boolean }> {
    ...
    await this.deps.scheduler.timeout({
      task: Promise.allSettled([
        ...
        Promise.resolve().then(() => this.deps.providerExecutionPort.cancelGeneration(targetSessionId)),
      ]).then((results) => {
