...
export const readSettingsSnapshot = (configPresenter: IConfigPresenter): SettingsSnapshotValues => ({
...
export const pickSettingsSnapshot = (
  ...
  for (const key of selectedKeys) {
    (result as Record<SettingsKey, SettingsSnapshotValues[SettingsKey] | undefined>)[key] = snapshot[key];
  }

  return result;
};

export const applySettingChange = (configPresenter: IConfigPresenter, change: SettingsChange): void => {
  switch (change.key) {
    case "fontSizeLevel":
...
export function createSettingsRouteAdapter(configPresenter: IConfigPresenter): SettingsRouteAdapter {
  return {
