...
const createLegacyDefaultStepState = (id: LegacyGuidedOnboardingStepId): LegacyGuidedOnboardingStepState => ({
  id,
  status: "pending",
  startedAt: null,
  completedAt: null,
...
const isPreviousGuidedOnboardingStepId = (value: unknown): value is PreviousGuidedOnboardingStepId =>
  typeof value === "string" && PREVIOUS_GUIDED_ONBOARDING_STEP_IDS.includes(value as PreviousGuidedOnboardingStepId);

const isVersion3GuidedOnboardingStepId = (value: unknown): value is Version3GuidedOnboardingStepId =>
  typeof value === "string" && VERSION3_GUIDED_ONBOARDING_STEP_IDS.includes(value as Version3GuidedOnboardingStepId);
...
const migratePreviousState = (candidate: StoredGuidedOnboardingStateCandidate, now: number): GuidedOnboardingState => {
  ...
  const previousSteps = {
    "open-settings": normalizePreviousStepState("open-settings", previousStepMap.get("open-settings")),
    "select-provider": normalizePreviousStepState("select-provider", previousStepMap.get("select-provider")),
    "provider-api-key": normalizePreviousStepState("provider-api-key", previousStepMap.get("provider-api-key")),
  ...
  for (const stepId of ["provider-api-key", "mcp", "skills", "switch-agent", "switch-model", "first-chat"] as const) {
  ...
  const nextStateBase: GuidedOnboardingState = {
    ...
    lastActiveAt:
      ...
      candidate.lastActiveAt >= 0
        ...
        : now,
    currentStepId: null,
    steps: GUIDED_ONBOARDING_STEP_IDS.map((stepId) => steps.get(stepId) ?? createDefaultStepState(stepId)),
  };
...
const migrateVersion3State = (candidate: StoredGuidedOnboardingStateCandidate, now: number): GuidedOnboardingState => {
  const fallback = createDefaultState(now);
  const version3StepMap = resolveStepMap(VERSION3_GUIDED_ONBOARDING_STEP_IDS, candidate.steps);
  const version3Steps = {
    "select-provider": normalizeVersion3StepState("select-provider", version3StepMap.get("select-provider")),
    "provider-api-key": normalizeVersion3StepState("provider-api-key", version3StepMap.get("provider-api-key")),
...
const migrateLegacyState = (candidate: StoredGuidedOnboardingStateCandidate, now: number): GuidedOnboardingState => {
  const fallback = createDefaultState(now);
  ...
  const status =
    candidate.status === "active" || candidate.status === "completed" || candidate.status === "idle"
      ? candidate.status
      : fallback.status;
  const legacyCurrentStepId = isLegacyGuidedOnboardingStepId(candidate.currentStepId) ? candidate.currentStepId : null;
  const steps = new Map<GuidedOnboardingStepId, GuidedOnboardingStepState>(
  ...
  if (legacySteps["first-chat"].status !== "pending") {
    setStepState(steps, "first-chat", cloneStepStatus(legacySteps["first-chat"], legacySteps["first-chat"].status));
  }
  ...
  const nextStateBase: GuidedOnboardingState = {
    ...
    lastActiveAt:
      ...
      candidate.lastActiveAt >= 0
        ...
        : now,
    currentStepId: null,
    steps: SHARED_GUIDED_ONBOARDING_STEP_IDS.map((stepId) => steps.get(stepId) ?? createDefaultStepState(stepId)),
  };
...
const normalizeState = (raw: unknown, now: number): GuidedOnboardingState => {
  ...
  const steps = SHARED_GUIDED_ONBOARDING_STEP_IDS.map((stepId) => normalizeStepState(stepId, stepMap.get(stepId)));
...
const persistState = (configPresenter: ConfigPresenterPort, state: GuidedOnboardingState): GuidedOnboardingState => {
...
export function startGuidedOnboarding(
  ...
  const requestedStepId =
    options.stepId && SHARED_GUIDED_ONBOARDING_STEP_IDS.includes(options.stepId) ? options.stepId : undefined;
  ...
  const candidateStep = candidateStepId ? baseState.steps.find((step) => step.id === candidateStepId) : undefined;
  const nextStepId =
  ...
  const steps: GuidedOnboardingStepState[] = baseState.steps.map((step) => {
    ...
    return {
      ...step,
      status: step.status === "completed" || step.status === "skipped" ? step.status : ("in_progress" as const),
...
export function setGuidedOnboardingStepStatus(
  ...
  const nextStepId = input.status === "in_progress" ? input.stepId : findNextPendingStepId(nextStateBase);
...
export function resetGuidedOnboarding(configPresenter: ConfigPresenterPort, now = Date.now()): GuidedOnboardingState {
