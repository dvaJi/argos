...
export function readConfigEntries(
  ...
  const assignValue = <K extends ConfigEntryKey>(key: K, value: ConfigEntryValues[K] | undefined) => {
    if (value !== undefined) {
      values[key] = value;
    }
  };
  ...
  if (shouldRead("defaultModel")) {
    assignValue("defaultModel", configPresenter.getSetting<{ providerId: string; modelId: string }>("defaultModel"));
  }
  if (shouldRead("default_system_prompt")) {
    assignValue("default_system_prompt", configPresenter.getSetting<string>("default_system_prompt"));
  }
  ...
  if (shouldRead("artifact_think_collapse")) {
    assignValue("artifact_think_collapse", configPresenter.getSetting<boolean>("artifact_think_collapse"));
  }
  ...
  if (shouldRead("providerTimestamps")) {
    assignValue("providerTimestamps", configPresenter.getSetting<Record<string, number>>("providerTimestamps"));
  }
...
export function readVoiceAiConfig(configPresenter: IConfigPresenter): {
  ...
  return {
    audioFormat: configPresenter.getSetting<string>("voiceAI_audioFormat") ?? VOICE_AI_DEFAULTS.audioFormat,
    ...
    temperature: configPresenter.getSetting<number>("voiceAI_temperature") ?? VOICE_AI_DEFAULTS.temperature,
...
export function readGeminiSafety(configPresenter: IConfigPresenter, key: string): string {
  return configPresenter.getSetting<string>(`geminiSafety_${key}`) || "HARM_BLOCK_THRESHOLD_UNSPECIFIED";
}
...
export async function readAcpState(configPresenter: IConfigPresenter): Promise<{
  ...
  const [enabled, agents] = await Promise.all([configPresenter.getAcpEnabled(), configPresenter.getAcpAgents()]);
