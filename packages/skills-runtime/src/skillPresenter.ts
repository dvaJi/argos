...
export const SKILL_CONFIG = {
  ...
  SIDECAR_DIR: ".argos-meta",

  /** Draft skill configuration */
  DRAFT_ROOT_DIR: "argos-skill-drafts",
 ...
 *
 * Responsibilities:
 * - Discover and parse SKILL.md files from ~/.argos/skills/
...
export class SkillPresenter implements ISkillPresenter {
  ...
  private readonly ports: SkillHostPorts;
  ...
  constructor(
    ...
    ports: SkillHostPorts,
  ) {
    this.ports = ports;
    // Skills directory: ~/.argos/skills/
    this.skillsDir = this.resolveSkillsDir();
    this.sidecarDir = path.join(this.skillsDir, SKILL_CONFIG.SIDECAR_DIR);
    this.draftsRoot = path.join(ports.paths.tempDir(), SKILL_CONFIG.DRAFT_ROOT_DIR);
  ...
  private resolveSkillsDir(): string {
    ...
    const homePath = this.ports.paths.homeDir();
    const homeDir = homePath ? path.resolve(homePath) : path.resolve(".");
    const fallbackDir = path.join(homeDir, ".argos", "skills");
    ...
    const brokenPrefix = `${homeDir}.argos`;
    ...
    if (hasBrokenPrefix && hasBoundaryAfterPrefix) {
      ...
      return path.join(homeDir, ".argos", suffix);
    }
  ...
  private repairPortableDefaultSkillsPath(configuredPath: string, homeDir: string): string | null {
    ...
    const match =
      slashPath.match(/^\/Users\/[^/]+\/\.argos\/skills(?:\/(.*))?$/i) ??
      slashPath.match(/^[A-Za-z]:\/Users\/[^/]+\/\.argos\/skills(?:\/(.*))?$/i);
    ...
    return path.join(homeDir, ".argos", "skills", ...suffixParts);
  }
  ...
  async discoverSkills(): Promise<SkillMetadata[]> {
    ...
    try {
      const workerResult = await this.ports.services!.discoverMetadata!({
    ...
    this.ports.events.broadcast(SKILL_EVENTS.DISCOVERED, skills);
    this.ports.events.publish("skills.catalog.changed", {
      reason: "discovered",
      skills,
      version: Date.now(),
    });
  ...
  private async discoverExternalSkills(): Promise<SkillMetadata[]> {
    ...
    const homeDir = this.ports.paths.homeDir();
  ...
  private getBuiltinSkillsDirCandidates(): string[] {
    return this.ports.paths.bundledSkillRoots();
  }
  ...
  async installFromZip(zipPath: string, options?: SkillInstallOptions): Promise<SkillInstallResult> {
    ...
    const tempDir = fs.mkdtempSync(path.join(this.ports.paths.tempDir(), "argos-skill-"));
    try {
  ...
  async installFromUrl(url: string, options?: SkillInstallOptions): Promise<SkillInstallResult> {
    const tempZipPath = path.join(this.ports.paths.tempDir(), `argos-skill-${Date.now()}.zip`);
    try {
  ...
  private async installFromDirectory(folderPath: string, options?: SkillInstallOptions): Promise<SkillInstallResult> {
    try {
      ...
      this.ports.events.broadcast(SKILL_EVENTS.INSTALLED, { name: skillName });
      this.ports.events.publish("skills.catalog.changed", {
        reason: "installed",
        name: skillName,
        version: Date.now(),
  ...
  async uninstallSkill(name: string): Promise<SkillInstallResult> {
    try {
      ...
      this.ports.events.broadcast(SKILL_EVENTS.UNINSTALLED, { name });
      this.ports.events.publish("skills.catalog.changed", {
        reason: "uninstalled",
        name,
        version: Date.now(),
  ...
  async openSkillsFolder(): Promise<void> {
    this.ensureSkillsDir();
    await this.ports.services?.openPath?.(this.skillsDir);
  }
  ...
  async setActiveSkills(conversationId: string, skills: string[]): Promise<string[]> {
    try {
      ...
      if (activated.length > 0) {
        this.ports.events.broadcast(SKILL_EVENTS.ACTIVATED, {
          conversationId,
          skills: activated,
        });
        this.ports.events.publish("skills.session.changed", {
          conversationId,
          skills: activated,
          change: "activated",
          version: Date.now(),
      ...
      if (deactivated.length > 0) {
        this.ports.events.broadcast(SKILL_EVENTS.DEACTIVATED, {
          conversationId,
          skills: deactivated,
        });
        this.ports.events.publish("skills.session.changed", {
  ...
  watchSkillFiles(): void {
    ...
    this.watcher.on("change", async (filePath: string) => {
      if (path.basename(filePath) === "SKILL.md") {
        ...
        if (metadata) {
          ...
          this.metadataCache.set(metadata.name, metadata);
          this.ports.events.broadcast(SKILL_EVENTS.METADATA_UPDATED, metadata);
          this.ports.events.publish("skills.catalog.changed", {
    ...
    this.watcher.on("add", async (filePath: string) => {
      if (path.basename(filePath) === "SKILL.md") {
        ...
        if (metadata) {
          ...
          this.metadataCache.set(metadata.name, metadata);
          this.ports.events.broadcast(SKILL_EVENTS.INSTALLED, {
            name: metadata.name,
          });
          this.ports.events.publish("skills.catalog.changed", {
            reason: "installed",
    ...
    this.watcher.on("unlink", (filePath: string) => {
      if (path.basename(filePath) === "SKILL.md") {
        ...
        this.metadataCache.delete(skillName);
        this.contentCache.delete(skillName);
        this.ports.events.broadcast(SKILL_EVENTS.UNINSTALLED, {
          name: skillName,
        });
        this.ports.events.publish("skills.catalog.changed", {
          reason: "uninstalled",
