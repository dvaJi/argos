...
export function MermaidBlock({ node, className }: MermaidBlockProps) {
  ...
  if (!svg) {
    return (
      <div className={`flex items-center justify-center p-4 text-muted-foreground text-xs ${className ?? ""}`}>
        Loading diagram...
      </div>
    );
  }
  ...
  return (
    <div ref={containerRef} className={`overflow-auto ${className ?? ""}`} dangerouslySetInnerHTML={{ __html: svg }} />
  );
}
