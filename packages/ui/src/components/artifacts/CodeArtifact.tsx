...
export function CodeArtifact({ block, messageId, threadId, className }: CodeArtifactProps) {
  ...
  const isPreviewable = useMemo(() => codeLanguage === "html" || codeLanguage === "svg", [codeLanguage]);
  ...
  useEffect(() => {
    ...
    if (editor) editor.updateOptions({ fontFamily: getFormattedCodeFontFamily() });
  }, []);

  useEffect(() => {
    getEditorView()?.updateOptions({ fontFamily: getFormattedCodeFontFamily() });
  ...
  if (isMermaid) {
    return <pre className={`text-xs p-3 bg-muted rounded-lg overflow-auto ${className ?? ""}`}>{block.content}</pre>;
  }

  return (
    <div className={`m-4 rounded-lg border border-border overflow-hidden shadow-sm ${className ?? ""}`}>
      <div className="flex justify-between items-center p-2 bg-muted text-xs">
        <span className="flex items-center space-x-2">
          <Icon icon={languageIcon} className="w-4 h-4" />
          <span className="text-gray-600 dark:text-gray-400 font-mono font-bold">{displayLanguage}</span>
        </span>
        {isPreviewable ? (
