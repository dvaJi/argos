...
function estimateHeight(msg: MessageListItem): number {
  ...
  if (msg.role === "user") {
    ...
    return clamp(USER_BASE + Math.ceil(Math.max(textLen, richLen) / CHARS_PER_LINE) * LINE_H + files * 34);
  }
  ...
  for (const block of msg.content) {
    switch (block.type) {
      case "content":
        h += Math.max(
          48,
          Math.ceil((typeof block.content === "string" ? block.content.length : 0) / CHARS_PER_LINE) * LINE_H,
        );
        break;
      case "tool_call":
        h += 150;
