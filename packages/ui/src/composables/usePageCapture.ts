...
// Core capture routine (module-scope so the hook's callback stays free of
// component-scope closures). `onScrollConfigured` reports the container plus
// its original scroll-behavior once the override has been applied, so the
// caller can always restore it afterwards.
const performCapture = async (
  config: CaptureConfig,
  tabClient: ReturnType<typeof createTabClient>,
  onScrollConfigured: (scrollContainer: HTMLElement, originalScrollBehavior: string) => void,
