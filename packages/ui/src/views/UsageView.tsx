...
function deriveUsageViewData(data: UsageStatsOutput, selectedService: string | undefined) {
...
function UsageWindowSwitcher({
  window: selectedWindow,
  onSelect,
}: {
  window: UsageWindow;
  onSelect: (next: UsageWindow) => void;
}) {
  return (
    <div className="flex items-center gap-1 rounded-lg border border-border bg-card p-1">
      {WINDOWS.map((item) => (
        <button
          ...
          className={`usage-press rounded-md px-3 py-1.5 text-xs font-medium transition-colors ${selectedWindow === item.id ? "bg-accent text-foreground" : "text-muted-foreground hover:text-foreground"}`}
        >
          {item.label}
        </button>
      ))}
...
function UsageErrorBanner({ error, isLoading, onRetry }: { error: string; isLoading: boolean; onRetry: () => void }) {
  return (
  ...
  return (
    <Empty>
      <EmptyHeader>
        ...
        <EmptyDescription>
          Run a conversation with an agent to start tracking token usage and cost. Usage appears here once sessions
          finish.
        </EmptyDescription>
      </EmptyHeader>
    </Empty>
...
function CostSummaryCard({
  ...
  summary: UsageStatsOutput["summary"];
  dataKey: number;
  isLoading: boolean;
  onRefresh: () => void;
}) {
      ...
      <CardContent className="flex flex-col justify-between gap-6 p-6 lg:flex-row lg:items-end">
        <div>
          <div className="flex items-center gap-2">
            <p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Raw token cost</p>
          </div>
          <div
            key={`value-${dataKey}`}
...
function ServiceShareList({
  services,
  selectedService,
  onSelectService,
}: {
  services: ReturnType<typeof deriveUsageViewData>["services"];
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <button
        type="button"
        onClick={() => onSelectService(undefined)}
...
function DailyUsageChartCardHeader({
  chartMode,
  onChartModeChange,
}: {
...
function DailyUsageChartBody({
  chartSeries,
  hasChartActivity,
  chartMode,
}: {
  chartSeries: ReturnType<typeof deriveUsageViewData>["chartSeries"];
  hasChartActivity: boolean;
  chartMode: "cost" | "tokens";
}) {
  if (hasChartActivity) {
    return <DailyUsageChart points={chartSeries} mode={chartMode} />;
  }
  return (
...
function UsageMetricStrip({ summary }: { summary: UsageStatsOutput["summary"] }) {
  return (
    <>
      <Metric
      ...
      <Metric
        ...
        hint={summary.reasoningTokens > 0 ? `includes ${formatTokens(summary.reasoningTokens)} reasoning` : undefined}
      />
      <Metric
...
function ModelBreakdownRow({ item }: { item: ReturnType<typeof deriveUsageViewData>["modelBreakdown"][number] }) {
  return (
    <TableRow>
      <TableCell>
        <div className="flex min-w-0 items-center gap-2.5">
          <ModelIcon modelId={item.id} customClass="h-4 w-4 shrink-0" isDark={themeStore.state.isDark} />
          <span className="truncate font-mono text-xs">{item.label}</span>
        </div>
      </TableCell>
      <TableCell>
        <div className="flex min-w-0 items-center gap-2">
          <ModelIcon modelId={item.providerId} customClass="h-3.5 w-3.5 shrink-0" isDark={themeStore.state.isDark} />
          <span className="truncate text-xs text-muted-foreground">{harnessLabel(item.providerId)}</span>
        </div>
      </TableCell>
...
function ModelBreakdownTable({
  ...
  modelBreakdown: ReturnType<typeof deriveUsageViewData>["modelBreakdown"];
}) {
  return (
    <>
...
export default function UsageView() {
  ...
  const filtered = data ? deriveUsageViewData(data, selectedService) : null;
    ...
    <div className="h-full w-full overflow-y-auto">
      <div className="mx-auto flex w-full max-w-6xl flex-col gap-8 p-6 lg:p-8">
        ...
        <header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          ...
          <UsageWindowSwitcher window={window} onSelect={selectWindow} />
        </header>

        {error && <UsageErrorBanner error={error} isLoading={isLoading} onRetry={() => void load(window)} />}

        {isLoading && !data && <UsageLoadingSkeleton />}

        {data && !hasData && !isLoading && <UsageEmptyState />}
          ...
          <>
            ...
            <Card className="usage-reveal" key={`chart-${dataKey}`}>
              <CardHeader className="flex-row items-center justify-between space-y-0">
                <DailyUsageChartCardHeader chartMode={chartMode} onChartModeChange={setChartMode} />
              </CardHeader>
              <CardContent>
                <DailyUsageChartBody
            ...
            <section
              ...
              key={`metrics-${dataKey}`}
            >
              <UsageMetricStrip summary={summary} />
            </section>
