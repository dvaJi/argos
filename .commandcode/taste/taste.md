...
# Architecture
- Structure the project with separate core/daemon (CLI backend as an npm package), UI (frontend app), and shell (Electron/Tauri) layers, following the model of CodeNomad/NeuralNomadsAI. Confidence: 0.75
- Place exported CLI packages (standalone binaries that users can run) in `apps/`, not `packages/`, following Turborepo conventions — things in `apps/` are deployable/exported artifacts, while `packages/` are internal libraries consumed by apps. Confidence: 0.60

# Error Handling
- NOOP stubs for unimplemented features should return a distinguishable error (e.g., an "unimplemented" marker or descriptive message), not silently return `null`. This makes it clear the feature is not yet wired rather than hiding a potential bug. Confidence: 0.75
