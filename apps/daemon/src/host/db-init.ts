...
const CORE_TABLES = [
  ...
  `CREATE TABLE IF NOT EXISTS deepchat_sessions (
    ...
    agent_id TEXT NOT NULL DEFAULT 'deepchat',
    agent_type TEXT NOT NULL DEFAULT 'deepchat',
    project_dir TEXT,
    model_id TEXT,
    provider_id TEXT,
  ...
  `CREATE TABLE IF NOT EXISTS deepchat_messages (
    ...
    created_at INTEGER NOT NULL,
    updated_at INTEGER NOT NULL,
    FOREIGN KEY (session_id) REFERENCES deepchat_sessions(id) ON DELETE CASCADE
  )`,
  ...
  `CREATE TABLE IF NOT EXISTS deepchat_user_messages (
    ...
    FOREIGN KEY (message_id) REFERENCES deepchat_messages(id) ON DELETE CASCADE,
    FOREIGN KEY (session_id) REFERENCES deepchat_sessions(id) ON DELETE CASCADE
  )`,
...
const INDEXES = [
  `CREATE INDEX IF NOT EXISTS idx_deepchat_messages_session ON deepchat_messages(session_id)`,
  `CREATE INDEX IF NOT EXISTS idx_deepchat_messages_role ON deepchat_messages(role)`,
  `CREATE INDEX IF NOT EXISTS idx_deepchat_user_messages_session ON deepchat_user_messages(session_id)`,
  `CREATE INDEX IF NOT EXISTS idx_deepchat_assistant_blocks_session ON deepchat_assistant_blocks(session_id)`,
  `CREATE INDEX IF NOT EXISTS idx_deepchat_assistant_blocks_message ON deepchat_assistant_blocks(message_id)`,
...
function runMigrations(_db: BunDatabase, _currentVersion: number): void {
  ...
  // Example:
  // if (currentVersion < 2) {
  //   db.exec("ALTER TABLE deepchat_sessions ADD COLUMN priority INTEGER DEFAULT 0");
  // }
}
