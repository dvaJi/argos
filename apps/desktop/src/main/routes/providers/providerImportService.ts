...
const CC_SWITCH_APP_TYPES = ["claude", "claude-desktop", "gemini", "opencode", "openclaw", "hermes"] as const;
...
const hasHttpBaseUrl = (value: string | undefined): boolean => /^https?:\/\//i.test(normalizeUrl(value));

const normalizeName = (value: string | undefined): string => normalizeToken(value).replace(/[^a-z0-9-]/g, "");
...
const replaceDisallowedControlCharacters = (value: string): string => {
  ...
  for (const character of value) {
    ...
    result += codePoint < 32 && codePoint !== 9 && codePoint !== 10 && codePoint !== 13 ? " " : character;
  }
  return result;
};
...
const buildWindowsDisplayPath = (base: "%APPDATA%" | "%USERPROFILE%" | "%HOME%", relativePath: string) =>
...
const toUnixRelativeDisplayPath = (relativePath: string): string => relativePath.split(path.sep).join("/");

const isSupportedScanPlatform = (platform: NodeJS.Platform): platform is "darwin" | "linux" | "win32" =>
...
export class ProviderImportService {
  ...
  async scan(): Promise<ProviderImportScanResult> {
    ...
    const results = await Promise.all(SOURCE_DEFINITIONS.map((definition) => this.readSource(definition)));
  ...
  apply(selections: { sessionId: string; selections: ProviderImportSelection[] }): ProviderImportApplyResult {
    this.pruneSessions();
    ...
    const selectedProviders = PROVIDER_IMPORT_SOURCE_IDS.flatMap((sourceId) => {
      ...
      return session.rawProviders.filter((provider) => provider.sourceId === sourceId && selectedIds.has(provider.id));
    });
    ...
    const reservedProviderIds = new Set(this.configPresenter.getProviders().map((provider) => provider.id));
    ...
    for (const rawProvider of selectedProviders) {
      ...
      if (mapping.targetKind === "unsupported" || !this.hasRequiredCredentials(rawProvider, mapping)) {
        resultByProviderId.set(rawProvider.id, this.buildResult(rawProvider, mapping, "skipped"));
        continue;
      }

      const planned = this.planProvider(rawProvider, mapping, reservedProviderIds, plannedCustomFingerprints);
      ...
      resultByProviderId.set(rawProvider.id, this.buildResult(rawProvider, mapping, "skipped", planned));
    }
  ...
  private async readSource(definition: SourceDefinition): Promise<SourceReadResult> {
    ...
    try {
      ...
      const previewPairs = readableProviders
        ...
        .filter(({ provider, preview }) => provider.sourceId !== "cc-switch" || preview.targetKind !== "unsupported");
  ...
  private async readProviders(definition: SourceDefinition, sourcePath: string): Promise<ProviderImportRawProvider[]> {
  ...
  private async readCherryStudio(definition: SourceDefinition, dbPath: string): Promise<ProviderImportRawProvider[]> {
  ...
  private createLevelDbSnapshot(dbPath: string): string | null {
    const snapshotPath = fs.mkdtempSync(path.join(os.tmpdir(), "argos-cherry-import-"));
    try {
  ...
  private readHermes(definition: SourceDefinition, configPath: string): ProviderImportRawProvider[] {
    const config = parseYaml(fs.readFileSync(configPath, "utf8"));
    const providers = config?.llm?.providers;
    return this.readYamlProviders(definition, Array.isArray(providers) ? providers : []);
  }

  private readOpenClaw(definition: SourceDefinition, configPath: string): ProviderImportRawProvider[] {
    const config = parseYaml(fs.readFileSync(configPath, "utf8"));
    const providers = config?.providers;
    return this.readYamlProviders(definition, Array.isArray(providers) ? providers : []);
  }

  private readYamlProviders(definition: SourceDefinition, providers: unknown[]): ProviderImportRawProvider[] {
    return providers.flatMap((provider, index) => {
      if (!provider || typeof provider !== "object") return [];
      const item = provider as Record<string, unknown>;
  ...
  private parseCcSwitchClaudeConfig(settingsConfig: Record<string, unknown>, meta: Record<string, unknown>) {
  ...
  private normalizeSourceApiType(value: string): string {
    ...
    if (normalized === "anthropic" || normalized === "anthropic-messages" || normalized === "anthropic-messages-api") {
      return "anthropic";
    }
    if (normalized === "gemini" || normalized === "gemini-native" || normalized === "google-generative-ai") {
      return "gemini";
    }
    if (
    ...
    if (normalized === "bedrock-converse" || normalized === "bedrock-converse-stream" || normalized === "aws-bedrock") {
      return "aws-bedrock";
    }
    return normalized;
  }
  ...
  private toPreview(rawProvider: ProviderImportRawProvider): ProviderImportProviderPreview {
    ...
    const warnings = [
      ...
      ...(!hasCredentials && mapping.targetKind !== "unsupported" ? (["missing_api_key"] as const) : []),
  ...
  private hasRequiredCredentials(rawProvider: ProviderImportRawProvider, mapping: ProviderImportMapping): boolean {
  ...
  private isConfigured(rawProvider: ProviderImportRawProvider, mapping: ProviderImportMapping): boolean {
    const providers = this.configPresenter.getProviders();
    ...
    if (mapping.targetKind === "custom") {
      ...
      return providers.some(
        (provider) =>
          provider.custom && fingerprint && this.getFingerprint(provider.baseUrl, provider.apiKey) === fingerprint,
      );
    }
  ...
  private buildCustomProviderBaseId(rawProvider: ProviderImportRawProvider): string {
    const rawId = normalizeToken(rawProvider.sourceProviderId || rawProvider.name).replace(/[^a-z0-9-]/g, "-");
  ...
  private planProvider(
    ...
    reservedProviderIds = new Set(this.configPresenter.getProviders().map((provider) => provider.id)),
    plannedCustomFingerprints = new Map<string, string>(),
  ): ProviderImportPlannedProvider {
    const providers = this.configPresenter.getProviders();
    const targetProviderId =
      mapping.targetKind === "custom"
        ? this.resolveCustomProviderId(rawProvider, mapping, providers, reservedProviderIds, plannedCustomFingerprints)
    ...
    const provider: LLM_PROVIDER = current
      ? {
          ...
          baseUrl: mapping.importMode === "credentials_only" ? current.baseUrl : rawProvider.baseUrl || current.baseUrl,
          enable: true,
        }
      : {
          id: targetProviderId,
    ...
    return {
      ...
      provider,
      models:
        mapping.importMode === "credentials_only" ? [] : this.buildModelMeta(targetProviderId, rawProvider.models),
    };
  }
  ...
  private applyPlannedProviders(plannedProviders: ProviderImportPlannedProvider[]): ProviderImportApplyResultItem[] {
  ...
  private buildResult(
    ...
    return {
      ...
      targetKind: mapping.targetKind,
      targetProviderId,
      targetProviderName: mapping.targetKind === "custom" ? rawProvider.name : mapping.targetProviderName,
      status,
  ...
  private resolveCcSwitchDesktopAppPathsPath(): string {
    if (this.platform === "win32") {
      return path.join(this.appDataDir, CC_SWITCH_DESKTOP_CONFIG_DIR_NAME, CC_SWITCH_DESKTOP_APP_PATHS_FILE_NAME);
    }

    if (this.platform === "linux") {
      const xdgConfigDir = (process.env.XDG_CONFIG_HOME ?? "").trim();
      const configBaseDir = xdgConfigDir || path.join(this.homeDir, ".config");
      return path.join(configBaseDir, CC_SWITCH_DESKTOP_CONFIG_DIR_NAME, CC_SWITCH_DESKTOP_APP_PATHS_FILE_NAME);
    }
