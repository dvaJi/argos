...
function resolvePricedModel(providerId: string, modelId: string): ProviderModel | undefined {
  return providerDbLoader.getModel(providerId, modelId) ?? providerDbLoader.getModel("aihubmix", modelId);
}
...
export function normalizeUsageStatsBackfillStatus(value: unknown): UsageStatsBackfillStatus {
  ...
  const status =
    input.status === "running" || input.status === "completed" || input.status === "failed" || input.status === "idle"
      ? input.status
      : "idle";
...
export function isUsageBackfillRunningStale(status: UsageStatsBackfillStatus, now = Date.now()): boolean {
...
export function normalizeUsageCounts(metadata: MessageMetadata): {
  ...
  const cacheWriteInputTokens = inputTokens > 0 ? Math.min(rawCacheWrite, remainingInputTokens) : rawCacheWrite;
...
export function resolveUsageProviderId(metadata: MessageMetadata, fallbackProviderId?: string | null): string | null {
  return normalizeTextId(metadata.provider) ?? normalizeTextId(fallbackProviderId) ?? null;
}

export function resolveUsageModelId(metadata: MessageMetadata, fallbackModelId?: string | null): string | null {
...
export function getModelLabel(providerId: string, modelId: string): string {
  const model = providerDbLoader.getModel(providerId, modelId) ?? providerDbLoader.getModel("aihubmix", modelId);
