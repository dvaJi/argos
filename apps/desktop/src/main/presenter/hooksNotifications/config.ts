...
const normalizeHookItem = (input: LooseHookCommandItem | undefined, index: number): HookCommandItem => ({
  id: typeof input?.id === "string" && input.id.trim() ? input.id.trim() : randomUUID(),
  name: typeof input?.name === "string" && input.name.trim() ? input.name.trim() : `Hook ${index + 1}`,
...
export const normalizeHooksNotificationsConfig = (input: unknown): HooksNotificationsSettings => {
  ...
  if (isRecord(input) && Array.isArray(input.hooks)) {
    input.hooks.forEach((item, index) => {
      warnUnknownKeys(`hooksNotifications.hooks[${index}]`, item, ["id", "name", "enabled", "command", "events"]);
    });
  }
  ...
  const hooks = rawHooks.reduce<HookCommandItem[]>((items, item, index) => {
    ...
    if (!parsedItem.success) {
      log.warn(`[HooksNotifications] Invalid hook at index ${index}, skipping: ${parsedItem.error?.message}`);
      return items;
    }
