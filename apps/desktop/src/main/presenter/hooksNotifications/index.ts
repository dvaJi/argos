...
const HOOK_COMMAND_PLACEHOLDER_ENV_MAP = {
  event: "ARGOS_HOOK_EVENT",
  time: "ARGOS_HOOK_TIME",
  isTest: "ARGOS_HOOK_IS_TEST",
  conversationId: "ARGOS_CONVERSATION_ID",
  workdir: "ARGOS_WORKDIR",
  agentId: "ARGOS_AGENT_ID",
  providerId: "ARGOS_PROVIDER_ID",
  modelId: "ARGOS_MODEL_ID",
  messageId: "ARGOS_MESSAGE_ID",
  toolName: "ARGOS_TOOL_NAME",
  toolCallId: "ARGOS_TOOL_CALL_ID",
} as const;
...
export const expandHookCommandPlaceholders = (command: string, platform: NodeJS.Platform = process.platform): string =>
  command.replace(/{{\s*([a-zA-Z][a-zA-Z0-9]*)\s*}}/g, (match, key: string) => {
    const envName = HOOK_COMMAND_PLACEHOLDER_ENV_MAP[key as keyof typeof HOOK_COMMAND_PLACEHOLDER_ENV_MAP];
    if (!envName) {
      return match;
    }
...
const redactSensitiveText = (text: string, secrets: string[]): string => {
  ...
  output = output.replace(/https?:\/\/(discord(?:app)?\.com)\/api\/webhooks\/\S+/gi, "***REDACTED***");
...
export class HooksNotificationsService {
  ...
  private async dispatchEventAsync(event: HookEventName, context: HookDispatchContext): Promise<void> {
  ...
  private async buildPayload(event: HookEventName, context: HookDispatchContext): Promise<HookEventPayload> {
    ...
    return {
      ...
      tool: context.tool
        ? {
            ...
            paramsPreview: context.tool.params ? truncateText(context.tool.params, PREVIEW_TEXT_LIMIT) : undefined,
            responsePreview: context.tool.response
              ? truncateText(context.tool.response, PREVIEW_TEXT_LIMIT)
              : undefined,
            error: context.tool.error ? truncateText(context.tool.error, PREVIEW_TEXT_LIMIT) : undefined,
          }
        : null,
  ...
  private async runHookCommand(hook: HookCommandItem, payload: HookEventPayload): Promise<HookTestResult> {
  ...
  private async executeHookCommand(command: string, payload: HookEventPayload): Promise<HookCommandResult> {
    ...
    const env: Record<string, string> = {
      ...process.env,
      ARGOS_HOOK_EVENT: payload.event,
      ARGOS_HOOK_TIME: payload.time,
      ARGOS_HOOK_IS_TEST: payload.isTest ? "true" : "false",
      ARGOS_CONVERSATION_ID: payload.session.conversationId ?? "",
      ARGOS_WORKDIR: payload.session.workdir ?? "",
      ARGOS_AGENT_ID: payload.session.agentId ?? "",
      ARGOS_PROVIDER_ID: payload.session.providerId ?? "",
      ARGOS_MODEL_ID: payload.session.modelId ?? "",
      ARGOS_MESSAGE_ID: payload.user?.messageId ?? "",
      ARGOS_TOOL_NAME: payload.tool?.name ?? "",
      ARGOS_TOOL_CALL_ID: payload.tool?.callId ?? "",
    };
