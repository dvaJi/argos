...
const SUPPORTED_LANGUAGES = new Set(["en", "ca", "sv", "es", "fr", "de", "it", "pt", "pl", "ru", "nl"]);
...
export class VoiceAIProvider extends BaseLLMProvider {
  ...
  public async *coreStream(
    ...
    try {
      const { audioBase64, mimeType } = await this.generateSpeech(text, modelId, temperature, modelConfig);
  ...
  private getTtsConfig(): VoiceAITtsConfig {
    const audioFormat = this.configPresenter.getSetting<string>("voiceAI_audioFormat") || DEFAULT_AUDIO_FORMAT;
    ...
    return {
      ...
      model,
      language,
      temperature: typeof temperatureSetting === "number" ? temperatureSetting : DEFAULT_TEMPERATURE,
  ...
  private async resolveAudioFromJson(
    ...
    const rootMimeType = this.pickString(data, ["mime_type", "content_type", "contentType"]) || fallbackMimeType;

    const audioField = data.audio;
    if (audioField && typeof audioField === "object") {
      const audioData = audioField as Record<string, unknown>;
      const audioMimeType = this.pickString(audioData, ["mime_type", "content_type", "contentType"]) || rootMimeType;
      const audioValue =
  ...
  private async generateSpeech(
    ...
    if (!SUPPORTED_LANGUAGES.has(config.language)) {
      throw new Error(
        `Unsupported language code: ${config.language}. Supported languages: ${Array.from(SUPPORTED_LANGUAGES).join(
          ", ",
        )}`,
      );
    }
