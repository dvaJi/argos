...
export class OllamaProvider extends BaseLLMProvider {
  ...
  private alignModelsWithCliList(sdkModels: OllamaModel[], cliModels: OllamaModel[]): OllamaModel[] {
  ...
  private mergeModelInfo(
    ...
    const mergedVisionEmbeddingLength = primary?.vision?.embedding_length ?? secondary?.vision?.embedding_length;
    const mergedVision =
  ...
  private resolveOllamaModelMeta(model: OllamaModel, cachedModel?: MODEL_META): MODEL_META {
    ...
    const resolvedType = capabilitySet.has("embedding") ? ModelType.Embedding : (cachedModel?.type ?? ModelType.Chat);
    ...
    return {
      ...
      contextLength: model.model_info?.context_length ?? cachedModel?.contextLength ?? DEFAULT_MODEL_CONTEXT_LENGTH,
  ...
  protected async fetchProviderModels(): Promise<MODEL_META[]> {
    try {
      const [localModels, runningModels] = await Promise.all([this.listModels(), this.listRunningModels()]);
      ...
      for (const runningModel of runningModels) {
        ...
        const merged = existing ? this.mergeOllamaModels(existing, runningModel) : this.mergeOllamaModels(runningModel);
    ...
    try {
      ...
      const context_length =
        this.findModelInfoNumber(
          ...
          (key) => key.endsWith(".context_length") && !key.includes(".vision.") && !key.includes(".audio."),
        ) ?? DEFAULT_MODEL_CONTEXT_LENGTH;
      const embedding_length =
        this.findModelInfoNumber(
          entries,
          exactPrefixes.map((prefix) => `${prefix}.embedding_length`),
          (key) => key.endsWith(".embedding_length") && !key.includes(".vision.") && !key.includes(".audio."),
        ) ?? 512;
      ...
      const general = {
        ...
        ...(typeof quantizationVersion === "number" ? { quantization_version: quantizationVersion } : {}),
      };
      ...
      console.warn(`Failed to get info for model ${model.name}, using defaults:`, (error as Error).message);
      return {
        ...model,
        model_info: {
          context_length: 4096,
  ...
  public async listModels(): Promise<OllamaModel[]> {
    ...
    try {
      ...
      const enrichedModels = await Promise.all(models.map(async (model) => this.attachModelInfo(model)));
  ...
  public async pullModel(modelName: string, onProgress?: (progress: ProgressResponse) => void): Promise<boolean> {
