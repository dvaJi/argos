  ...
  async syncModelScopeMcpServers(
    ...
    try {
      const syncTask = async () => {
        ...
        const convertedServers = mcpServers
          .map<ConvertedServer | null>((server) => {
            try {
              ...
              console.log(`[ModelScope MCP Sync] Converted operational server: ${displayName} (${name})`);
        ...
        console.log(`[ModelScope MCP Sync] Successfully converted ${convertedServers.length} servers`);
        ...
        for (const serverEntry of convertedServers) {
          try {
            ...
            const success = await this.options.configPresenter.addMcpServer(serverName, serverEntry.config);
            if (success) {
              console.log(`[ModelScope MCP Sync] Successfully imported server: ${serverEntry.displayName}`);
              result.imported++;
            } else {
