...
export class RateLimitManager {
  ...
  initializeProviderRateLimitConfigs(): void {
    ...
    console.log(`[RateLimitManager] Initialized rate limit configs for ${providers.length} providers`);
  }

  updateProviderRateLimit(providerId: string, enabled: boolean, qpsLimit: number): void {
    let finalConfig = { enabled, qpsLimit };
    if (finalConfig.qpsLimit !== undefined && (finalConfig.qpsLimit <= 0 || !isFinite(finalConfig.qpsLimit))) {
  ...
  async executeWithRateLimit(providerId: string, options?: ExecuteWithRateLimitOptions): Promise<void> {
    ...
    return new Promise<void>((resolve, reject) => {
      ...
      console.log(`[RateLimitManager] Request queued for ${providerId}, queue length: ${state.queue.length}`);
  ...
  private async processRateLimitQueue(providerId: string): Promise<void> {
    ...
    try {
      ...
      console.error(`[RateLimitManager] Error processing rate limit queue for ${providerId}:`, error);
      while (state.queue.length > 0) {
        const queueItem = state.queue.shift();
        if (queueItem) {
  ...
  private buildQueueSnapshot(providerId: string, state: ProviderRateLimitState): RateLimitQueueSnapshot {
    const intervalMs = (1 / state.config.qpsLimit) * 1000;
    const nextAllowedTime = state.lastRequestTime + intervalMs;
