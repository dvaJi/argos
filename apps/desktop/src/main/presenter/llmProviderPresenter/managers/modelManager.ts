...
export class ModelManager {
  ...
  async getModelList(providerId: string): Promise<MODEL_META[]> {
    ...
    console.log(`[ModelManager] getModelList: received ${models.length} models from provider "${providerId}"`);
    ...
    models = models.map((model) => {
      ...
      if (model.providerId && model.providerId !== providerId) {
        ...
        console.warn(`[ModelManager] getModelList: Model ${model.id} missing providerId, setting to "${providerId}"`);
        model.providerId = providerId;
      }
  ...
  async batchUpdateModelStatusQuiet(providerId: string, statusMap: Record<string, boolean>): Promise<void> {
  ...
  async getCustomModels(providerId: string): Promise<MODEL_META[]> {
    try {
      ...
      console.warn(`Failed to get custom models from provider instance ${providerId}, falling back to config:`, error);
