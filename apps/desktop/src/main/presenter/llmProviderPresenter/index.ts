...
export class LLMProviderPresenter implements ILlmProviderPresenter {
  ...
  async setCurrentProvider(providerId: string): Promise<void> {
    // If there are any active streams, stop them first
    ...
    this.currentProviderId = providerId;
    // Ensure the new provider instance is initialized
  ...
  async batchUpdateModelStatus(providerId: string, updates: { modelId: string; enabled: boolean }[]): Promise<void> {
  ...
  /**
   * Update the provider's rate limit configuration
   */
  ...
  /**
   * Get the rate limit status of all providers
   */
  ...
  private async stopAllStreams(): Promise<void> {
    const promises = Array.from(this.activeStreams.keys()).map((eventId) => this.stopStream(eventId));
  ...
  async generateCompletionStandalone(
    ...
    try {
      const llmResponse = await (abortPromise ? Promise.race([completionPromise, abortPromise]) : completionPromise);
  ...
  async transcribeAudioStandalone(
    ...
    try {
      return (
        await provider.transcribeAudio(modelId, normalizedAudioBase64, normalizedMimeType, filename, {
          signal,
        })
      ).trim();
    } catch (error) {
  ...
  async generateVideoStandalone(
    ...
    const collect = async () => {
      for await (const event of stream) {
        ...
        if (event.type === "image_data" && event.image_data.mimeType.trim().toLowerCase().startsWith("video/")) {
          videos.push({
            data: event.image_data.data,
            mimeType: event.image_data.mimeType,
          });
  ...
  async check(providerId: string, modelId?: string): Promise<{ isOk: boolean; errorMsg: string | null }> {
    try {
      const provider = this.getProviderInstance(providerId);

      // If modelId is provided, test it via the completions method
      if (modelId) {
        try {
          ...
          if (response && (response.content || response.content === "" || response.reasoning_content)) {
        ...
        // If no modelId is provided, fall back to the provider's own check method for basic verification
        console.log(`[LLMProviderPresenter] No modelId provided, using provider's own check method for ${providerId}`);
        try {
  ...
  async updateCustomModel(providerId: string, modelId: string, updates: Partial<MODEL_META>): Promise<boolean> {
  ...
  // Get the OllamaProvider instance
   ...
   * @param providerId the provider ID
   * @param modelId the model ID
   * @returns the model's embedding dimensions
   */
  async getDimensions(providerId: string, modelId: string): Promise<{ data: LLM_EMBEDDING_ATTRS; errorMsg?: string }> {
