...
import { buildCapabilitySnapshot, buildClientCapabilities, type AcpCapabilitySnapshot } from "./acpCapabilities";
...
export class AcpProcessManager implements AgentProcessManager<AcpProcessHandle, AcpAgentConfig> {
  ...
  private getFallbackWorkdir(): string {
    const tempDir = path.join(app.getPath("temp"), "argos-acp", "sessions");
    try {
  ...
  private async spawnProcessOnce(
    ...
    try {
      const initPayload = {
        ...
        clientInfo: { name: "Argos", version: app.getVersion() },
      };
      this.debugLog.append(agent.id, {
        kind: "request",
        action: "initialize",
  ...
  private async spawnAgentProcess(
    ...
    const processedCommand = this.runtimeHelper.replaceWithRuntimeCommand(expandedCommand, useBundledRuntime, true);
