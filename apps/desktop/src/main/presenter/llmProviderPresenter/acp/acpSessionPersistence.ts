...
export class AcpSessionPersistence {
  ...
  async mergeMetadata(conversationId: string, agentId: string, metadata: Record<string, unknown>): Promise<void> {
  ...
  async syncRemoteSessions(input: AcpRemoteSessionSyncInput): Promise<AcpRemoteSessionSyncResult> {
    ...
    for (const remoteSession of input.sessions) {
      ...
      const item = await this.withRemoteSessionSyncLock(input.agentId, remoteSession.sessionId, () =>
  ...
  private async syncRemoteSession(
    ...
    if (existing) {
      return this.updateRemoteSessionLink(input, remoteSession, existing, sessionWorkdir, metadata, syncedAt);
    }
    ...
    try {
      await this.saveSessionData(conversationId, input.agentId, remoteSession.sessionId, sessionWorkdir, "idle", {
        ...metadata,
        acpSync: {
          importedAt: syncedAt,
      ...
      await this.deleteConversationSilently(conversationId);
      return this.updateRemoteSessionLink(input, remoteSession, concurrentExisting, sessionWorkdir, metadata, syncedAt);
    }
  ...
  private async withRemoteSessionSyncLock<T>(agentId: string, sessionId: string, task: () => Promise<T>): Promise<T> {
    return this.withKeyLock(this.remoteSessionSyncLocks, `${agentId}::${sessionId}`, task);
  }

  private async withKeyLock<T>(locks: Map<string, Promise<void>>, key: string, task: () => Promise<T>): Promise<T> {
  ...
  private async deleteConversationSilently(conversationId: string): Promise<void> {
    try {
      ...
      console.warn(`[ACP] Failed to delete duplicate imported conversation ${conversationId}:`, error);
    }
  }
  ...
  private resolveExistingSessionWorkdir(existingWorkdir: string | null | undefined, syncedWorkdir: string): string {
  ...
  private buildRemoteSessionTitle(agentName: string, session: schema.SessionInfo): string {
    ...
    const shortSessionId = session.sessionId.length > 12 ? session.sessionId.slice(0, 12) : session.sessionId;
