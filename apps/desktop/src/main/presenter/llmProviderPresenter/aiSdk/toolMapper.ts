...
function intersectRequiredKeys(variants: JsonSchema[]): string[] | undefined {
  ...
  const requiredLists = variants.map((variant) =>
    Array.isArray(variant.required) ? variant.required.filter((key): key is string => typeof key === "string") : [],
  );
...
function mergePropertySchemas(existing: unknown, incoming: unknown): unknown {
  ...
  if (existing.type === incoming.type && typeof existing.const === "string" && typeof incoming.const === "string") {
    return {
      type: existing.type,
...
export function normalizeToolInputSchema(schema: Record<string, unknown>): Record<string, unknown> {
  ...
  const variants = branchKey
    ? (normalized[branchKey] as unknown[]).filter(isObjectSchema).filter((item) => item.type === "object")
    : [];
  ...
  if (!variants.length) {
    ...
    const additionalProperties =
      typeof normalized.additionalProperties === "boolean" || isObjectSchema(normalized.additionalProperties)
  ...
  const required = branchKey === "allOf" ? unionRequiredKeys(variants) : intersectRequiredKeys(variants);
  ...
  return {
    ...
    ...(required ? { required } : {}),
    ...(variants.every((variant) => variant.additionalProperties === false) ? { additionalProperties: false } : {}),
  };
}
