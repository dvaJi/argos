...
function mapUserContent(content: ChatMessage["content"], options: MapMessagesToModelMessagesOptions): any[] {
  ...
  return content
    .map((part) => {
      ...
      if (part.type === "image_url" && part.image_url && typeof part.image_url.url === "string" && part.image_url.url) {
...
export function mapMessagesToModelMessages(
  ...
  const consumeToolCall = (source: PendingToolCall[], preferredId?: string): PendingToolCall | undefined => {
    if (preferredId) {
  ...
  const modelMessages = messages.reduce<ModelMessage[]>((acc, message) => {
          ...
          for (const toolCall of message.tool_calls) {
            ...
            assistantContent.push({
              ...
              ...(sanitizeProviderOptions(toolCall.provider_options)
                ...
                : {}),
              input: typeof rawArgs === "string" ? (tryParseJson(rawArgs) ?? { raw: rawArgs }) : rawArgs,
            });
          }
    ...
    if (message.role === "tool") {
      const serialized =
        typeof message.content === "string" ? message.content : serializeChatMessageContent(message.content);
        ...
        for (const part of splitParts) {
          ...
          acc.push({
            ...
            content: [
              {
                type: "text",
                text: buildFunctionCallRecordContent(pending.name, tryParseJson(pending.args || "{}") ?? {}, part),
              },
            ],
          } as ModelMessage);
        }
  ...
  if (!options.supportsNativeTools && options.tools.length > 0) {
    return applyLegacyFunctionCallPrompt(modelMessages, options.tools, options.buildLegacyFunctionCallPrompt);
  }
