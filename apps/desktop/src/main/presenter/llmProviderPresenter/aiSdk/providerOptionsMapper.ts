...
function supportsGrokReasoningEffort(modelId: string): boolean {
  return ["grok-3-mini", "grok-3-mini-fast"].some((model) => modelId.toLowerCase().includes(model.toLowerCase()));
}

export function buildProviderOptions(params: BuildProviderOptionsParams): ProviderOptionsMappingResult {
  const providerOptions: ProviderOptionsRecord = {};
  let messages = params.messages;
  const reasoningPortrait = modelCapabilities.getReasoningPortrait?.(params.capabilityProviderId, params.modelId);
  ...
  const shouldSendThinkingConfig = hasThinkingConfig && (reasoningPortrait ? reasoningEnabled : true);
  ...
  switch (params.apiType) {
    ...
    case "openai_responses": {
      ...
      if (params.providerId === "siliconcloud" && supportsSiliconcloudThinking(params.modelId) && reasoningEnabled) {
      ...
      if (
        ...
        config.enable_thinking = true;
        const dbBudget = modelCapabilities.getThinkingBudgetRange(params.providerId, params.modelId).default;
    ...
    case "bedrock": {
      ...
      const anthropicReasoningToggle = hasAnthropicReasoningToggle(params.capabilityProviderId, reasoningPortrait);
      const config: Record<string, unknown> = {
      ...
      if (shouldSendThinkingConfig) {
        config.thinkingConfig = {
          ...(params.modelConfig.thinkingBudget !== undefined
            ...
            : {}),
          ...(params.modelConfig.reasoningEffort ? { thinkingLevel: params.modelConfig.reasoningEffort } : {}),
