...
function buildPromptCacheKey(providerId: string, modelId: string, conversationId?: string): string | undefined {
  ...
  return `argos:${normalizeId(providerId)}:${normalizeId(modelId)}:${digest}`;
}

function findOpenAIChatBreakpoint(messages: PromptCacheOpenAIMessage[]): PromptCacheBreakpointPlan | undefined {
...
function findAnthropicBreakpoint(messages: PromptCacheAnthropicMessage[]): PromptCacheBreakpointPlan | undefined {
  let prefixEnd = messages.length;
...
export function applyOpenAIChatExplicitCacheBreakpoint(
  ...
  let nextContent: PromptCacheOpenAIMessage["content"] = content as PromptCacheOpenAIMessage["content"];
  ...
  if (typeof content === "string") {
    ...
    nextContent = content.map((part, index) => {
      if (index !== contentIndex || part?.type !== "text" || typeof part.text !== "string" || !part.text.trim()) {
        return part;
      }
...
export function applyAnthropicExplicitCacheBreakpoint(
  ...
  if (typeof content === "string") {
    ...
    nextContent = content.map((block, index) => {
      if (index !== contentIndex || block?.type !== "text" || typeof block.text !== "string" || !block.text.trim()) {
        return block;
      }
