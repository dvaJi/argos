...
import { ISyncPresenter, IConfigPresenter, ISQLitePresenter, SyncBackupInfo, CloudSyncResult } from "@shared/presenter";
...
export class SyncPresenter implements ISyncPresenter {
  ...
  public async listBackups(): Promise<SyncBackupInfo[]> {
    ...
    const entries = fs
      ...
      .map((fileName) => {
        ...
        const createdAt = match ? Number(match[1]) : fs.statSync(path.join(backupsDir, fileName)).mtimeMs;
  ...
  public async importFromSync(
    ...
    const extractionDir = path.join(app.getPath("temp"), `argos-backup-${Date.now()}`);
    ...
    try {
      ...
      tempCurrentFiles.appSettings = this.createTempBackup(this.APP_SETTINGS_PATH, "app-settings.json");
      tempCurrentFiles.customPrompts = this.createTempBackup(this.CUSTOM_PROMPTS_PATH, "custom_prompts.json");
      tempCurrentFiles.systemPrompts = this.createTempBackup(this.SYSTEM_PROMPTS_PATH, "system_prompts.json");
      tempCurrentFiles.mcpSettings = this.createTempBackup(this.MCP_SETTINGS_PATH, "mcp-settings.json");
  ...
  private ensureSqliteConfigStorageReady(): void {
    const getConfigTables = () => (this.sqlitePresenter as unknown as Partial<SQLitePresenter>).configTables;
  ...
  private addOptionalFile(files: Record<string, Uint8Array>, zipPath: string, filePath: string): void {
  ...
  private mergeAppSettingsPreservingSync(backupPath: string, targetPath: string): void {
    ...
    const mergedSettings = {
      ...
      ...Object.fromEntries(
        Object.entries(preservedSettings).filter(([, value]) => value !== undefined && value !== null),
      ),
    };
