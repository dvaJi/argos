...
const LEGACY_SENSITIVE_SQLITE_KEYS = [...LEGACY_SENSITIVE_APP_SETTING_KEYS, "customPrompts", "systemPrompts"];
...
export class SyncConfigImportService {
  ...
  private readLegacyPromptStores(extractionDir: string, payload: LegacyConfigPayload): void {
    const customPrompts = this.readPromptStore(path.join(extractionDir, LEGACY_CONFIG_PATHS.customPrompts));
    if (customPrompts) {
      payload.sections.customPrompts = true;
      payload.customPrompts = customPrompts;
    }

    const systemPrompts = this.readPromptStore(path.join(extractionDir, LEGACY_CONFIG_PATHS.systemPrompts));
    if (systemPrompts) {
  ...
  private readLegacyMcpSettings(extractionDir: string, payload: LegacyConfigPayload): void {
    ...
    const defaultServers = new Set([
      ...
      ...normalizeStringArray(typeof mcpSettings.defaultServer === "string" ? [mcpSettings.defaultServer] : []),
    ]);
  ...
  private applyLegacyConfigPayload(
    ...
    if (overwrite) {
      if (payload.sections.providers) {
        configTables.replaceProviders(payload.providers, payload.providerOrder, payload.providerTimestamps);
      }
      if (payload.sections.providers || payload.sections.providerModels) {
        configTables.clearAllProviderModels();
      }
      if (payload.sections.providers || payload.sections.modelStatuses || payload.sections.providerModels) {
      ...
      if (payload.sections.sensitiveAppSettings || payload.sections.customPrompts || payload.sections.systemPrompts) {
    ...
    if (payload.modelStatuses.length > 0) {
      for (const status of payload.modelStatuses) {
        if (overwrite || !configTables.hasModelStatus(status.statusKey)) {
          configTables.setModelStatus(status.statusKey, status.providerId, status.modelId, status.enabled);
        }
      }
    }
  ...
  private mergeAppSettingArray(
    ...
    const existingIds = new Set(
      existing.map((item) => item.id).filter((id): id is string => typeof id === "string" && id.length > 0),
    );
  ...
  private mergeProviderModels(
    ...
    for (const group of this.groupProviderModels(providerModels)) {
      ...
      if (missingModels.length > 0) {
        configTables.replaceProviderModels(group.providerId, group.source, [...existing, ...missingModels]);
      }
    }
  }
  ...
  private collectProviderIds(payload: LegacyConfigPayload, appSettings: Record<string, unknown>): string[] {
  ...
  private parseModelStatusKey(statusKey: string, providerIds: string[]): { providerId: string; modelId: string } {
