...
function buildDefaultImageName(mimeType: string, suggestedName?: string): string {
  ...
  return `argos-image-${formatImageTimestamp(new Date())}.${extension}`;
}
...
export class FilePresenter implements IFilePresenter {
  ...
  private async resolveRemoteImage(source: string, fallbackMimeType?: string): Promise<ResolvedImageData> {
    ...
    const mimeType =
      responseMimeType || normalizeImageMimeType(fallbackMimeType) || inferImageMimeTypeFromPath(url.pathname);
  ...
  private async resolveUserDataReadPath(relativePath: string): Promise<string> {
    ...
    const basePath = await fs.realpath(this.userDataPath).catch(() => path.resolve(this.userDataPath));
