...
export class GitHubCopilotDeviceFlow {
  ...
  /**
   * Get the Copilot API token
   * Exchange the OAuth token for a Copilot API token
   */
  public async getCopilotToken(): Promise<string> {
    ...
    // Use the OAuth token to get the Copilot token from the GitHub API
  ...
  /**
   * Check whether a valid authentication state already exists
   */
  public async checkExistingAuth(externalToken?: string): Promise<string | null> {
    try {
      // If an external token is provided, use it
      if (externalToken) {
        this.oauthToken = externalToken;

        // Try to get an API token to verify the authentication state
        try {
      ...
      // Check the internally stored token
      if (this.oauthToken) {
        // Try to get an API token to verify the authentication state
        try {
          await this.getCopilotToken();
  ...
  private async pollForAccessToken(deviceCodeResponse: DeviceCodeResponse): Promise<string> {
    return new Promise((resolve, reject) => {
      ...
      const poll = async () => {
        ...
        if (pollCount > 100) {
          // Increase the maximum number of polling attempts
...
/**
 * Create a global GitHub Copilot Device Flow instance
 */
let globalDeviceFlowInstance: GitHubCopilotDeviceFlow | null = null;
let globalDeviceFlowClientId: string | null = null;

export function getGlobalGitHubCopilotDeviceFlow(clientIdOverride?: string): GitHubCopilotDeviceFlow {
  const effectiveClientId = clientIdOverride?.trim() || process.env.VITE_GITHUB_CLIENT_ID;
