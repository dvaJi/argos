 ...
 * - serializeToSkillMd: Serialize to Argos SKILL.md format
...
export class FormatConverter {
   ...
   */
  serializeToExternal(skill: CanonicalSkill, targetToolId: string, options?: Record<string, unknown>): string {
  ...
  /**
   * Serialize CanonicalSkill to Argos SKILL.md format (Claude Code format)
   */
  serializeToSkillMd(skill: CanonicalSkill, options?: Record<string, unknown>): string {
    // Argos uses Claude Code format as the native format
   ...
   */
  autoDetectAndParse(content: string, context: Omit<ParseContext, "toolId">): CanonicalSkill | null {
