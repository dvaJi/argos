...
import { ConflictStrategy, type ExternalToolConfig } from "@argos/shared/types/skillSync";
...
export function resolveSafePath(targetPath: string, baseDir: string): string | null {
  try {
    ...
    const absoluteTarget = path.isAbsolute(targetPath) ? path.resolve(targetPath) : path.resolve(baseDir, targetPath);
    ...
    try {
      ...
      try {
        ...
        const baseDirWithSep = realBaseDir.endsWith(path.sep) ? realBaseDir : `${realBaseDir}${path.sep}`;
...
export async function validateImportOperation(
  ...
  try {
    ...
    result.valid = false;
    result.errors.push(`Cannot access source: ${error instanceof Error ? error.message : "Unknown error"}`);
  }
