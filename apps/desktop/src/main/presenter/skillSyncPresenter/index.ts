 ...
 * - Importing skills from external tools to Argos
 * - Exporting skills from Argos to external tools
 */
...
export class SkillSyncPresenter implements ISkillSyncPresenter {
  ...
  private compareWithCacheAndSkills(
    ...
    for (const result of scanResults) {
      ...
      for (const skill of result.skills) {
        ...
        // 2. It's not already imported into Argos
  ...
  // ============================================================================
  // Import Operations (External Tool → Argos)
  // ============================================================================
  ...
  // ============================================================================
  // Export Operations (Argos → External Tool)
  // ============================================================================
  ...
  private async createTempSkillFolder(skill: CanonicalSkill): Promise<string> {
    const tempDir = path.join(app.getPath("temp"), `argos-skill-${Date.now()}-${skill.name}`);
  ...
  private getImportWarnings(skill: CanonicalSkill, _sourceToolId: string): string[] {
    ...
    // Check if source has features that Argos also supports
    // (no warnings needed for import since Argos supports most features)
