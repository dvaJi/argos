 ...
 */
const COPILOT_TO_ARGOS_TOOLS: Record<string, string> = {
  read: "Read",
  edit: "Edit",
  runCommands: "Bash",
...
const ARGOS_TO_COPILOT_TOOLS: Record<string, string> = {
  Read: "read",
  Edit: "edit",
  Bash: "runCommands",
...
export class CopilotAdapter implements IFormatAdapter {
  ...
  parse(content: string, context: ParseContext): CanonicalSkill {
    ...
    // Map tool names from Copilot to Argos format
  ...
  serialize(skill: CanonicalSkill, _options?: Record<string, unknown>): string {
    ...
    // Map tool names from Argos to Copilot format
    if (skill.allowedTools && skill.allowedTools.length > 0) {
      frontmatter.tools = this.mapArgosTools(skill.allowedTools);
    }
  ...
  /**
   * Map Copilot tool names to Argos format
   */
  private mapCopilotTools(tools: unknown): string[] | undefined {
    ...
    for (const tool of tools) {
      if (typeof tool === "string") {
        const argosTool = COPILOT_TO_ARGOS_TOOLS[tool] || tool;
        if (!mapped.includes(argosTool)) {
          mapped.push(argosTool);
        }
      }
    }
  ...
  /**
   * Map Argos tool names to Copilot format
   */
  private mapArgosTools(tools: string[]): string[] {
    const mapped: string[] = [];
    for (const tool of tools) {
      const copilotTool = ARGOS_TO_COPILOT_TOOLS[tool] || tool.toLowerCase();
