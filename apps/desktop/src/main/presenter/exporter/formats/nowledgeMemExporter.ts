...
export function convertArgosToNowledgeMemFormat(conversation: CONVERSATION, messages: Message[]): NowledgeMemThread {
  ...
  return {
    ...
    metadata: {
      conversation: {
        ...
        description: `Exported from Argos - ${messages.length} messages`,
        tags: ["argos-export", conversation.settings.providerId, conversation.settings.modelId].filter(
          Boolean,
        ) as string[],
        settings: {
...
export function buildNowledgeMemExportContent(conversation: CONVERSATION, messages: Message[]): string {
  try {
    const nowledgeMemThread = convertArgosToNowledgeMemFormat(conversation, messages);
