...
import type { ArgosUserMessageLinkRow } from "../sqlitePresenter/tables/argosUserMessageLinks";
import type { ArgosUserMessageRow } from "../sqlitePresenter/tables/argosUserMessages";
import {
...
import { appendMessageRecordToTape, appendMessageReplacementToTape, appendMessageRetractionToTape } from "./tapeFacts";

function shouldConvertPendingBlockToError(status: AssistantMessageBlock["status"]): status is "pending" | "loading" {
  ...
  const normalizedBlocks: AssistantMessageBlock[] = Array.isArray(blocks)
    ? blocks.map(
        (block): AssistantMessageBlock =>
          shouldConvertPendingBlockToError(block.status) ? { ...block, status: "error" as const } : block,
      )
    : [];
...
type StructuredMessageMaps = {
  userRows: Map<string, ArgosUserMessageRow>;
  fileRows: Map<string, ArgosUserMessageFileRow[]>;
  linkRows: Map<string, ArgosUserMessageLinkRow[]>;
  assistantRows: Map<string, ArgosAssistantBlockRow[]>;
};

function normalizePersistedActionType(actionType: string | null): AssistantMessageBlock["action_type"] | undefined {
  if (actionType === "tool_call_permission" || actionType === "question_request" || actionType === "rate_limit") {
    return actionType;
  }
...
export class ArgosMessageStore {
  ...
  createUserMessage(sessionId: string, orderSeq: number, content: UserMessageContent): string {
    ...
    this.sqlitePresenter.argosMessagesTable.insert({
      id,
      sessionId,
      orderSeq,
      role: "user",
  ...
  updateAssistantContent(messageId: string, blocks: AssistantMessageBlock[]): void {
    this.sqlitePresenter.argosAssistantBlocksTable.replaceForMessage(messageId, blocks);
    this.sqlitePresenter.argosMessagesTable.updateStatus(messageId, "pending");
  }

  updateMessageStatus(messageId: string, status: "pending" | "sent" | "error"): void {
    this.sqlitePresenter.argosMessagesTable.updateStatus(messageId, status);
  }

  finalizeAssistantMessage(messageId: string, blocks: AssistantMessageBlock[], metadata: string): void {
    this.sqlitePresenter.argosAssistantBlocksTable.replaceForMessage(messageId, blocks);
    this.sqlitePresenter.argosMessagesTable.updateContentAndStatus(messageId, JSON.stringify(blocks), "sent", metadata);
  ...
  updateCompactionMessage(
    ...
    this.runInDatabaseTransaction(() => {
      this.sqlitePresenter.argosMessagesTable.updateContentAndStatus(
        messageId,
  ...
  setMessageError(messageId: string, blocks: AssistantMessageBlock[], metadata?: string): void {
    ...
    if (metadata === undefined) {
      this.sqlitePresenter.argosMessagesTable.updateContentAndStatus(messageId, serializedBlocks, "error");
      this.upsertAssistantSearchDocument(messageId, blocks);
      this.appendLiveTapeFacts(messageId);
      return;
    }
    this.sqlitePresenter.argosMessagesTable.updateContentAndStatus(messageId, serializedBlocks, "error", metadata);
    this.upsertAssistantSearchDocument(messageId, blocks);
    this.persistUsageStats(messageId, metadata, "live");
    this.appendLiveTapeFacts(messageId);
  }
  ...
  getMessages(sessionId: string): ChatMessageRecord[] {
    const rows = this.sqlitePresenter.argosMessagesTable.getBySession(sessionId);
  ...
  listMessagesPage(
    ...
    const rows = this.sqlitePresenter.argosMessagesTable.listPageBySession(sessionId, {
      limit: limit + 1,
  ...
  getMessagesUpToOrderSeq(sessionId: string, maxOrderSeq: number): ChatMessageRecord[] {
    const rows = this.sqlitePresenter.argosMessagesTable.getBySessionUpToOrderSeq(sessionId, maxOrderSeq);
    return this.toRecords(rows);
  }

  getMessageIds(sessionId: string): string[] {
    return this.sqlitePresenter.argosMessagesTable.getIdsBySession(sessionId);
  }

  getMessage(messageId: string): ChatMessageRecord | null {
    const row = this.sqlitePresenter.argosMessagesTable.get(messageId);
    if (!row) return null;
    return this.toRecord(row);
  }

  getLastUserMessageBeforeOrAt(sessionId: string, orderSeq: number): ChatMessageRecord | null {
    const row = this.sqlitePresenter.argosMessagesTable.getLastUserMessageBeforeOrAtOrderSeq(sessionId, orderSeq);
    if (!row) return null;
    return this.toRecord(row);
  }

  updateMessageContent(messageId: string, content: string): void {
    this.sqlitePresenter.argosMessagesTable.updateContent(messageId, content);
    const row = this.sqlitePresenter.argosMessagesTable.get(messageId);
    if (!row) {
      return;
    }
    ...
    if (row.role === "user") {
      ...
      if (updated) {
        appendMessageReplacementToTape(this.sqlitePresenter.argosTapeEntriesTable, updated, "message_content_updated");
      }
      return;
    }
    ...
    if (row.status === "sent" || row.status === "error") {
      this.upsertMessageSearchDocument(row.session_id, messageId, "assistant", content, row.updated_at);
    }
    const updated = this.getMessage(messageId);
    if (updated) {
      appendMessageReplacementToTape(this.sqlitePresenter.argosTapeEntriesTable, updated, "message_content_updated");
  ...
  getNextOrderSeq(sessionId: string): number {
    return this.sqlitePresenter.argosMessagesTable.getMaxOrderSeq(sessionId) + 1;
  }

  deleteBySession(sessionId: string): void {
    this.sqlitePresenter.argosSearchDocumentsTable.deleteBySession(sessionId);
    this.sqlitePresenter.argosAssistantBlocksTable.deleteBySession(sessionId);
    this.sqlitePresenter.argosUserMessageLinksTable.deleteBySession(sessionId);
    this.sqlitePresenter.argosUserMessageFilesTable.deleteBySession(sessionId);
    this.sqlitePresenter.argosUserMessagesTable.deleteBySession(sessionId);
    this.sqlitePresenter.argosMessageTracesTable.deleteBySessionId(sessionId);
    this.sqlitePresenter.argosMessageSearchResultsTable.deleteBySessionId(sessionId);
    this.sqlitePresenter.argosMessagesTable.deleteBySession(sessionId);
  }
  ...
  deleteMessage(messageId: string): void {
    this.runInDatabaseTransaction(() => {
      ...
      if (record) {
        appendMessageRetractionToTape(this.sqlitePresenter.argosTapeEntriesTable, record, "message_deleted");
      }
      this.sqlitePresenter.argosSearchDocumentsTable.delete(`message:${messageId}`);
      this.sqlitePresenter.argosAssistantBlocksTable.delete(messageId);
      this.sqlitePresenter.argosUserMessageLinksTable.delete(messageId);
      this.sqlitePresenter.argosUserMessageFilesTable.delete(messageId);
      this.sqlitePresenter.argosUserMessagesTable.delete(messageId);
      this.sqlitePresenter.argosMessageTracesTable.deleteByMessageIds([messageId]);
      this.sqlitePresenter.argosMessageSearchResultsTable.deleteByMessageIds([messageId]);
      this.sqlitePresenter.argosMessagesTable.delete(messageId);
    });
  }

  deleteFromOrderSeq(sessionId: string, fromOrderSeq: number): void {
    this.runInDatabaseTransaction(() => {
      const records = this.getMessages(sessionId).filter((record) => record.orderSeq >= fromOrderSeq);
      for (const record of records) {
        appendMessageRetractionToTape(
          this.sqlitePresenter.argosTapeEntriesTable,
          record,
      ...
      if (messageIds.length > 0) {
        this.sqlitePresenter.argosSearchDocumentsTable.deleteByMessageIds(messageIds);
        this.sqlitePresenter.argosAssistantBlocksTable.deleteByMessageIds(messageIds);
        this.sqlitePresenter.argosUserMessageLinksTable.deleteByMessageIds(messageIds);
        this.sqlitePresenter.argosUserMessageFilesTable.deleteByMessageIds(messageIds);
        this.sqlitePresenter.argosUserMessagesTable.deleteByMessageIds(messageIds);
        this.sqlitePresenter.argosMessageTracesTable.deleteByMessageIds(messageIds);
        this.sqlitePresenter.argosMessageSearchResultsTable.deleteByMessageIds(messageIds);
      }
      this.sqlitePresenter.argosMessagesTable.deleteFromOrderSeq(sessionId, fromOrderSeq);
    });
  }
  ...
  addSearchResult(row: {
    ...
    this.sqlitePresenter.argosMessageSearchResultsTable.add({
  ...
  getSearchResults(messageId: string, searchId?: string): SearchResult[] {
    const rows = this.sqlitePresenter.argosMessageSearchResultsTable.listByMessageId(messageId);
    ...
    for (const row of rows) {
      try {
        ...
        console.warn("[ArgosMessageStore] Failed to parse search result row:", error);
      }
    }
  ...
  insertMessageTrace(row: {
    ...
    return this.sqlitePresenter.argosMessageTracesTable.insert(row);
  }

  listMessageTraces(messageId: string): MessageTraceRecord[] {
    const rows = this.sqlitePresenter.argosMessageTracesTable.listByMessageId(messageId);
    return rows.map((row) => ({
      id: row.id,
  ...
  getMessageTraceCount(messageId: string): number {
    return this.sqlitePresenter.argosMessageTracesTable.countByMessageId(messageId);
  }

  cloneSentMessagesToSession(sourceSessionId: string, targetSessionId: string, maxOrderSeq: number): number {
    const sourceRows = this.sqlitePresenter.argosMessagesTable
    ...
    for (const record of sourceRecords) {
      ...
      this.upsertMessageSearchDocument(targetSessionId, nextId, record.role, record.content, record.updatedAt);
      nextOrderSeq += 1;
    }
  ...
  recoverPendingMessages(): number {
    const pendingRows = this.sqlitePresenter.argosMessagesTable.getByStatus("pending");
    const recoveredRecords = new Map(this.toRecords(pendingRows).map((record) => [record.id, record]));
    ...
    for (const row of pendingRows) {
      ...
      if (row.role === "assistant") {
        const blocks = this.parseAssistantBlocks(recoveredRecords.get(row.id)?.content ?? row.content);
        const recoveredBlocks = buildTerminalErrorBlocks(blocks, "common.error.sessionInterrupted");
        this.sqlitePresenter.argosAssistantBlocksTable.replaceForMessage(row.id, recoveredBlocks);
        this.sqlitePresenter.argosMessagesTable.updateContentAndStatus(
          row.id,
        ...
        this.sqlitePresenter.argosMessagesTable.updateStatus(row.id, "error");
      }
      recoveredCount += 1;
    }
  ...
  backfillMessageRow(row: ArgosMessageRow): void {
    if (row.role === "user") {
      ...
      this.sqlitePresenter.argosAssistantBlocksTable.replaceForMessage(row.id, this.parseAssistantBlocks(row.content));
    }

    if (row.status === "sent" || row.status === "error") {
      this.upsertMessageSearchDocument(row.session_id, row.id, row.role, this.materializeContent(row), row.updated_at);
    }
  }

  private shouldKeepPending(row: ArgosMessageRow): boolean {
    if (row.role !== "assistant") {
      return false;
    }
  ...
  private appendLiveTapeFacts(messageId: string): void {
    if (!this.sqlitePresenter.argosTapeEntriesTable) {
      return;
    }
    ...
    appendMessageRecordToTape(this.sqlitePresenter.argosTapeEntriesTable, record, "live");
  }

  private toRecord(row: ArgosMessageRow): ChatMessageRecord {
    return this.toRecords([row])[0]!;
  }

  private toRecords(rows: ArgosMessageRow[]): ChatMessageRecord[] {
    if (rows.length === 0) {
      return [];
    }
  ...
  private materializeContent(row: ArgosMessageRow, maps?: StructuredMessageMaps): string {
    if (row.role === "user") {
      const userRow = maps?.userRows.get(row.id) ?? this.sqlitePresenter.argosUserMessagesTable.get(row.id);
      if (!userRow) {
        return row.content;
      }

      const fileRows =
        maps?.fileRows.get(row.id) ?? this.sqlitePresenter.argosUserMessageFilesTable.listByMessageIds([row.id]);
      const linkRows =
        maps?.linkRows.get(row.id) ?? this.sqlitePresenter.argosUserMessageLinksTable.listByMessageIds([row.id]);
    ...
    const assistantRows =
      maps?.assistantRows.get(row.id) ?? this.sqlitePresenter.argosAssistantBlocksTable.listByMessageId(row.id);
  ...
  private buildCompactionBlocks(status: "compacting" | "compacted"): AssistantMessageBlock[] {
    return [
      {
        type: "content",
        content: status === "compacting" ? "Compacting conversation context..." : "Conversation context compacted.",
  ...
  private persistUserContent(messageId: string, content: UserMessageContent): void {
    ...
    this.sqlitePresenter.argosUserMessageLinksTable.replaceForMessage(messageId, content.links);
  }

  private toMessageFile(row: ArgosUserMessageFileRow): MessageFile {
  ...
  private toAssistantBlock(row: ArgosAssistantBlockRow): AssistantMessageBlock {
  ...
  private loadStructuredMaps(messageIds: string[]): StructuredMessageMaps {
    const userRows = this.sqlitePresenter.argosUserMessagesTable.listByMessageIds(messageIds);
    const fileRows = this.sqlitePresenter.argosUserMessageFilesTable.listByMessageIds(messageIds);
    const linkRows = this.sqlitePresenter.argosUserMessageLinksTable.listByMessageIds(messageIds);
    const assistantRows = this.sqlitePresenter.argosAssistantBlocksTable.listByMessageIds(messageIds);
  ...
  private upsertAssistantSearchDocument(messageId: string, blocks: AssistantMessageBlock[]): void {
    const messageRow = this.sqlitePresenter.argosMessagesTable.get(messageId);
    if (!messageRow) {
      return;
    }
  ...
  private persistUsageStats(messageId: string, metadataRaw: string, source: "backfill" | "live"): void {
    ...
    try {
      ...
      const sessionRow = this.sqlitePresenter.argosSessionsTable.get(messageRow.session_id);
      ...
      logger.error("Failed to persist argos usage stats", { messageId, source }, error);
      return;
    }
  }
}
