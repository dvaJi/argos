...
import type {
  ArgosTapeEntryKind,
  ArgosTapeEntryRow,
  ArgosTapeSearchInput,
} from "../sqlitePresenter/tables/argosTapeEntries";

export interface EffectiveTapeView {
  rows: ArgosTapeEntryRow[];
...
function tapeEntryToMessageRecord(row: ArgosTapeEntryRow): ChatMessageRecord | null {
  if (row.kind !== "message") {
    return null;
  }
...
function readMessageRetractionId(row: ArgosTapeEntryRow): string | null {
...
function isAuditEvent(row: ArgosTapeEntryRow): boolean {
  return (
    row.name === "message/retracted" || row.name === "message/compaction_indicator" || row.name === "migration/backfill"
  );
}

function readToolStatus(row: ArgosTapeEntryRow): string | null {
  const meta = parseJsonObject(row.meta_json);
  return typeof meta.status === "string" ? meta.status : null;
}

function toolRank(row: ArgosTapeEntryRow, includePending: boolean): number {
...
function readToolIdentity(row: ArgosTapeEntryRow): ToolIdentity | null {
...
function shouldReplaceToolRow(
  current: ArgosTapeEntryRow | undefined,
  next: ArgosTapeEntryRow,
  includePending: boolean,
): boolean {
  if (!current) {
    return true;
...
function matchesKinds(row: ArgosTapeEntryRow, kinds: ArgosTapeEntryKind[] | undefined): boolean {
  return !kinds?.length || kinds.includes(row.kind);
}

function matchesCreatedAt(row: ArgosTapeEntryRow, options: ArgosTapeSearchInput): boolean {
  if (Number.isFinite(options.startCreatedAt) && row.created_at < (options.startCreatedAt as number)) {
    return false;
  }
...
function matchesQuery(row: ArgosTapeEntryRow, normalizedQuery: string): boolean {
...
export function buildEffectiveTapeView(
  ...
  const toolRows = new Map<string, { row: ArgosTapeEntryRow; messageId: string }>();
  const anchorRows: ArgosTapeEntryRow[] = [];
  const eventRows: ArgosTapeEntryRow[] = [];
...
export function getLastEffectiveTokenUsage(rows: ArgosTapeEntryRow[]): number | null {
