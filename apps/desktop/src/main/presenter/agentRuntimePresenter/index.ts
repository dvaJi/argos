...
import { ArgosSessionStore, type SessionSummaryState } from "./sessionStore";
...
export class AgentRuntimePresenter implements IAgentImplementation {
  ...
  private readonly sessionStore: ArgosSessionStore;
  private readonly messageStore: ArgosMessageStore;
  private readonly tapeService: ArgosTapeService;
  private readonly pendingInputStore: ArgosPendingInputStore;
  private readonly pendingInputCoordinator: PendingInputCoordinator;
  private readonly runtimeState: Map<string, ArgosSessionState> = new Map();
  ...
  constructor(
    ...
    this.sessionStore = new ArgosSessionStore(sqlitePresenter);
    this.messageStore = new ArgosMessageStore(sqlitePresenter);
    this.tapeService = new ArgosTapeService(sqlitePresenter);
    this.pendingInputStore = new ArgosPendingInputStore(sqlitePresenter);
    ...
    if (recovered > 0) {
      console.log(`ArgosAgent: recovered ${recovered} pending messages to error status`);
    }

    const recoveredPendingInputs = this.pendingInputCoordinator.recoverClaimedInputsAfterRestart();
    if (recoveredPendingInputs > 0) {
      console.log(`ArgosAgent: recovered ${recoveredPendingInputs} sessions with claimed pending inputs`);
    }
  ...
  async initSession(
    ...
    console.log(
      `[ArgosAgent] initSession id=${sessionId} provider=${config.providerId} model=${config.modelId} permission=${permissionMode} projectDir=${projectDir ?? "<none>"}`,
    );
    ...
    this.sessionAgentIds.set(sessionId, config.agentId?.trim() || this.getSessionAgentId(sessionId) || "argos");
  ...
  async getSessionState(sessionId: string): Promise<ArgosSessionState | null> {
    return await this.getResolvedSessionState(sessionId, "full");
  }

  async getSessionListState(sessionId: string): Promise<ArgosSessionState | null> {
  ...
  private async getResolvedSessionState(
  ...
  ): Promise<ArgosSessionState | null> {
    ...
    this.getSessionAgentId(sessionId);
    const rebuilt: ArgosSessionState = {
  ...
  async processMessage(
    ...
    console.log(
      `[ArgosAgent] processMessage session=${sessionId} content="${normalizedInput.text.slice(0, 60)}" projectDir=${projectDir ?? "<none>"}`,
    );
    ...
    try {
      ...
      const useContextBudget = this.shouldUseArgosContextBudget(state.providerId, modelConfig);
      this.throwIfAbortRequested(preStreamAbortSignal);
      ...
      const contextBudgetLength = this.resolveArgosContextBudgetLength(
        state.providerId,
      ...
      if (actionBlock.action_type === "question_request") {
        ...
        if (response.granted) {
          ...
          if (execution.terminalError) {
            ...
            publishArgosEvent("chat.stream.failed", {
  ...
  private dispatchTerminalHooks(sessionId: string, state: ArgosSessionState | undefined, result: ProcessResult): void {
  ...
  private dispatchHook(
    ...
    try {
      this.hooksBridge?.dispatch(event, {
        ...context,
        agentId: this.getSessionAgentId(context.sessionId) ?? "argos",
      });
    } catch (error) {
      console.warn(`[ArgosAgent] Failed to dispatch ${event} hook:`, error);
    }
  }
  ...
  private shouldUseArgosContextBudget(
  ...
  private shouldBypassArgosContextBudget(
    providerId?: string | null,
    modelConfig?: Pick<ModelConfig, "apiEndpoint" | "endpointType" | "type"> | null,
    modelId?: string | null,
  ): boolean {
    return !this.shouldUseArgosContextBudget(providerId, modelConfig, modelId);
  }

  private resolveArgosContextBudgetLength(
    providerId: string | null | undefined,
    contextLength: number,
    modelConfig?: Pick<ModelConfig, "apiEndpoint" | "endpointType" | "type"> | null,
    modelId?: string | null,
  ): number {
    return this.shouldBypassArgosContextBudget(providerId, modelConfig, modelId)
  ...
  private toTapeAnchorResult(row: ArgosTapeEntryRow): AgentTapeAnchorResult {
  ...
  private async runStreamForMessage(args: {
    ...
    const shouldBypassContextBudget = this.shouldBypassArgosContextBudget.bind(this);
      ...
      const result = await processStream({
        ...
        hooks: {
          ...
          onInterleavedReasoningGap: (gap) => {
            console.warn(
              `[ArgosAgent] Interleaved reasoning gap detected for ${gap.providerId}/${gap.modelId}. Update provider DB metadata at ${gap.providerDbSourceUrl}.`,
            );
            if (!traceEnabled) {
              return;
            }
            persistMessageTrace({
              ...
              modelId: state.modelId,
              payload: {
                endpoint: "argos://interleaved-reasoning-gap",
                headers: {},
                body: gap,
              },
            });
  ...
  private emitRateLimitWaitingMessage(
    ...
    publishArgosEvent("chat.stream.updated", {
      kind: "snapshot",
      requestId,
      sessionId,
      messageId,
  ...
  private async buildSystemPromptWithSkills(
    ...
    if (skillsEnabled && skillPresenter) {
      if (skillPresenter.getMetadataList) {
        try {
          ...
          console.warn(`[ArgosAgent] Failed to load skills metadata for session ${sessionId}:`, error);
        }
      }
      ...
      if (!activeSkillNamesOverride && skillPresenter.getActiveSkills) {
        try {
          ...
          console.warn(`[ArgosAgent] Failed to load active skills for session ${sessionId}:`, error);
        }
      }
    }
    ...
    if (skillsEnabled && skillPresenter?.loadSkillContent && normalizedActiveSkills.length > 0) {
      ...
      for (const skillName of normalizedActiveSkills) {
        try {
          ...
          console.warn(`[ArgosAgent] Failed to load skill content for "${skillName}" in session ${sessionId}:`, error);
        }
      }
    ...
    try {
      ...
      console.warn(`[ArgosAgent] Failed to build env prompt for session ${sessionId}:`, error);
    }
    ...
    if (this.toolPresenter) {
      try {
        ...
        console.warn(`[ArgosAgent] Failed to build tooling prompt for session ${sessionId}:`, error);
      }
    }
  ...
  private buildSkillsMetadataPrompt(
    ...
    if (capabilities.canManageDraftSkills && skillDraftSuggestionsEnabled) {
      ...
      lines.push(
        "Only propose one draft per task, do it after the main answer is complete, and use `argos_question` to ask whether the user wants to keep the draft.",
      );
  ...
  private async resolveProviderPermissionInteraction(input: ProviderPermissionInteractionInput): Promise<void> {
    ...
    if (resolution.status === "stale") {
      console.warn(`[ArgosAgent] Clearing stale ACP permission request ${input.requestId}:`, resolution.error);
    }
  ...
  private updateSubagentToolCallProgress(
    ...
    try {
      ...
      console.warn("[ArgosAgent] Failed to persist subagent tool progress:", error);
    }
  }
  ...
  private async loadToolDefinitionsForSession(
    ...
    try {
      ...
      console.error("[ArgosAgent] failed to fetch tool definitions:", error);
      return [];
    }
  }
  ...
  private async normalizeToolResultContent(params: {
    ...
    try {
      ...
      console.warn("[ArgosAgent] Failed to normalize screenshot tool output:", {
        sessionId: params.sessionId,
        toolCallId: params.toolCallId,
        error: message,
      });
  ...
  private emitMessageRefresh(sessionId: string, messageId: string): void {
    ...
    try {
      ...
      console.warn("[ArgosAgent] Failed to emit internal message refresh:", error);
    }
  }
  ...
  private resolvePersistedSessionProjectDir(sessionId: string): string | null {
    try {
      ...
      console.warn("[ArgosAgent] Failed to resolve persisted project directory:", {
        sessionId,
        error,
      });
      return null;
