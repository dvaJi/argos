...
function isUserCanceledAlreadyFinalized(io: IoParams): boolean {
  ...
  return blocks.some((block) => block.type === "error" && block.content === USER_CANCELED_GENERATION_ERROR);
}
...
function normalizeProviderPermissionType(permissionType: unknown): "read" | "write" | "all" | "command" {
...
function parseStreamingPermissionCommandInfo(raw: unknown): PendingPermissionCommandInfo | undefined {
  ...
  return {
    ...
    ...(typeof value.signature === "string" && value.signature.trim() ? { signature: value.signature.trim() } : {}),
...
function toStreamingProviderPermission(permission: PermissionRequestPayload): PendingPermissionPayload {
  const toolName =
  ...
  const providerId =
    typeof permission.providerId === "string" && permission.providerId.trim()
      ...
      : undefined;
  const requestId =
    typeof permission.requestId === "string" && permission.requestId.trim() ? permission.requestId.trim() : undefined;
  const command =
    typeof permission.command === "string" && permission.command.trim() ? permission.command.trim() : undefined;
  ...
  const metadata =
    permission.metadata && typeof permission.metadata === "object" && !Array.isArray(permission.metadata)
...
function appendStreamingProviderPermissionBlock(
  ...
  const toolArgs = typeof permissionPayload.tool_call_params === "string" ? permissionPayload.tool_call_params : "";
  ...
  const actionBlock: AssistantMessageBlock = {
    ...
    tool_call: {
      ...
      ...(typeof permissionPayload.server_description === "string"
        ...
        : {}),
      ...(typeof permissionPayload.server_icons === "string" ? { server_icons: permissionPayload.server_icons } : {}),
    },
    extra: {
    ...
    while (true) {
      ...
      for await (const event of stream) {
        ...
        if (event.type === "permission") {
          const { actionBlock, permission, tool } = appendStreamingProviderPermissionBlock(state, event.permission);
      ...
      if (executed.pendingInteractions.length > 0) {
        console.log(`[ProcessStream] paused for user interaction count=${executed.pendingInteractions.length}`);
