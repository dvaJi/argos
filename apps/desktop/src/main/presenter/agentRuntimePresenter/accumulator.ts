...
function getCurrentBlock(
  ...
  if (last && last.status === "pending" && (last.type === "content" || last.type === "reasoning_content")) {
    const lastProviderOptionsJson =
      typeof last.extra?.providerOptionsJson === "string" ? last.extra.providerOptionsJson : undefined;
...
function serializeProviderOptions(providerOptions?: ChatMessageProviderOptions): string | undefined {
  if (!providerOptions) {
    return undefined;
  }
      ...
      if (pending) {
        ...
        if (block?.tool_call) {
          ...
          block.extra = {
            ...
            ...(providerOptions ? { providerOptionsJson: serializeProviderOptions(providerOptions) } : {}),
          };
        }
