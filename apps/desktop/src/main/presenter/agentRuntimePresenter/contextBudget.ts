...
export function getUsableContextLength(contextLength: number): number {
  ...
  return Math.max(AGENT_MIN_EFFECTIVE_OUTPUT_TOKENS, Math.floor(contextLength - AGENT_CONTEXT_SAFETY_MARGIN_TOKENS));
}

export function capAgentRequestMaxTokens(maxTokens: number, contextLength: number = Number.MAX_SAFE_INTEGER): number {
  const normalizedMaxTokens = Number.isFinite(maxTokens) ? Math.floor(maxTokens) : AGENT_MIN_EFFECTIVE_OUTPUT_TOKENS;
...
export function capAgentDefaultMaxTokens(maxTokens: number, contextLength: number): number {
  return Math.min(capAgentRequestMaxTokens(maxTokens, contextLength), AGENT_DEFAULT_MAX_OUTPUT_TOKENS_CAP);
}
...
export function fitRequestMessagesToContextWindow(params: {
  ...
  return fitMessagesToContextWindow(
    ...
    Math.max(params.minimumProtectedTailCount ?? 0, resolveProtectedRequestTailCount(params.messages)),
  );
}
...
export function resolveEffectiveRequestMaxTokens(params: {
  ...
  const remaining = Math.floor(
    getUsableContextLength(params.contextLength) - estimateMessagesTokens(params.messages) - params.toolReserveTokens,
  );
  if (remaining <= 0) {
...
export function preflightRequestContext(params: {
  ...
  const requestedMaxTokens = capAgentRequestMaxTokens(params.requestedMaxTokens, params.contextLength);
  ...
  const remainingOutputTokens = hasFiniteContext
    ...
    : requestedMaxTokens;
  const fitsWithinContext = !hasFiniteContext || remainingOutputTokens >= AGENT_MIN_EFFECTIVE_OUTPUT_TOKENS;
  const effectiveMaxTokens = hasFiniteContext
    ? remainingOutputTokens <= 0
      ? 0
      : Math.max(AGENT_MIN_EFFECTIVE_OUTPUT_TOKENS, Math.min(requestedMaxTokens, remainingOutputTokens))
    : requestedMaxTokens;
...
export function buildRequestContextOverflowErrorMessage(preflight: RequestContextPreflightResult): string {
  const diagnostics = buildRequestContextBudgetDiagnostics(preflight);
  const formatTokenCount = (value: number): string => (Number.isFinite(value) ? String(Math.floor(value)) : "unknown");
