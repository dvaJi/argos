...
export function buildTriagePrompt(spanText: string): string {
  const span = spanText.length > MAX_TRIAGE_SPAN_CHARS ? spanText.slice(-MAX_TRIAGE_SPAN_CHARS) : spanText;
  return [
...
export type MemoryCandidateParseResult =
  ...
  | { ok: false; reason: "empty-response" | "missing-json-array" | "invalid-json" | "non-array" };
...
export function parseMemoryCandidates(raw: string): MemoryCandidateParseResult {
  ...
  for (const entry of parsed) {
    ...
    const category = isAgentMemoryCategory(rawCategory) ? rawCategory : undefined;
    ...
    candidates.push({ kind: kind ?? "semantic", category, content, importance });
...
export function buildReflectionPrompt(previousSelfModel: string | null, memories: string[]): string {
...
export function personaChangeRatio(previous: string | null | undefined, next: string | null | undefined): number {
