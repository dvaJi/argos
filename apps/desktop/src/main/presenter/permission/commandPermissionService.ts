...
const BUILD_COMMANDS = /\b(git\s+(pull|push|checkout|switch|merge)|npm|pnpm|yarn|bun|pip|pip3|cargo|make|gradle|mvn)\b/;
...
export class CommandPermissionRequiredError extends Error {
  ...
  constructor(responseContent: string, permissionRequest: CommandPermissionRequiredError["permissionRequest"]) {
...
export class CommandPermissionService {
  ...
  checkPermission(conversationId: string | undefined, command: string): CommandPermissionCheckResult {
