...
const FTS_TABLE_NAME = "argos_search_documents_fts";
const FTS_TRIGGER_NAMES = [
  "argos_search_documents_ai",
  "argos_search_documents_ad",
  "argos_search_documents_au",
] as const;
...
export class ArgosSearchDocumentsTable extends BaseTable {
  ...
  getCreateTableSQL(): string {
    return `
      CREATE TABLE IF NOT EXISTS argos_search_documents (
      ...
      CREATE INDEX IF NOT EXISTS idx_argos_search_documents_session
        ON argos_search_documents(session_id, updated_at DESC);
      CREATE INDEX IF NOT EXISTS idx_argos_search_documents_message
        ON argos_search_documents(message_id);
    `;
  }
  ...
  private hasCompatibleFtsTable(): boolean {
    ...
    return (
      ...
      /content\s*=\s*'argos_search_documents'/.test(normalized) &&
  ...
  upsert(row: {
    ...
    this.db
      .prepare(
        `INSERT INTO argos_search_documents (
          document_key,
          session_id,
          message_id,
          document_kind,
  ...
  delete(documentKey: string): void {
    this.db.prepare("DELETE FROM argos_search_documents WHERE document_key = ?").run(documentKey);
  }
  ...
  deleteBySession(sessionId: string): void {
    this.db.prepare("DELETE FROM argos_search_documents WHERE session_id = ?").run(sessionId);
  }

  searchFts(query: string, limit: number): Array<ArgosSearchDocumentRow & { rank: number }> {
    ...
    return this.db
      .prepare(
        `SELECT
           ...
           d.content,
           d.updated_at,
           bm25(argos_search_documents_fts) AS rank
         FROM argos_search_documents_fts
         JOIN argos_search_documents d
           ON d.rowid = argos_search_documents_fts.rowid
         WHERE argos_search_documents_fts MATCH ?
         ORDER BY rank ASC, d.updated_at DESC
         LIMIT ?`,
      )
      .all(matchQuery, limit) as Array<ArgosSearchDocumentRow & { rank: number }>;
  }

  searchLike(query: string, limit: number): Array<ArgosSearchDocumentRow & { rank: number }> {
    ...
    return this.db
      .prepare(
        `SELECT
           ...
           updated_at,
           0 AS rank
         FROM argos_search_documents
         WHERE lower(title) LIKE ?
            OR lower(content) LIKE ?
         ORDER BY updated_at DESC
         LIMIT ?`,
      )
      .all(likeQuery, likeQuery, limit) as Array<ArgosSearchDocumentRow & { rank: number }>;
  }
  ...
  private ensureFtsTable(): void {
    try {
      this.db.transaction(() => {
        ...
        this.db.exec(`
          ...
          USING fts5(
            title,
            content,
            content='argos_search_documents',
            content_rowid='rowid'
          );
        `);
      ...
      console.warn("[ArgosSearchDocumentsTable] FTS5 unavailable, falling back to LIKE search.", error);
    }
  }
  ...
  private ensureFtsTriggers(): void {
    this.dropFtsTriggers();
    this.db.exec(`
      CREATE TRIGGER argos_search_documents_ai
      AFTER INSERT ON argos_search_documents
      BEGIN
        INSERT INTO ${FTS_TABLE_NAME}(rowid, title, content)
        VALUES (new.rowid, new.title, new.content);
      END;

      CREATE TRIGGER argos_search_documents_ad
      AFTER DELETE ON argos_search_documents
      BEGIN
        INSERT INTO ${FTS_TABLE_NAME}(${FTS_TABLE_NAME}, rowid, title, content)
        VALUES('delete', old.rowid, old.title, old.content);
      END;

      CREATE TRIGGER argos_search_documents_au
      AFTER UPDATE OF title, content ON argos_search_documents
      BEGIN
        INSERT INTO ${FTS_TABLE_NAME}(${FTS_TABLE_NAME}, rowid, title, content)
        VALUES('delete', old.rowid, old.title, old.content);
        INSERT INTO ${FTS_TABLE_NAME}(rowid, title, content)
