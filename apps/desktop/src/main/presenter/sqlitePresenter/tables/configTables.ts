...
export class ConfigTables extends BaseTable {
  ...
  hasConfigMigration(id = CONFIG_STORAGE_MIGRATION_ID): boolean {
    const row = this.db.prepare("SELECT 1 FROM config_migrations WHERE id = ?").get(id) as { 1: number } | undefined;
    return Boolean(row);
  }
  ...
  replaceProviders(providers: LLM_PROVIDER[], order: string[] = [], timestamps: Record<string, number> = {}): void {
    this.db.transaction(() => {
  ...
  upsertProvider(provider: LLM_PROVIDER, options: { sortOrder?: number; lastUsedAt?: number | null } = {}): void {
    const timestamp = now();
  ...
  replaceProviderModels(providerId: string, source: "provider" | "custom", models: MODEL_META[]): void {
    this.db.transaction(() => {
      this.db.prepare("DELETE FROM provider_models WHERE provider_id = ? AND source = ?").run(providerId, source);
  ...
  getModelStatus(statusKey: string): boolean | undefined {
    const row = this.db.prepare("SELECT enabled FROM model_status WHERE status_key = ?").get(statusKey) as
      | { enabled: number }
      | undefined;
  ...
  getModelConfigStoreEntry<TValue = unknown>(cacheKey: string): TValue | undefined {
    const row = this.db.prepare("SELECT config_json FROM model_configs WHERE cache_key = ?").get(cacheKey) as
      | { config_json: string }
      | undefined;
  ...
  setModelConfigStoreEntry(cacheKey: string, value: unknown): void {
    ...
    const existing = this.db.prepare("SELECT created_at FROM model_configs WHERE cache_key = ?").get(cacheKey) as
      | { created_at: number }
      | undefined;
    ...
    this.db
      ...
      .run(cacheKey, providerId, modelId, source, stringifyJson(value), existing?.created_at ?? timestamp, timestamp);
  }
  ...
  listMcpServers(): Record<string, MCPServerConfig> {
    ...
    return Object.fromEntries(
      rows.map((row) => [row.name, parseJson<MCPServerConfig>(row.config_json, {} as MCPServerConfig)]),
    );
  }
  ...
  hasAppSetting(key: string): boolean {
    const row = this.db.prepare("SELECT 1 FROM app_settings WHERE key = ?").get(key) as { 1: number } | undefined;
    return Boolean(row);
  }
  ...
  setAgentMcpSelections(selections: string[], agentId = SHARED_AGENT_MCP_SELECTION_ID, isBuiltin = false): void {
  ...
  private getProvider(providerId: string): ProviderRow | undefined {
    return this.db.prepare("SELECT * FROM providers WHERE id = ?").get(providerId) as ProviderRow | undefined;
  }
  ...
  private upsertMcpServer(name: string, config: MCPServerConfig, sortOrder: number): void {
    const timestamp = now();
    const existing = this.db.prepare("SELECT created_at FROM mcp_servers WHERE name = ?").get(name) as
      | { created_at: number }
      | undefined;
    this.db
      .prepare(
  ...
  private getJsonSetting<TValue = unknown>(
    ...
    const row = this.db.prepare(`SELECT value_json FROM ${table} WHERE key = ?`).get(key) as SettingsRow | undefined;
    return row ? parseJson<TValue | undefined>(row.value_json, undefined) : undefined;
  }

  private setJsonSetting(table: "mcp_settings" | "agent_settings" | "app_settings", key: string, value: unknown) {
    this.db
      .prepare(
  ...
  private deleteJsonSetting(table: "mcp_settings" | "agent_settings" | "app_settings", key: string) {
    this.db.prepare(`DELETE FROM ${table} WHERE key = ?`).run(key);
  }

  private listJsonSettings(table: "mcp_settings" | "agent_settings" | "app_settings"): Record<string, unknown> {
