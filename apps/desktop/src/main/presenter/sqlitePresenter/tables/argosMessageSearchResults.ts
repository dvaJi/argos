...
export interface ArgosMessageSearchResultRow {
  id: string;
  session_id: string;
  message_id: string;
...
export class ArgosMessageSearchResultsTable extends BaseTable {
  ...
  getCreateTableSQL(): string {
    return `
      CREATE TABLE IF NOT EXISTS argos_message_search_results (
        id TEXT PRIMARY KEY,
      ...
      CREATE INDEX IF NOT EXISTS idx_search_results_message
        ON argos_message_search_results(message_id, created_at ASC);
      CREATE INDEX IF NOT EXISTS idx_search_results_message_search
        ON argos_message_search_results(message_id, search_id, rank);
      CREATE INDEX IF NOT EXISTS idx_search_results_session
        ON argos_message_search_results(session_id, created_at DESC);
    `;
  }
  ...
  add(row: {
    ...
    const result = this.db
      .prepare(
        `INSERT OR IGNORE INTO argos_message_search_results (
           id,
           session_id,
           message_id,
           search_id,
  ...
  listByMessageId(messageId: string): ArgosMessageSearchResultRow[] {
    return this.db
      .prepare(
        `SELECT * FROM argos_message_search_results
         WHERE message_id = ?
         ORDER BY created_at ASC, CASE WHEN rank IS NULL THEN 2147483647 ELSE rank END ASC`,
      )
      .all(messageId) as ArgosMessageSearchResultRow[];
  }

  deleteBySessionId(sessionId: string): void {
    this.db.prepare("DELETE FROM argos_message_search_results WHERE session_id = ?").run(sessionId);
  }
  ...
  deleteByMessageIds(messageIds: string[]): void {
    ...
    this.db
      .prepare(`DELETE FROM argos_message_search_results WHERE message_id IN (${placeholders})`)
      .run(...messageIds);
  }
