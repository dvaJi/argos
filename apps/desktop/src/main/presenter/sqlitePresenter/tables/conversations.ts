...
export class ConversationsTable extends BaseTable {
  ...
  async get(conversationId: string): Promise<CONVERSATION> {
    ...
    const settings = {
      ...
      reasoningEffort: isReasoningEffort(result.reasoning_effort) ? result.reasoning_effort : undefined,
      ...
      searchStrategy: result.search_strategy ? (result.search_strategy as "turbo" | "max") : undefined,
  ...
  async update(conversationId: string, data: Partial<CONVERSATION>): Promise<void> {
    ...
    if (data.settings) {
      ...
      if (data.settings.agentWorkspacePath !== undefined) {
        ...
        params.push(data.settings.agentWorkspacePath !== null ? data.settings.agentWorkspacePath : null);
      }
      if (data.settings.acpWorkdirMap !== undefined) {
        updates.push("acp_workdir_map = ?");
        params.push(data.settings.acpWorkdirMap ? JSON.stringify(data.settings.acpWorkdirMap) : null);
      }
    }
