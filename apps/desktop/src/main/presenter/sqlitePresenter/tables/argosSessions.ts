...
export interface ArgosSessionRow {
  id: string;
  provider_id: string;
  model_id: string;
...
export interface ArgosSessionSummaryRow {
  summary_text: string | null;
  summary_cursor_order_seq: number | null;
  summary_updated_at: number | null;
}
...
export class ArgosSessionsTable extends BaseTable {
  ...
  override createTable(): void {
    ...
    if (recordedVersion > latestVersion) {
      const message = `Recorded argos_sessions schema version ${recordedVersion} exceeds supported version ${latestVersion}. Refusing to create table from a downgraded schema.`;
  ...
  private getRecoveryMigrationStatements(): string[] {
    ...
    if (!this.hasColumn("system_prompt")) {
      statements.push("ALTER TABLE argos_sessions ADD COLUMN system_prompt TEXT;");
    }
    if (!this.hasColumn("temperature")) {
      statements.push("ALTER TABLE argos_sessions ADD COLUMN temperature REAL;");
    }
    if (!this.hasColumn("context_length")) {
      statements.push("ALTER TABLE argos_sessions ADD COLUMN context_length INTEGER;");
    }
    if (!this.hasColumn("top_p")) {
      statements.push("ALTER TABLE argos_sessions ADD COLUMN top_p REAL;");
    }
    if (!this.hasColumn("max_tokens")) {
      statements.push("ALTER TABLE argos_sessions ADD COLUMN max_tokens INTEGER;");
    }
    if (!this.hasColumn("thinking_budget")) {
      statements.push("ALTER TABLE argos_sessions ADD COLUMN thinking_budget INTEGER;");
    }
    if (!this.hasColumn("reasoning_effort")) {
      statements.push("ALTER TABLE argos_sessions ADD COLUMN reasoning_effort TEXT;");
    }
    if (!this.hasColumn("verbosity")) {
      statements.push("ALTER TABLE argos_sessions ADD COLUMN verbosity TEXT;");
    }
    if (!this.hasColumn("summary_text")) {
      statements.push("ALTER TABLE argos_sessions ADD COLUMN summary_text TEXT;");
    }
    if (!this.hasColumn("summary_cursor_order_seq")) {
      statements.push("ALTER TABLE argos_sessions ADD COLUMN summary_cursor_order_seq INTEGER NOT NULL DEFAULT 1;");
    }
    if (!this.hasColumn("summary_updated_at")) {
      statements.push("ALTER TABLE argos_sessions ADD COLUMN summary_updated_at INTEGER;");
    }
    if (!this.hasColumn("timeout_ms")) {
      statements.push("ALTER TABLE argos_sessions ADD COLUMN timeout_ms INTEGER;");
    }
    if (!this.hasColumn("force_interleaved_thinking_compat")) {
      statements.push("ALTER TABLE argos_sessions ADD COLUMN force_interleaved_thinking_compat INTEGER;");
    }
    if (!this.hasColumn("reasoning_visibility")) {
      statements.push("ALTER TABLE argos_sessions ADD COLUMN reasoning_visibility TEXT;");
    }
    if (!this.hasColumn("image_generation_options_json")) {
      statements.push("ALTER TABLE argos_sessions ADD COLUMN image_generation_options_json TEXT;");
    }
    if (!this.hasColumn("video_generation_options_json")) {
      statements.push("ALTER TABLE argos_sessions ADD COLUMN video_generation_options_json TEXT;");
    }
  ...
  getMigrationSQL(version: number): string | null {
    if (version === 12) {
      return `
        ALTER TABLE argos_sessions ADD COLUMN system_prompt TEXT;
        ALTER TABLE argos_sessions ADD COLUMN temperature REAL;
        ALTER TABLE argos_sessions ADD COLUMN context_length INTEGER;
        ALTER TABLE argos_sessions ADD COLUMN max_tokens INTEGER;
        ALTER TABLE argos_sessions ADD COLUMN thinking_budget INTEGER;
        ALTER TABLE argos_sessions ADD COLUMN reasoning_effort TEXT;
        ALTER TABLE argos_sessions ADD COLUMN verbosity TEXT;
      `;
    }
    if (version === 14) {
      return `
        ALTER TABLE argos_sessions ADD COLUMN summary_text TEXT;
        ALTER TABLE argos_sessions ADD COLUMN summary_cursor_order_seq INTEGER NOT NULL DEFAULT 1;
        ALTER TABLE argos_sessions ADD COLUMN summary_updated_at INTEGER;
      `;
    }
    if (version === 19) {
      return `
    ...
    if (version === 20) {
      return `
        ALTER TABLE argos_sessions ADD COLUMN reasoning_visibility TEXT;
      `;
    }
    if (version === 23) {
    ...
    if (version === 24) {
      return "ALTER TABLE argos_sessions ADD COLUMN timeout_ms INTEGER;";
    }
    if (version === 27) {
      return "ALTER TABLE argos_sessions ADD COLUMN image_generation_options_json TEXT;";
    }
    if (version === 28) {
      return "ALTER TABLE argos_sessions ADD COLUMN video_generation_options_json TEXT;";
    }
    if (version === 29) {
      return "ALTER TABLE argos_sessions ADD COLUMN top_p REAL;";
    }
    return null;
  }
  ...
  private serializeImageGenerationOptions(value: ImageGenerationOptions | undefined): string | null {
  ...
  private serializeVideoGenerationOptions(value: VideoGenerationOptions | undefined): string | null {
  ...
  create(
    ...
    generationSettings?: Partial<ArgosSessionGenerationSettings>,
  ): void {
    this.db
      .prepare(
        `INSERT INTO argos_sessions (
           id,
           provider_id,
           model_id,
           permission_mode,
  ...
  get(id: string): ArgosSessionRow | undefined {
    return this.db.prepare("SELECT * FROM argos_sessions WHERE id = ?").get(id) as ArgosSessionRow | undefined;
  }

  getGenerationSettings(id: string): Partial<ArgosSessionGenerationSettings> | null {
    const row = this.get(id);
    if (!row) {
      return null;
    }

    const settings: Partial<ArgosSessionGenerationSettings> = {};
  ...
  updatePermissionMode(id: string, mode: "default" | "full_access"): void {
    this.db.prepare("UPDATE argos_sessions SET permission_mode = ? WHERE id = ?").run(mode, id);
  }

  updateSessionModel(id: string, providerId: string, modelId: string): void {
    this.db
      .prepare("UPDATE argos_sessions SET provider_id = ?, model_id = ? WHERE id = ?")
      .run(providerId, modelId, id);
  }

  updateGenerationSettings(id: string, settings: Partial<ArgosSessionGenerationSettings>): void {
    ...
    if (Object.prototype.hasOwnProperty.call(settings, "forceInterleavedThinkingCompat")) {
      ...
      params.push(
        settings.forceInterleavedThinkingCompat === undefined ? null : settings.forceInterleavedThinkingCompat ? 1 : 0,
      );
    }
    ...
    params.push(id);
    this.db.prepare(`UPDATE argos_sessions SET ${updates.join(", ")} WHERE id = ?`).run(...params);
  }

  getSummaryState(id: string): ArgosSessionSummaryRow | null {
    const row = this.db
      .prepare("SELECT summary_text, summary_cursor_order_seq, summary_updated_at FROM argos_sessions WHERE id = ?")
      .get(id) as ArgosSessionSummaryRow | undefined;
  ...
  updateSummaryState(
    ...
    this.db
      ...
      .run(state.summaryText ?? null, Math.max(1, state.summaryCursorOrderSeq), state.summaryUpdatedAt ?? null, id);
  }
  ...
  delete(id: string): void {
    this.db.prepare("DELETE FROM argos_sessions WHERE id = ?").run(id);
  }
}
