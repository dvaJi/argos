...
function readSchemaSnapshot(db: Database): Map<string, SchemaSnapshotTable> {
  ...
  for (const { name } of tableRows) {
    ...
    snapshot.set(name, {
      columns: new Map(columns.map((column) => [column.name, normalizeDeclaredType(column.type)] as const)),
  ...
  diagnose(): DatabaseSchemaDiagnosis {
    ...
    for (const table of this.catalog) {
      ...
      for (const column of table.columns) {
        ...
        if (column.checkType && column.declaredType && (actualType === null || actualType !== column.declaredType)) {
          issues.push(
            createIssue({
...
export class DatabaseRepairService {
  ...
  repair(): DatabaseRepairReport {
    ...
    this.db.transaction(() => {
      for (const table of this.catalog) {
        const tableIssues = diagnosisBeforeRepair.issues.filter((issue) => issue.table === table.name);
    ...
    const repairedIssues = diagnosisBeforeRepair.issues.filter((issue) => repairedIssueKeys.has(buildIssueKey(issue)));
