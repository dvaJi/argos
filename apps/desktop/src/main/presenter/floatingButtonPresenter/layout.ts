...
function mapSessionAgent(
  ...
  return {
    ...
    type: session.agentId === "argos" ? "argos" : "acp",
  };
}
...
export function getCollapsedWidgetSize(activeCount: number): Pick<WidgetRect, "width" | "height"> {
  return activeCount > 0 ? { ...FLOATING_WIDGET_LAYOUT.collapsedBusy } : { ...FLOATING_WIDGET_LAYOUT.collapsedIdle };
}

export function getExpandedWidgetSize(sessionCount: number): Pick<WidgetRect, "width" | "height"> {
  const visibleSessions = Math.max(1, Math.min(sessionCount, FLOATING_WIDGET_LAYOUT.maxVisibleSessions));
...
export function getWidgetSizeForSnapshot(snapshot: FloatingWidgetSnapshot): Pick<WidgetRect, "width" | "height"> {
...
export function repositionWidgetForResize(
  ...
  const clampedX = Math.max(workArea.x, Math.min(nextX, workArea.x + workArea.width - nextSize.width));
...
export function getPeekedCollapsedBounds(
  ...
  const x = dockSide === "left" ? workArea.x - hiddenWidth : workArea.x + workArea.width - bounds.width + hiddenWidth;
