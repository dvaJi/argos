...
export class FloatingButtonWindow {
  ...
  constructor(config: FloatingButtonConfig, persistedBounds: FloatingButtonBounds | null = null) {
    ...
    this.dockSide = persistedBounds?.dockSide ?? (config.position.endsWith("left") ? "left" : "right");
  }
  ...
  public async create(): Promise<void> {
    ...
    try {
      ...
      this.dockSide = inferDockSide(initialBounds, screen.getDisplayMatching(initialBounds).workArea);
  ...
  private resolveInitialBounds(): WidgetRect {
    ...
    const y = Math.max(workArea.y, Math.min(defaultPosition.y, workArea.y + workArea.height - height));
  ...
  private getDefaultPosition(): { x: number; y: number } {
    ...
    return {
      x: isRight ? workArea.x + workArea.width - width - this.config.offset.x : workArea.x + this.config.offset.x,
      y: isBottom ? workArea.y + workArea.height - height - this.config.offset.y : workArea.y + this.config.offset.y,
    };
  }
