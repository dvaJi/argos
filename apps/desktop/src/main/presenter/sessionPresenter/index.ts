...
export class SessionPresenter implements ISessionPresenter {
  ...
  initializeLegacyRuntime(): void {
    ...
    eventBus.on(TAB_EVENTS.CLOSED, (webContentsId: number) => {
      ...
      if (activeConversationId) {
        ...
        console.log(`SessionPresenter: Cleaned up conversation binding for closed webContents ${webContentsId}.`);
      }
    });
    eventBus.on(TAB_EVENTS.RENDERER_TAB_READY, () => {
      void this.broadcastThreadListUpdate().catch((error) => {
        if (
          ...
          console.info("[SessionPresenter] Skip legacy thread list broadcast on tab ready: legacy tables not found.");
          return;
        }
    ...
    // Process all unfinished messages on initialization
  ...
  async createSession(params: CreateSessionParams): Promise<string> {
    ...
    return this.createConversation(params.title, params.settings ?? {}, webContentsId, params.options ?? {});
  }
  ...
  async getSessionList(page: number, pageSize: number): Promise<{ total: number; sessions: Session[] }> {
  ...
  async updateSessionSettings(sessionId: string, settings: Partial<Session["config"]>): Promise<void> {
  ...
  async findTabForSession(sessionId: string, preferredWindowType?: "main" | "floating"): Promise<number | null> {
  ...
  async generateTitle(sessionId: string): Promise<string> {
    ...
    const assistantModel = this.configPresenter.getSetting<{ providerId: string; modelId: string }>("assistantModel");
    ...
    try {
      ...
      console.warn("[SessionPresenter] Failed to generate title with assistant model, fallback to conversation model", {
        preferredProviderId,
        preferredModelId,
        fallbackProviderId,
        fallbackModelId,
  ...
  private clearActiveConversationBindingInternal(webContentsId: number, options: { notify?: boolean } = {}): void {
  ...
  async openConversationInNewWindow(payload: {
    ...
    const existingWebContentsId = await this.conversationManager.findWebContentsForConversation(conversationId);
    ...
    if (messageId || childConversationId) {
      await presenter.windowPresenter.sendToWebContents(targetWebContentsId, CONVERSATION_EVENTS.SCROLL_TO_MESSAGE, {
        conversationId,
        messageId,
        childConversationId,
      });
  ...
  async openConversationInNewTab(payload: {
    ...
    if (typeof tabId === "number") {
      ...
      if (messageId || childConversationId) {
        await presenter.windowPresenter.sendToWebContents(tabId, CONVERSATION_EVENTS.SCROLL_TO_MESSAGE, {
          conversationId,
          messageId,
          childConversationId,
        });
  ...
  async createConversation(
    ...
    const conversationId = await this.conversationManager.createConversation(title, settings, webContentsId, options);
  ...
  async updateConversationSettings(conversationId: string, settings: Partial<CONVERSATION_SETTINGS>): Promise<void> {
    await this.conversationManager.updateConversationSettings(conversationId, settings);
  }

  async getConversationList(page: number, pageSize: number): Promise<{ total: number; list: CONVERSATION[] }> {
  ...
  async getSearchResults(messageId: string, searchId?: string): Promise<SearchResult[]> {
    ...
    if (searchId) {
      ...
      // Legacy compatibility: if no matching searchId, fall back to results without a searchId
  ...
  // Look up the generation status for a specific session
   ...
   */
  private applyVariantSelection(messages: Message[], selectedVariantsMap: Record<string, string>): Message[] {
  ...
  /**
   * Create a fork of the conversation
   * @param targetConversationId Source conversation ID
   * @param targetMessageId Target message ID (all messages up to and including this message will be copied)
   * @param newTitle New conversation title
   * @param settings New conversation settings
   * @param selectedVariantsMap Selected variant map (optional)
  ...
  async createChildConversationFromSelection(payload: {
    ...
    const targetWebContentsId =
      typeof webContentsId === "number" ? webContentsId : typeof tabId === "number" ? tabId : undefined;
   ...
   * @param format Export format ('markdown' | 'html' | 'txt')
   ...
   */
  async updateNowledgeMemConfig(config: { baseUrl?: string; apiKey?: string; timeout?: number }): Promise<void> {
  ...
  private toSession(conversation: CONVERSATION): Session {
    const boundWebContentsIds = this.conversationManager.getWebContentsIdsByConversation(conversation.id);
    const webContentsId = boundWebContentsIds.length > 0 ? boundWebContentsIds[0] : null;
    const targetContents = typeof webContentsId === "number" ? electronWebContents.fromId(webContentsId) : null;
    const targetWindow =
      targetContents && !targetContents.isDestroyed() ? BrowserWindow.fromWebContents(targetContents) : null;
    ...
    const windowType =
      targetWindow == null
        ...
        : floatingWindow && floatingWindow.id === targetWindow.id
          ? "floating"
          : "main";
    const settings = conversation.settings as unknown as Omit<Session["config"], "sessionId" | "title" | "isPinned">;
    ...
    return {
      ...
      context: {
        resolvedChatMode: (conversation.settings.chatMode ?? "chat") as Session["context"]["resolvedChatMode"],
