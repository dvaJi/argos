...
const sortFilesByModified = async (files: string[]) => {
  const entries = await Promise.all(files.map(async (file) => ({ file, mtimeMs: await getMtime(file) })));
...
export async function searchFiles(
  ...
  if (!cached) {
    const targetLimit = Math.min(Math.max(offset + pageSize + 1, DEFAULT_CACHE_LIMIT), MAX_CACHE_FILES);
    ...
    const sortedFiles = sortBy === "modified" ? await sortFilesByModified(trimmedFiles) : sortFilesByName(trimmedFiles);
