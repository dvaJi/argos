...
export function resolveWorkspacePath(workspaceRoot: string, inputPath: string): string | null {
  ...
  const absolute = path.isAbsolute(expanded) ? path.resolve(expanded) : path.resolve(workspaceRoot, expanded);
  ...
  const workspaceWithSep = workspaceReal.endsWith(path.sep) ? workspaceReal : `${workspaceReal}${path.sep}`;
