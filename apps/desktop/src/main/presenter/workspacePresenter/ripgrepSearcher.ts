...
export class RipgrepSearcher {
  static async *files(
    ...
    const rgExecutable = ripgrepPath ? path.join(ripgrepPath, process.platform === "win32" ? "rg.exe" : "rg") : "rg";
    ...
    try {
      for await (const line of rl) {
        ...
        const pathValue = typeof parsed.data?.path === "string" ? parsed.data.path : parsed.data?.path?.text;
