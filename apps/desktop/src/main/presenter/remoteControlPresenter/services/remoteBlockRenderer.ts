...
const normalizeText = (value: string | undefined | null): string => (value ?? "").replace(/\r\n/g, "\n").trim();
...
const summarizeText = (
  ...
  return {
    preview: normalized.length > limit ? `${normalized.slice(0, limit).trimEnd()}\n...[truncated]` : normalized,
...
const formatSearchSnippet = (result: SearchResult): string => {
  ...
  return snippet.length > SEARCH_SNIPPET_LIMIT ? `${snippet.slice(0, SEARCH_SNIPPET_LIMIT).trimEnd()}...` : snippet;
};
...
const formatSearchBlock = (
  ...
  const label = normalizeText(typeof block.extra?.label === "string" ? block.extra.label : "") || "search";
...
const formatImageNoticeBlock = (block: AssistantMessageBlock, asset?: RemoteGeneratedImageAsset): string => {
...
const escapeTracePreview = (value: string): string => value.replace(/\\/g, "\\\\").replace(/"/g, '\\"');
...
const getTraceEmoji = (toolName: string): string => {
  ...
  if (normalized === "grep" || normalized === "find" || normalized.includes("search") || normalized.includes("grep")) {
    return "🔎";
  }

  if (normalized === "read" || normalized === "cat" || normalized.includes("read") || normalized.includes("open")) {
    return "📖";
  }

  if (normalized === "write" || normalized === "edit" || normalized.includes("write") || normalized.includes("edit")) {
    return "📝";
  }
...
const getProcessLogLines = (block: AssistantMessageBlock): string[] => {
  ...
  const toolName = normalizeText(block.tool_call?.name) || "unknown_tool";
  const lines = [`${getTraceEmoji(toolName)} ${toolName}: "${getTracePreview(block.tool_call?.params, toolName)}"`];

  if (block.status === "error") {
    lines.push(`❌ ${toolName}: "${getTracePreview(block.tool_call?.response || block.content, undefined, "error")}"`);
  }
...
const extractTrailingPendingNarrativeBlocks = (blocks: AssistantMessageBlock[]): AssistantMessageBlock[] => {
  const trailing: AssistantMessageBlock[] = [];

  for (let index = blocks.length - 1; index >= 0; index -= 1) {
    const block = blocks[index];
    if (block.status !== "pending" || (block.type !== "content" && block.type !== "reasoning_content")) {
      break;
    }
...
export const buildRemoteDraftText = (blocks: AssistantMessageBlock[]): string =>
  extractTrailingPendingNarrativeBlocks(blocks)
    .map((block) => {
      ...
      return block.type === "reasoning_content" ? formatReasoningBlock(content) : formatAnswerBlock(content);
    })
    .filter(Boolean)
    .join("\n\n")
    .trim();
...
export const buildRemoteStatusText = (blocks: AssistantMessageBlock[], pendingInteraction: boolean = false): string => {
...
export const buildRemoteRenderableBlocks = async (
  ...
  for (const [index, block] of blocks.entries()) {
    ...
    if (block.type === "search" && block.status !== "pending") {
      const searchId =
        normalizeText(typeof block.extra?.searchId === "string" ? block.extra.searchId : "") || normalizeText(block.id);
      const searchBlock = formatSearchBlock(block, await loadSearchResults(messageId, searchId || undefined));
      rendered.push({
