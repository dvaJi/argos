...
export class RemoteConversationRunner {
  ...
  async sendInput(
    ...
    const previousActiveEventId = this.deps.agentRuntimePresenter.getActiveGeneration(session.id)?.eventId ?? null;
  ...
  async stop(endpointKey: string): Promise<boolean> {
    ...
    const stopped = await this.deps.agentRuntimePresenter.cancelGenerationByEventId(session.id, activeEventId);
    if (stopped) {
      this.bindingStore.clearActiveEvent(endpointKey);
    }
    return stopped;
  ...
  private async resolveChatWindow(): Promise<BrowserWindow | null> {
    ...
    if (focusedWindow && !focusedWindow.isDestroyed() && chatWindows.some((window) => window.id === focusedWindow.id)) {
