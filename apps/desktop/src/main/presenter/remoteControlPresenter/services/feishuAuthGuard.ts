...
export class FeishuAuthGuard {
  ...
  ensureAuthorized(message: FeishuInboundMessage): FeishuAuthResult {
    ...
    return {
      ok: false,
      message: "This Feishu account is not paired. Open Remote settings and use the current /pair code.",
    };
  }
  ...
  pair(message: FeishuInboundMessage, rawCode: string): string {
    ...
    if (pairing.code !== normalizedCode) {
      const result = this.bindingStore.recordPairCodeFailure("feishu", REMOTE_PAIR_CODE_MAX_FAILURES);
      if (result.exhausted) {
