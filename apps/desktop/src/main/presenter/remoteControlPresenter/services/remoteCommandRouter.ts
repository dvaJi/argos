...
export class RemoteCommandRouter {
  ...
  private formatStartMessage(isAuthorized: boolean): string {
    const statusLine = isAuthorized
      ? "Status: paired"
      : "Status: not paired. Use /pair <code> from Argos Remote settings.";

    return ["Argos Telegram remote control is ready.", statusLine, "Use /help to see the available commands."].join(
      "\n",
    );
  }
    ...
    return [
      ...
      ...TELEGRAM_REMOTE_COMMANDS.map((item) =>
        item.command === "pair"
          ...
          : item.command === "new"
            ? "/new [title] - Start a new Argos session"
            : item.command === "use"
  ...
  private formatAgentButtonLabel(agent: TelegramAgentOption): string {
    const typeLabel = agent.agentType === "acp" ? "ACP" : "Argos";
