...
export class DiscordAuthGuard {
  ...
  ensureAuthorized(message: DiscordInboundMessage): DiscordAuthResult {
    ...
    return {
      ...
      message:
        message.chatType === "dm"
          ? "This Discord DM is not paired. Generate a pair code in Argos Remote settings, then use /pair <code>."
          : "This Discord channel is not authorized. Generate a pair code in Argos Remote settings, then use /pair <code> in this channel.",
    };
  }
  ...
  pair(message: DiscordInboundMessage, rawCode: string): string {
    ...
    if (!pairing.code || !pairing.expiresAt || pairing.expiresAt <= Date.now()) {
      ...
      return "Pairing code is missing or expired. Generate a new code from Argos Remote settings.";
    }

    if (pairing.code !== normalizedCode) {
      const result = this.bindingStore.recordPairCodeFailure("discord", REMOTE_PAIR_CODE_MAX_FAILURES);
      if (result.exhausted) {
        return "Too many invalid pairing attempts. The current pairing code has expired. Generate a new code from Argos Remote settings.";
      }
