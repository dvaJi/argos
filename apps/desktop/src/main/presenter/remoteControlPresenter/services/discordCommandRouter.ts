...
export class DiscordCommandRouter {
    ...
    try {
      ...
      switch (command) {
        ...
        case "use": {
          ...
          const session = await this.deps.runner.useSessionByIndex(endpointKey, index - 1, bindingMeta);
          return {
            ...
            replies: [
              openResult.status === "ok"
                ...
                : openResult.status === "windowNotFound"
                  ? "Could not find a Argos desktop window. Open Argos and try /open again."
      ...
      if (message.kind === "interaction") {
        return {
          replies: ["Slash commands only support remote control commands. Send a regular message to chat with Argos."],
        };
      }
  ...
  private async handleModelCommand(
    ...
    const updatedSession = await this.deps.runner.setSessionModel(endpointKey, provider.providerId, model.modelId);
  ...
  private async handleAgentCommand(
    ...
    return {
      replies: [
        [
          `Agent switched to ${result.agent.agentName} [${result.agent.agentId}] (${result.agent.agentType === "acp" ? "ACP" : "Argos"}).`,
  ...
  private formatModelOverview(session: SessionWithState, providers: TelegramModelProviderOption[]): string {
    return [
      ...
      ...providers.flatMap((provider) => [
        ...
        ...provider.models.map((model) => `- ${model.modelName} (${provider.providerId} ${model.modelId})`),
      ]),
    ].join("\n");
  }
  ...
  private formatAgentOverview(session: SessionWithState, agents: TelegramAgentOption[]): string {
    return [
      ...
      ...agents.map(
        (agent) =>
          `- ${agent.agentName} [${agent.agentId}] (${agent.agentType === "acp" ? "ACP" : "Argos"}${agent.source ? `, ${agent.source}` : ""})`,
      ),
    ].join("\n");
  }
  ...
  private formatStartMessage(chatType: DiscordInboundMessage["chatType"], authorized: boolean): string {
    if (authorized) {
      return [
        "Argos Discord Remote is ready.",
    ...
    if (chatType === "channel") {
      return [
        "Argos Discord Remote is online.",
        "Authorize this channel with /pair <code> before using remote control here.",
      ].join("\n");
    }

    return ["Argos Discord Remote is online.", "Pair this DM with /pair <code> before using remote control."].join(
      "\n",
    );
  }
