...
export abstract class ChannelAdapter extends EventEmitter implements IChannelAdapter {
  ...
  async connect(): Promise<void> {
    ...
    this.connectPromise = (async () => {
      ...
      try {
        ...
        this.setStatus({
          connected: true,
          state: this.statusSnapshot.state === "error" ? "running" : this.statusSnapshot.state || "running",
          lastError: null,
        });
      } catch (error) {
  ...
  protected setStatus(next: Partial<ChannelStatusSnapshot> & { state?: ChannelStatusSnapshot["state"] }): void {
    this.statusSnapshot = {
      ...this.statusSnapshot,
      ...next,
  ...
  protected emitLog(level: ChannelLogLevel, message: string, context?: Record<string, unknown>): void {
  ...
  protected async downloadFileAsBase64(url: string, filename: string): Promise<FileAttachment | null> {
    const response = await this.fetchBinaryAttachment(url);
    if (!response) {
      return null;
    }
  ...
  private async fetchBinaryAttachment(url: string): Promise<{
    ...
    const declaredSize =
      typeof contentLength === "string" && contentLength.trim() ? Number.parseInt(contentLength, 10) : Number.NaN;
