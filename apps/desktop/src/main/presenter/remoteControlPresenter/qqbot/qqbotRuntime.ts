...
export class QQBotRuntime {
  ...
  private async acceptDispatch(payload: Parameters<QQBotParser["parseDispatch"]>[0], runId: number): Promise<void> {
    if (!this.isCurrentRun(runId)) {
      return;
    }
  ...
  private async deliverConversation(
    ...
    while (this.isCurrentRun(runId)) {
      ...
      if (snapshot.completed) {
        ...
        const didFlushProcessOutput = await this.flushBufferedProcessMessages(endpointKey, sendContext, {
  ...
  private getOrCreateToolBuffer(endpointKey: string, sourceMessageId: string | null): QQBotToolBufferState | null {
  ...
  private migrateToolBufferSourceMessageId(state: QQBotToolBufferState, nextSourceMessageId: string): void {
    ...
    state.pendingProcessSegments = state.pendingProcessSegments.map((batch) => ({
      ...
      keys: this.dedupeKeysInOrder(
        batch.keys.map((key) => this.rewriteToolBufferKey(key, previousSourceMessageId, nextSourceMessageId)),
      ),
    }));
    state.sourceMessageId = nextSourceMessageId;
  }

  private rewriteToolBufferKey(key: string, previousSourceMessageId: string, nextSourceMessageId: string): string {
  ...
  private buildBufferedProcessText(state: QQBotToolBufferState, batch: QQBotPendingProcessBatch): string {
    return batch.keys
  ...
  private markProcessBatchFlushed(state: QQBotToolBufferState, batch: QQBotPendingProcessBatch): void {
  ...
  private async sendGeneratedImages(
    ...
    for (const asset of snapshot.generatedImages ?? []) {
      ...
      try {
        ...
        sendContext.sentCount -= 1;
        sendContext.nextMsgSeq -= 1;
        await this.sendText(sendContext, "[Image] Delivery failed - see local copy in the app.").catch(() => undefined);
      }
    }
  }
