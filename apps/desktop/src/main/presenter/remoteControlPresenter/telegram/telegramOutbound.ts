...
export const createTelegramDraftId = (): number => Math.max(1, Math.trunc(Math.random() * 2_000_000_000));
...
const collectText = (blocks: AssistantMessageBlock[], predicate: (block: AssistantMessageBlock) => boolean): string =>
  blocks
    .filter(predicate)
...
export const chunkTelegramText = (text: string, limit: number = TELEGRAM_OUTBOUND_TEXT_LIMIT): string[] => {
