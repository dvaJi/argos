...
const normalizeText = (value: string | null | undefined): string | undefined => value?.trim() || undefined;
...
const extractAttachmentFromItem = (item: WeixinIlinkMessageItem, index: number) => {
  ...
  if (item.file_item) {
    ...
    return {
      id: `${index}`,
      filename: item.file_item.filename?.trim() || item.file_item.file_name?.trim() || `attachment-${index + 1}`,
      mediaType,
...
export class WeixinIlinkParser {
  parseMessage(accountId: string, raw: WeixinIlinkInboundApiMessage): WeixinIlinkInboundMessage | null {
    ...
    return {
      ...
      text,
      messageId:
        String(raw.message_id ?? "").trim() || `${userId}:${String(raw.seq ?? raw.create_time_ms ?? Date.now())}`,
