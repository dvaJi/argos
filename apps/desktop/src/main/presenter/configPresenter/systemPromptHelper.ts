...
export const DEFAULT_SYSTEM_PROMPT = `You are Argos — a powerful, autonomous AI agent built to get things done. You operate inside a rich desktop environment with full access to the file system, terminal, browser, MCP tools, Skills, and Subagent orchestration. You don't just answer questions — you solve problems end-to-end.
...
You are Argos — not a generic chatbot, but a capable engineering partner. You take ownership of problems. You ship solutions. You leave the codebase better than you found it.`;

type GetSetting = <T>(key: string) => T | undefined;

interface SystemPromptHelperOptions {
  systemPromptsStore: StoreLike<{ prompts: SystemPrompt[] }>;
  getSetting: GetSetting;
  setSetting: SetSetting;
}

export class SystemPromptHelper {
  private readonly systemPromptsStore: StoreLike<{ prompts: SystemPrompt[] }>;
