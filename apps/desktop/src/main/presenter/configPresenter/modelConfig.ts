...
export class ModelConfigHelper {
  ...
  constructor(appVersion: string = "0.0.0", storeFactory?: StoreFactory) {
    if (storeFactory) {
      this.modelConfigStore = storeFactory<ModelConfigStoreSchema>({
        name: "model-config",
      });
    } else {
      // Fallback: create a minimal in-memory store
      const data = {} as ModelConfigStoreSchema & Record<string, unknown>;
  ...
  /**
   * Get model config (priority: user-defined > remote cache/local built-in Provider DB strict match > default fallback)
   * Strict matching requires providerId and modelId to be exactly equal; fuzzy matching is no longer performed.
   */
  getModelConfig(modelId: string, providerId?: string): ModelConfig {
    ...
    // Lowercase for strict DB matching; read user config as-is first, then try lowercase key
    const normModelIdRaw = modelId ? modelId.toLowerCase() : modelId;
    // Compat with Google Gemini SDK's `models/` prefixed model IDs
    ...
    if (providerId) {
      ...
      if (cachedEntry?.config) {
        ...
        // Second attempt: lowercase key (compat for historical storage keys with inconsistent case)
    ...
    // Strict match: look up from Provider DB only when providerId is provided
