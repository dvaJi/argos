...
export class UiSettingsHelper {
  ...
  getAutoCompactionTriggerThreshold(): number {
    return this.normalizeAutoCompactionTriggerThreshold(this.getSetting<number>("autoCompactionTriggerThreshold"));
  }
  ...
  getAutoCompactionRetainRecentPairs(): number {
    return this.normalizeAutoCompactionRetainRecentPairs(this.getSetting<number>("autoCompactionRetainRecentPairs"));
  }
  ...
  private normalizeStoredFont(value?: string | null): string {
    ...
    if (this.systemFontsCache?.length) {
      const match = this.systemFontsCache.find((font) => font.toLowerCase() === collapsed.toLowerCase());
  ...
  private normalizeAutoCompactionTriggerThreshold(value: unknown): number {
    ...
    return Math.min(AUTO_COMPACTION_TRIGGER_THRESHOLD_MAX, Math.max(AUTO_COMPACTION_TRIGGER_THRESHOLD_MIN, rounded));
  }
