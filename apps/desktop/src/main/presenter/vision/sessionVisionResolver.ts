...
type SessionVisionResolverParams = {
  ...
  agentId?: string | null;
  signal?: AbortSignal;
  configPresenter: Pick<IConfigPresenter, "getModelConfig" | "resolveArgosAgentConfig" | "isKnownModel">;
  logLabel?: string;
};
...
export async function resolveSessionVisionTarget(
  ...
  try {
    throwIfAbortRequested(params.signal);
    const agentConfig = await params.configPresenter.resolveArgosAgentConfig(agentId);
    throwIfAbortRequested(params.signal);
