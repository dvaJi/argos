...
export class FloatingChatWindow {
  ...
  private registerWindowContent(): void {
    ...
    try {
      ...
      if (tabPresenter) {
        const webContentsId = this.window.webContents.id;
        logger.info(`Registering floating window webContents bridge, WebContents ID: ${webContentsId}`);
  ...
  private unregisterWindowContent(): void {
    ...
    try {
      ...
      if (tabPresenter) {
        const webContentsId = this.window.webContents.id;
        logger.info(`Unregistering floating window webContents bridge, WebContents ID: ${webContentsId}`);
  ...
  private calculatePosition(floatingButtonPosition?: FloatingButtonPosition): {
    ...
    const finalY = Math.max(workArea.y, Math.min(idealY, workArea.y + workArea.height - windowHeight));
