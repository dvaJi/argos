    ...
    app.on("activate", function () {
      ...
      if (allWindows.length === 0) {
        ...
        if (!targetWindow.isDestroyed()) {
          ...
          console.warn("eventListenerSetupHook: App activated but target window is destroyed, creating new window.");
          presenter.windowPresenter.createAppWindow({
            initialRoute: "chat",
          });
        }
    ...
    eventBus.on(TRAY_EVENTS.CHECK_FOR_UPDATES, async () => {
      try {
        ...
        const triggerUpdateCheck = () => {
          presenter.windowPresenter.sendToWindow(settingsWindowId, SETTINGS_EVENTS.CHECK_FOR_UPDATES);
        };
