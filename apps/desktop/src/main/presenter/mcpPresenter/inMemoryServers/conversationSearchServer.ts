...
export class ConversationSearchServer {
  ...
  private async searchConversations(query: string, limit: number = 10, offset: number = 0): Promise<SearchResult> {
    try {
      ...
      const rows = db.prepare(conversationSql).all(searchQuery, searchQuery, searchQuery, searchQuery, limit, offset);

      const conversations = rows.map((row: any) => {
        const matchedText = row.matchedContent ? this.getSearchableContent(String(row.matchedContent)) : "";
        const snippet = matchedText ? this.createSnippet(matchedText, query) : `Title match: ${String(row.title)}`;
  ...
  private async getConversationStats(days: number = 30) {
    try {
      ...
      const totalMessages = db.prepare("SELECT COUNT(*) as count FROM argos_messages").get();
    ...
    try {
      ...
      if (Array.isArray(parsed)) {
        const segments = parsed
          .map((block: { content?: unknown }) => (typeof block?.content === "string" ? block.content : ""))
  ...
  private createSnippet(content: string, query: string, maxLength: number = 200): string {
    ...
    // Highlight keywords - escape special characters and validate safety
