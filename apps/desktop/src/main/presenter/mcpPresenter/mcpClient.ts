  ...
  async connect(): Promise<void> {
    ...
    try {
      ...
      this.client = new Client(
        { name: "Argos", version: app.getVersion() },
        {
          capabilities: {
            sampling: {},
          },
    ...
    if (signal) {
      decision = await new Promise<McpSamplingDecision>((resolve, reject) => {
        const onAbort = () => {
          ...
          void presenter.mcpPresenter.cancelSamplingRequest(payload.requestId, "cancelled by server").catch((error) => {
...
export async function createMcpClient(serverName: string): Promise<McpClient> {
  // Get MCP server config from configPresenter
