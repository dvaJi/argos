...
export class NowledgeMemPresenter {
   ...
   */
  async submitThread(thread: NowledgeMemThread): Promise<NowledgeMemApiResponse<NowledgeMemThread>> {
    try {
      await this.ensureConfigLoaded();
      ...
      return {
        ...
        error: response.ok
          ? undefined
          : responseData.error || responseData.message || rawText || `HTTP ${response.status}: ${response.statusText}`,
      };
    } catch (error) {
  ...
  validateThreadForSubmission(thread: NowledgeMemThread): {
    ...
    if (thread.messages) {
      thread.messages.forEach((message, index) => {
        ...
        if (message.content && message.content.length > 50000) {
          warnings.push(`Message ${index + 1} content is very large (${message.content.length} characters)`);
        }
      });
    }
    ...
    if (jsonSize > 10000000) {
      // 10MB
      errors.push(`Thread data is too large (${Math.round(jsonSize / 1024 / 1024)}MB). Maximum size is 10MB`);
    } else if (jsonSize > 5000000) {
      // 5MB
      warnings.push(`Thread data is large (${Math.round(jsonSize / 1024 / 1024)}MB). Upload may take some time`);
    }
