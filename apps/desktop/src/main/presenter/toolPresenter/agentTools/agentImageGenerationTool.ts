...
export class AgentImageGenerationTool {
  ...
  async call(
    ...
    if (!model) {
      return this.buildErrorResult(
        ...
        "No available image generation model is configured for this Argos Agent.",
        parsed.data,
      );
    }
    ...
    try {
      const result = await this.options.runtimePort
        ...
        .generateImageStandalone(model.providerId, parsed.data.prompt, model.modelId, imageOptions, {
  ...
  private async resolveImageGenerationModel(conversationId?: string): Promise<ImageGenerationModelSelection | null> {
    if (!conversationId) {
      return null;
    }
    ...
    try {
      ...
      if (!session || session.agentType !== "argos") {
        return null;
      }

      const config = await this.options.configPresenter.resolveArgosAgentConfig(session.agentId);
