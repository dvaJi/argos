...
function buildToolDefinition(name: TapeToolName, description: string, schema: z.ZodTypeAny): MCPToolDefinition {
  return {
    ...
    function: {
      name,
      description,
      parameters: toJSONSchema(schema, { unrepresentable: "any" }) as {
        type: string;
    ...
    server: {
      ...
      icons: "T",
      description: "Argos session tape tools",
    },
  };
}

function createTapeResult(toolName: TapeToolName, result: unknown, summary: string): AgentToolCallResult {
...
function toTapeAnchorOverview(anchor: { name: string | null; entryId: number; createdAt: number }): TapeAnchorOverview {
  return {
    name: anchor.name,
...
export class AgentTapeToolHandler {
  ...
  getToolDefinitions(): MCPToolDefinition[] {
    return [
      buildToolDefinition(
        ...
        "Inspect this Argos-scoped append-only tape subset inspired by bub tape.info. Returns entry counts, anchor state, token usage, and migration status for the current session.",
        tapeInfoSchema,
      ),
      buildToolDefinition(
        TAPE_TOOL_NAMES.search,
        "Search this Argos-scoped append-only tape subset inspired by bub tape.search. Supports text query plus optional kind and created-at filters for the current session.",
        tapeSearchSchema,
      ),
      buildToolDefinition(
        TAPE_TOOL_NAMES.anchors,
        "List recent bub-style anchors for this Argos session tape. Use this before handoff when you need to inspect recent phase transitions or reconstruction checkpoints.",
        tapeAnchorsSchema,
      ),
      buildToolDefinition(
        TAPE_TOOL_NAMES.handoff,
        "Write a bub-style phase-transition anchor to this Argos session tape. The anchor becomes the durable reconstruction marker for later context builds; include a compact summary when earlier history should be carried forward.",
        tapeHandoffSchema,
      ),
    ];
  }
  ...
  async call(
    ...
    return createTapeResult(toolName, overview, `Wrote tape handoff anchor ${overview.name ?? "unknown"}.`);
  }
}
