...
export const CHAT_SETTINGS_TOOL_NAMES = {
  toggle: "argos_settings_toggle",
  setLanguage: "argos_settings_set_language",
  setTheme: "argos_settings_set_theme",
  setFontSize: "argos_settings_set_font_size",
  open: "argos_settings_open",
} as const;
...
const buildError = (errorCode: ApplyError["errorCode"], message: string, details?: unknown): ApplyError => ({
  ok: false,
  errorCode,
  message,
  ...(details ? { details } : {}),
...
export class ChatSettingsToolHandler {
  ...
  private async ensureSkillActive(conversationId?: string): Promise<ApplyChatSettingResult | null> {
    ...
    if (!activeSkills.includes(CHAT_SETTINGS_SKILL_NAME)) {
      return buildError("skill_inactive", "argos-settings skill is not active.");
    }
    return null;
  }
  ...
  async setLanguage(raw: unknown, conversationId?: string): Promise<ApplyChatSettingResult> {
    ...
    try {
      ...
      return buildError(
        "apply_failed",
        "Failed to apply Argos language.",
        error instanceof Error ? error.message : String(error),
      );
    }
  ...
  async setFontSize(raw: unknown, conversationId?: string): Promise<ApplyChatSettingResult> {
    ...
    try {
      ...
      return buildError(
        "apply_failed",
        "Failed to apply Argos font size.",
        error instanceof Error ? error.message : String(error),
      );
    }
  ...
  if (allowToggle) {
    definitions.push({
      ...
      server: {
        name: "argos-settings",
        icons: "settings",
        description: "Argos settings control",
      },
    });
  }
