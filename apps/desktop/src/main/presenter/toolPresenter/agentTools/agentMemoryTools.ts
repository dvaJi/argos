...
function buildToolDefinition(name: MemoryToolName, description: string, schema: z.ZodTypeAny): MCPToolDefinition {
  return {
    type: "function",
    function: {
      name,
    ...
    server: {
      ...
      icons: "\u{1F9E0}",
      description: "Argos long-term memory tools",
    },
  };
}

function createMemoryResult(toolName: MemoryToolName, result: unknown, summary: string): AgentToolCallResult {
...
export class AgentMemoryToolHandler {
  ...
  getToolDefinitions(): MCPToolDefinition[] {
    return [
      ...
      buildToolDefinition(MEMORY_TOOL_NAMES.recall, "Recall relevant long-term memories for a query.", recallSchema),
      buildToolDefinition(
