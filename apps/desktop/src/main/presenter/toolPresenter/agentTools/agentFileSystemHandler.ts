...
import { validateGlobPattern, validateRegexPattern } from "@argos/shared/regexValidator";
...
const ReadFileArgsSchema = z.object({
  ...
  limit: z
    ...
    .positive()
    .optional()
    .describe("Maximum characters to read per file. Large files are auto-truncated if not specified"),
});
...
const GlobSearchArgsSchema = z.object({
  ...
  sortBy: z.enum(["name", "modified"]).default("name").describe("Sort results by name or modification time"),
});
...
export class AgentFileSystemHandler {
  ...
  constructor(allowedDirectories: string[], options: { conversationId?: string; allowExternalAccess?: boolean } = {}) {
  ...
  private async validatePath(
    ...
    try {
      ...
      try {
        ...
        if (pathResolutionError instanceof Error && pathResolutionError.message.startsWith("Access denied")) {
  ...
  private assertSessionReadAllowed(candidatePath: string): void {
    ...
    const sessionWithSeparator = sessionDir.endsWith(path.sep) ? sessionDir : `${sessionDir}${path.sep}`;
  ...
  private async runRipgrepSearch(
    ...
    const rgExecutable = process.platform === "win32" ? path.join(ripgrepPath, "rg.exe") : path.join(ripgrepPath, "rg");
  ...
  async editText(args: unknown, baseDirectory?: string): Promise<string> {
    ...
    if (parsed.data.operation === "edit_lines" && parsed.data.edits) {
      ...
      try {
        ...
        throw new Error(error instanceof Error ? error.message : `Invalid pattern: ${String(error)}`);
      }
      ...
      .map((match) => {
        ...
        if (match.beforeContext && match.beforeContext.length > 0) {
          const beforeLines = match.beforeContext
            .map((line, i) => `${match.file}:${match.line - match.beforeContext!.length + i}: ${line}`)
            .join("\n");
          output = beforeLines + "\n" + output;
        }
  ...
  async textReplace(args: unknown, baseDirectory?: string): Promise<string> {
    ...
    const result = await this.replaceTextInFile(validPath, parsed.data.pattern, parsed.data.replacement, {
  ...
  async editFile(args: unknown, baseDirectory?: string): Promise<string> {
    ...
    const { originalCode, updatedCode } = this.buildTruncatedDiff(normalizedContent, modifiedContent, 3);
    const language = getLanguageFromFilename(validPath);
    const response: DiffToolResponse = {
      success: true,
      originalCode,
