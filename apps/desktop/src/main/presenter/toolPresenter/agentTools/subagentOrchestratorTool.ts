...
const SUBAGENT_WORKDIR_RULE = "Every child session inherits the same working directory as the parent session.";
...
export class SubagentOrchestratorTool {
  ...
  private buildRunProgressResult(run: MutableRunState, label = "Subagent run status"): AgentToolCallResult {
    const content = [
      `${label}: \`${run.runId}\``,
      `Status: ${run.status}`,
      "",
  ...
  private async waitForRunCompletion(run: MutableRunState, timeoutMs: number, signal?: AbortSignal): Promise<void> {
  ...
  private async getAvailableSession(conversationId?: string): Promise<ConversationSessionInfo | null> {
    if (!conversationId) {
      return null;
    }
    ...
    return session.agentType === "argos" &&
  ...
  private buildSlotIdParameter(slots: ArgosSubagentSlot[]) {
    ...
    const slotLines = normalizedSlots.map((slot) => {
      const target = slot.targetType === "self" ? "current agent" : (slot.targetAgentId?.trim() ?? "configured agent");
  ...
  async getToolDefinition(conversationId?: string): Promise<MCPToolDefinition | null> {
    ...
    return {
        ...
        parameters: {
          ...
          properties: {
            operation: {
              ...
              description: "Use run to start tasks. Use list/info/log/wait/kill to manage background runs.",
            },
            mode: {
              type: "string",
              enum: ["parallel", "chain"],
              description: "Required for operation=run. Choose whether delegated tasks run concurrently or one by one.",
            },
            tasks: {
              type: "array",
              maxItems: 5,
              ...
              items: {
                ...
                properties: {
                  ...
                  title: {
                    type: "string",
                    description: "Short task label shown in progress cards and the final aggregate result.",
                  },
                  prompt: {
                    type: "string",
            ...
            background: {
              type: "boolean",
              description: "When true, start operation=run in the background and return a runId immediately.",
            },
            runId: {
              type: "string",
      ...
      server: {
        ...
        icons: "🧩",
        description: "Argos subagent orchestration",
      },
    };
  }
  ...
  async call(
    ...
    if (parent.agentType !== "argos" || parent.sessionKind !== "regular" || parent.subagentEnabled !== true) {
      throw new Error("subagent_orchestrator is only available in Argos regular sessions with subagents enabled.");
    }
    ...
    const tasks = taskSpecs.map((task, index): MutableTaskState => {
      ...
      const targetAgentId = slot.targetType === "self" ? parent.agentId : (slot.targetAgentId?.trim() ?? null);
      if (!targetAgentId) {
    ...
    const updateTaskStatusFromRuntime = (task: MutableTaskState) => {
      ...
      if (task.runtimeStatus === "error") {
        task.status = "error";
        task.resultSummary = task.resultSummary || summarizeResult(task.responseMarkdown) || "Child session failed.";
        maybeResolveTask(task);
        return;
      }
    ...
    const unsubscribe = this.runtimePort.subscribeArgosSessionUpdates((update) => {
    ...
    const runTask = async (task: MutableTaskState): Promise<void> => {
      ...
      try {
        ...
        task.resultSummary = error instanceof Error ? error.message : "Subagent session failed unexpectedly.";
        maybeResolveTask(task);
        await this.finalizeTaskTape({
          parentSessionId: parent.sessionId,
          runId,
