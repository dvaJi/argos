...
import { SUBAGENT_ORCHESTRATOR_TOOL_NAME, SubagentOrchestratorTool } from "./subagentOrchestratorTool";
...
import { FilePermissionRequiredError, type FilePermissionLevel } from "../../permission/filePermissionService";
...
export class AgentToolManager {
  ...
  private readonly fileSystemSchemas = {
    ...
    write: z.object({
      ...
      base_directory: z
        .string()
        .optional()
        .describe("Base directory for resolving relative paths. Required when using skills with relative paths."),
    }),
    edit: z.object({
      path: z.string(),
      oldText: z.string().max(10000).describe("The exact text to find and replace (case-sensitive)"),
    ...
    exec: z.object({
      ...
      timeoutMs: z.number().min(100).max(600000).optional().describe("Optional timeout in milliseconds"),
      description: z
        .string()
        .min(5)
        .max(100)
        .optional()
        .describe('Brief description of what the command does (e.g., "Install dependencies", "Start dev server")'),
    ...
    process: z.object({
      ...
      limit: z.number().int().min(1).optional().describe("Maximum characters to return for log action"),
  ...
  private readonly skillSchemas = {
    ...
    skill_view: z.object({
      ...
      file_path: z.string().min(1).optional().describe("Optional file path under the skill root to inspect"),
    }),
    skill_run: z.object({
      skill: z.string().min(1).describe("Active skill name that owns the script"),
      script: z.string().min(1).describe("Script path under the skill root, usually scripts/<name>.<ext>"),
    ...
    skill_manage: z.discriminatedUnion("action", [
      ...
      z.object({
        ...
        draftId: z.string().describe("Opaque draft ID returned by skill_manage create"),
        filePath: z.string().describe("Relative file path under references/, templates/, scripts/, or assets/"),
  ...
  public syncContext(context: { chatMode: "agent" | "acp agent"; agentWorkspacePath: string | null }): void {
  ...
  async getAllToolDefinitions(context: {
    ...
    // 2.1. Progress checklist tool (argos regular sessions only)
    if (isAgentMode && this.planTool) {
      defs.push(this.planTool.getToolDefinition());
    }

    // 2.15. Session tape tools (Argos sessions only)
    ...
    // 2.25. Image generation tool (argos agent sessions with an image model)
    ...
    // 2.5. Subagent orchestration tool (argos regular sessions only)
    if (isAgentMode && context.conversationId && this.subagentOrchestratorTool) {
      try {
        const subagentToolDefinition = await this.subagentOrchestratorTool.getToolDefinition(context.conversationId);
    ...
    if (isAgentMode && this.isSkillsEnabled() && context.conversationId) {
      try {
        ...
        logger.warn("[AgentToolManager] Failed to load Argos settings tools", { error });
      }
    }
  ...
  private async callProcessTool(
    ...
    switch (action) {
      ...
      case "log": {
        ...
        const result = await backgroundExecSessionManager.log(conversationId, sessionId, offset, limit);
        return {
          content: JSON.stringify(result, null, 2),
        };
      }
  ...
  private async callFileSystemTool(
    ...
    if (toolName === "exec") {
      ...
      const bashHandler = new AgentBashHandler(allowedDirectories, this.commandPermissionHandler, this.configPresenter);
      ...
      const content =
        typeof commandResult.output === "string" ? commandResult.output : JSON.stringify(commandResult.output);
      return {
        content,
        rawData: {
          content,
    ...
    try {
      switch (toolName) {
        case "read": {
          ...
          const prepared = await this.getFilePresenter().prepareFileCompletely(validPath, mimeType, "llm-friendly");
          return {
            content: this.paginateReadContent(readArgs.path, prepared.content || "", readArgs.offset, readArgs.limit),
          };
        }
        case "write":
          await this.assertFileAccessPermission(
  ...
  private async buildAllowedDirectories(
    ...
    if (includeRuntimeRoots) {
      addPath(path.join(app.getPath("home"), ".argos"));
      addPath(app.getPath("temp"));
      addPath(path.join(app.getPath("userData"), "temp"));
    }

    if (conversationId) {
      const approved = this.runtimePort.getApprovedFilePaths(conversationId, options.requiredPermission ?? "read");
  ...
  private async resolveActiveSkillRoots(conversationId: string): Promise<string[]> {
    ...
    const metadataByName = new Map(
      metadataList.filter((metadata) => metadata?.name?.trim()).map((metadata) => [metadata.name.trim(), metadata]),
    );
    const roots: string[] = [];
    ...
    for (const skillName of activeSkillNames) {
      ...
      if (!metadata) {
        logger.warn("[AgentToolManager] Active skill metadata missing during file allowlist build", {
          conversationId,
  ...
  private async resolveValidatedReadPath(
    ...
    try {
      ...
      if (!allowExternalFileAccess && !fileSystemHandler.isPathAllowedAbsolute(realPath)) {
        throw new Error(`Access denied - symlink target outside allowed directories: ${requestedPath}`);
      }
  ...
  private paginateReadContent(pathLabel: string, fullContent: string, offset?: number, limit?: number): string {
    const start = Math.max(0, offset ?? 0);
    const totalLength = fullContent.length;

    let effectiveLimit = limit;
    let autoTruncated = false;
    if (effectiveLimit === undefined && totalLength - start > AgentToolManager.READ_FILE_AUTO_TRUNCATE_THRESHOLD) {
      effectiveLimit = AgentToolManager.READ_FILE_AUTO_TRUNCATE_THRESHOLD;
      autoTruncated = true;
    }

    const content =
      effectiveLimit !== undefined ? fullContent.slice(start, start + effectiveLimit) : fullContent.slice(start);
  ...
  private async readImageWithVisionFallback(
    ...
    try {
      ...
      const modelConfig = this.configPresenter.getModelConfig(visionTarget.modelId, visionTarget.providerId);
  ...
  private async assertFileAccessPermission(
    ...
    const targets =
      permissionType === "write" ? this.collectWriteTargets(toolName, args) : this.collectReadTargets(toolName, args);
    ...
    throw new FilePermissionRequiredError(`components.messageBlockPermissionRequest.description.${permissionType}`, {
      toolName,
  ...
  private getDefaultAgentWorkspacePath(): string {
    const tempDir = path.join(app.getPath("temp"), "argos-agent", "workspaces");
    try {
      fs.mkdirSync(tempDir, { recursive: true });
    } catch (error) {
      logger.warn("[AgentToolManager] Failed to create default workspace, using system temp:", error);
    ...
    if (!this.chatSettingsHandler) {
      this.chatSettingsHandler = new ChatSettingsToolHandler({
        ...
        windowRuntime: {
          ...
          sendToWindow: (windowId, channel, ...args) => this.runtimePort.sendToWindow(windowId, channel, ...args),
        },
      });
    }
  ...
  private getSkillExecutionService(): SkillExecutionService {
    if (!this.skillExecutionService) {
      this.skillExecutionService = new SkillExecutionService(this.getSkillPresenter(), this.configPresenter, {
        resolveConversationWorkdir: (conversationId) => this.getWorkdirForConversation(conversationId),
      });
    }
  ...
  private buildSkillManageToolResult(result: SkillManageResult): Record<string, unknown> {
    return {
      ...
      ...result,
      ...(result.success === true && result.action === "create" && result.draftId && result.skillName
        ? {
            skillDraft: {
              status: "created",
  ...
  private async callChatSettingsTool(
          ...
          return {
            ...
            rawData: {
              ...
              permissionRequest: {
                ...
                description: "Opening Argos settings requires approval.",
                conversationId,
                rememberable: false,
              },
            },
    ...
    throw new Error(`Unknown Argos settings tool: ${toolName}`);
  }
}
