...
export class ToolPresenter implements IToolPresenter {
  ...
  buildToolSystemPrompt(context: { conversationId?: string; toolDefinitions?: MCPToolDefinition[] }): string {
    ...
    const offloadPath =
      resolveToolOffloadTemplatePath(conversationId) ?? "~/.argos/sessions/<conversationId>/tool_<toolCallId>.offload";
    ...
    const sections = [
      ...
      this.buildSettingsPrompt(groupedTools.get("argos-settings") ?? []),
  ...
  private buildTapePrompt(tools: MCPToolDefinition[]): string {
    ...
    const lines = ["## Tape Tools", `Argos tape tools are available in this session: ${names}.`];

    if (toolNames.has(TAPE_TOOL_NAMES.info)) {
      lines.push("`tape_info` inspects this Argos-scoped tape subset inspired by bub tape.info.");
    }
  ...
  private buildSettingsPrompt(tools: MCPToolDefinition[]): string {
    ...
    const names = tools.map((tool) => `\`${tool.function.name}\``).join(", ");
    return [
      "## Argos Settings Tools",
      `Argos settings tools are available in this session: ${names}.`,
