...
export class AgentSessionPresenter {
  ...
  async createSession(input: CreateSessionInput, webContentsId: number): Promise<SessionWithState> {
    const agentId = input.agentId || "argos";
    console.log(`[AgentSessionPresenter] createSession agent=${agentId} webContentsId=${webContentsId}`);
    const normalizedInput = this.normalizeCreateSessionInput(input);
    const agentType = await this.getAgentType(agentId);
    const argosAgentConfig = agentType === "argos" ? await this.resolveArgosAgentConfigCompat(agentId) : null;
    const projectDir =
      input.projectDir?.trim() ||
      argosAgentConfig?.defaultProjectPath?.trim() ||
      this.getDefaultProjectPathCompat() ||
      null;
    const disabledAgentTools =
      agentType === "argos"
        ? this.normalizeDisabledAgentTools(input.disabledAgentTools ?? argosAgentConfig?.disabledAgentTools)
        : [];
    const subagentEnabled = this.resolveSessionSubagentEnabled(
      agentType,
      input.subagentEnabled,
      argosAgentConfig?.subagentEnabled,
    );
    ...
    const providerId =
      input.providerId ?? argosAgentConfig?.defaultModelPreset?.providerId ?? defaultModel?.providerId ?? "";
    const modelId = input.modelId ?? argosAgentConfig?.defaultModelPreset?.modelId ?? defaultModel?.modelId ?? "";
    const permissionMode: PermissionMode =
      input.permissionMode !== undefined
        ? input.permissionMode === "default"
          ? "default"
          : "full_access"
        : argosAgentConfig?.permissionMode === "default"
          ? "default"
          : "full_access";
    const generationSettings = this.mergeArgosDefaultGenerationSettings(argosAgentConfig, input.generationSettings);
  ...
  async createDetachedSession(input: CreateDetachedSessionInput): Promise<SessionWithState> {
    const agentId = input.agentId?.trim() || "argos";
  ...
  async createSubagentSession(input: {
    ...
    const subagentMeta: ArgosSubagentMeta = {
      slotId,
      displayName,
    ...
    throw lastError instanceof Error ? lastError : new Error(`Failed to create subagent session for slot ${slotId}.`);
  }
  ...
  async ensureAcpDraftSession(input: {
    ...
    const agent = await this.resolveAgentImplementation(agentId);
    const permissionMode: PermissionMode = input.permissionMode === "default" ? "default" : "full_access";
  ...
  async sendMessage(sessionId: string, content: string | SendMessageInput): Promise<MessageStartResult> {
    ...
    await this.syncAcpSessionWorkdir(providerId, sessionId, session.agentId, session.projectDir ?? null);
    if (agent.queuePendingInput) {
  ...
  async queuePendingInput(sessionId: string, content: string | SendMessageInput) {
    ...
    await this.syncAcpSessionWorkdir(providerId, sessionId, currentSession.agentId, currentSession.projectDir ?? null);
  ...
  async forkSession(sourceSessionId: string, targetMessageId: string, newTitle?: string): Promise<SessionWithState> {
    ...
    const generationSettings = agent.getGenerationSettings ? await agent.getGenerationSettings(sourceSessionId) : null;

    const title = this.buildForkTitle(sourceSession.title, newTitle);
    const targetSessionId = this.sessionManager.create(sourceSession.agentId, title, sourceSession.projectDir ?? null, {
      isDraft: false,
    });
  ...
  async getLightweightSessionsByIds(sessionIds: string[]): Promise<SessionListItem[]> {
    const dedupedIds = Array.from(new Set(sessionIds.map((sessionId) => sessionId.trim()).filter(Boolean)));
    return this.dedupeAndSortSessionListItems(
      this.sessionManager.getMany(dedupedIds).map((record) => this.mapSessionRecordToListItem(record)),
    );
  }
  ...
  async translateText(text: string, locale?: string, agentId?: string): Promise<string> {
    ...
    const translated = await this.llmProviderPresenter.generateCompletion(providerId, messages, modelId, 0.2, 1024);
  ...
  async getAgents(): Promise<Agent[]> {
    ...
    return agents.filter((agent) => agent.type === "argos" || acpEnabled);
  }
  ...
  async setSessionSubagentEnabled(sessionId: string, enabled: boolean): Promise<SessionWithState> {
    ...
    if ((await this.getAgentType(session.agentId)) !== "argos") {
      throw new Error("Only Argos sessions can change subagent state.");
    }
  ...
  async setSessionModel(sessionId: string, providerId: string, modelId: string): Promise<SessionWithState> {
    const session = this.sessionManager.get(sessionId);
    if (!session) {
      throw new Error(`Session not found: ${sessionId}`);
    }
  ...
  async setSessionProjectDir(sessionId: string, projectDir: string | null): Promise<SessionWithState> {
    const session = this.sessionManager.get(sessionId);
    if (!session) {
      throw new Error(`Session not found: ${sessionId}`);
    }
    ...
    const agent = await this.resolveAgentImplementation(session.agentId);
    const state = await agent.getSessionState(sessionId);
    const providerId = state?.providerId?.trim() || ((await this.getAgentType(session.agentId)) === "acp" ? "acp" : "");
  ...
  async updateSessionDisabledAgentTools(sessionId: string, disabledAgentTools: string[]): Promise<string[]> {
    const session = this.sessionManager.get(sessionId);
    if (!session) {
      throw new Error(`Session not found: ${sessionId}`);
    }
  ...
  private async generateSessionTitle(
    ...
    try {
      ...
      console.warn(`[AgentSessionPresenter] title generation skipped for session=${sessionId}:`, error);
    }
  }
  ...
  private async resolveAgentImplementation(agentId: string): Promise<IAgentImplementation> {
    ...
    if (agentType === "argos" || agentType === "acp") {
      return this.agentRegistry.resolve("argos");
    }
  ...
  private async getAgentType(agentId: string): Promise<"argos" | "acp" | null> {
    if (typeof this.configPresenter.getAgentType !== "function") {
      const resolvedAgentId = resolveAcpAgentAlias(agentId);
      if (resolvedAgentId === "argos") {
        return "argos";
      }
      const fallbackAgent = await this.configPresenter.getAgent?.(resolvedAgentId);
      if (fallbackAgent?.type === "acp" || fallbackAgent?.type === "argos") {
  ...
  private async resolveArgosAgentConfigCompat(
    agentId: string,
  ): Promise<Awaited<ReturnType<IConfigPresenter["resolveArgosAgentConfig"]>> | null> {
    if (typeof this.configPresenter.resolveArgosAgentConfig !== "function") {
      return {} as Awaited<ReturnType<IConfigPresenter["resolveArgosAgentConfig"]>>;
    }
  ...
  private async resolveAssistantModelSelection(
    ...
    if ((await this.getAgentType(agentId)) === "argos") {
      const config = await this.resolveArgosAgentConfigCompat(agentId);
  ...
  private mergeArgosDefaultGenerationSettings(
    config: Awaited<ReturnType<IConfigPresenter["resolveArgosAgentConfig"]>> | null,
  ...
  private async assessTransferSession(session: SessionRecord): Promise<{
    ...
    if (agent.listPendingInputs) {
      try {
        ...
        console.warn(`[AgentSessionPresenter] Failed to inspect pending input for session=${session.id}:`, error);
  ...
  private async resolveTransferTargetContext(
    ...
    const config = await this.resolveArgosAgentConfigCompat(resolvedAgentId);
    const defaultModel = this.configPresenter.getDefaultModel();
    ...
    if (!providerId || !modelId) {
      throw new Error("Target Argos agent does not have a default model.");
    }
    ...
    return {
      ...
      providerId,
      modelId,
      projectDir: currentProject || config?.defaultProjectPath?.trim() || this.getDefaultProjectPathCompat() || null,
      permissionMode: config?.permissionMode === "default" ? "default" : "full_access",
      generationSettings: this.mergeArgosDefaultGenerationSettings(config),
      disabledAgentTools: this.normalizeDisabledAgentTools(config?.disabledAgentTools),
      subagentEnabled: this.resolveSessionSubagentEnabled(agentType, undefined, config?.subagentEnabled),
    };
  }
  ...
  private async hasSessionMessages(agent: IAgentImplementation, sessionId: string): Promise<boolean> {
    try {
      const ids = await agent.getMessageIds(sessionId);
      return ids.length > 0;
    } catch (error) {
      console.warn(`[AgentSessionPresenter] Failed to inspect message ids for session=${sessionId}:`, error);
      return true;
    }
  }
  ...
  private async ensureSessionRuntimeInitialized(
    ...
    if (state.permissionMode && state.permissionMode !== config.permissionMode && agent.setPermissionMode) {
      await agent.setPermissionMode(sessionId, config.permissionMode);
    }

    await this.syncAcpSessionWorkdir(config.providerId, sessionId, config.agentId ?? config.modelId, config.projectDir);
  }
