  ...
  private async importRows(payload: {
    ...
    await this.sqlitePresenter.runTransaction(() => {
      for (const conversation of payload.conversations) {
        ...
        const agentId = providerId === "acp" && modelId ? modelId : "argos";
        ...
        if (!this.sqlitePresenter.argosSessionsTable.get(sessionId)) {
          this.sqlitePresenter.argosSessionsTable.create(sessionId, providerId, modelId, "full_access", {
        ...
        for (const row of messageRows) {
          ...
          if (this.sqlitePresenter.argosMessagesTable.get(messageId)) {
            nextOrderSeq += 1;
            continue;
          }
        ...
        for (const [oldMessageId, attachmentRows] of attachmentRowsByMessage.entries()) {
          ...
          for (const attachmentRow of attachmentRows) {
            ...
            for (const result of parsedResults) {
              const inserted = this.sqlitePresenter.argosMessageSearchResultsTable.add({
