...
const yoBrowserSchemas = {
  ...
  cdp_send: z.object({
    ...
    params: z.record(z.string(), z.unknown()).optional().describe("Parameters for the selected CDP method"),
  }),
};
...
export function getYoBrowserToolDefinitions(): MCPToolDefinition[] {
  return [
    toDefinition("get_browser_status", "Get the current session browser status", yoBrowserSchemas.get_browser_status),
    toDefinition("load_url", "Create the session browser on demand and load a URL into it", yoBrowserSchemas.load_url),
    toDefinition(
      "cdp_send",
