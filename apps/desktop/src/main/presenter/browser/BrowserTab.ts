...
export class BrowserTab {
  ...
  constructor(webContents: WebContents, cdpManager: CDPManager, screenshotManager: ScreenshotManager) {
  ...
  async sendCdpCommand(method: string, params?: Record<string, unknown>): Promise<unknown> {
    ...
    if (method === "Page.navigate") {
      ...
      if (hasCommittedCrossDocumentNavigation) {
        this.beginMainFrameNavigation(typeof params?.url === "string" && params.url.trim() ? params.url : this.url);
      }
    ...
    await this.evaluate(
      (sel, targetValues) => {
        ...
        for (const option of options) {
          const shouldSelect = targetValues.includes(option.value) || targetValues.includes(option.text);
      ...
      const buildSelector = (element: Element): string => {
        ...
        while (current && current.nodeType === Node.ELEMENT_NODE && parts.length < 5) {
          ...
          if (parent) {
            ...
            const matchingSiblings = siblings.filter((child: Element) => child.nodeName === current!.nodeName);
  ...
  private async highlightElements(selectors: string[]): Promise<() => Promise<void>> {
    await this.evaluate((list) => {
      list.forEach((selector, index) => {
        const element = document.querySelector<HTMLElement>(selector);
        if (element) {
          element.dataset.__argosOriginalOutline = element.style.outline;
          element.style.outline = "2px solid #ff5f6d";
          element.style.outlineOffset = "2px";
          element.dataset.__argosHighlightIndex = String(index);
        }
      });
    }, selectors);
    ...
    return async () => {
      await this.evaluate(() => {
        document.querySelectorAll<HTMLElement>("[data-__argos-highlight-index]").forEach((el) => {
          if (el.dataset.__argosOriginalOutline !== undefined) {
            el.style.outline = el.dataset.__argosOriginalOutline;
          } else {
            el.style.outline = "";
          }
          delete el.dataset.__argosHighlightIndex;
          delete el.dataset.__argosOriginalOutline;
        });
      });
    };
  }
  ...
  private async evaluate<T>(fn: (...args: any[]) => T, ...args: any[]): Promise<T> {
    ...
    await this.ensureInteractiveReadyOrWait("evaluate script");
    const session = await this.ensureSession();
    const serializedArgs = JSON.stringify(args, (_key, value) => (value === undefined ? null : value));
  ...
  private validateKeyInput(key: string, count: number): string {
    ...
    const singleCharCandidate = rawKey.length === 1 ? rawKey : trimmedKey.length === 1 ? trimmedKey : "";
    ...
    try {
      await new Promise<void>((resolvePromise, rejectPromise) => {
        ...
        onFailLoad = (
          ...
          settle(() => rejectPromise(new Error(`Navigation failed ${errorCode}: ${errorDescription}`)));
        };
  ...
  private async ensureSession() {
    ...
    // Security check: only browser tabs loading external web pages are allowed to bind CDP
  ...
  private isInteractiveReadyTimeoutError(error: unknown): error is Error {
    return error instanceof Error && error.message.startsWith(INTERACTIVE_READY_TIMEOUT_MESSAGE_PREFIX);
  }
