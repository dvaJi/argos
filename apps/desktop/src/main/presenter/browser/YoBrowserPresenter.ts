...
export class YoBrowserPresenter implements IYoBrowserPresenter {
  ...
  async loadUrl(
    ...
    if (activitySource === "agent") {
      await this.runAgentActivity(normalizedSessionId, { kind: "navigation", action: "navigate" }, navigate);
    } else {
      await navigate();
    }
  ...
  async sendCdpCommand(
    ...
    if (activitySource === "agent" && descriptor) {
      return await this.runAgentActivity(sessionId, descriptor, () => state.page.sendCdpCommand(method, params));
    }
  ...
  private setupPageListeners(state: SessionBrowserState, contents: WebContents): void {
    ...
    contents.on(
      "did-fail-load",
      (_event, errorCode: number, _errorDescription: string, _validatedURL: string, isMainFrame) => {
  ...
  private emitWindowCreated(sessionId: string): void {
    ...
    publishArgosEvent("browser.status.changed", {
      sessionId,
      reason: "created",
      windowId: payload.status.page ? (this.sessionBrowsers.get(sessionId)?.attachedWindowId ?? null) : null,
  ...
  private emitOpenRequested(sessionId: string, windowId: number, url: string): void {
    ...
    publishArgosEvent("browser.open.requested", {
      ...payload,
      version: Date.now(),
    });
  }
  ...
  private emitWindowCount(): void {
    eventBus.sendToRenderer(YO_BROWSER_EVENTS.WINDOW_COUNT_CHANGED, SendTarget.ALL_WINDOWS, this.sessionBrowsers.size);
  }
  ...
  private emitBrowserActivity(
    ...
    publishArgosEvent("browser.activity.changed", payload);
  ...
  private describeCdpActivity(method: string, params?: Record<string, unknown>): BrowserActivityDescriptor | null {
    switch (method) {
      case "Page.navigate":
  ...
  private describeMouseActivity(params?: Record<string, unknown>): BrowserActivityDescriptor | null {
  ...
  private describeRuntimeEvaluateActivity(params?: Record<string, unknown>): BrowserActivityDescriptor {
  ...
  private inferScrollDirection(params?: Record<string, unknown>): YoBrowserActivityDirection | undefined {
  ...
  private extractPointFromRuntimeExpression(expression: string): YoBrowserActivityPoint | undefined {
