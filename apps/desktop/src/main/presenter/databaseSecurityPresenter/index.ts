...
export class DatabaseSecurityPresenter {
  ...
  getStatus(): DatabaseSecurityStatus {
    ...
    return {
      ...
      manualUnlockRequired:
        metadata.enabled &&
        (!safeStorageAvailable || metadata.passwordStorage !== "safeStorage" || !metadata.wrappedPassword),
  ...
  async resolveStartupPassword(unlockProvider: UnlockProvider): Promise<string | undefined> {
    ...
    if (safeStorageAvailable && metadata.passwordStorage === "safeStorage" && metadata.wrappedPassword) {
      try {
  ...
  private attachMigrationTarget(
    ...
    if (targetPassword) {
      ...
      db.prepare(`ATTACH DATABASE ? AS ${MIGRATION_TARGET_SCHEMA} KEY ?`).run(tempPath, targetPassword);
      return;
    }
  ...
  private qualifyCreateTableSql(sql: string): string {
    return sql.replace(
      ...
      (_match, ifNotExists: string | undefined) => `CREATE TABLE ${ifNotExists ?? ""}${MIGRATION_TARGET_SCHEMA}.`,
    );
  }
  ...
  private copySqliteSequence(db: Database.Database): void {
    ...
    const insert = db.prepare(`INSERT INTO ${MIGRATION_TARGET_SCHEMA}.sqlite_sequence (name, seq) VALUES (?, ?)`);
  ...
  private collectValidationCounts(dbPath: string, password: string | undefined): Record<string, number> {
  ...
  private replaceDatabaseWithRollback(dbPath: string, tempPath: string, rollbackPath: string): void {
