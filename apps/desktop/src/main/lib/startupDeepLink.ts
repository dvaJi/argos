...
const SECONDARY_STARTUP_ENV_KEYS = ["DEEPLINK_URL", "argos_deeplink"] as const;
...
export const isDeepLinkUrl = (value: string | null | undefined): value is string => {
  ...
  return normalized.startsWith("argos://") || normalized.startsWith("argos:");
};
...
export const storeStartupDeepLink = (url: string, _env: NodeJS.ProcessEnv = process.env): string | null => {
