...
function toStringEnvEntries(input: NodeJS.ProcessEnv | Record<string, string> | undefined): Record<string, string> {
  if (!input) {
    return {};
  }
...
export function getPathEntriesFromEnv(input: NodeJS.ProcessEnv | Record<string, string> | undefined): string[] {
  const env = toStringEnvEntries(input);
  ...
  return await new Promise<Record<string, string>>((resolve, reject) => {
    ...
    child.on("exit", (code, signal) => {
      ...
      if (code !== 0 && signal === null) {
        reject(new Error(`Shell environment command exited with code ${code}, stderr: ${stderr.substring(0, 200)}`));
        return;
      }
