...
export const getBackgroundExecConfig = () => ({
  ...
  maxOutputChars:
    parseInt(process.env.OPENCLAW_BASH_PENDING_MAX_OUTPUT_CHARS || process.env.PI_BASH_MAX_OUTPUT_CHARS || "500", 10) ||
    500,
...
export class BackgroundExecSessionManager {
  ...
  async log(conversationId: string, sessionId: string, offset = 0, limit = 1000): Promise<LogResult> {
    const session = this.getSession(conversationId, sessionId);
    session.lastAccessedAt = Date.now();
    await this.waitForSessionDrain(session);
  ...
  async remove(conversationId: string, sessionId: string): Promise<void> {
    ...
    if (session.outputFilePath && fs.existsSync(session.outputFilePath)) {
      try {
        ...
        logger.warn(`[BackgroundExec] Failed to remove output file ${session.outputFilePath}:`, error);
      }
    }
  ...
  async shutdown(): Promise<void> {
    ...
    await Promise.all(
      allSessions.map(({ conversationId, sessionId }) => this.remove(conversationId, sessionId).catch(() => {})),
    );
  }
  ...
  private setupOutputHandling(session: BackgroundSession, config: ReturnType<typeof getConfig>): void {
  ...
  private appendOutput(session: BackgroundSession, data: string, config: ReturnType<typeof getConfig>): void {
  ...
  private hasPersistedOutput(session: BackgroundSession, config: ReturnType<typeof getConfig>): boolean {
    return session.outputFilePath !== null && session.totalOutputLength > config.offloadThresholdChars;
  }

  private getPersistedOutputLength(session: BackgroundSession, config: ReturnType<typeof getConfig>): number {
  ...
  private queueOutputWrite(session: BackgroundSession, data: string, mode: "append" | "truncate"): void {
    if (!session.outputFilePath) {
      if (mode === "append" && data) {
        session.outputBuffer += data;
      }
  ...
  private async finalizeSession(
    ...
    logger.info(`[BackgroundExec] Session ${session.sessionId} closed with code ${code}, signal ${signal}`);
  }

  private buildCompletionResult(session: BackgroundSession, previewChars: number): SessionCompletionResult {
    const config = getConfig();
  ...
  private createOutputFilePath(sessionDir: string, sessionId: string, outputPrefix?: string): string {
...
class BackgroundExecUtilityProxy {
  ...
  async start(
    ...
    const result = await this.request<StartSessionResult>("start", [conversationId, command, cwd, options]);
  ...
  private getCrashedCompletionResult(conversationId: string, sessionId: string): SessionCompletionResult | null {
  ...
  private touchOrCompleteSession(conversationId: string, sessionId: string, status: PollResult["status"]): void {
