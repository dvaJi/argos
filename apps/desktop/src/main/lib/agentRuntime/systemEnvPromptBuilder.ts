...
function resolveModelDisplayName(
  ...
  try {
    ...
    console.warn(`[SystemEnvPromptBuilder] Failed to resolve model display name for ${providerId}/${modelId}:`, error);
  }
...
export function buildRuntimeCapabilitiesPrompt(
  ...
  if (options.hasProcess) {
    lines.push("- Use process(list|poll|log|write|kill|remove) to manage background terminal sessions.");
  }
...
export async function buildSystemEnvPrompt(options: BuildSystemEnvPromptOptions = {}): Promise<string> {
  ...
  const { modelName, exactModelId } = resolveModelIdentity(options.providerId, options.modelId, options.modelLookup);
