...
import { detectMimeType, isLikelyTextFile } from "@argos/file-adapters/mime";
...
export async function shouldRejectAgentBinaryRead(filePath: string, mimeType: string): Promise<boolean> {
  ...
  if (ALWAYS_BINARY_MIMES.has(mimeType) || mimeType.startsWith("audio/") || mimeType.startsWith("video/")) {
    return true;
  }
...
export function buildBinaryReadGuidance(filePath: string, mimeType: string, mode: "agent" | "acp"): string {
