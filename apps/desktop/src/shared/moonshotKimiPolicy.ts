...
const FIXED_TEMPERATURE_MODELS = new Set(["kimi-k2.5", "kimi-k2.6", "moonshotai/kimi-k2.5", "moonshotai/kimi-k2.6"]);
...
export const getMoonshotKimiTemperaturePolicy = (
  ...
  const baseModelId = isThinkingVariant ? normalizedModelId.slice(0, -THINKING_SUFFIX.length) : normalizedModelId;
...
export const resolveMoonshotKimiTemperaturePolicy = (
  ...
  return {
    ...policy,
    reasoningEnabled,
    temperature: reasoningEnabled ? policy.thinkingEnabledTemperature : policy.thinkingDisabledTemperature,
