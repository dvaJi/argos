...
const matchesToolContractName = (toolName: string | undefined | null, expectedName: string): boolean => {
...
const extractToolSpecificSummaryValue = (value: unknown, context?: ToolCallSummaryContext): unknown => {
  if (!isRecord(value)) {
    return undefined;
  }
  ...
  if (matchesToolContractName(context?.toolName, "read") || matchesToolContractName(context?.toolName, "write")) {
...
export const summarizeToolCallPreview = (
  ...
  try {
    ...
    return formatSummaryValue(extractToolSpecificSummaryValue(parsed, context) ?? extractFirstSummaryValue(parsed));
  } catch {
