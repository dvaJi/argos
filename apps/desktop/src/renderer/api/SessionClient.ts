...
export function createSessionClient(bridge: ArgosBridge = getArgosBridge()) {
  async function create(input: CreateSessionInput) {
    return await bridge.invoke(sessionsCreateRoute.name, input as ArgosRouteInput<typeof sessionsCreateRoute.name>);
  }
  ...
  async function updateQueuedInput(sessionId: string, itemId: string, content: string | SendMessageInput) {
  ...
  async function exportSession(sessionId: string, format: "markdown" | "html" | "txt" | "nowledge-mem") {
  ...
  async function setAcpSessionConfigOption(sessionId: string, configId: string, value: string | boolean) {
  ...
  async function updateSessionGenerationSettings(
    sessionId: string,
    settings: ArgosRouteInput<typeof sessionsUpdateGenerationSettingsRoute.name>["settings"],
  ) {
  ...
  function onPendingInputsChanged(listener: (payload: { sessionId: string; version: number }) => void) {
