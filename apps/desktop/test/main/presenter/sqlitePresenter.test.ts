...
describeIfSqlite("SQLitePresenter legacy schema bootstrap", () => {
  ...
  it("repairs missing legacy conversation tables when schema version is already advanced", async () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "argos-sqlite-presenter-"));
    ...
    expect(tables).toEqual([{ name: "conversations" }, { name: "message_attachments" }, { name: "messages" }]);
  ...
  it("migrates new_sessions active_skills when schema version is already at 14", async () => {
    ...
    const versions = checkDb.prepare("SELECT version FROM schema_versions ORDER BY version ASC").all() as Array<{
      version: number;
    }>;
  ...
  it("creates fresh new_sessions tables with disabled_agent_tools column", async () => {
    ...
    const columnNames = new Set(newSessionColumns.map((column) => column.name));
    const environmentColumns = checkDb.prepare("PRAGMA table_info(new_environments)").all() as Array<{
      name: string;
    }>;
  ...
  it("migrates ACP agent aliases without requiring legacy conversations tables", async () => {
    ...
    presenter.argosSessionsTable.create("session-1", "acp", "kimi-cli");
    ...
    expect(presenter.argosSessionsTable.get("session-1")?.model_id).toBe("kimi");
  ...
  it("recreates new_sessions with applied columns when schema version is already 16", async () => {
    ...
    const row = checkDb
      .prepare("SELECT is_draft, active_skills, disabled_agent_tools FROM new_sessions WHERE id = ?")
      .get("session-1") as
      | {
          is_draft: number;
  ...
  it("migrates new_environments from existing session history when schema version is 16", async () => {
    ...
    bootstrapDb.exec(`
      ...
      VALUES
        ('s1', 'argos', 'One', '/work/app-a', 0, 0, '[]', '[]', 100, 200),
        ('s2', 'argos', 'Two', '/work/app-a', 0, 0, '[]', '[]', 150, 300),
        ('s3', 'agent-1', 'Temp', NULL, 0, 0, '[]', '[]', 200, 250),
        ('s4', 'argos', 'Draft', '/work/draft', 0, 1, '[]', '[]', 300, 400),
        ('s5', 'argos', 'Empty', '', 0, 0, '[]', '[]', 500, 600);
  ...
  it("does not duplicate environment rows when reopening an already migrated database", async () => {
    ...
    const checkDb = new DatabaseCtor(dbPath);
    const rows = checkDb.prepare("SELECT path, session_count, last_used_at FROM new_environments").all() as Array<{
      path: string;
      session_count: number;
      last_used_at: number;
    }>;
  ...
  it("recreates argos_sessions with applied columns when schema version is already 14", async () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "argos-sqlite-presenter-"));
    tempDirs.push(tempDir);
    ...
    const presenter = new SQLitePresenterCtor(dbPath);
    presenter.argosSessionsTable.create("session-1", "openai", "gpt-4o");
    presenter.close();

    const checkDb = new DatabaseCtor(dbPath);
    const argosColumns = checkDb.prepare("PRAGMA table_info(argos_sessions)").all() as Array<{
      name: string;
    }>;
    const columnNames = new Set(argosColumns.map((column) => column.name));
    ...
    const row = checkDb
      .prepare(
        "SELECT system_prompt, summary_text, summary_cursor_order_seq, force_interleaved_thinking_compat, reasoning_visibility, timeout_ms, image_generation_options_json FROM argos_sessions WHERE id = ?",
      )
      .get("session-1") as
      | {
  ...
  it("migrates force_interleaved_thinking_compat when schema version is already 18", async () => {
    ...
    const presenter = new SQLitePresenterCtor(dbPath);
    presenter.argosSessionsTable.updateGenerationSettings("session-1", {
    ...
    const row = checkDb
      .prepare("SELECT force_interleaved_thinking_compat FROM argos_sessions WHERE id = ?")
      .get("session-1") as
      | {
          force_interleaved_thinking_compat: number | null;
        }
  ...
  it("repairs missing argos_sessions columns when schema version is already 22", async () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "argos-sqlite-presenter-"));
    tempDirs.push(tempDir);
    ...
    const presenter = new SQLitePresenterCtor(dbPath);
    const diagnosis = await presenter.diagnoseSchema();
    expect(diagnosis.issues.some((issue) => issue.name === "force_interleaved_thinking_compat")).toBe(true);
    ...
    const row = checkDb
      .prepare(
        "SELECT force_interleaved_thinking_compat, reasoning_visibility, timeout_ms, image_generation_options_json FROM argos_sessions WHERE id = ?",
      )
      .get("session-1") as
      | {
          force_interleaved_thinking_compat: number | null;
  ...
  it("repairs missing timeout_ms and image settings in argos_sessions when schema version is already 24", async () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "argos-sqlite-presenter-"));
    tempDirs.push(tempDir);
    ...
    const row = checkDb
      .prepare(
        "SELECT reasoning_visibility, timeout_ms, image_generation_options_json FROM argos_sessions WHERE id = ?",
      )
      .get("session-1") as
      | {
          reasoning_visibility: string | null;
  ...
  it("runs the v23, v24, and v27 recovery migrations for argos_sessions when schema version is 22", async () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "argos-sqlite-presenter-"));
    tempDirs.push(tempDir);
  ...
  it("returns child sessions when filtering by parentSessionId without includeSubagents", async () => {
    ...
    presenter.newSessionsTable.create("parent-session", "argos", "Parent session", "/workspace", {
      sessionKind: "regular",
    });
    presenter.newSessionsTable.create("child-session", "argos", "Child session", "/workspace", {
  ...
  it("migrates argos_usage_stats to include cache_write_input_tokens without losing rows", async () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "argos-sqlite-presenter-"));
    tempDirs.push(tempDir);
    ...
    const checkDb = new DatabaseCtor(dbPath);
    const usageColumns = checkDb.prepare("PRAGMA table_info(argos_usage_stats)").all() as Array<{
      name: string;
    }>;
