...
function createMockSqlitePresenter() {
  ...
  const argosMessagesTable = {
    ...
    updateContentAndStatus(messageId: string, content: string, status: MessageRow["status"], metadata?: string) {
...
describe("AgentSessionPresenter usage dashboard", () => {
  ...
  it("backfills current argos_messages and uses session provider/model fallback", async () => {
    const { presenter, sqlitePresenter, configPresenter } = createPresenter();

    sqlitePresenter.argosSessionsTable.create("session-1", "openai", "gpt-4o");
    ...
    const row = sqlitePresenter.argosUsageStatsTable.getByMessageId("message-1");
  ...
  it("keeps a single stats row when live finalize updates a previously backfilled message", async () => {
    ...
    const messageStore = new ArgosMessageStore(sqlitePresenter);
    ...
    expect(sqlitePresenter.argosUsageStatsTable.count()).toBe(1);
    const row = sqlitePresenter.argosUsageStatsTable.getByMessageId("message-1");
    expect(row).toMatchObject({
      source: "live",
  ...
  it("reads dashboard data from argos_usage_stats only", async () => {
    const { presenter, sqlitePresenter } = createPresenter();
  ...
  it("returns session count and most active day from usage stats summary", async () => {
    ...
    sqlitePresenter.argosUsageStatsTable.upsert({
      messageId: "message-1",
      sessionId: "session-1",
