...
describe("NewSessionManager", () => {
  ...
  describe("create", () => {
    it("creates a session, syncs environments, and returns the generated id", () => {
      const id = manager.create("argos", "Hello world", "/tmp/workspace");
      ...
      expect(sqlitePresenter.argosSearchDocumentsTable.upsert).toHaveBeenCalledWith({
  ...
  describe("list", () => {
    ...
    it("passes filters through", () => {
      manager.list({ agentId: "argos" });
      expect(sqlitePresenter.newSessionsTable.list).toHaveBeenCalledWith({ agentId: "argos" });
    });
  });
  ...
  describe("update", () => {
    it("maps camelCase fields to snake_case and syncs affected environment paths", () => {
      ...
      expect(sqlitePresenter.argosSearchDocumentsTable.refreshSessionTitle).toHaveBeenCalledWith("s1", "New Title");
  ...
  describe("disabled tools", () => {
    it("syncs the related environment after updating disabled tools", () => {
      ...
      expect(sqlitePresenter.newSessionsTable.updateDisabledAgentTools).toHaveBeenCalledWith("s1", ["exec"]);
