...
import os from "os";
import type { IModelConfig, LLM_PROVIDER, MCPServerConfig, MODEL_META } from "../../../src/shared/types/presenters";
...
describe("SyncConfigImportService", () => {
  ...
  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "argos-sync-config-"));
  ...
  it("preserves local rows in increment mode and replaces them in overwrite mode", () => {
    ...
    try {
      expect(incrementTables.listProviders().find((item) => item.id === "imported")?.name).toBe("Local Imported");
    ...
    try {
      expect(overwriteTables.listProviders().find((item) => item.id === "imported")?.name).toBe("Imported");
...
function writeLegacyConfigFixture(extractionDir: string) {
  writeJson(path.join(extractionDir, "configs", "app-settings.json"), {
    ...
    imported_models: [{ id: "gpt-4", name: "GPT 4", group: "default", providerId: "imported", enabled: true }],
  ...
  writeJson(path.join(extractionDir, "configs", "provider_models", "models_imported.json"), {
    ...
    custom_models: [{ id: "custom-store", name: "Custom Store", group: "custom", providerId: "imported" }],
  });
