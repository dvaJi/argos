...
describe("DeeplinkPresenter", () => {
  ...
  it("routes MCP imports through settings IPC instead of localStorage injection", async () => {
    ...
    const url = `argos://mcp/install?code=${Buffer.from(JSON.stringify(payload)).toString("base64")}`;
  ...
  it("routes built-in provider imports to settings and stores the preview for replay", async () => {
    ...
    const url = `argos://provider/install?v=1&data=${Buffer.from(JSON.stringify(payload)).toString("base64")}`;
    ...
    expect(presenterMock.windowPresenter.sendToWindow).toHaveBeenNthCalledWith(1, 9, SETTINGS_EVENTS.NAVIGATE, {
      routeName: "settings-provider",
    });
    expect(presenterMock.windowPresenter.sendToWindow).toHaveBeenNthCalledWith(2, 9, SETTINGS_EVENTS.PROVIDER_INSTALL);
  });
