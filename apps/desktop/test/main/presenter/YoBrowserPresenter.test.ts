...
describe("YoBrowserPresenter", () => {
  ...
  const setupPresenter = async () => {
    ...
    const getSessionWebContents = (sessionId: string) => {
      return ((presenter as any).sessionBrowsers.get(sessionId)?.view?.webContents ?? null) as MockWebContents | null;
    };
  ...
  it("starts session navigation immediately and resolves after dom-ready", async () => {
    ...
    await presenter.updateSessionBrowserBounds("session-a", 1, { x: 12, y: 18, width: 320, height: 480 }, true);
  ...
  it("resolves loadUrl only after the first dom-ready", async () => {
    ...
    await Promise.resolve();
    await presenter.attachSessionBrowser("session-a", 1);
    await presenter.updateSessionBrowserBounds("session-a", 1, { x: 10, y: 20, width: 300, height: 400 }, true);
    await Promise.resolve();
  ...
  it("does not emit WINDOW_UPDATED for pure bounds changes", async () => {
    ...
    await presenter.updateSessionBrowserBounds("session-a", 1, { x: 0, y: 0, width: 240, height: 360 }, true);
    await presenter.updateSessionBrowserBounds("session-a", 1, { x: 8, y: 16, width: 256, height: 384 }, true);

    const updatedEvents = sendToRendererMock.mock.calls.filter(([event]) => event === "yo-browser:window-updated");
  ...
  it("keeps session browsers isolated when switching the attached session", async () => {
    ...
    await Promise.resolve();
    await presenter.attachSessionBrowser("session-a", 1);
    await presenter.updateSessionBrowserBounds("session-a", 1, { x: 10, y: 10, width: 300, height: 400 }, true);
    await vi.advanceTimersByTimeAsync(130);
    ...
    await presenter.updateSessionBrowserBounds("session-b", 1, { x: 10, y: 10, width: 300, height: 400 }, true);
    await vi.advanceTimersByTimeAsync(130);
  ...
  it("publishes agent navigation activity without exposing page content", async () => {
    ...
    const loadPromise = presenter.loadUrl("session-a", "https://example.com", undefined, undefined, "agent");
    await Promise.resolve();
    getSessionWebContents("session-a")?.emitDomReady();
    await loadPromise;

    const activityPayloads = sendToRendererMock.mock.calls
      .filter(([event]) => event === "argos:event")
  ...
  it("maps Runtime.evaluate click and scroll scripts to visible action cues", async () => {
    ...
    await presenter.sendCdpCommand("session-a", "Runtime.evaluate", { expression: "window.scrollBy(0, 600)" }, "agent");
  ...
  it("maps Runtime.evaluate client coordinates by axis name", async () => {
    ...
    expect(extractPoint('document.dispatchEvent(new MouseEvent("click", { clientY: 12.4, clientX: 98.6 }))')).toEqual({
      x: 99,
      y: 12,
    });
    expect(
  ...
  it("does not show overlay activity when the host window is backgrounded", async () => {
    ...
    const activityEvents = sendToRendererMock.mock.calls.filter(([event]) => event === "argos:event");
