...
vi.mock("../../src/main/presenter/sqlitePresenter/dbType", async () => {
  ...
  class MockDatabase {
    ...
    private upsertRow(tableName: string, row: MockRow) {
      ...
      const existingIndex = table.findIndex((entry) => String(entry.id ?? entry.conv_id ?? "") === rowId);
      if (existingIndex >= 0) {
...
vi.mock("../../../src/main/presenter/sqlitePresenter/importData", async () => {
  ...
  class MockDataImporter {
    ...
    private getRowKey(tableName: string, row: MockRow): string {
      ...
      return String(row.id ?? row.conv_id ?? row.status_key ?? row.cache_key ?? row.name ?? row.key ?? "");
    }
  }
...
describe("SyncPresenter backup import", () => {
  ...
  beforeEach(() => {
    ...
    userDataDir = fs.mkdtempSync(path.join(os.tmpdir(), "argos-user-"));
  ...
  it("backs up migrated config through agent.db and keeps app-settings lightweight", async () => {
    ...
    const appSettings = JSON.parse(Buffer.from(files[ZIP_PATHS.appSettings]).toString("utf-8")) as Record<
      string,
      unknown
    >;
  ...
  it("imports backup incrementally without overwriting existing data", async () => {
    ...
    expect(configImportMocks.importLegacyConfig).toHaveBeenCalledWith(
      expect.stringContaining("argos-backup-"),
      "increment",
    );
    ...
    const appSettings = JSON.parse(fs.readFileSync(path.join(userDataDir, "app-settings.json"), "utf-8"));
    ...
    const customPrompts = JSON.parse(fs.readFileSync(path.join(userDataDir, "custom_prompts.json"), "utf-8"));
    expect(customPrompts.prompts).toEqual([
      { id: "prompt-local", title: "Local prompt" },
      { id: "prompt-imported", title: "Imported prompt" },
    ]);

    const systemPrompts = JSON.parse(fs.readFileSync(path.join(userDataDir, "system_prompts.json"), "utf-8"));
    expect(systemPrompts.prompts).toEqual([
      { id: "system-local", title: "Local system prompt" },
      { id: "system-imported", title: "Imported system prompt" },
    ]);

    const mcpSettings = JSON.parse(fs.readFileSync(path.join(userDataDir, "mcp-settings.json"), "utf-8"));
  ...
  it("skips invalid backup-looking zip files during cloud upload", async () => {
    ...
    expect(cloudStorageMocks.uploadBackup).toHaveBeenCalledWith(path.join(syncDir, validBackupFile), validBackupFile);
  });
  ...
  it("overwrites existing data when import mode is OVERWRITE", async () => {
    ...
    const customPrompts = JSON.parse(fs.readFileSync(path.join(userDataDir, "custom_prompts.json"), "utf-8"));
    expect(customPrompts.prompts).toEqual([{ id: "prompt-imported", title: "Imported prompt only" }]);
...
function createLocalState(
  ...
  fs.writeFileSync(path.join(userDataDir, "app-settings.json"), JSON.stringify(data.appSettings, null, 2));
  fs.writeFileSync(path.join(userDataDir, "custom_prompts.json"), JSON.stringify(data.customPrompts, null, 2));
  fs.writeFileSync(path.join(userDataDir, "system_prompts.json"), JSON.stringify(data.systemPrompts, null, 2));
  fs.writeFileSync(path.join(userDataDir, "mcp-settings.json"), JSON.stringify(data.mcpSettings, null, 2));
}
...
function createBackupArchive(
  ...
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "argos-backup-src-"));
  ...
  fs.writeFileSync(path.join(configsDir, "app-settings.json"), JSON.stringify(data.appSettings, null, 2));
  fs.writeFileSync(path.join(configsDir, "custom_prompts.json"), JSON.stringify(data.customPrompts, null, 2));
  fs.writeFileSync(path.join(configsDir, "system_prompts.json"), JSON.stringify(data.systemPrompts, null, 2));
  fs.writeFileSync(path.join(configsDir, "mcp-settings.json"), JSON.stringify(data.mcpSettings, null, 2));
  ...
  files[ZIP_PATHS.appSettings] = new Uint8Array(Buffer.from(JSON.stringify(data.appSettings, null, 2), "utf-8"));
  files[ZIP_PATHS.customPrompts] = new Uint8Array(Buffer.from(JSON.stringify(data.customPrompts, null, 2), "utf-8"));
  files[ZIP_PATHS.systemPrompts] = new Uint8Array(Buffer.from(JSON.stringify(data.systemPrompts, null, 2), "utf-8"));
  files[ZIP_PATHS.mcpSettings] = new Uint8Array(Buffer.from(JSON.stringify(data.mcpSettings, null, 2), "utf-8"));
