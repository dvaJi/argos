...
describe("detectMimeType", () => {
  ...
  it("treats TypeScript source files as application/typescript instead of video/mp2t", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "argos-mime-"));
  ...
  it("detects common office and document extensions by filename", async () => {
    ...
    await expect(detectMimeType("/tmp/report.docm")).resolves.toBe("application/vnd.ms-word.document.macroenabled.12");
  ...
  it("maps common attachment MIME types to processing adapters", () => {
    ...
    expect(map.get("application/vnd.openxmlformats-officedocument.wordprocessingml.document")).toBe(DocFileAdapter);
    expect(map.get("application/vnd.ms-word.document.macroenabled.12")).toBe(DocFileAdapter);
    expect(map.get("application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")).toBe(ExcelFileAdapter);
    expect(map.get("application/vnd.ms-excel.sheet.macroenabled.12")).toBe(ExcelFileAdapter);
    expect(map.get("application/vnd.openxmlformats-officedocument.presentationml.presentation")).toBe(PptFileAdapter);
    expect(map.get("application/vnd.ms-powerpoint.presentation.macroenabled.12")).toBe(PptFileAdapter);
