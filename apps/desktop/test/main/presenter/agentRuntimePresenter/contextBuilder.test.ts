...
function createMockMessageStore(messages: any[] = []) {
  return {
    getMessages: vi.fn<(...args: any[]) => any>().mockReturnValue(messages),
  } as any;
}

function makeUserRecord(orderSeq: number, text: string, status: "sent" | "pending" | "error" = "sent") {
  return {
    id: `user-${orderSeq}`,
    sessionId: "s1",
    orderSeq,
...
function makeUserRecordWithFiles(orderSeq: number, text: string, files: Array<Record<string, unknown>>) {
  return {
    id: `user-${orderSeq}`,
    sessionId: "s1",
    orderSeq,
...
function makeAssistantRecord(orderSeq: number, text: string, status: "sent" | "pending" | "error" = "sent") {
  return {
    id: `asst-${orderSeq}`,
    sessionId: "s1",
    orderSeq,
    role: "assistant" as const,
    content: JSON.stringify([{ type: "content", content: text, status: "success", timestamp: Date.now() }]),
    status,
    isContextEdge: 0,
    metadata: "{}",
    createdAt: Date.now(),
...
function makeAssistantErrorRecord(orderSeq: number, errorMessage: string, partialText: string = "") {
  return {
    id: `asst-${orderSeq}`,
    sessionId: "s1",
    orderSeq,
    ...
    content: JSON.stringify([
      ...(partialText ? [{ type: "content", content: partialText, status: "success", timestamp: Date.now() }] : []),
...
function makeAssistantErrorWithToolRecord(orderSeq: number, text: string, toolResponse: string, errorMessage: string) {
  return {
    id: `asst-${orderSeq}`,
    sessionId: "s1",
    orderSeq,
...
function makeAssistantWithToolProviderOptionsRecord(orderSeq: number, text: string, toolResponse: string) {
  return {
    id: `asst-${orderSeq}`,
    sessionId: "s1",
    orderSeq,
...
describe("buildContext", () => {
  ...
  it("preserves partial assistant content before the failure reason", () => {
    const messages = [makeUserRecord(1, "do work"), makeAssistantErrorRecord(2, "timeout", "Partial answer")];
    const store = createMockMessageStore(messages);
    const result = buildContext("s1", "continue", "", 10000, 4096, store);
  ...
  it("adds empty reasoning_content for settled tool calls when empty interleaved preservation is enabled", () => {
    const messages = [makeUserRecord(1, "Use a tool"), makeAssistantWithToolRecord(2, "", "All good")];
    const store = createMockMessageStore(messages);
    const result = buildContext("s1", "next", "", 10000, 4096, store, false, {
      preserveInterleavedReasoning: true,
      preserveEmptyInterleavedReasoning: true,
  ...
  it("builds from provided history records without rereading newer persisted messages", () => {
    ...
    const store = createMockMessageStore(messages);
    const result = buildContext("s1", "new user already persisted", "System", 10000, 4096, store, false, {
  ...
  it("converts audio files to input_audio when audio input is enabled", () => {
    ...
    expect(
      userMessageParts.some(
        (part) => part.type === "text" && typeof part.text === "string" && part.text.includes("Audio file path:"),
      ),
    ).toBe(false);
  });
