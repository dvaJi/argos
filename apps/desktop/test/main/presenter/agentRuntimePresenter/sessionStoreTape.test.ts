...
const sqlitePresenterModule = sqliteModule ? await import("../../../../src/main/presenter/sqlitePresenter") : null;
...
const ArgosSessionStore = sessionStoreModule?.ArgosSessionStore;
const SQLitePresenterCtor = SQLitePresenter!;
const ArgosSessionStoreCtor = ArgosSessionStore!;
...
describeIfSqlite("ArgosSessionStore tape summary state", () => {
  function createStore() {
    const sqlitePresenter = new SQLitePresenterCtor(":memory:");
    const store = new ArgosSessionStoreCtor(sqlitePresenter);
  ...
  it("creates a bootstrap anchor for each session", () => {
    ...
    expect(sqlitePresenter.argosTapeEntriesTable.getBySession("s1")).toMatchObject([
      {
        session_id: "s1",
        entry_id: 1,
        kind: "anchor",
    ...
    expect(sqlitePresenter.argosTapeEntriesTable.getBySession("s2")).toMatchObject([
      {
        session_id: "s2",
        entry_id: 1,
        kind: "anchor",
  ...
  it("prefers compaction summary anchors over legacy summary columns", () => {
    ...
    expect(sqlitePresenter.argosTapeEntriesTable.getLatestSummaryAnchor("s1")).toMatchObject({
      name: "compaction/manual",
      created_at: 100,
    });
  ...
  it("uses handoff anchors as context reconstruction state", () => {
    ...
    sqlitePresenter.argosTapeEntriesTable.appendAnchor({
      sessionId: "s1",
      name: "handoff/manual",
      state: {
  ...
  it("compares summary state against tape reconstruction anchors before writing compaction anchors", () => {
    ...
    expect(sqlitePresenter.argosTapeEntriesTable.getLatestReconstructionAnchor("s1")).toMatchObject({
      name: "compaction/auto",
      created_at: 200,
    });
  ...
  it("does not write a stale anchor when summary compare-and-set fails", () => {
    ...
    expect(sqlitePresenter.argosTapeEntriesTable.getLatestSummaryAnchor("s1")).toBeUndefined();
