    ...
    argosTapeEntriesTable: (argosTapeEntriesTable = {
      ...
      append: vi.fn<(...args: any[]) => any>((input: any) => {
        ...
        const existing = input.idempotent
          ? tapeEntries.find(
              (entry) =>
                entry.session_id === input.sessionId && entry.provenance_key && entry.provenance_key === provenanceKey,
            )
          : undefined;
        if (existing) {
          return existing;
        ...
        const row = {
          ...
          entry_id:
            Math.max(
              0,
              ...tapeEntries.filter((entry) => entry.session_id === input.sessionId).map((entry) => entry.entry_id),
            ) + 1,
          kind: input.kind,
...
function makeArgosUserRow(orderSeq: number, text: string, id = `u${orderSeq}`) {
  return {
    id,
    session_id: "s1",
    order_seq: orderSeq,
...
function makeArgosAssistantRow(
  orderSeq: number,
  content: string,
  id = `a${orderSeq}`,
  ...
  return {
    ...
    content: JSON.stringify([{ type: "content", content, status: "success", timestamp: Date.now() }]),
    status,
    is_context_edge: 0,
    metadata: "{}",
    created_at: Date.now(),
...
describe("AgentRuntimePresenter", () => {
  ...
  function installSessionRows(initialRows: any[]) {
    ...
    sqlitePresenter.argosMessagesTable.getBySession.mockImplementation((sessionId: string) =>
      rows.filter((row) => row.session_id === sessionId),
    );
    sqlitePresenter.argosMessagesTable.get.mockImplementation((id: string) => rows.find((row) => row.id === id));
    sqlitePresenter.argosMessagesTable.getLastUserMessageBeforeOrAtOrderSeq.mockImplementation(
      (sessionId: string, orderSeq: number) =>
        [...rows]
          .reverse()
          .find((row) => row.session_id === sessionId && row.role === "user" && row.order_seq <= orderSeq),
    );
    sqlitePresenter.argosMessagesTable.getIdsFromOrderSeq.mockImplementation(
      (sessionId: string, fromOrderSeq: number) =>
        rows.filter((row) => row.session_id === sessionId && row.order_seq >= fromOrderSeq).map((row) => row.id),
    );
    sqlitePresenter.argosMessagesTable.deleteFromOrderSeq.mockImplementation(
      (sessionId: string, fromOrderSeq: number) => {
        rows = rows.filter((row) => row.session_id !== sessionId || row.order_seq < fromOrderSeq);
      },
    );
    sqlitePresenter.argosMessagesTable.getMaxOrderSeq.mockImplementation((sessionId: string) =>
      rows.reduce(
        (maxOrderSeq, row) => (row.session_id === sessionId ? Math.max(maxOrderSeq, row.order_seq) : maxOrderSeq),
        0,
      ),
    );
  ...
  describe("constructor (crash recovery)", () => {
      ...
      sqlitePresenter.argosMessagesTable.getByStatus.mockReturnValue([
        {
          id: "m1",
          role: "assistant",
          content: JSON.stringify([{ type: "content", content: "partial", status: "pending", timestamp: 1 }]),
        },
      ]);

      new AgentRuntimePresenter(llmProvider, configPresenter, sqlitePresenter, toolPresenter, undefined, {
        skillPresenter: getSkillPresenterMock(),
      });

      expect(consoleSpy).toHaveBeenCalledWith("ArgosAgent: recovered 1 pending messages to error status");
    ...
    it("only recovers claimed pending inputs for sessions that still exist", () => {
      ...
      sqlitePresenter.argosSessionsTable.get.mockImplementation((sessionId: string) =>
      ...
      expect(sqlitePresenter.argosPendingInputsTable.update).toHaveBeenCalledTimes(1);
      expect(sqlitePresenter.argosPendingInputsTable.update).toHaveBeenCalledWith("pending-existing", {
        state: "pending",
        claimed_at: null,
      });
      expect(consoleSpy).toHaveBeenCalledWith("ArgosAgent: recovered 1 sessions with claimed pending inputs");
      consoleSpy.mockRestore();
    });
  });
  ...
  describe("getSessionState", () => {
    ...
    it("returns null for unknown session", async () => {
      sqlitePresenter.argosSessionsTable.get.mockReturnValue(undefined);
  ...
  describe("processMessage", () => {
    it("creates user and assistant messages with correct order_seq", async () => {
      sqlitePresenter.argosMessagesTable.getMaxOrderSeq
    ...
    it("includes conversation history in LLM call", async () => {
      ...
      sqlitePresenter.argosMessagesTable.getBySession.mockReturnValue(existingMessages);
      sqlitePresenter.argosMessagesTable.getMaxOrderSeq.mockReturnValueOnce(2).mockReturnValueOnce(3);
    ...
    it("compacts old turns into summary before building prompt", async () => {
      ...
      expect(sqlitePresenter.argosSessionsTable.updateSummaryState).toHaveBeenCalledWith(
        "s1",
        expect.objectContaining({
    ...
    it("emits and clears an ephemeral rate-limit message while waiting for the provider gate", async () => {
      ...
      const typedStreamUpdates = (publishArgosEvent as ReturnType<typeof vi.fn>).mock.calls
    ...
    it("does not call provider.coreStream when a queued request is canceled", async () => {
      ...
      llmProvider.executeWithRateLimit.mockImplementation(
        (_providerId: string, options?: { signal?: AbortSignal; onQueued?: (snapshot: any) => void }) =>
      ...
      (processStream as ReturnType<typeof vi.fn>).mockImplementation(
        async (params: {
          ...
          try {
            ...
            return {
              status: error instanceof Error && error.name === "AbortError" ? ("aborted" as const) : ("error" as const),
              stopReason: error instanceof Error && error.name === "AbortError" ? "user_stop" : "error",
    ...
    it("does not call provider.coreStream when cancellation lands right after rate-limit wait", async () => {
      llmProvider.executeWithRateLimit.mockImplementation(
        async (_providerId: string, options?: { signal?: AbortSignal; onQueued?: (snapshot: any) => void }) => {
          options?.onQueued?.({
            providerId: "openai",
            qpsLimit: 1,
            currentQps: 1,
    ...
    it("keeps system prompt section order: user prompt -> runtime -> env -> skills -> tooling -> permission -> verification", async () => {
      ...
      expect(callArgs.messages.filter((message: { role: string }) => message.role === "system")).toHaveLength(1);
    ...
    it("emits a refresh for the persisted user message before streaming starts", async () => {
      ...
      (processStream as ReturnType<typeof vi.fn>).mockImplementationOnce(async () => {
        refreshCountAtStreamStart = (eventBus.sendToRenderer as ReturnType<typeof vi.fn>).mock.calls.filter(
          (call: any[]) => call[0] === "stream:end",
        ).length;
    ...
    it("throws for unknown session", async () => {
      await expect(agent.processMessage("unknown", "hi")).rejects.toThrow("Session unknown not found");
    });

    it("persists files when message input is object", async () => {
      sqlitePresenter.argosMessagesTable.getMaxOrderSeq.mockReturnValueOnce(0).mockReturnValueOnce(1);
    ...
    it("skips Argos runtime prompt layers and local tools for ACP-backed subagent sessions", async () => {
      sqlitePresenter.newSessionsTable.get.mockReturnValue({
        id: "s-acp-subagent",
    ...
    it("injects request trace context when trace debug is enabled", async () => {
      configPresenter.getSetting.mockImplementation((key: string) => (key === "traceDebugEnabled" ? true : undefined));
      ...
      await traceContext.persist({
        ...
        body: {
          ...
          nested: {
            token: "argos-token-9999",
          },
        },
      });

      expect(sqlitePresenter.argosMessageTracesTable.insert).toHaveBeenCalledTimes(1);
      const inserted = sqlitePresenter.argosMessageTracesTable.insert.mock.calls[0][0];
    ...
    it("does not inject request trace context when trace debug is disabled", async () => {
      configPresenter.getSetting.mockImplementation((key: string) => (key === "traceDebugEnabled" ? false : undefined));
      ...
      expect(sqlitePresenter.argosMessageTracesTable.insert).not.toHaveBeenCalled();
    });
    ...
    it("persists interleaved reasoning gaps into traces when trace debug is enabled", async () => {
      ...
      expect(inserted.endpoint).toBe("argos://interleaved-reasoning-gap");
      expect(body).toEqual({
        providerId: "zenmux",
        modelId: "moonshotai/kimi-k2.5",
        providerDbSourceUrl: "https://example.com/dist/all.json",
  ...
  describe("generation settings", () => {
    ...
    it("updates generation settings with minimal validation and keeps invalid fields unchanged", async () => {
      ...
      expect(sqlitePresenter.argosSessionsTable.updateGenerationSettings).toHaveBeenCalledWith(
        "s1",
        expect.objectContaining({
          temperature: 9,
          contextLength: 128000,
    ...
    it("normalizes Moonshot Kimi generation temperatures from model reasoning defaults", async () => {
      ...
      configPresenter.getReasoningPortrait.mockImplementation((providerId: string, modelId: string) => {
        if (providerId === "moonshot" && modelId === "moonshotai/kimi-k2.6") {
          return {
            supported: true,
            defaultEnabled: true,
    ...
    it("inherits interleaved thinking defaults and allows explicit session disable", async () => {
      ...
      expect(sqlitePresenter.argosSessionsTable.updateGenerationSettings).toHaveBeenLastCalledWith(
        "s1",
        expect.objectContaining({
          forceInterleavedThinkingCompat: false,
        }),
      ...
      const interleavedConfig = (agent as any).resolveInterleavedReasoningConfig("openai", "gpt-4", disabled);
    ...
    it("drops new-api anthropic adaptive reasoning overrides when backend reasoning is disabled", async () => {
      configPresenter.getCapabilityProviderId.mockImplementation((providerId: string, modelId: string) =>
    ...
    it("drops zenmux anthropic adaptive reasoning overrides when backend reasoning is disabled", async () => {
      configPresenter.getCapabilityProviderId.mockImplementation((providerId: string, modelId: string) =>
        providerId === "zenmux" && modelId === "anthropic/claude-opus-4-7" ? "anthropic" : providerId,
      );
      configPresenter.getModelConfig.mockImplementation((modelId: string, providerId: string) => {
    ...
    it("keeps system prompt and resets other settings to the new model defaults", async () => {
      ...
      configPresenter.getThinkingBudgetRange.mockImplementation((providerId: string, modelId: string) => {
        if (providerId === "anthropic" && modelId === "claude-3-5-sonnet") {
          return { min: 0, max: 4096, default: 256 };
        }
        return { min: 0, max: 8192, default: 512 };
      });
      configPresenter.getReasoningEffortDefault.mockImplementation((providerId: string, modelId: string) => {
        if (providerId === "anthropic" && modelId === "claude-3-5-sonnet") {
          return "low";
        }
        return "medium";
      });
      configPresenter.getVerbosityDefault.mockImplementation((providerId: string, modelId: string) => {
        if (providerId === "anthropic" && modelId === "claude-3-5-sonnet") {
          return "high";
        }
        return "medium";
      ...
      expect(sqlitePresenter.argosSessionsTable.updateSessionModel).toHaveBeenCalledWith(
        "s1",
        "anthropic",
        "claude-3-5-sonnet",
      );
    ...
    it("drops unsupported reasoning and verbosity settings when switching models", async () => {
      ...
      configPresenter.supportsReasoningCapability.mockImplementation(
        (providerId: string, modelId: string) => !(providerId === "openai" && modelId === "gpt-4o-mini"),
      );
    ...
    it("rejects model switching while the session is generating", async () => {
      await agent.initSession("s1", { providerId: "openai", modelId: "gpt-4" });
      ((agent as any).runtimeState as Map<string, ArgosSessionState>).set("s1", {
        status: "generating",
        providerId: "openai",
        modelId: "gpt-4",
        permissionMode: "full_access",
      ...
      expect(sqlitePresenter.argosSessionsTable.updateSessionModel).not.toHaveBeenCalled();
    });
  });
  ...
  describe("summary invalidation", () => {
    it("resets summary when deleting history before cursor", async () => {
      await agent.initSession("s1", { providerId: "openai", modelId: "gpt-4" });
      sqlitePresenter.argosSessionsTable.updateSummaryState("s1", {
        summaryText: "summary",
      ...
      expect(sqlitePresenter.argosSessionsTable.resetSummaryState).toHaveBeenCalledWith("s1");
  ...
  describe("session compaction state", () => {
    ...
    it("bypasses Argos context preflight for oversized ACP provider calls", async () => {
      await agent.initSession("s1", {
        providerId: "acp",
      ...
      expect(JSON.stringify((eventBus.sendToRenderer as ReturnType<typeof vi.fn>).mock.calls)).not.toContain(
        "Request was not sent",
      );
      expect(JSON.stringify(sqlitePresenter.argosMessagesTable.updateContentAndStatus.mock.calls)).not.toContain(
        "Request was not sent",
      );
    });
    ...
    it("bypasses chat context preflight for image generation endpoints", async () => {
      ...
      const requestMessages = [{ role: "user" as const, content: makeTextWithEstimatedTokens(9000) }];
    ...
    it("preflights provider calls with a safety margin and compacts before low-output pressure calls", async () => {
      ...
      await agent.processMessage("s1", "Hello");
      sqlitePresenter.argosMessagesTable.getBySession.mockReturnValue(createSentTurnRecords(3));
      llmProvider.generateText.mockClear();
      ...
      const totalRequestTokens =
        estimateMessagesTokens(providerMessages) + estimateToolReserveTokens(providerTools) + providerMaxTokens;
    ...
    it("emits compacting before compacted on successful compaction", async () => {
      ...
      const insertRows = sqlitePresenter.argosMessagesTable.insert.mock.calls.map(([row]: any[]) => row);
      ...
      const compactionInsert = sqlitePresenter.argosMessagesTable.insert.mock.calls.find(
        ([row]: any[]) => typeof row?.metadata === "string" && row.metadata.includes('"messageType":"compaction"'),
      )?.[0];
      ...
      const finalizedCompaction = sqlitePresenter.argosMessagesTable.updateContentAndStatus.mock.calls.find(
        ([, , , metadata]: any[]) => typeof metadata === "string" && metadata.includes('"messageType":"compaction"'),
      );
    ...
    it("manually compacts without creating a user turn or streaming", async () => {
      ...
      sqlitePresenter.argosMessagesTable.getBySession.mockReturnValue(createSentTurnRecords(3));
      sqlitePresenter.argosMessagesTable.getMaxOrderSeq.mockReturnValue(6);
    ...
    it("marks the session generating before manual compaction preparation awaits", async () => {
      sqlitePresenter.argosMessagesTable.getBySession.mockReturnValue(createSentTurnRecords(1));
      sqlitePresenter.argosMessagesTable.getMaxOrderSeq.mockReturnValue(2);
    ...
    it("does not manually compact ACP sessions", async () => {
      ...
      await expect(agent.compactSession("s1")).rejects.toThrow(
        "Manual compaction is only available for Argos agent sessions.",
      );
      expect(prepareSpy).not.toHaveBeenCalled();
    });
    ...
    it("falls back to the previous compacted state when compaction fails", async () => {
      ...
      sqlitePresenter.argosMessagesTable.getBySession.mockReturnValue(createSentTurnRecords(4));
      sqlitePresenter.argosMessagesTable.getMaxOrderSeq
        .mockReturnValueOnce(8)
      ...
      expect(sqlitePresenter.argosMessagesTable.delete).toHaveBeenCalledWith("mock-msg-id");
    });
  ...
  describe("retry context overflow recovery", () => {
    it("recovers an unfittable retry provider request before calling the provider", async () => {
      ...
      installSessionRows([
        makeArgosUserRow(1, "A".repeat(100)),
        makeArgosAssistantRow(2, "B".repeat(100)),
        makeArgosUserRow(3, "C".repeat(100)),
        makeArgosAssistantRow(4, "D".repeat(100)),
        makeArgosUserRow(5, "E".repeat(100)),
        makeArgosAssistantRow(6, "F".repeat(100)),
        makeArgosUserRow(7, "retry target", "retry-user"),
        makeArgosAssistantRow(8, "failed answer", "retry-assistant", "error"),
      ]);
    ...
    it("fails retry with budget guidance when the current input cannot fit", async () => {
      ...
      installSessionRows([
        makeArgosUserRow(1, makeTextWithEstimatedTokens(9000), "retry-user"),
        makeArgosAssistantRow(2, "failed answer", "retry-assistant", "error"),
      ]);
      ...
      const providerCoreStream = llmProvider.getProviderInstance.mock.results[0].value.coreStream;
      const errorUpdate = sqlitePresenter.argosMessagesTable.updateContentAndStatus.mock.calls
  ...
  describe("respondToolInteraction", () => {
    ...
    it("handles question_option and resumes assistant message", async () => {
      ...
      expect(sqlitePresenter.argosMessagesTable.updateContent).toHaveBeenCalledWith("m1", expect.any(String));

      const updatedBlocks = JSON.parse(sqlitePresenter.argosMessagesTable.updateContent.mock.calls[0][1]);
    ...
    it("preserves reasoning_content when resuming after a question answer", async () => {
      ...
      sqlitePresenter.argosMessagesTable.updateContent.mockImplementation((id: string, content: string) => {
        if (id === row.id) {
          row.content = content;
        }
      });
      ...
      expect(result).toEqual({ resumed: true });
      const callArgs = (processStream as ReturnType<typeof vi.fn>).mock.calls[0][0];
      const assistantMessage = callArgs.messages.find((message: any) => message.role === "assistant");
    ...
    it("treats an aborted resume signal as cancellation even for non-abort errors", async () => {
      ...
      const [messageId, contentJson, status] =
        sqlitePresenter.argosMessagesTable.updateContentAndStatus.mock.calls.at(-1);
    ...
    it("handles question_other and waits for user message without resume", async () => {
      ...
      expect(sqlitePresenter.argosMessagesTable.updateStatus).toHaveBeenCalledWith("m1", "sent");
      expect(processStream).not.toHaveBeenCalled();

      const updatedBlocks = JSON.parse(sqlitePresenter.argosMessagesTable.updateContent.mock.calls[0][1]);
      expect(updatedBlocks[0].tool_call.response).toBe("User chose to answer with a follow-up message.");
      expect(updatedBlocks[0].status).toBe("success");
      expect(updatedBlocks[1].status).toBe("success");
    });
    ...
    it("cleans deferred offload files when resume budget downgrades the tool result", async () => {
      ...
      try {
        ...
        expect(result).toEqual({ resumed: true });
        const updateCalls = sqlitePresenter.argosMessagesTable.updateContent.mock.calls;
        const updatedBlocks = JSON.parse(updateCalls[updateCalls.length - 1][1]);
        expect(updatedBlocks[0].tool_call.response).toContain("remaining context window is insufficient");
        expect(updatedBlocks[0].tool_call.response).not.toContain("[Tool output offloaded]");
        const postToolUseCalls = hookDispatcher.dispatchEvent.mock.calls.filter(([event]) => event === "PostToolUse");
    ...
    it("handles ACP permission grant through live provider resolver without deferred tool resume", async () => {
      ...
      (agent as any).activeProviderPermissions.set("acp-req-1", {
        ...
        resolve: async (granted: boolean) => {
          ...
          sqlitePresenter.argosMessagesTable.updateContent(
            "m1",
            JSON.stringify([
              {
                type: "tool_call",
  ...
  describe("permission mode", () => {
    it("setPermissionMode updates runtime and db", async () => {
      ...
      expect(sqlitePresenter.argosSessionsTable.updatePermissionMode).toHaveBeenCalledWith("s1", "default");
    });
  ...
  describe("disabled tools", () => {
    ...
    it("publishes typed stream failure when deferred tool execution returns a terminal error", async () => {
      ...
      try {
        ...
        expect(result).toEqual({ resumed: false });
        expect(publishArgosEvent).toHaveBeenCalledWith(
          "chat.stream.failed",
          expect.objectContaining({
            requestId: "m1",
            sessionId: "s1",
    ...
    it("re-reads the latest message content before persisting subagent progress", async () => {
      ...
      sqlitePresenter.argosMessagesTable.get
    ...
    it("falls back to the current session agent vision model when the current model has no vision", async () => {
      ...
      expect(configPresenter.resolveArgosAgentConfig).toHaveBeenCalledWith("persisted-agent");
      expect(configPresenter.agentSupportsCapability).toHaveBeenCalledWith("persisted-agent", "vision");
      expect(llmProvider.executeWithRateLimit).toHaveBeenCalledWith(
        "google",
        expect.objectContaining({
          signal: undefined,
