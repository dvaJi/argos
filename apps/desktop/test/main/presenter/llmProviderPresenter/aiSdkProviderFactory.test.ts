...
describe("AI SDK provider factory", () => {
  ...
  it("normalizes anthropic-style base urls to a v1 prefix", () => {
    ...
    expect(normalizeAnthropicBaseUrl("https://zenmux.ai/api/anthropic/")).toBe("https://zenmux.ai/api/anthropic/v1");
  });
  ...
  it("avoids duplicating the messages suffix", () => {
    ...
    expect(normalizeAnthropicBaseUrl("https://proxy.example.com/messages")).toBe("https://proxy.example.com");
  });
  ...
  it("normalizes vertex express-mode base urls to the publishers/google prefix", () => {
    ...
    expect(normalizeVertexBaseUrl("https://zenmux.ai/api/vertex-ai/v1/publishers/google", "api-key", "v1")).toBe(
      "https://zenmux.ai/api/vertex-ai/v1/publishers/google",
    );
  });
  ...
  it("normalizes gemini base urls to a v1beta prefix", () => {
    ...
    expect(normalizeGeminiBaseUrl("https://api.newapi.ai/v1beta")).toBe("https://api.newapi.ai/v1beta");
    expect(normalizeGeminiBaseUrl("https://api.newapi.ai/v1beta1")).toBe("https://api.newapi.ai/v1beta1");
  });
  ...
  it("normalizes Ollama base urls for SDK and OpenAI-compatible endpoints", () => {
    ...
    expect(normalizeOllamaSdkHost("http://example.com/ollama/api")).toBe("http://example.com/ollama");

    expect(normalizeOllamaOpenAIBaseUrl("http://localhost:11434")).toBe("http://localhost:11434/v1");
    expect(normalizeOllamaOpenAIBaseUrl("http://localhost:11434/api")).toBe("http://localhost:11434/v1");
    expect(normalizeOllamaOpenAIBaseUrl("http://localhost:11434/v1")).toBe("http://localhost:11434/v1");
    expect(normalizeOllamaOpenAIBaseUrl("http://example.com/ollama/api")).toBe("http://example.com/ollama/v1");
  });
  ...
  it("preserves deployment-based azure urls and legacy api versions", () => {
    expect(normalizeAzureBaseUrl("https://example.openai.azure.com/openai/deployments/argos-prod", undefined)).toEqual({
      baseURL: "https://example.openai.azure.com/openai",
      apiVersion: "2024-02-01",
      useDeploymentBasedUrls: true,
      deploymentName: "argos-prod",
    });

    expect(
      normalizeAzureBaseUrl("https://example.openai.azure.com/openai/deployments/argos-prod", "2025-04-01-preview"),
    ).toEqual({
      baseURL: "https://example.openai.azure.com/openai",
  ...
  it("builds azure responses endpoints without duplicating v1 segments", () => {
    ...
    expect(context.endpoint).toBe("https://example.openai.azure.com/openai/v1/responses?api-version=v1");
    expect(context.embeddingEndpoint).toBe("https://example.openai.azure.com/openai/v1/embeddings?api-version=v1");
    expect(context.imageEndpoint).toBe("https://example.openai.azure.com/openai/v1/images/generations?api-version=v1");
  ...
  it("uses deployment ids from azure deployment-scoped urls", () => {
    const context = createAiSdkProviderContext({
      ...
      provider: {
        ...
        name: "Azure OpenAI",
        apiKey: "test-key",
        baseUrl: "https://example.openai.azure.com/openai/deployments/argos-prod",
        enable: false,
      } as any,
      configPresenter: {
        getSetting: () => undefined,
    ...
    expect(context.endpoint).toBe(
      "https://example.openai.azure.com/openai/deployments/argos-prod/responses?api-version=2024-02-01",
    );
    expect(context.resolvedModelId).toBe("argos-prod");
  });
