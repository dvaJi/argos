...
describe("AI SDK runtime", () => {
  ...
  it("fails cleanly when chat-audio TTS content is text without audio data", async () => {
    ...
    await expect(drainStream()).rejects.toThrow("TTS response missing audio data in choices[0].message.audio.data");
  });
  ...
  it.each(["anthropic/claude-opus-4-7", "claude-opus-4-7-think"])(
    ...
    async (modelId) => {
      ...
      const request = mockStreamText.mock.calls[0]?.[0] as Record<string, unknown>;
      expect(context.configPresenter.getCapabilityProviderId).toHaveBeenCalledWith("aihubmix", modelId);
      expect(context.configPresenter.supportsTemperatureControl).toHaveBeenCalledWith("anthropic", modelId);
      expect(request).not.toHaveProperty("temperature");
      expect(tracePayloads[0]?.body).not.toHaveProperty("temperature");
      expect(events).toEqual([]);
    },
  ...
  it("omits temperature and topP for new-api anthropic routes that disable sampling controls", async () => {
    ...
    const request = mockStreamText.mock.calls[0]?.[0] as Record<string, unknown>;
    expect(context.configPresenter.supportsTemperatureControl).toHaveBeenCalledWith("anthropic", "claude-opus-4-8");
    expect(request).not.toHaveProperty("temperature");
