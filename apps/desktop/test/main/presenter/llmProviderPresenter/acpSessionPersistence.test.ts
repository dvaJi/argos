...
import type { AcpSessionEntity, ISQLitePresenter } from "@argos/shared/types/presenters";
...
describe("AcpSessionPersistence remote session sync", () => {
  ...
  it("cleans up the new conversation when remote session save cannot be recovered", async () => {
    ...
    expect(sqlitePresenter.createConversation).toHaveBeenCalledWith("Remote title", expect.any(Object));
