...
describe("OllamaProvider.fetchModels", () => {
  ...
  const originalAllowInsecureTls = process.env.ARGOS_ALLOW_INSECURE_TLS;

  beforeEach(() => {
    mockOllamaConstructorOptions.length = 0;
    mockExecFile.mockReset();
    delete process.env.ARGOS_ALLOW_INSECURE_TLS;
  ...
  afterEach(() => {
    if (originalAllowInsecureTls === undefined) {
      ...
      process.env.ARGOS_ALLOW_INSECURE_TLS = originalAllowInsecureTls;
    }
  });
  ...
  it("merges local and running models, keeps running-only models, and preserves capabilities", async () => {
    ...
    expect(models.map((model) => model.id)).toEqual(["deepseek-r1:1.5b", "nomic-embed-text:latest", "qwen3:8b"]);
    ...
    expect(configPresenter.ensureModelStatus).toHaveBeenCalledWith("ollama", "deepseek-r1:1.5b", true);
    expect(configPresenter.ensureModelStatus).toHaveBeenCalledWith("ollama", "nomic-embed-text:latest", true);
  ...
  it("treats latest tags from ollama list as a successful untagged pull", async () => {
    ...
    mockExecFile.mockImplementationOnce((_command, _args, _options, callback) => {
      callback(
        null,
        ["NAME          ID              SIZE      MODIFIED", "qwen3:latest  500a1f067a9f    5.2 GB    now"].join("\n"),
        "",
      );
    });
