...
describe("AcpProcessManager config cache fallback", () => {
  ...
  it("falls back when a warmup workdir no longer exists", () => {
    ...
    try {
      ...
      expect(normalizePathValue(resolved)).toContain("/tmp/argos-acp/sessions");
    } finally {
  ...
  it("falls back to the ACP temp workdir instead of process.cwd() when session workdir is missing", async () => {
    ...
    expect(normalizePathValue(terminalRequest.cwd)).toContain("/argos-acp/sessions");
  });
  ...
  it("keeps explicit PATH overrides ahead of bundled runtime and shell PATH", async () => {
    ...
    try {
      ...
      const ports = createAcpTestPorts();
      ports.runtime.buildSpawnEnv = (env) => ({
        ...env,
      ...
      expect(env.ACP_IDE).toBe("argos");
      expect(env.ARGOS_ACP_AGENT_ID).toBe("agent-1");
  ...
  it("moves only the broken npx hash directory and retries once", async () => {
    const npxRoot = normalizePathValue("/tmp/argos-acp-npx/_npx");
