...
describe("AgentRepository", () => {
  it("resolves default Argos subagent slots for the builtin agent", () => {
    ...
    repository.ensureBuiltinArgosAgent({ name: "Argos", config: {} });
    const config = repository.resolveArgosAgentConfig("argos");
  ...
  it("inherits Argos image generation model from the builtin agent", () => {
    const now = Date.now();
    const rows = new Map<string, any>([
      [
        "argos",
    ...
    expect(repository.resolveArgosAgentConfig("custom-agent").imageGenerationModel).toEqual({
      providerId: "openai",
      modelId: "gpt-image-1",
    });
  });
