...
function mockSkillTree(relativeRoots: string[]) {
  const tree = new Map<string, Array<ReturnType<typeof createDirEntry> | ReturnType<typeof createFileEntry>>>();
...
function getWatcherHandler(eventName: string) {
  const watcherInstance = (watch as Mock).mock.results[(watch as Mock).mock.results.length - 1]?.value as
    | { on: Mock }
    | undefined;
...
describe("SkillPresenter", () => {
  ...
  beforeEach(() => {
    ...
    discoveryWorkerMock.discoverSkillMetadataInWorker.mockRejectedValue(new Error("worker unavailable"));
  ...
  describe("constructor", () => {
    ...
    it("should repair malformed .argos path segments", async () => {
      (mockConfigPresenter.getSkillsPath as Mock).mockReturnValue("/mock/home.argos/skills");
    ...
    it("should repair stale POSIX default skills paths from another user profile", async () => {
      (mockConfigPresenter.getSkillsPath as Mock).mockReturnValue("/Users/legacy-user/.argos/skills");
      (app.getPath as Mock).mockImplementation((name: string) => {
        if (name === "home") return "/mock/home";
        if (name === "temp") return "/mock/temp";
        return "/mock/" + name;
    ...
    it("should repair stale Windows default skills paths from another user profile", async () => {
      (mockConfigPresenter.getSkillsPath as Mock).mockReturnValue("C:\\Users\\legacy-user\\.argos\\skills\\nested");
      (app.getPath as Mock).mockImplementation((name: string) => {
        if (name === "home") return "/mock/home";
        if (name === "temp") return "/mock/temp";
        return "/mock/" + name;
  ...
  describe("discoverSkills", () => {
    ...
    it("should discover skills from directories with SKILL.md", async () => {
      ...
      expect(eventBus.sendToRenderer).toHaveBeenCalledWith(SKILL_EVENTS.DISCOVERED, "all", expect.any(Array));
    });
    ...
    it("sorts manifest paths before resolving duplicate skill names", async () => {
      ...
      expect((skillPresenter as any).parseSkillMetadata.mock.calls.map((call: unknown[]) => call[0])).toEqual([
        firstPath,
        secondPath,
      ]);
      expect(skills).toEqual([
    ...
    it("falls back to main-thread discovery when the worker fails", async () => {
      discoveryWorkerMock.discoverSkillMetadataInWorker.mockRejectedValue(new Error("worker failed"));
  ...
  describe("getMetadataList", () => {
    ...
    it("shows plugin-owned skills after registration and hides them after unregistering owner", async () => {
      ...
      (matter as unknown as Mock).mockImplementation((content: string) => {
        if (content === "plugin-skill") {
          return {
            ...
            content: "Plugin root: `${PLUGIN_ROOT}`. Arch: `${PROCESS_ARCH}`. Owner: `${OWNER_PLUGIN_ID}`.",
          };
        }
      ...
      await skillPresenter.registerPluginSkill({
        ownerPluginId: "com.argos.plugins.fixture",
        id: "plugin-skill",
      ...
      expect(pluginSkillContent?.content).toContain("Owner: `com.argos.plugins.fixture`");
      ...
      await skillPresenter.unregisterPluginSkillsByOwner("com.argos.plugins.fixture");

      expect((await skillPresenter.getMetadataList()).map((skill) => skill.name)).toEqual(["regular-skill"]);
  ...
  describe("loadSkillContent", () => {
    ...
    it("should load skill content by name", async () => {
      ...
      expect(content?.content).toContain("/.argos/skills/test-skill`.");
      ...
      expect(content?.content).toContain("When this skill needs script execution, prefer `skill_run` over `exec`.");
    });
  ...
  describe("viewSkill", () => {
    beforeEach(async () => {
      ...
      (fs.readdirSync as Mock).mockImplementation((target: string) => {
        ...
        if (target === `${DEFAULT_SKILLS_DIR}/engineering/test-skill`) {
          return [createFileEntry("SKILL.md"), createDirEntry("references"), createDirEntry("scripts")];
        }
    ...
    it("activates a skill after viewing the main SKILL.md in a new-agent session", async () => {
      ...
      expect(await skillPresenter.getActiveSkills("conv-view-auto-activate")).toEqual(["test-skill"]);
    ...
    it("rejects oversized skill markdown files before loading content", async () => {
      ...
      expect(fs.readFileSync).not.toHaveBeenCalledWith(expect.stringContaining("/test-skill/SKILL.md"), "utf-8");
    });
  ...
  describe("manageDraftSkill", () => {
    ...
    it("views draft content without exposing draft paths", async () => {
      ...
      const draftPath = "/mock/temp/argos-skill-drafts/conv-draft/draft-12345678-1234-1234-1234-123456789abc";
      (fs.existsSync as Mock).mockImplementation((target: string) => {
        return target === draftPath || target === `${draftPath}/SKILL.md` || target === "/mock/temp/argos-skill-drafts";
      });
      (fs.readFileSync as Mock).mockImplementation((target: string) => {
      ...
      const result = await skillPresenter.viewDraftSkill("conv-draft", "draft-12345678-1234-1234-1234-123456789abc");
      ...
      expect(result).not.toHaveProperty("draftPath");
      expect(fs.writeFileSync).toHaveBeenCalledWith(`${draftPath}/.lastActivity`, expect.any(String), "utf-8");
    });
    ...
    it("installs draft content and removes the temp draft", async () => {
      ...
      (fs.existsSync as Mock).mockImplementation((target: string) => {
        ...
        return (
          ...
          target === "/mock/temp/argos-skill-drafts" ||
      ...
      const result = await skillPresenter.installDraftSkill("conv-draft", "draft-12345678-1234-1234-1234-123456789abc");
    ...
    it("discards draft content and removes empty conversation draft folder", async () => {
      const draftPath = "/mock/temp/argos-skill-drafts/conv-draft/draft-12345678-1234-1234-1234-123456789abc";
      const conversationPath = "/mock/temp/argos-skill-drafts/conv-draft";
      (fs.existsSync as Mock).mockImplementation((target: string) => {
      ...
      const result = await skillPresenter.discardDraftSkill("conv-draft", "draft-12345678-1234-1234-1234-123456789abc");
  ...
  describe("cleanupExpiredDrafts", () => {
    it("uses the last activity marker instead of the draft directory mtime", () => {
      const now = 1_000_000;
      const conversationDir = "/mock/temp/argos-skill-drafts/conv-clean";
      ...
      (skillPresenter as any).draftsRoot = "/mock/temp/argos-skill-drafts";
      (fs.existsSync as Mock).mockImplementation((target: string) => {
        return (
          target === "/mock/temp/argos-skill-drafts" ||
      ...
      (fs.readdirSync as Mock).mockImplementation((target: string) => {
        if (target === "/mock/temp/argos-skill-drafts") {
  ...
  describe("installFromFolder", () => {
    ...
    it("should successfully install a valid skill", async () => {
      ...
      (fs.existsSync as Mock).mockImplementation((p: string) => {
        ...
        if (p.includes("/.argos/skills/new-skill")) return false;
        // Skills dir exists
        return true;
      });
      (fs.readFileSync as Mock).mockReturnValue("test");
  ...
  describe("saveSkillWithExtension", () => {
    beforeEach(async () => {
      mockSkillTree(["test-skill"]);
      (fs.existsSync as Mock).mockImplementation((target: string) => {
        if (target.endsWith("/.argos-meta/test-skill.json")) {
          return true;
        }
        return true;
      });
    ...
    it("saves skill content and extension together", async () => {
      ...
      const result = await skillPresenter.saveSkillWithExtension("test-skill", "new content", extension);
      ...
      expect(fs.writeFileSync).toHaveBeenCalledWith(
        expect.stringContaining("/.argos-meta/test-skill.json"),
    ...
    it("rolls back skill content when extension save fails", async () => {
      ...
      (fs.writeFileSync as Mock).mockImplementation((target: string, content: string) => {
        if (target.endsWith("/.argos-meta/test-skill.json") && content === JSON.stringify(extension, null, 2)) {
  ...
  describe("skill runtime extensions", () => {
    ...
    it("should save and load sidecar runtime config", async () => {
      ...
      (fs.existsSync as Mock).mockImplementation(
        (target: string) => !target.includes("/scripts") || target.endsWith("/.argos-meta/test-skill.json"),
      );
      (fs.readFileSync as Mock).mockImplementation((target: string) => {
        if (target.endsWith("/.argos-meta/test-skill.json")) {
    ...
    it("reads raw skill file content by skill name", async () => {
      ...
      expect(fs.promises.readFile).toHaveBeenCalledWith(expect.stringContaining("/test-skill/SKILL.md"), "utf-8");
    });
    ...
    it("should discover runnable scripts under scripts directory", async () => {
      (fs.existsSync as Mock).mockImplementation(
        (target: string) => !target.endsWith("/.argos-meta/test-skill.json") || target.includes("/scripts"),
      );
      (fs.readdirSync as Mock).mockImplementation((target: string) => {
    ...
    it("should remove sidecar config when uninstalling a skill", async () => {
      ...
      expect(fs.rmSync).toHaveBeenCalledWith(expect.stringContaining("/.argos-meta/test-skill.json"), {
        force: true,
      });
    });
  });
  ...
  describe("getActiveSkills", () => {
    it("should return empty skills for new agent sessions", async () => {
      ...
      expect(skillSessionStatePort.getPersistedNewSessionSkills).toHaveBeenCalledWith("new-session-1");
    ...
    it("returns persisted active skills for new agent sessions", async () => {
      ...
      expect(skillSessionStatePort.setPersistedNewSessionSkills).toHaveBeenCalledWith("new-session-2", ["skill-1"]);
    });
    ...
    it("filters invalid persisted skills for new agent sessions", async () => {
      ...
      expect(skillSessionStatePort.setPersistedNewSessionSkills).toHaveBeenCalledWith("new-session-2b", ["exists"]);
    });
    ...
    it("repairs imported legacy sessions when persisted skills are empty", async () => {
      ...
      expect(skillSessionStatePort.repairImportedLegacySessionSkills).toHaveBeenCalledWith("legacy-session-conv-123");
    });
    ...
    it("filters invalid skills after imported legacy session repair", async () => {
      ...
      expect(active).toEqual(["exists"]);
      expect(skillSessionStatePort.setPersistedNewSessionSkills).toHaveBeenCalledWith("legacy-session-conv-456", [
        "exists",
      ]);
    });
  });
  ...
  describe("setActiveSkills", () => {
    ...
    it("does not emit activated event for retired raw legacy conversations", async () => {
      ...
      expect(eventBus.sendToRenderer).not.toHaveBeenCalledWith(SKILL_EVENTS.ACTIVATED, "all", expect.anything());
    });
    ...
    it("does not emit deactivated event for retired raw legacy conversations", async () => {
      ...
      expect(eventBus.sendToRenderer).not.toHaveBeenCalledWith(SKILL_EVENTS.DEACTIVATED, "all", expect.anything());
    });
    ...
    it("persists active skills for new-agent sessions", async () => {
      ...
      expect(active).toEqual(["skill-1"]);
      expect(skillSessionStatePort.setPersistedNewSessionSkills).toHaveBeenCalledWith("new-session-3", ["skill-1"]);
    });
  });
  ...
  describe("clearNewAgentSessionSkills", () => {
    ...
    it("clears persisted active skills for new-agent sessions", async () => {
      ...
      expect(skillSessionStatePort.setPersistedNewSessionSkills).toHaveBeenCalledWith("new-session-4", []);
    });
  });
  ...
  describe("watchSkillFiles", () => {
    ...
    it("updates cached metadata when a changed skill is renamed without conflicts", async () => {
      ...
      expect(eventBus.sendToRenderer).toHaveBeenCalledWith(SKILL_EVENTS.METADATA_UPDATED, "all", renamedMetadata);
    });
