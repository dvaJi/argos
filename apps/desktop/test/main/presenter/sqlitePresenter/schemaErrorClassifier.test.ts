...
describe("schemaErrorClassifier", () => {
  it("classifies missing column errors", () => {
    expect(classifySchemaError(new Error("table argos_sessions has no column named reasoning_visibility"))).toEqual({
  ...
  it("classifies missing table errors", () => {
    expect(classifySchemaError(new Error("no such table: argos_message_traces"))).toEqual({
      reason: "missing-table",
      dedupeKey: "missing-table:argos_message_traces",
    });
  });

  it("builds a repair suggestion payload for repairable schema errors", () => {
    expect(
      buildDatabaseRepairSuggestedPayload(new Error("table argos_sessions has 14 columns but 16 values were supplied")),
    ).toEqual({
      ...
      dedupeKey: "column-count-mismatch:argos_sessions",
    });
  });
});
