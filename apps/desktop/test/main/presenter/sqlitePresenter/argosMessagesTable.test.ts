...
import { ArgosMessagesTable } from "@/presenter/sqlitePresenter/tables/argosMessages";
      ...
      if (sql.includes("FROM argos_messages m") && sql.includes("ORDER BY m.order_seq DESC")) {
        return {
          all: (
            ...
            const cursorOrderSeq = sql.includes("m.order_seq < ?") ? (_orderSeqOrLimit as number) : null;
            ...
            const filtered = rows
              .filter((row) => {
                ...
                return row.order_seq < cursorOrderSeq || (row.order_seq === cursorOrderSeq && row.id < cursorId);
              })
              .sort((left, right) => right.order_seq - left.order_seq || right.id.localeCompare(left.id));
...
describe("ArgosMessagesTable", () => {
  it("allows fetching 501 rows for hasMore detection when the requested page size is 500", () => {
    ...
    const table = new ArgosMessagesTable(db);
