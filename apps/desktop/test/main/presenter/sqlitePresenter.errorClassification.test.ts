...
describe("sqlitePresenter error classification", () => {
  ...
  it("keeps corruption errors classified as destructive", async () => {
    const { isDestructiveDatabaseError } = await import("../../../src/main/presenter/sqlitePresenter");
  ...
  it("treats open failures as non-destructive", async () => {
    ...
    expect(isDestructiveDatabaseError(new Error("SQLITE_CANTOPEN: unable to open database file"))).toBe(false);
