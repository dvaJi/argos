...
import { AdapterRegistry, ChannelManager } from "@argos/remote-control-runtime/channelManager";
import type {
  ...
  IChannelAdapter,
  SendMessageOptions,
} from "@argos/remote-control-runtime/types/channel";
...
const createConfig = (channelType: string, source: "builtin" | "plugin" = "builtin"): ChannelAdapterConfig => ({
  channelId: "default",
  channelType,
  agentId: "argos",
  channelConfig: {},
...
describe("ChannelManager", () => {
  it("creates adapters from both builtin and plugin registries", async () => {
    ...
    const builtinFactory: ChannelFactory = {
      ...
      create: (config) => new FakeAdapter(config.channelType, config.channelId, config.agentId, config.configSignature),
    };
  ...
  it("tracks adapter status and dispatches stream hooks", async () => {
    ...
    const adapter = new FakeAdapter("telegram", "default", "argos");
