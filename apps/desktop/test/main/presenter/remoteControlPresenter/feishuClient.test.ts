...
import { FeishuClient } from "@argos/remote-control-runtime/feishu/feishuClient";
...
describe("FeishuClient", () => {
  ...
  it("sends image replies with the uploaded image key", async () => {
    const workspace = await fs.mkdtemp(path.join(os.tmpdir(), "argos-feishu-client-"));
  ...
  it("fails fast when the image file is missing", async () => {
    ...
    await expect(
      client.sendImage(
        ...
        path.join(os.tmpdir(), "missing-argos-feishu-image.png"),
      ),
