...
import { QQBotParser } from "@argos/remote-control-runtime/qqbot/qqbotParser";
...
describe("QQBotParser", () => {
  ...
  it("uses unique fallback filenames for unnamed attachments", () => {
    ...
    expect(parsed?.attachments.map((attachment) => attachment.filename)).toEqual(["attachment-1", "attachment-2"]);
  });
});
