...
describe("convertMarkdownToTelegramHtml", () => {
  ...
  it("converts bold, italic, and strikethrough markers", () => {
    expect(convertMarkdownToTelegramHtml("**bold** _italic_ ~~gone~~")).toBe("<b>bold</b> <i>italic</i> <s>gone</s>");
  });
  ...
  it("renders inline code with HTML escaping", () => {
    expect(convertMarkdownToTelegramHtml("use `<div>` here")).toBe("use <code>&lt;div&gt;</code> here");
  });

  it("renders fenced code blocks with language class and escapes contents", () => {
    const input = "```ts\nconst a = 1 < 2\n```";
    expect(convertMarkdownToTelegramHtml(input)).toBe('<pre><code class="language-ts">const a = 1 &lt; 2</code></pre>');
  });
  ...
  it("renders GFM pipe tables as preformatted fixed-width text", () => {
    ...
    expect(convertMarkdownToTelegramHtml(input)).toBe("<pre>Name  | Value\n------|------\nAlpha | 1\nBeta  | 22</pre>");
  });

  it("does not convert pipe table text inside fenced code blocks", () => {
    const input = "```\n| A | B |\n| --- | --- |\n| 1 | 2 |\n```";
    expect(convertMarkdownToTelegramHtml(input)).toBe("<pre>| A | B |\n| --- | --- |\n| 1 | 2 |</pre>");
  });

  it("auto-closes a dangling fenced block at a chunk boundary", () => {
    const input = "```ts\nconst a = 1";
    expect(convertMarkdownToTelegramHtml(input)).toBe('<pre><code class="language-ts">const a = 1</code></pre>');
  });
