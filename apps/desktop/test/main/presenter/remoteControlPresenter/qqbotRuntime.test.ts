...
} from "@argos/remote-control-runtime/types";
...
const createInboundMessage = (target: QQBotTransportTarget, messageSeq: number = 1): QQBotInboundMessage => ({
  kind: "message",
...
const createProcessSegment = (sourceMessageId: string, index: number, text: string): RemoteDeliverySegment => ({
...
const createAnswerSegment = (sourceMessageId: string, index: number, text: string): RemoteDeliverySegment => ({
...
describe("QQBotRuntime", () => {
  ...
  it.each([
  ...
  ])("waits for completion before sending $label final text", async ({ target, message, getSendMock }) => {
    vi.useFakeTimers();
    ...
    expect(sendMock).toHaveBeenCalledWith(createExpectedPayload(target, message.messageSeq, "Final answer"));
  ...
  it("flushes the latest tool batch when answer text appears and sends the final answer on completion", async () => {
    ...
    const { execution, getSnapshot } = createExecution([
      ...
      },
      {
        ...
        text: "Draft answer",
        deliverySegments: [
          createProcessSegment(sourceMessageId, 0, '💻 shell_command: "pwd"\n📖 read_file: "/tmp/report.md"'),
    ...
    expect(client.sendC2CMessage).toHaveBeenNthCalledWith(
      1,
      createExpectedPayload(C2C_TARGET, 1, '💻 shell_command: "pwd"\n📖 read_file: "/tmp/report.md"'),
    );
    ...
    expect(client.sendC2CMessage).toHaveBeenNthCalledWith(2, createExpectedPayload(C2C_TARGET, 2, "Final answer"));
  });
  ...
  it("flushes each process batch in segment order while keeping answer delivery final-only", async () => {
    ...
    expect(client.sendGroupMessage).toHaveBeenNthCalledWith(
      1,
      createExpectedPayload(GROUP_TARGET, 2, '📖 read_file: "/tmp/a.md"\n💻 shell_command: "git status"'),
    );
    ...
    expect(client.sendGroupMessage).toHaveBeenNthCalledWith(3, createExpectedPayload(GROUP_TARGET, 4, "Final answer"));
  });
  ...
  it("keeps flushed process state when the source message id changes mid-conversation", async () => {
    ...
    expect(getSnapshot).toHaveBeenCalledTimes(3);
    expect(client.sendC2CMessage).toHaveBeenCalledTimes(2);
    expect(client.sendC2CMessage).toHaveBeenNthCalledWith(2, createExpectedPayload(C2C_TARGET, 9, "Final answer"));
  });
  ...
  it("flushes the buffered tool batch before timeout text", async () => {
    ...
    const { execution } = createExecution([
      {
        completed: false,
        deliverySegments: [createProcessSegment("assistant-msg-1", 0, '💻 shell_command: "sleep 1"')],
      },
    ]);
    ...
    expect(client.sendC2CMessage).toHaveBeenNthCalledWith(
      2,
      createExpectedPayload(C2C_TARGET, 3, "The current conversation timed out before finishing. Please try again."),
    );
  });
  ...
  it("omits the no-response terminal text when buffered tool output was sent", async () => {
    ...
    const { execution } = createExecution([
      {
        completed: true,
        deliverySegments: [createProcessSegment("assistant-msg-1", 0, '📖 read_file: "/tmp/empty.md"')],
  ...
  it("keeps the final reply slot reserved when the passive reply limit is almost exhausted", async () => {
    ...
    expect(client.sendC2CMessage).toHaveBeenCalledTimes(1);
    expect(client.sendC2CMessage).toHaveBeenCalledWith(createExpectedPayload(C2C_TARGET, 5, "Final answer"));
  });
  ...
  it("falls back to legacy trace text snapshots when delivery segments are unavailable", async () => {
    ...
    expect(client.sendC2CMessage).toHaveBeenNthCalledWith(
      1,
      createExpectedPayload(C2C_TARGET, 7, '💻 shell_command: "git status"\n📖 read_file: "/tmp/a.md"'),
    );
    ...
    expect(client.sendC2CMessage).toHaveBeenCalledTimes(2);
    expect(client.sendC2CMessage).toHaveBeenNthCalledWith(2, createExpectedPayload(C2C_TARGET, 8, "Final answer"));
  });
  ...
  it("flushes buffered tool text before sending the internal error reply", async () => {
      ...
      getSnapshot: vi.fn<(...args: any[]) => any>(async () => {
        if (snapshotCallCount === 0) {
          ...
          return {
            ...
            text: "",
            traceText: "",
            deliverySegments: [createProcessSegment("assistant-msg-1", 0, '💻 shell_command: "pwd"')],
            fullText: "",
            finalText: "",
            completed: false,
    ...
    expect(client.sendC2CMessage).toHaveBeenNthCalledWith(
      2,
      createExpectedPayload(C2C_TARGET, 7, "An internal error occurred while processing your request."),
    );
  });
