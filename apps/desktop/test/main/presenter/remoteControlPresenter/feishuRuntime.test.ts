...
import { FeishuRuntime } from "@argos/remote-control-runtime/feishu/feishuRuntime";
import {
...
describe("FeishuRuntime", () => {
  ...
  it("streams answer text beside a persistent trace log", async () => {
    ...
    try {
      ...
      await vi.waitFor(() => {
        ...
        expect(harness.bindingStore.clearRemoteDeliveryState).toHaveBeenCalledWith("feishu:oc_1:root");
      });
  ...
  it("preserves null messageId holes so later updates do not target the wrong chunk", async () => {
    ...
    try {
      ...
      const initialText = firstChunk + "B".repeat(FEISHU_OUTBOUND_TEXT_LIMIT) + "C".repeat(FEISHU_OUTBOUND_TEXT_LIMIT);
      ...
      await vi.waitFor(() => {
        expect(harness.client.updateMarkdown).not.toHaveBeenCalledWith("om_bot_3", changedMiddleChunk);
      });
  ...
  it("appends later process and answer segments in Argos order instead of rewriting the first answer", async () => {
    vi.useFakeTimers();
  ...
  it("stops polling and sends a timeout message when a conversation never completes", async () => {
    ...
    try {
      ...
      await vi.advanceTimersByTimeAsync(FEISHU_CONVERSATION_POLL_TIMEOUT_MS + TELEGRAM_STREAM_POLL_INTERVAL_MS);
  ...
  it("does not deliver conversation output from a previous run after restart", async () => {
    ...
    expect(harness.client.sendMarkdown).not.toHaveBeenCalledWith(expect.anything(), "stale conversation output");
