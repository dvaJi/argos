...
describe("TelegramPoller", () => {
  ...
  it("stops retrying and reports error on Telegram 409 conflict", async () => {
    ...
    expect(onFatalError).toHaveBeenCalledWith(expect.stringContaining("terminated by other getUpdates request"));
  });
  ...
  it("logs per-update delivery failures, advances offset, and keeps polling", async () => {
    ...
    expect(setPollOffset.mock.invocationCallOrder[0]).toBeLessThan(client.sendMessage.mock.invocationCallOrder[0]);
  ...
  it("preserves null messageId holes from stored delivery state so edits stay aligned", async () => {
    ...
    try {
      ...
      const initialText =
        firstChunk + " " + "B".repeat(TELEGRAM_OUTBOUND_TEXT_LIMIT) + " " + "C".repeat(TELEGRAM_OUTBOUND_TEXT_LIMIT);
  ...
  it("does not append a terminal segment when the latest answer already matches after a process segment", async () => {
    ...
    try {
      ...
      expect(client.sendMessage.mock.calls.filter(([, text]) => text === "Final answer")).toHaveLength(1);
  ...
  it("ignores expired callback query and not-modified edit errors", async () => {
    ...
    expect(warnSpy).not.toHaveBeenCalledWith("[TelegramPoller] Failed to answer callback query:", expect.anything());
