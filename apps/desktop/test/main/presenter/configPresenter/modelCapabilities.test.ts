...
describe("ModelCapabilities reasoning portraits", () => {
  ...
  it("looks up provider DB capabilities with canonical model ids", () => {
    ...
    expect(capabilities.getCapabilityModel("anthropic", "claude-opus-4-8")?.id).toBe("claude-opus-4-8");
    expect(capabilities.getCapabilityModel("anthropic", "anthropic/claude-opus-4.8")?.id).toBe("claude-opus-4-8");
    expect(capabilities.getCapabilityModel("anthropic", "anthropic.claude-opus-4.8")?.id).toBe("claude-opus-4-8");
    expect(capabilities.supportsTemperatureControl("anthropic", "anthropic/claude-opus-4.8")).toBe(false);
    expect(capabilities.supportsTemperatureControl("anthropic", "anthropic.claude-opus-4.8")).toBe(false);
  });
  ...
  it("finds best capability model matches across provider and model id variants", () => {
    ...
    expect(capabilities.findCapabilityModelMatch("google/gemini-3.5-flash", ["gemini"])).toMatchObject({
      providerId: "google",
      model: expect.objectContaining({
        id: "gemini-3.5-flash",
      }),
  ...
  it("reads temperature support from provider DB without model-id fallbacks", () => {
    ...
    expect(capabilities.supportsTemperatureControl("anthropic", "anthropic/claude-opus-4-7")).toBe(false);
