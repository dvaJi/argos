...
describe("McpConfHelper", () => {
  ...
  it("does not recreate the Apple built-in server after the user removed it", async () => {
    ...
    mcpStore.set("mcpServers", {});
    mcpStore.set("removedBuiltInServers", ["argos/apple-server"]);

    helper.onUpgrade(undefined);

    expect(mcpStore.get("mcpServers")["argos/apple-server"]).toBeUndefined();
  });
  ...
  it("removes the unpublished Computer Use demo MCP server config", async () => {
    ...
    mcpStore.set("mcpServers", {
      "argos/computer-use": legacyServer,
      demo: {
        command: "demo",
        args: [],
        env: {},
    ...
    mcpStore.set("removedBuiltInServers", ["argos/computer-use", "demo"]);

    const servers = await helper.getMcpServers();

    expect(servers["argos/computer-use"]).toBeUndefined();
