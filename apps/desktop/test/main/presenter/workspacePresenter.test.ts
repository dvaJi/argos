...
import { ARGOS_EVENT_CHANNEL } from "../../../src/shared/contracts/channels";
...
describe("WorkspacePresenter watchers", () => {
  ...
  beforeEach(() => {
    ...
    workspacePath = fs.mkdtempSync(path.join(os.tmpdir(), "argos-workspace-"));
  ...
  it("debounces file-system invalidations into a single fs refresh event", async () => {
    ...
    const legacyCalls = sendToRendererMock.mock.calls.filter(([channel]) => channel === WORKSPACE_EVENTS.INVALIDATED);
    const typedCalls = sendToRendererMock.mock.calls.filter(([channel]) => channel === ARGOS_EVENT_CHANNEL);
  ...
  it("emits git invalidations from git metadata watcher changes", async () => {
    ...
    expect(sendToRendererMock).toHaveBeenCalledWith(ARGOS_EVENT_CHANNEL, "all_windows", {
      name: "workspace.invalidated",
      payload: {
        workspacePath,
        kind: "git",
...
describe("WorkspacePresenter readFilePreview", () => {
  ...
  beforeEach(() => {
    workspacePath = fs.mkdtempSync(path.join(os.tmpdir(), "argos-workspace-preview-"));
  });
...
describe("WorkspacePresenter resolveMarkdownLinkedFile", () => {
  ...
  beforeEach(() => {
    workspacePath = fs.mkdtempSync(path.join(os.tmpdir(), "argos-workspace-links-"));
  ...
  it("supports file urls and absolute file paths", async () => {
    ...
    expect(fileUrlResolution?.path).toBe(normalizeForAccess(path.join(workspacePath, "docs", "root.md")));
    expect(absoluteResolution?.path).toBe(normalizeForAccess(path.join(workspacePath, "docs", "root.md")));
  });
...
describe("workspacePreviewProtocol helpers", () => {
  ...
  beforeEach(() => {
    workspacePath = fs.mkdtempSync(path.join(os.tmpdir(), "argos-workspace-protocol-"));
  ...
  it("rejects unregistered roots and outside-root preview URLs", () => {
    ...
    registerWorkspacePreviewRoot(workspacePath);
    expect(createWorkspacePreviewUrl(workspacePath, path.join(workspacePath, "..", "outside.txt"))).toBeNull();
  });
