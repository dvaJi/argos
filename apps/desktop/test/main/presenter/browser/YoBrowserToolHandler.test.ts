...
describe("YoBrowserToolHandler", () => {
  ...
  it("routes load_url through the conversation session id", async () => {
    ...
    expect(presenter.loadUrl).toHaveBeenCalledWith("session-a", "https://example.com", undefined, undefined, "agent");
  ...
  it("returns a recoverable browser-unavailable error before cdp_send", async () => {
    ...
    await expect(handler.callTool("cdp_send", { method: "Page.reload" }, "session-a")).rejects.toMatchObject({
  ...
  it("maps YoBrowserNotReadyError to the recoverable unavailable error", async () => {
    ...
    await expect(handler.callTool("cdp_send", { method: "Page.captureScreenshot" }, "session-a")).rejects.toMatchObject(
      {
        name: "YoBrowserUnavailableError",
        payload: {
          error: expect.objectContaining({
