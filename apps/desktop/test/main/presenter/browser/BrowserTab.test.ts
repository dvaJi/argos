...
describe("BrowserTab", () => {
  ...
  it("allows exploration after timeout when the renderer probe finds a live document", async () => {
    ...
    await expect(tab.sendCdpCommand("Page.navigate", { url: "https://example.com/probe" })).resolves.toEqual({
      frameId: "frame-1",
      loaderId: "loader-1",
    });
  ...
  it("does not treat loading documents with a body as interactive during the renderer probe", async () => {
    ...
    await expect(tab.sendCdpCommand("Page.navigate", { url: "https://example.com/loading-probe" })).resolves.toEqual({
      frameId: "frame-1",
      loaderId: "loader-1",
    });
  ...
  it("does not treat body-less documents as interactive during the renderer probe", async () => {
    ...
    await expect(tab.sendCdpCommand("Page.navigate", { url: "https://example.com/bodyless-probe" })).resolves.toEqual({
      frameId: "frame-1",
      loaderId: "loader-1",
    });
  ...
  it("marks Page.navigate as loading so the next tool waits for the new document", async () => {
    ...
    await expect(tab.sendCdpCommand("Page.navigate", { url: "https://example.com/next" })).resolves.toEqual({
      frameId: "frame-1",
      loaderId: "loader-1",
    });
  ...
  it("does not reset readiness for same-document Page.navigate responses without loaderId", async () => {
    ...
    await expect(tab.sendCdpCommand("Page.navigate", { url: "https://example.com#section" })).resolves.toEqual({
      frameId: "frame-1",
    });
  ...
  it("does not reset readiness for failed Page.navigate responses with errorText", async () => {
    ...
    await expect(tab.sendCdpCommand("Page.navigate", { url: "https://example.com/fail" })).resolves.toEqual({
      frameId: "frame-1",
      loaderId: "loader-1",
      errorText: "net::ERR_ABORTED",
    });
