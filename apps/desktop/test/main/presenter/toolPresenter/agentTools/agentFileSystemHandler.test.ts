...
describe("AgentFileSystemHandler diff responses", () => {
  ...
  it("limits directoryTree depth based on depth option", async () => {
    ...
    await fs.writeFile(path.join(testDir, "level1", "level2", "level3", "file3.txt"), "file3", "utf-8");

    const depthZero = JSON.parse(await handler.directoryTree({ path: testDir, depth: 0 })) as Array<{
      name: string;
      type: string;
      children?: unknown;
    }>;
