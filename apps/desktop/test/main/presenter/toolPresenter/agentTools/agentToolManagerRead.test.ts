...
vi.mock("electron", () => ({
  app: {
    getPath: (name: string) => {
      if (name === "temp") {
        return path.join(os.tmpdir(), "argos-electron-temp");
      }
      if (name === "home") {
        return path.join(os.tmpdir(), "argos-electron-home");
      }
      if (name === "userData") {
        return path.join(os.tmpdir(), "argos-electron-user-data");
      }
      return os.tmpdir();
    },
  },
...
describe("AgentToolManager read routing", () => {
  ...
  it("allows reading outside the workspace when external file access is enabled", async () => {
    const externalDir = await fs.mkdtemp(path.join(os.tmpdir(), "argos-read-external-"));
  ...
  it("requests permission for external reads in default access mode", async () => {
    const externalFile = path.join(path.parse(workspaceDir).root, "argos-outside-default.txt");
  ...
  it("requests permission for workspace symlinks that point outside", async () => {
    const externalDir = await fs.mkdtemp(path.join(os.tmpdir(), "argos-read-link-target-"));
    const externalFile = path.join(externalDir, "outside.txt");
    ...
    try {
      ...
      const errorCode =
        error instanceof Error && "code" in error ? String((error as Error & { code?: string }).code) : "";
    ...
    const permission = await manager.preCheckToolPermission("read", { path: "linked-outside.txt" }, "conv1");
  ...
  it("does not request external read permission when full access is enabled", async () => {
    const externalFile = path.join(path.parse(workspaceDir).root, "argos-outside-full-access.txt");
  ...
  it("allows writing outside the workspace when external file access is enabled", async () => {
    const externalDir = await fs.mkdtemp(path.join(os.tmpdir(), "argos-write-external-"));
  ...
  it("requests permission for new external writes without broadening to the parent directory", async () => {
    const externalFile = path.join(path.parse(workspaceDir).root, `argos-write-target-${Date.now()}.txt`);
  ...
  it("uses filePresenter llm-friendly content for document files with offset/limit", async () => {
    ...
    const result = (await manager.callTool("read", { path: "report.pdf", offset: 2, limit: 3 }, "conv1")) as {
      content: string;
    };
  ...
  it("falls back to the agent vision model when the current model has no vision", async () => {
    ...
    const result = (await manager.callTool("read", { path: "image-agent-vision.png" }, "conv1")) as {
      content: string;
    };

    expect(result.content).toContain("fallback image description");
    expect(configPresenter.resolveArgosAgentConfig).toHaveBeenCalledWith("agent-vision");
  ...
  it("falls back to image metadata when the conversation cannot be found", async () => {
    ...
    const result = (await manager.callTool("read", { path: "image-missing-conversation.png" }, "conv1")) as {
      content: string;
    };
  ...
  it("surfaces runtime errors while resolving the conversation vision target", async () => {
    ...
    await expect(manager.callTool("read", { path: "image-session-error.png" }, "conv1")).rejects.toThrow(
