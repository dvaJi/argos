...
describe("SplashWindowManager", () => {
  ...
  it("removes lifecycle listeners and clears activities on close", async () => {
    ...
    expect(eventBus.listenerCount(LIFECYCLE_EVENTS.HOOK_EXECUTED)).toBe(initialHookExecutedCount + 1);
    expect(eventBus.listenerCount(LIFECYCLE_EVENTS.HOOK_COMPLETED)).toBe(initialHookCompletedCount + 1);
    expect(eventBus.listenerCount(LIFECYCLE_EVENTS.HOOK_FAILED)).toBe(initialHookFailedCount + 1);
    expect(eventBus.listenerCount(LIFECYCLE_EVENTS.ERROR_OCCURRED)).toBe(initialErrorOccurredCount + 1);
