...
describe("BackgroundExecSessionManager", () => {
  ...
  it("rejects missing working directories before spawn can report a misleading shell ENOENT", async () => {
    ...
    await expect(manager.start("conv-1", "echo test", "/missing/workspace", { timeout: 0 })).rejects.toThrow(
  ...
  it("decodes split UTF-8 output from running sessions", async () => {
    ...
    await expect(manager.waitForCompletionOrYield("conv-1", result.sessionId, 100)).resolves.toMatchObject({
      kind: "completed",
      result: {
        status: "done",
        output: "chinese.txt\n",
...
describe("backgroundExecSessionManager utility proxy", () => {
  ...
  it("forks the dedicated entrypoint for the utility host", async () => {
    ...
    expect(mockUtilityProcessFork).toHaveBeenCalledWith(
      expect.stringMatching(/[\\/]mock[\\/]app[\\/]out[\\/]main[\\/]backgroundExecUtilityHost\.js$/),
      ["--argos-exec-utility-host"],
      expect.objectContaining({
  ...
  it("returns crashed completion results without starting a fresh utility host", async () => {
    ...
    await expect(backgroundExecSessionManager.waitForCompletionOrYield("conv-1", "bg_crashed", 10)).resolves.toEqual({
      kind: "completed",
      result: {
        status: "error",
