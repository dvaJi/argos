...
function expectNoHealthCommandProbes(calls: [command: string, args: string[], options?: unknown][]): void {
...
describe("RtkRuntimeService", () => {
  ...
  it.each([
    ...
    expect(result.rtkFallbackReason).toBe("Bypassed RTK rewrite: unsupported find compound predicates or actions");
  });
  ...
  it("tries system RTK when bundled RTK version check fails", async () => {
    ...
    expect(new Set(runCommand.mock.calls.map(([command, args]) => JSON.stringify([command, args])))).toEqual(
      new Set([JSON.stringify(["/runtime/rtk/rtk.exe", ["--version"]]), JSON.stringify(["rtk", ["--version"]])]),
    );
    expectNoHealthCommandProbes(runCommand.mock.calls);
  });
