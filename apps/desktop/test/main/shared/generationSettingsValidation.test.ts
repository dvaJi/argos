...
describe("validateGenerationNumericField timeout bounds", () => {
  ...
  it("rejects timeout values outside the supported range", () => {
    expect(validateGenerationNumericField("timeout", MODEL_TIMEOUT_MIN_MS - 1)).toBe("timeout_too_small");
    expect(validateGenerationNumericField("timeout", MODEL_TIMEOUT_MAX_MS + 1)).toBe("timeout_too_large");
  });
});
