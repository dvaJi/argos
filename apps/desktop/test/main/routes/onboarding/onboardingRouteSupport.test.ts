...
describe("onboardingRouteSupport", () => {
  ...
  it("advances through required steps and allows skipping optional steps", () => {
    ...
    const afterMcp = setGuidedOnboardingStepStatus(presenter, { stepId: "mcp", status: "completed" }, 404);
    const afterSkills = setGuidedOnboardingStepStatus(presenter, { stepId: "skills", status: "completed" }, 405);
  ...
  it("falls back to the next pending step when start targets a terminal step", () => {
    ...
    setGuidedOnboardingStepStatus(presenter, { stepId: "select-provider", status: "completed" }, 451);
  ...
  it("completes onboarding and marks remaining optional steps skipped", () => {
    ...
    setGuidedOnboardingStepStatus(presenter, { stepId: "select-provider", status: "completed" }, 501);
    ...
    expect(state.steps.filter((step) => !step.required).every((step) => step.status === "skipped")).toBe(true);
  ...
  it("migrates the legacy provider and chat flow into the new eight-step sequence", () => {
    ...
    expect(state.steps.find((step) => step.id === "mcp")).toEqual(expect.objectContaining({ status: "skipped" }));
    expect(state.steps.find((step) => step.id === "skills")).toEqual(expect.objectContaining({ status: "completed" }));
