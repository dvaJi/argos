...
const createHome = () => mkdtempSync(path.join(tmpdir(), "argos-provider-import-"));
...
describe("ProviderImportService", () => {
  ...
  it("keeps Codex-only CC Switch Desktop override providers hidden", async () => {
    ...
    expect(result.providers).not.toEqual(expect.arrayContaining([expect.objectContaining({ sourceId: "cc-switch" })]));
  });
  ...
  it("does not select providers by default when Argos already has a config", async () => {
    homeDir = createHome();
    writeFile(
      path.join(homeDir, ".hermes/config.yaml"),
      [
  ...
  it("uses Cherry Studio custom data directory from app config", async () => {
    homeDir = createHome();
    const defaultCherryPath = path.join(homeDir, "Library/Application Support/CherryStudio/Local Storage/leveldb");
  ...
  it("reads Cherry Studio providers from a LevelDB snapshot", async () => {
    homeDir = createHome();
    const cherryPath = path.join(homeDir, "Library/Application Support/CherryStudio/Local Storage/leveldb");
  ...
  it("applies selected providers in source order and lets later sources overwrite earlier ones", async () => {
    ...
    expect(configPresenter.getCurrentProviders().find((provider) => provider.id === "openai")).toMatchObject({
      apiKey: "sk-openclaw",
      enable: true,
    });
  });
