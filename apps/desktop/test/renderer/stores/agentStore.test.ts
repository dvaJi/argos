...
const createAgent = (id: string, overrides: Record<string, unknown> = {}): Record<string, unknown> => ({
  id,
  name: id,
  type: id === "argos" ? "argos" : "acp",
  agentType: id === "argos" ? "argos" : "acp",
  enabled: true,
  ...overrides,
});
...
describe("agent store incremental refresh", () => {
  ...
  it("removes deleted ACP agents without refetching the full agent list", async () => {
    ...
    expect(store.agents.value.map((agent) => agent.id)).toEqual(["argos", "acp-1"]);
  });
  ...
  it("falls back to a full refresh when the change is not scoped", async () => {
    const { store, sessionClient, emitAgentsChanged } = await setupStore({
      initialAgents: [createAgent("argos", { name: "Argos" })],
    });
