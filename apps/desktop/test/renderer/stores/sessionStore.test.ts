...
type SetupStoreOptions = {
  ...
  enabledAgents?: Array<{ id: string; name?: string; type?: "argos" | "acp"; enabled?: boolean }>;
...
describe("sessionStore.getFilteredGroups", () => {
  ...
  it("uses the last path segment for Windows project labels", async () => {
    ...
    store.sessions.value = [
      {
        ...
        agentId: "argos",
        status: "none",
        projectDir: "C:\\Users\\Argos\\workspace",
        providerId: "openai",
        modelId: "gpt-4",
        isPinned: false,
        isDraft: false,
    ...
    expect(groups[0]?.id).toBe("C:\\Users\\Argos\\workspace");
  ...
  it("keeps a stable unique id for project groups with the same folder name", async () => {
    ...
    store.sessions.value = [
      {
        ...
        agentId: "argos",
        status: "none",
        projectDir: "/tmp/company-a/argos",
        providerId: "openai",
        modelId: "gpt-4",
        isPinned: false,
        isDraft: false,
      ...
      },
      {
        ...
        agentId: "argos",
        status: "none",
        projectDir: "/tmp/company-b/argos",
        providerId: "openai",
        modelId: "gpt-4",
        isPinned: false,
        isDraft: false,
    ...
    expect(groups.map((group: SessionListTestItem) => group.label)).toEqual(["argos", "argos"]);
  });
...
describe("sessionStore.startNewConversation", () => {
  it("selects the first enabled agent from the all-agents welcome state", async () => {
    const { store, agentStore, pageRouter, sessionClient } = await setupStore({
      ...
      enabledAgents: [{ id: "argos" }, { id: "acp-a", type: "acp" }],
    });

    await store.startNewConversation({ refresh: true });

    expect(agentStore.setSelectedAgent).toHaveBeenCalledWith("argos");
