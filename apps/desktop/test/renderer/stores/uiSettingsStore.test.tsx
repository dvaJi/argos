...
describe("uiSettingsStore", () => {
      ...
      if (routeName === "settings.update") {
        return {
          ...
          values: Object.fromEntries(
            input.changes.map((change: { key: string; value: unknown }) => [change.key, change.value]),
          ),
        };
      }
  ...
  it("hydrates from the typed settings snapshot and reacts to typed settings.changed events", async () => {
    ...
    const listener = on.mock.calls[0]?.[1] as
      | ((payload: { changedKeys: string[]; version: number; values: Record<string, unknown> }) => void)
      | undefined;

    listener?.({
      changedKeys: ["fontSizeLevel", "notificationsEnabled", "privacyModeEnabled", "launchAtLoginEnabled"],
      version: 3,
      values: {
        fontSizeLevel: 4,
  ...
  it("waits for the initial snapshot before applying an update result", async () => {
    ...
    expect(invoke.mock.calls.map(([routeName]) => routeName)).toEqual(["settings.getSnapshot", "settings.update"]);
    expect(store.fontSizeLevel).toBe(4);
  });
});
