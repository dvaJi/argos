...
describe("LinkNode", () => {
  ...
  it("opens http links in YoBrowser by default", async () => {
    ...
    expect(window.argos.invoke).toHaveBeenCalledWith(
      "browser.loadUrl",
  ...
  it("opens http links in the system browser on Alt click", async () => {
    ...
    expect(window.argos.invoke).not.toHaveBeenCalledWith("browser.loadUrl", expect.anything());
  });
  ...
  it("opens local markdown links in the workspace preview", async () => {
    const argosInvoke = window.argos.invoke as ReturnType<typeof vi.fn>;
    argosInvoke.mockImplementation((routeName: string) => {
    ...
    expect(window.argos.invoke).toHaveBeenCalledWith("workspace.resolveMarkdownLinkedFile", {
      workspacePath: "/repo",
      href: "./docs/README.md",
      sourceFilePath: "/repo/guide.md",
    });
  ...
  it("keeps same-document fragments inside the current document", async () => {
    ...
    expect(sidepanelStore.openBrowser).not.toHaveBeenCalled();
    expect(window.argos.invoke).not.toHaveBeenCalledWith("browser.loadUrl", expect.anything());
    expect(window.argos.invoke).not.toHaveBeenCalledWith("workspace.resolveMarkdownLinkedFile", expect.anything());
    expect(window.api.openExternal).not.toHaveBeenCalled();
  });
});
