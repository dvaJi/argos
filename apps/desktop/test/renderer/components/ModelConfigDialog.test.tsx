...
describe("ModelConfigDialog reasoning portraits", () => {
  it("renders the speech recognition model setting for chat models", async () => {
    ...
    expect(container.textContent).toContain("settings.model.modelConfig.speechRecognition.description");
  });
  ...
  it("shows interleaved thinking when an OpenAI-compatible model defaults to interleaved mode", async () => {
    ...
    expect(container.textContent).toContain("settings.model.modelConfig.interleavedThinking.description");
  });
  ...
  it("hides interleaved thinking for Responses providers", async () => {
    ...
    expect(container.textContent).not.toContain("settings.model.modelConfig.interleavedThinking.label");
  });
  ...
  it("renders full effort options for non-grok-3-mini xAI portraits", async () => {
    ...
    expect(container.textContent).toContain("settings.model.modelConfig.reasoningEffort.options.minimal");
    expect(container.textContent).toContain("settings.model.modelConfig.reasoningEffort.options.medium");
  });
  ...
  it("treats official anthropic effort portraits as editable toggles with conditional subsettings", async () => {
    ...
    expect(container.textContent).not.toContain("settings.model.modelConfig.reasoningVisibility.label");

    // PLACEHOLDER: was (wrapper.vm as any).config.reasoning = true
    await act(async () => {});

    expect(container.textContent).toContain("settings.model.modelConfig.reasoningEffort.options.max");
    expect(container.textContent).toContain("settings.model.modelConfig.reasoningVisibility.label");
    expect(container.textContent).toContain("settings.model.modelConfig.reasoningVisibility.options.omitted");
    expect(container.textContent).toContain("settings.model.modelConfig.reasoningVisibility.options.summarized");
  });
...
describe("ModelConfigDialog OpenAI image generation settings", () => {
  ...
  it("keeps ordinary OpenAI chat models on the generic model form", async () => {
    ...
    expect(container.textContent).not.toContain("settings.model.modelConfig.imageGeneration.size.label");
