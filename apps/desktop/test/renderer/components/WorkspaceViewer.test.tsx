...
describe("WorkspaceViewer", () => {
  ...
  it("shows restore label while fullscreen is active", async () => {
    ...
    expect(screen.getByTestId("workspace-viewer-fullscreen-toggle").getAttribute("title")).toBe("common.restore");
  });
  ...
  it("shows raw artifact preview through preview pane fallback", async () => {
    ...
    expect(body.className.split(" ")).toEqual(expect.arrayContaining(["min-h-0", "flex-1", "overflow-hidden"]));
    const previewPane = screen.getByTestId("preview-pane");
    expect(previewPane.className.split(" ")).toEqual(expect.arrayContaining(["h-full", "min-h-0", "w-full"]));
  ...
  it("renders code pane only for text files", async () => {
    ...
    expect(codePane.className.split(" ")).toEqual(expect.arrayContaining(["h-full", "min-h-0", "w-full"]));
  ...
  it("shows preview and code tabs for markdown files", async () => {
    ...
    const codeButton = screen.getAllByRole("button").find((button) => button.textContent?.includes("artifacts.code"));
  ...
  it("shows info pane for unsupported files", async () => {
    ...
    expect(infoPane.className.split(" ")).toEqual(expect.arrayContaining(["h-full", "min-h-0", "w-full"]));
    expect(screen.queryByTestId("preview-pane")).toBeNull();
    expect(screen.queryByTestId("code-pane")).toBeNull();
  });
});
