...
const createModelOptionSelector = (modelId: string, providerId?: string): string => {
  ...
  return providerId ? `${selector}[data-provider-id="${escapeAttributeValue(providerId)}"]` : selector;
};

export async function selectAgent(page: Page, preferredAgentId = "argos"): Promise<void> {
  ...
  const button = (await preferredAgent.isVisible().catch(() => false)) ? preferredAgent : fallbackAgent;
  ...
  await expect
    .poll(
      async () => {
        ...
        return nextSelectedModelId === modelId && (!providerId || nextSelectedProviderId === providerId);
      },
      {
...
export async function getActiveSessionId(page: Page): Promise<string> {
  const activeSession = page.locator('[data-testid="sidebar-session-item"][data-active="true"]').first();
...
export async function openSessionById(page: Page, sessionId: string): Promise<void> {
  const sessionItem = page.locator(`[data-testid="sidebar-session-item"][data-session-id="${sessionId}"]`).first();
