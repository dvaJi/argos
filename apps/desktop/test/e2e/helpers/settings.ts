...
export async function verifyProviderConnection(settingsPage: Page, modelId: string): Promise<void> {
  ...
  const option = settingsPage.locator(`[data-testid="model-check-option"][data-model-id="${modelId}"]`).first();
  await expect(option, `Model "${modelId}" was not found in the provider check dialog.`).toBeVisible({
    timeout: 30_000,
  });
  await option.click();
