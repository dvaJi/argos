...
metadata:
  argosFeature: feishu-integration
---

# feishu-tools

This plugin is an MCP server tool surface exposed by Argos's Feishu plugin. Do not ask the user to classify the plugin as an MCP server, a CLI tool, or another plugin type. When a request is about Feishu/Lark content and matching tools are available, invoke the relevant tool directly.
