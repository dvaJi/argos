---
name: computer-use
description: Drive native desktop apps through Argos's built-in Computer Use tools. Use when the user asks to operate, inspect, automate, or perform a GUI task in a real desktop application.
platforms:
  - darwin
  - win32
  - linux
...
Use Argos's plugin-provided Computer Use tools as the only action surface for this skill. Do not
ask the user to install `cua-driver`, configure an external server, or put anything on PATH for the
bundled Argos plugin.
...
- macOS runtime bundle: packaged builds prefer
  `Argos.app/Contents/Helpers/Argos Computer Use.app`; the plugin-local fallback is
...
6. Act with the matching Argos tool: `click`, `right_click`, `double_click`, `drag`, `scroll`,
...
- macOS: use `check_permissions` for Accessibility and Screen Recording status. The embedded
  daemon is a direct child of Argos, so the grants belong to the signed Argos host app. Do
...
intentionally denied because clipboard plaintext is privacy-sensitive and Argos has no reviewed
