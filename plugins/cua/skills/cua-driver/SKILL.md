---
name: cua-driver
description: Drive native macOS apps through Argos's plugin-provided Computer Use MCP tools. Use when the user asks to operate, inspect, automate, or perform a GUI task in a real macOS application.
platforms:
  - darwin
metadata:
  argosFeature: computer-use
---

# cua-driver

Use Argos's CUA plugin MCP tools for macOS app automation. Treat the tools exposed by the `cua-driver` MCP server as the only action surface for this skill.

## Runtime Context

- Plugin id: `${OWNER_PLUGIN_ID}`.
- Plugin root: `${PLUGIN_ROOT}`.
- Helper app bundle: `${PLUGIN_ROOT}/runtime/darwin/${PROCESS_ARCH}/Argos Computer Use.app`.
- Helper binary: `${PLUGIN_ROOT}/runtime/darwin/${PROCESS_ARCH}/Argos Computer Use.app/Contents/MacOS/cua-driver`.
...
Use `check_permissions` for permission status and prompting. If Accessibility or Screen Recording is missing, tell the user to grant it to `Argos Computer Use.app` from the helper app bundle path in this skill's runtime context.
