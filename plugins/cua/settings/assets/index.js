...
function getPluginApi() {
  const api = window.argosPlugin;
  if (!api) {
    throw new Error("Argos plugin settings bridge is unavailable. Restart Argos and reopen this page.");
  }
  return api;
}
...
function friendlyErrorMessage(value) {
  ...
  if (
    ...
    /(runtime|artifact).*(integrity|missing|modified|mismatch)|integrity.*(runtime|artifact)/i.test(raw)
  ) {
    return "The CUA runtime is missing or modified. Reinstall Argos or the CUA plugin, then check Windows Security > Protection history.";
  }
  if (/PowerShell 5\.1|positional JSON arg|argos-permission-probe/i.test(raw)) {
...
async function refreshStatus() {
  ...
  const runtimeUnavailable = status.runtime?.state === "missing" || status.runtime?.state === "error";
  ...
  if (status.activationError) {
    ...
    setMessage(
      friendlyErrorMessage(status.activationError) ||
        "CUA runtime validation failed. Repair or reinstall Argos or the CUA plugin.",
      "error",
    ...
    if (status.runtime?.state === "missing") {
      setMessage(
        currentPlatform === "win32"
          ? "The CUA runtime is missing. Reinstall Argos or the CUA plugin, then check Windows Security > Protection history."
          : "The CUA runtime is missing. Repair or reinstall Argos or the CUA plugin.",
        "error",
      );
    ...
    setMessage(
      "Argos stopped this runtime after an unclean exit. Retry only when the desktop is safe to control.",
      "warning",
      cuaMcp.lastError,
    );
...
async function runRuntimeAction(actionId, progressMessage, successMessage) {
  ...
  try {
    ...
    if (!result.ok) {
      ...
      setMessage(friendlyErrorMessage(result.error || "Runtime operation failed"), "error", result.error);
      return;
    }
    await refreshStatus();
...
testRuntimeNode?.addEventListener("click", async () => {
  await runRuntimeAction("runtime.test", "Testing the CUA runtime...", "Runtime test completed successfully.");
});
...
async function checkPermissions() {
  ...
  if (!result.ok || !result.data) {
    ...
    setMessage(friendlyErrorMessage(result.error || "Permission check failed"), "error", result.error);
    return;
  }
...
document.getElementById("guide")?.addEventListener("click", async () => {
  try {
    ...
    if (!result.ok) {
      setMessage(friendlyErrorMessage(result.error || "Failed to open permission setup"), "error", result.error);
      return;
    }
...
projectLinkNode?.addEventListener("click", async (event) => {
  ...
  try {
    ...
    if (!result.ok) {
      setMessage(friendlyErrorMessage(result.error || "Failed to open project"), "error", result.error);
    }
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    setMessage(friendlyErrorMessage(message), "error", message);
...
document.getElementById("disable")?.addEventListener("click", async () => {
  try {
    ...
    if (!result.ok) {
      setMessage(friendlyErrorMessage(result.error || "Failed to disable plugin"), "error", result.error);
      return;
    }
    await refreshStatus();
  } catch (error) {
