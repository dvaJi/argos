...
/// `~/Library/Application Support/<CFBundleName ?? "Argos Computer Use">/config.json`.
///
...
public actor ConfigStore {
    ...
    private let log = Logger(
        subsystem: "com.wefonk.argos.computeruse",
    ...
    /// `CFBundleName` from the running bundle (so `Argos Computer Use.app` with
    /// `CFBundleName=Argos Computer Use` writes under `Argos Computer Use/`). Falls back
    /// to the string literal `"Argos Computer Use"` when running from the raw
