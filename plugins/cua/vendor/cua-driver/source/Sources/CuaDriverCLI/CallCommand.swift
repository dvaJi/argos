...
struct CallCommand: AsyncParsableCommand {
    ...
    private func runInProcess(arguments: [String: Value]?) async throws {
        ...
        // is running from Argos Computer Use.app (the daemon). Any one-shot CLI
        ...
        // the IDE's bundle — not com.wefonk.argos.computeruse — and report
        // "NOT granted" even when the user has granted both permissions
        // to Argos Computer Use.app. If the daemon were up we'd already have
        ...
        if toolName == "check_permissions" {
            printToStderr(
                """
                ⚠️ Not running inside the cua-driver daemon process. Results may be inaccurate — TCC checks the calling process, not Argos Computer Use.app, so permissions granted to Argos Computer Use.app may read as "NOT granted" here.
                For authoritative results, start the daemon first: `open -n -g -a "Argos Computer Use" --args serve`, then re-run this check.
                """
            )
