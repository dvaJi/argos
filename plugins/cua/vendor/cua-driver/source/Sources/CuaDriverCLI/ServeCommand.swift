...
struct ServeCommand: ParsableCommand {
    static let configuration = CommandConfiguration(
        ...
        discussion: """
            ...
            Argos Computer Use.app. AX probes no-op silently and the daemon never becomes
            useful. To sidestep, `serve` detects that context and re-execs itself
            via `open -n -g -a "Argos Computer Use" --args serve`, which relaunches under
            LaunchServices so TCC attributes the process to com.wefonk.argos.computeruse.
    ...
    @Flag(
        ...
        help: """
            ...
            `open -n -g -a "Argos Computer Use"`. \
            Use when the calling context already has the right TCC responsibility \
            (running inside Argos Computer Use.app directly, or from a shell that's been \
    ...
    func run() throws {
        ...
        AppKitBootstrap.runBlockingAppKitWith {
            ...
            // while Argos settings owns the guided TCC setup flow.
...
extension ServeCommand {
    ...
    /// via `/usr/bin/open -n -g -a "Argos Computer Use" --args serve`. True when all of
    /// the following hold:
    ///
    ...
    ///     (bundlePath ends in `/Argos Computer Use.app`), so checking for its
    ...
    ///     living inside some `Argos Computer Use.app/Contents/MacOS/`. This
    ...
    /// com.wefonk.argos.computeruse — and the daemon's AppKitBootstrap silently
    /// no-ops. Bouncing through `open` relaunches under LaunchServices so
    /// TCC attributes the fresh process to Argos Computer Use.app.
    fileprivate func shouldRelaunchViaOpen() -> Bool {
        ...
        // `/Applications/Argos Computer Use.app/Contents/MacOS/cua-driver`
        ...
        // if that bare path actually resolves into a Argos Computer Use.app
    ...
    /// Spawn `/usr/bin/open -n -g -a "Argos Computer Use" --args serve [--socket …]`,
    ...
    fileprivate func relaunchViaOpen(socketPath: String) throws {
        FileHandle.standardError.write(
            Data(
                "cua-driver: relaunching via `open -n -g -a \"Argos Computer Use\" --args serve` for correct TCC context. Pass --no-relaunch to stay in this process.\n"
                    .utf8))
        ...
        // -g: keep the new instance in the background. Argos Computer Use.app is
        //     LSUIElement=true so it wouldn't take focus anyway, but this
        //     makes that explicit.
        process.arguments = ["-n", "-g", "-a", "Argos Computer Use", "--args", "serve"] + extraArgs
        ...
        if process.terminationStatus != 0 {
            FileHandle.standardError.write(
                Data(
                    "cua-driver: `open -n -g -a \"Argos Computer Use\" --args serve` exited \(process.terminationStatus). Check that `/Applications/Argos Computer Use.app` is installed, or pass --no-relaunch to bypass.\n"
                        .utf8))
            throw ExitCode(1)
        }
        ...
        if probeReachable {
            FileHandle.standardOutput.write(
                Data(
                    "cua-driver daemon is running (relaunched via Argos Computer Use.app)\n  socket: \(socketPath)\n"
                        .utf8))
            return
        }
        ...
        FileHandle.standardError.write(
            Data(
                "cua-driver: relaunched Argos Computer Use.app but no daemon appeared on \(socketPath) within 5s. Check Accessibility + Screen Recording grants for Argos Computer Use.app, or re-run with --no-relaunch to see in-process errors.\n"
                    .utf8))
        throw ExitCode(1)
    }
