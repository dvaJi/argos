...
/// Shared "is this binary running from inside an installed Argos Computer Use.app
...
/// `Argos Computer Use.app/Contents/MacOS/` directory.
///
/// That's the "installed via install-local.sh / install.sh" shape —
/// `/usr/local/bin/cua-driver` is a symlink into
/// `/Applications/Argos Computer Use.app`, and `realpath` walks into the
...
func isExecutableInsideCuaDriverApp() -> Bool {
    ...
    return resolved.contains("/Argos Computer Use.app/Contents/MacOS/")
}
