...
struct CuaDriverCommand: AsyncParsableCommand {
    static let configuration = CommandConfiguration(
        ...
        subcommands: [
            ArgosPermissionProbeCommand.self,
            MCPCommand.self,
            CallCommand.self,
...
struct MCPConfigCommand: ParsableCommand {
    ...
    func run() throws {
        ...
        // `/Applications/Argos Computer Use.app/...` even when `cua-driver`
...
struct ArgosPermissionProbeCommand: AsyncParsableCommand {
    static let configuration = CommandConfiguration(
        commandName: "argos-permission-probe",
        abstract: "Prompt for and write Argos helper TCC permission status JSON."
    )
...
struct CuaDriverEntryPoint {
    ...
    static func main() async {
        ...
        // / Spotlight / Dock on Argos Computer Use.app. LSUIElement=true keeps
...
struct MCPCommand: ParsableCommand {
    static let configuration = CommandConfiguration(
        ...
        discussion: """
            ...
            terminal — not to Argos Computer Use.app — so AX probes silently fail \
            ...
            (relaunching via `open -n -g -a "Argos Computer Use" --args serve` if not), \
            ...
            Argos Computer Use.app directly), or for diagnosing \
            in-process failures.
            """
    )
    ...
    @Flag(
        ...
        help: """
            ...
            without Argos Computer Use.app's TCC grants. Also toggleable via \
    ...
    func run() throws {
        ...
        // (shell-spawned bare binary that resolves into Argos Computer Use.app
        // bundle), gated by an explicit env / flag opt-out. When the
        // shell already has the right TCC context (e.g. Argos Computer Use.app
        ...
        AppKitBootstrap.runBlockingAppKitWith {
            // Keep MCP startup non-blocking. Permission setup is handled by Argos's
...
extension MCPCommand {
    ...
    fileprivate func shouldUseDaemonProxy() -> Bool {
        ...
        // When AppKit already attributes us to Argos Computer Use.app — either
        ...
        // Argos Computer Use.app bundle, otherwise there's nothing for the
    ...
    fileprivate func runViaDaemonProxy() throws {
        ...
        if !DaemonClient.isDaemonListening(socketPath: socketPath) {
            FileHandle.standardError.write(
                Data(
                    "cua-driver: mcp launched without Argos Computer Use.app's TCC grants; auto-launching the daemon via `open -n -g -a \"Argos Computer Use\" --args serve` and proxying MCP requests through it. Pass --no-daemon-relaunch to stay in-process.\n"
                        .utf8))
    ...
    /// Spawn `/usr/bin/open -n -g -a "Argos Computer Use" --args serve`. Mirror of
    ...
    fileprivate func launchDaemonViaOpen() throws {
        ...
        // -n: force a new instance. Argos Computer Use.app may already be
        ...
        // -g: keep the new instance backgrounded. Argos Computer Use.app is
        //     LSUIElement=true anyway, but this makes that explicit.
        process.arguments = ["-n", "-g", "-a", "Argos Computer Use", "--args", "serve"]
        ...
        if process.terminationStatus != 0 {
            FileHandle.standardError.write(
                Data(
                    "cua-driver: `open -n -g -a \"Argos Computer Use\" --args serve` exited \(process.terminationStatus). Check that `/Applications/Argos Computer Use.app` is installed, or pass --no-daemon-relaunch to bypass.\n"
                        .utf8))
            throw ExitCode(1)
        }
    }
    ...
    /// granted Accessibility / Screen Recording to Argos Computer Use.app yet
    ...
    fileprivate func waitForDaemon(socketPath: String, timeout: TimeInterval) throws {
        ...
        FileHandle.standardError.write(
            Data(
                "cua-driver: daemon did not appear on \(socketPath) within \(Int(timeout))s. If this is the first launch, grant Accessibility + Screen Recording to Argos Computer Use.app in System Settings and retry. Pass --no-daemon-relaunch to stay in-process.\n"
                    .utf8))
        throw ExitCode(1)
    }
...
/// `cua-driver update` — report the Argos-managed update path.
struct UpdateCommand: AsyncParsableCommand {
    static let configuration = CommandConfiguration(
        commandName: "update",
        abstract: "Show how Argos updates the bundled cua-driver helper."
    )
    ...
    func run() async throws {
        ...
        print("")
        print("Argos packages this cua-driver fork with the app.")
        print("Update Argos to receive newer Computer Use helper builds.")
        ...
        print("--apply is managed by Argos's app updater.")
        throw ExitCode(1)
    }
}
