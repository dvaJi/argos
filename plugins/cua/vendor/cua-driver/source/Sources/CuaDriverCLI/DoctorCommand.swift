...
struct DoctorCommand: AsyncParsableCommand {
    ...
    private func runProbes() async -> DoctorResult {
        ...
        // 2. Attribution check — are we attributed to Argos Computer Use.app or a shell?
        let bundleID = Bundle.main.bundleIdentifier ?? ""
        let isCorrectBundle = bundleID == "com.wefonk.argos.computeruse"
    ...
    private func recommend(
        ...
        if !axOk {
            return Recommendation(
                ...
                detail: """
                    Grant Accessibility to Argos Computer Use.app in System Settings → Privacy & Security → Accessibility, then restart the daemon:
                      open -n -g -a "Argos Computer Use" --args serve
                    Argos's bundled `cua-driver mcp` auto-relaunches through Argos Computer Use.app when needed.
                    """
            )
        }
        ...
        if !isCorrectBundle {
            return Recommendation(
                captureMode: nil,
                severity: .warning,
                summary: "TCC is attributed to the wrong process (not Argos Computer Use.app).",
                detail: """
                    Your shell or IDE is the responsible process for TCC, not Argos Computer Use.app.
                    Argos's bundled `cua-driver mcp` auto-relaunches through Argos Computer Use.app.
                    Or start the daemon manually: open -n -g -a "Argos Computer Use" --args serve
                    """
            )
        }
