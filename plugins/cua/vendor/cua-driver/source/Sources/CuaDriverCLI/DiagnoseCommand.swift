...
///     process sees them)
///   - install layout (/Applications/Argos Computer Use.app bundle + signature,
///     /usr/local/bin/cua-driver symlink resolution)
///   - TCC DB rows for `com.wefonk.argos.computeruse` (best-effort; system TCC DB
...
struct DiagnoseCommand: AsyncParsableCommand {
    ...
    private func installLayoutSection() -> String {
        ...
        let appPath = "/Applications/Argos Computer Use.app"
        ...
        let stalePaths = [
            ...
            "\(NSHomeDirectory())/Applications/Argos Computer Use.app",
    ...
    private func tccDatabaseSection() -> String {
        var lines: [String] = ["## tcc database rows for com.wefonk.argos.computeruse"]
        ...
        let sql = "SELECT service, client, client_type, auth_value, auth_reason, "
            + "hex(csreq) AS csreq_hex FROM access WHERE client='com.wefonk.argos.computeruse';"
    ...
    private func configPathsSection() -> String {
        ...
        let paths: [(String, String)] = [
            ...
            ("config dir",        "\(home)/Library/Application Support/Argos Computer Use"),
