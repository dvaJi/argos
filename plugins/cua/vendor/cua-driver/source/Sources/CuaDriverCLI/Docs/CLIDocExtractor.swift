...
enum CLIDocExtractor {
    ...
    private static var mcpDoc: CommandDoc {
        CommandDoc(
            ...
            discussion: """
                ...
                already has the right TCC grants (e.g. spawned from Argos Computer Use.app
            ...
            flags: [
                ...
                FlagDoc(name: "no-daemon-relaunch", shortName: nil, help: "Stay in the current process instead of auto-launching a daemon and proxying through its Unix socket when invoked from a shell without Argos Computer Use.app's TCC grants. Also toggleable via CUA_DRIVER_MCP_NO_RELAUNCH=1.", defaultValue: false),
            ],
            subcommands: []
        )
    }
    ...
    private static var serveDoc: CommandDoc {
        CommandDoc(
            ...
            flags: [
                FlagDoc(name: "no-relaunch", shortName: nil, help: "Stay in the current process instead of re-execing via `open -n -g -a \"Argos Computer Use\"`.", defaultValue: false),
            ],
            subcommands: []
        )
    }
