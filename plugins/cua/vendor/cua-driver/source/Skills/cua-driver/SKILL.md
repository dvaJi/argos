...
When Argos auto-pins this skill because the Computer Use MCP server is
...
2. Run `cua-driver check_permissions` (with the daemon up — see step 3).
   ...
   Accessibility and Screen Recording to `Argos Computer Use.app`, then stop.
   Pass `'{"prompt":false}'` for a purely read-only status check that
   won't steal focus.
3. Start the daemon with `open -n -g -a "Argos Computer Use" --args serve` (the
   recommended form — goes through LaunchServices so TCC attributes
   the process to Argos Computer Use.app). `cua-driver serve &` also works;
   the CLI auto-relaunches through `open -n -g -a "Argos Computer Use"` when it
...
- `open -n -g -a "Argos Computer Use" --args serve` — start persistent daemon
