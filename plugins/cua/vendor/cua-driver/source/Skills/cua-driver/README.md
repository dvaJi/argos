...
2. **`cua-driver` CLI + `Argos Computer Use.app`** — bundled with Argos.
   For local Argos builds:
   ```bash
   pnpm run build:cua-driver:mac:arm64
   ```
   The driver runs as an `.app` bundle because macOS TCC grants are
   tied to a stable bundle id (`com.wefonk.argos.computeruse`). The CLI symlink
   lets Claude invoke tools via plain shell.
3. **TCC grants on `Argos Computer Use.app`** — **Accessibility** and
...
- `cua-driver: command not found` → re-run the installer or add
  `.build/Argos Computer Use.app/Contents/MacOS/` to `$PATH`.
