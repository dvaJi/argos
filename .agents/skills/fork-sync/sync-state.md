...
| `ce6796e8` | #1776 | CUA cross-platform runtimes | done (PR #4) — platform/arch `engines.targets` targeting + `arch` threaded through settings-nav/plugin filtering. Deferred (separate): win32/linux CUA permission probes + Windows `launch_app` arg preflight (need the win32 runtime build). |
