---
name: argos-data-import
description: Help developers build third-party tools that import, inspect, migrate, or analyze Argos data. Use when Codex needs to work with Argos provider configuration, model configuration, MCP/app settings, sessions, messages, legacy chat data, `agent.db`, `chat.db`, SQLCipher encrypted SQLite, Electron safeStorage wrapped passwords, Tauri importers, or native macOS/Windows/Linux data access.
---
...
Use this skill to design or implement importers for Argos local data. Treat Argos's SQLite
...
1. Identify the source: live Argos profile, copied profile, sync backup, exported `agent.db`, or
   legacy `chat.db`.
...
5. Read [references/import-recipes.md](references/import-recipes.md) when writing extractor code,
   mapping Argos data to another app, or creating a compatibility import.

## Safety Rules

- Get explicit user consent before reading local Argos data. Provider keys, OAuth tokens, MCP
...
- Never open the active `agent.db` read-write from a third-party tool. Copy `agent.db`,
  `agent.db-wal`, and `agent.db-shm`, or use SQLite backup APIs through Argos itself.
- If Argos is running, either ask the user to quit it or make a WAL-aware snapshot before import.
