...
Use this reference when an importer must locate Argos data on disk or inside a backup.

## Primary Files

Argos stores the current main database at:
...
Electron derives the profile path from the packaged product name `Argos` unless the runtime
...
Argos sync backups use `database/agent.db` as the primary database payload in current backup
...
- If Argos is running, copy `agent.db`, `agent.db-wal`, and `agent.db-shm` together.
- If Argos is closed, `agent.db` alone is usually enough, but copying sidecars is still harmless.
...
- Do not delete `*.migration-tmp`, `*.migration-rollback`, `agent.db-wal`, or `agent.db-shm` from a
  user profile. Argos owns those lifecycle decisions.
