...
Use this reference when turning Argos data into an importer or migration tool.
...
LEFT JOIN argos_sessions ds ON ds.id = ns.id
...
6. Hydrate user messages from `argos_user_messages`, `argos_user_message_files`, and
   `argos_user_message_links`.
7. Hydrate assistant messages from `argos_assistant_blocks`; fall back to parsing
   `argos_messages.content`.
...
Do not rely only on `provider_json`; Argos deliberately stores common scalar fields in columns
...
- If structured user rows are missing, parse `argos_messages.content`.
- If structured assistant blocks are missing, parse `argos_messages.content`.
...
## Writing Back Into Argos

Avoid third-party direct writes to a user's live Argos database.

If a tool must generate data for Argos:

- Prefer creating an export file or backup package that Argos can import through its own code.
- If implementing inside Argos, use Presenter/table helpers instead of raw SQL.
...
- Keep `argos_messages.content` compatible even when structured tables are populated, because it
  remains the fallback path.
- Do not update `database-security.json` manually after rekeying; use Argos's migration flow.
...
- `argos_message_traces.headers_json` and `body_json`
...
SELECT COUNT(*) FROM argos_sessions;
SELECT COUNT(*) FROM argos_messages;
