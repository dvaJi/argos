...
`agent.db`. Verify against `sqlite_master` for user databases created by newer Argos versions.
...
- `cache_key`: primary key used by Argos config helpers.
...
Argos's current mainline session model is split across a thin registry and agent-specific data.
...
- `id`: session id.
- `agent_id`: agent implementation id. Argos chat sessions normally use the Argos agent id;
...
Argos-specific session config. `id` matches `new_sessions.id`.
...
Use these first for current rows; fall back to `argos_messages.content` if missing.

- `argos_user_messages`: `message_id`, `text`, `search_enabled`, `think_enabled`.
- `argos_user_message_files`: `message_id`, `ordinal`, `name`, `path`, `mime_type`, `size`,
  `metadata_json`.
- `argos_user_message_links`: `message_id`, `ordinal`, `url`.
...
Use `argos_assistant_blocks` first for assistant messages. It is especially important for
...
`argos_messages.content` as the fallback assistant block array.

### Event, Search, Trace, And Usage Tables

These are useful for richer import but optional for basic chat history.

- `argos_tape_entries`: append-only reconstruction/event facts per session.
- `argos_pending_inputs`: queued or steer-mode pending input payloads.
- `argos_search_documents` and FTS shadow tables: derived search index.
- `argos_message_search_results`: web/search results associated with messages.
- `argos_message_traces`: provider request traces. Treat as highly sensitive.
- `argos_usage_stats`: token/cost usage by message, provider, model, and date.

## Legacy Compatibility Tables

Current Argos keeps legacy tables for compatibility and import.
...
Prefer `new_sessions` and `argos_*` for current imports. Use legacy tables or `chat.db` only when
