...
3. If `metadata.enabled === true`, open with SQLCipher using the Argos SQLite password.
...
Argos uses `better-sqlite3-multiple-ciphers` and configures SQLCipher compatibility before
applying the key:
...
An Electron-based third-party importer has the best chance of using Argos's wrapped password.
...
async function readArgosPassword(argosUserData: string): Promise<string | null> {
  await app.whenReady()
  const metadataPath = path.join(argosUserData, 'database-security.json')
...
they can unwrap Argos's Electron safeStorage blob.
...
- macOS: Electron safeStorage depends on Keychain-backed OS crypto. Native Keychain access can be
  app-permission dependent and should not be treated as a stable Argos import API.
...
- Linux: safeStorage may use libsecret, KWallet, or a weaker backend reported as
  `safeStorageBackend`. If the user's desktop secret service is unavailable, Argos stores
...
- A missing WAL file can make recent rows disappear from a copied live database. Re-copy sidecars or
  ask the user to close Argos.
- Do not run rekey or migration operations from an importer. Argos's own migration flow copies
