...
describe("startupDeepLink utilities", () => {
  ...
  it("prefers stored startup deeplink over argv and secondary env keys", () => {
    ...
    expect(findStartupDeepLink(["electron", "deepchat://start?msg=argv"], env)).toBe("deepchat://start?msg=stored");
  });
  ...
  it("falls back to argv before secondary env deeplinks", () => {
    ...
    expect(findStartupDeepLink(["electron", "deepchat://start?msg=argv"], env)).toBe("deepchat://start?msg=argv");
  });

  it("stores and consumes startup deeplink exactly once", () => {
    const env = {} as NodeJS.ProcessEnv;

    expect(storeStartupDeepLink("deepchat://start?msg=hello", env)).toBe("deepchat://start?msg=hello");
  ...
  it("ignores strings that only contain a deeplink later in the value", () => {
    expect(findDeepLinkArg(["electron", "https://example.com/?next=deepchat://start?msg=1"])).toBe(null);
