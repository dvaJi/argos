...
describe("Integration: createSession end-to-end", () => {
  ...
  beforeEach(() => {
    ...
    agentPresenter = new AgentSessionPresenter(deepchatAgent as any, llmProvider, configPresenter, sqlitePresenter);
  });
  ...
  it("deleteSession cleans up all data", async () => {
    const session = await agentPresenter.createSession({ agentId: "deepchat", message: "To delete" }, 1);
  ...
  it("clearSessionMessages clears messages but keeps session row", async () => {
    const session = await agentPresenter.createSession({ agentId: "deepchat", message: "To clear" }, 1);
...
describe("Integration: multi-turn context", () => {
  ...
  beforeEach(() => {
    ...
    deepchatAgent = new AgentRuntimePresenter(llmProvider, configPresenter, sqlitePresenter, createMockToolPresenter());
    agentPresenter = new AgentSessionPresenter(deepchatAgent as any, llmProvider, configPresenter, sqlitePresenter);
  });

  it("second message includes first exchange in LLM context", async () => {
    // Send first message
    const session = await agentPresenter.createSession({ agentId: "deepchat", message: "Hello", projectDir: null }, 1);
...
describe("Integration: crash recovery", () => {
  it("pending messages are recovered to error status on init", () => {
    ...
    const [messageId, contentJson, status] = sqlitePresenter.deepchatMessagesTable.updateContentAndStatus.mock.calls[0];
    ...
    expect(consoleSpy).toHaveBeenCalledWith("DeepChatAgent: recovered 2 pending messages to error status");
