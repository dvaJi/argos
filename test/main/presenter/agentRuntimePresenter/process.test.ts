...
describe("processStream", () => {
  ...
  it("flushes ACP provider permission blocks immediately and keeps live permission updates mutable", async () => {
    ...
    const pendingBlocks = (messageStore.updateAssistantContent as ReturnType<typeof vi.fn>).mock.calls[0][1];
    ...
    const grantedBlocks = (messageStore.updateAssistantContent as ReturnType<typeof vi.fn>).mock.calls[1][1];
  ...
  it("treats AbortError thrown before the first event as aborted without writing an error block", async () => {
    ...
    expect(eventBus.sendToRenderer).not.toHaveBeenCalledWith("stream:error", "all", expect.anything());
  });
  ...
  it("yields after completed tool calls when a pending input should run next", async () => {
    ...
    const finalizedBlocks = (messageStore.finalizeAssistantMessage as ReturnType<typeof vi.fn>).mock.calls[0][1];
  ...
  it("refreshes tools for the next loop iteration after skill_view activates a skill", async () => {
    ...
    const refreshTools = vi.fn().mockResolvedValue([makeTool("skill_view"), makeTool("deepchat_settings_set_theme")]);
    ...
    const coreStream = vi.fn(function (_messages, _modelId, _modelConfig, _temperature, _maxTokens, tools) {
      ...
      if (callCount === 2) {
        expect(tools.map((tool) => tool.function.name)).toEqual(["skill_view", "deepchat_settings_set_theme"]);
        return (async function* () {
          yield {
            type: "tool_call_start",
            tool_call_id: "tc2",
  ...
  it("does not refresh tools after linked-file skill_view reads", async () => {
    ...
    const toolPresenter = {
      ...
      callTool: vi.fn().mockResolvedValue({
        content: '{"success":true,"name":"deepchat-settings","filePath":"references/guide.md","isPinned":false}',
        rawData: {
          toolCallId: "tc1",
          content: '{"success":true,"name":"deepchat-settings","filePath":"references/guide.md","isPinned":false}',
          isError: false,
      ...
      if (callCount === 1) {
        ...
        return (async function* () {
          ...
          yield {
            type: "tool_call_end",
            tool_call_id: "tc1",
            tool_call_arguments_complete: '{"name":"deepchat-settings","file_path":"references/guide.md"}',
          } as LLMCoreStreamEvent;
          yield { type: "stop", stop_reason: "tool_use" } as LLMCoreStreamEvent;
        })();
      }
  ...
  it("passes reasoning_content back after each interleaved tool-call loop", async () => {
    ...
    expect(toolCallAssistantMessages.map((message: any) => message.reasoning_content)).toEqual(["Think 1", "Think 2"]);
  });
  ...
  it("max tool calls limit", async () => {
    ...
    expect((toolPresenter.callTool as ReturnType<typeof vi.fn>).mock.calls.length).toBeLessThanOrEqual(128);
  ...
  it("abort during stream", async () => {
    ...
    expect(messageStore.setMessageError).toHaveBeenCalledWith("m1", expect.any(Array), expect.any(String));
    const abortMetadata = JSON.parse((messageStore.setMessageError as ReturnType<typeof vi.fn>).mock.calls[0][2]);
