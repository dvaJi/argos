...
describe("AgentRuntimePresenter", () => {
  ...
  function installSessionRows(initialRows: any[]) {
    ...
    sqlitePresenter.deepchatMessagesTable.get.mockImplementation((id: string) => rows.find((row) => row.id === id));
  ...
  describe("constructor (crash recovery)", () => {
    ...
    it("logs recovered count when > 0", () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith("DeepChatAgent: recovered 1 pending messages to error status");
    ...
    it("only recovers claimed pending inputs for sessions that still exist", () => {
      ...
      expect(sqlitePresenter.deepchatPendingInputsTable.update).toHaveBeenCalledWith("pending-existing", {
        state: "pending",
        claimed_at: null,
      });
      expect(consoleSpy).toHaveBeenCalledWith("DeepChatAgent: recovered 1 sessions with claimed pending inputs");
      consoleSpy.mockRestore();
    });
  });
  ...
  describe("processMessage", () => {
    ...
    it("includes conversation history in LLM call", async () => {
      ...
      sqlitePresenter.deepchatMessagesTable.getMaxOrderSeq.mockReturnValueOnce(2).mockReturnValueOnce(3);
    ...
    it("persists files when message input is object", async () => {
      sqlitePresenter.deepchatMessagesTable.getMaxOrderSeq.mockReturnValueOnce(0).mockReturnValueOnce(1);
  ...
  describe("generation settings", () => {
    ...
    it("inherits interleaved thinking defaults and allows explicit session disable", async () => {
      ...
      expect(sqlitePresenter.deepchatSessionsTable.updateGenerationSettings).toHaveBeenLastCalledWith(
        "s1",
        expect.objectContaining({
          forceInterleavedThinkingCompat: false,
        }),
  ...
  describe("session compaction state", () => {
    ...
    it("bypasses DeepChat context preflight for oversized ACP provider calls", async () => {
      ...
      expect(JSON.stringify(sqlitePresenter.deepchatMessagesTable.updateContentAndStatus.mock.calls)).not.toContain(
        "Request was not sent",
      );
    });
    ...
    it("emits compacting before compacted on successful compaction", async () => {
      ...
      const insertRows = sqlitePresenter.deepchatMessagesTable.insert.mock.calls.map(([row]: any[]) => row);
      ...
      const finalizedCompaction = sqlitePresenter.deepchatMessagesTable.updateContentAndStatus.mock.calls.find(
  ...
  describe("respondToolInteraction", () => {
    const makeAssistantRow = (overrides?: {
      ...
      sqlitePresenter.deepchatMessagesTable.get.mockImplementation((id: string) => (id === row.id ? row : undefined));
    ...
    it("handles question_option and resumes assistant message", async () => {
      ...
      expect(sqlitePresenter.deepchatMessagesTable.updateContent).toHaveBeenCalledWith("m1", expect.any(String));

      const updatedBlocks = JSON.parse(sqlitePresenter.deepchatMessagesTable.updateContent.mock.calls[0][1]);
    ...
    it("preserves reasoning_content when resuming after a question answer", async () => {
      ...
      sqlitePresenter.deepchatMessagesTable.updateContent.mockImplementation((id: string, content: string) => {
        if (id === row.id) {
          row.content = content;
        }
      });
    ...
    it("views a skill draft inline and keeps the confirmation pending", async () => {
      ...
      expect(processStream).not.toHaveBeenCalled();
      expect(sqlitePresenter.deepchatMessagesTable.updateStatus).toHaveBeenCalledWith("m1", "pending");
  ...
  describe("permission mode", () => {
    it("setPermissionMode updates runtime and db", async () => {
      ...
      expect(sqlitePresenter.deepchatSessionsTable.updatePermissionMode).toHaveBeenCalledWith("s1", "default");
    });
