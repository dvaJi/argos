...
describe("DeepChatSessionsTable.updateSummaryStateIfMatches", () => {
  ...
  it("restores the v23 recovery migration for missing forward columns", () => {
    ...
    expect(table.getMigrationSQL(24)).toBe("ALTER TABLE deepchat_sessions ADD COLUMN timeout_ms INTEGER;");
    ...
    expect(table.getMigrationSQL(23)).toContain("ALTER TABLE deepchat_sessions ADD COLUMN top_p REAL;");
  });
  ...
  it("writes image generation settings as normalized JSON", () => {
    ...
    expect(prepare).toHaveBeenCalledWith("UPDATE deepchat_sessions SET image_generation_options_json = ? WHERE id = ?");
