...
const createBundledFixture = async (
  ...
  if (includeSettings) {
    ...
    files["types/settings-preload.d.ts"] = new TextEncoder().encode("interface Window { deepchatPlugin?: unknown }\n");
  }
...
describe("PluginPresenter", () => {
  ...
  it("hides the CUA official plugin on unsupported platforms", async () => {
    ...
    expect((await winPresenter.listPlugins()).map((plugin) => plugin.id)).not.toContain("com.deepchat.plugins.cua");
    expect((await linuxPresenter.listPlugins()).map((plugin) => plugin.id)).not.toContain("com.deepchat.plugins.cua");
  });
