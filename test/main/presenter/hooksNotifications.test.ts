...
import { expandHookCommandPlaceholders, truncateText } from "../../../src/main/presenter/hooksNotifications";
import {
...
describe("hooksNotifications", () => {
  ...
  it("expandHookCommandPlaceholders resolves quoted env references on posix", () => {
    expect(
      expandHookCommandPlaceholders("node scripts/hook.js {{event}} {{conversationId}} {{toolName}}", "linux"),
    ).toBe('node scripts/hook.js "${DEEPCHAT_HOOK_EVENT}" "${DEEPCHAT_CONVERSATION_ID}" "${DEEPCHAT_TOOL_NAME}"');
  });

  it("expandHookCommandPlaceholders resolves quoted env references on windows", () => {
    expect(expandHookCommandPlaceholders("powershell -File scripts/hook.ps1 {{event}} {{isTest}}", "win32")).toBe(
      'powershell -File scripts/hook.ps1 "%DEEPCHAT_HOOK_EVENT%" "%DEEPCHAT_HOOK_IS_TEST%"',
    );
  });
