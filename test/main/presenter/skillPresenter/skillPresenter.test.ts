...
describe("SkillPresenter", () => {
  ...
  describe("constructor", () => {
    ...
    it("should repair stale POSIX default skills paths from another user profile", async () => {
      (mockConfigPresenter.getSkillsPath as Mock).mockReturnValue("/Users/legacy-user/.deepchat/skills");
      (app.getPath as Mock).mockImplementation((name: string) => {
        if (name === "home") return "/mock/home";
        if (name === "temp") return "/mock/temp";
        return "/mock/" + name;
    ...
    it("should repair stale Windows default skills paths from another user profile", async () => {
      (mockConfigPresenter.getSkillsPath as Mock).mockReturnValue("C:\\Users\\legacy-user\\.deepchat\\skills\\nested");
      (app.getPath as Mock).mockImplementation((name: string) => {
        if (name === "home") return "/mock/home";
        if (name === "temp") return "/mock/temp";
        return "/mock/" + name;
  ...
  describe("manageDraftSkill", () => {
    ...
    it("views draft content without exposing draft paths", async () => {
      ...
      const draftPath = "/mock/temp/deepchat-skill-drafts/conv-draft/draft-12345678-1234-1234-1234-123456789abc";
      (fs.existsSync as Mock).mockImplementation((target: string) => {
        return (
          target === draftPath || target === `${draftPath}/SKILL.md` || target === "/mock/temp/deepchat-skill-drafts"
        );
      });
      (fs.readFileSync as Mock).mockImplementation((target: string) => {
  ...
  describe("saveSkillWithExtension", () => {
    ...
    it("rolls back skill content when extension save fails", async () => {
      ...
      (fs.writeFileSync as Mock).mockImplementation((target: string, content: string) => {
        if (target.endsWith("/.deepchat-meta/test-skill.json") && content === JSON.stringify(extension, null, 2)) {
  ...
  describe("skill runtime extensions", () => {
    ...
    it("should save and load sidecar runtime config", async () => {
      ...
      (fs.existsSync as Mock).mockImplementation(
        (target: string) => !target.includes("/scripts") || target.endsWith("/.deepchat-meta/test-skill.json"),
      );
      (fs.readFileSync as Mock).mockImplementation((target: string) => {
        if (target.endsWith("/.deepchat-meta/test-skill.json")) {
    ...
    it("should discover runnable scripts under scripts directory", async () => {
      (fs.existsSync as Mock).mockImplementation(
        (target: string) => !target.endsWith("/.deepchat-meta/test-skill.json") || target.includes("/scripts"),
      );
      (fs.readdirSync as Mock).mockImplementation((target: string) => {
    ...
    it("should remove sidecar config when uninstalling a skill", async () => {
      ...
      expect(fs.rmSync).toHaveBeenCalledWith(expect.stringContaining("/.deepchat-meta/test-skill.json"), {
        force: true,
      });
    });
  });
