...
describe('WelcomePage', () => {
  it('marks init complete and navigates provider entry to provider settings', async () => {
    ...
    const guideImportButton = container.querySelector(
    ...
    expect(screen.getByTestId('welcome-provider-grid')).not.toHaveTextContent(
    ...
    const browseButton = buttons.find((b) =>
      b.textContent?.includes('welcome.page.browseProviders')
    )
  ...
  it('blocks background clicks and lets the spotlight coachmark continue the real select-provider step', async () => {
    ...
    expect(
      container.querySelector('[data-testid="welcome-guide-coachmark-primary-action"]')
    ).toBeFalsy()
