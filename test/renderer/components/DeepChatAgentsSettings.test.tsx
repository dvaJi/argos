...
describe('DeepChatAgentsSettings', () => {
  ...
  it('mounts and saves DeepChat agents without advanced model overrides', async () => {
    ...
    const DeepChatAgentsSettings = (
      await import('../../../src/renderer/settings/components/DeepChatAgentsSettings')
    ).default
    ...
    expect(
      (result.container.textContent?.indexOf('settings.deepchatAgents.visionModel') ?? -1) <
        (result.container.textContent?.indexOf('settings.deepchatAgents.imageGenerationModel') ??
          -1)
    ).toBe(true)
  ...
  it('shows an unsaved draft agent in the sidebar before persisting', async () => {
    ...
    expect(
      asideButtons.some((button) => button.textContent?.includes('settings.deepchatAgents.unnamed'))
    ).toBe(true)
    ...
    expect(updatedAsideButtons.some((button) => button.textContent?.includes('Draft Writer'))).toBe(
      true
    )
