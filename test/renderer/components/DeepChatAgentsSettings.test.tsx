...
describe("DeepChatAgentsSettings", () => {
  ...
  it("mounts and saves DeepChat agents without advanced model overrides", async () => {
    ...
    const DeepChatAgentsSettings = (await import("../../../src/renderer/settings/components/DeepChatAgentsSettings"))
      .default;

    const result = render(<DeepChatAgentsSettings />);

    await act(async () => {});

    expect(result.container.textContent).not.toContain("settings.deepchatAgents.temperature");
    expect(result.container.textContent).not.toContain("settings.deepchatAgents.reasoningEffort");
    expect(result.container.textContent).not.toContain("settings.deepchatAgents.verbosity");
    expect(result.container.textContent).not.toContain("settings.deepchatAgents.interleaved");
    ...
    expect(
      (result.container.textContent?.indexOf("settings.deepchatAgents.visionModel") ?? -1) <
        (result.container.textContent?.indexOf("settings.deepchatAgents.imageGenerationModel") ?? -1),
    ).toBe(true);
  ...
  it("keeps the editor header sticky so save actions stay visible while scrolling", async () => {
    ...
    const stickyHeader = screen.getByTestId("deepchat-agents-sticky-header");
  ...
  it("fills the system prompt field from a prompt template dialog", async () => {
    ...
    const textareas = Array.from(result.container.querySelectorAll("textarea")).filter((textarea) =>
      textarea.getAttribute("placeholder")?.includes("settings.deepchatAgents.systemPromptPlaceholder"),
    );
  ...
  it("shows an unsaved draft agent in the sidebar before persisting", async () => {
    ...
    expect(asideButtons.some((button) => button.textContent?.includes("settings.deepchatAgents.unnamed"))).toBe(true);

    const nameInput = Array.from(result.container.querySelectorAll("input")).find((input) =>
      input.getAttribute("placeholder")?.includes("settings.deepchatAgents.namePlaceholder"),
    );
  ...
  it("uses a flat target agent select for subagent slots", async () => {
    ...
    expect(result.container.textContent).not.toContain("settings.deepchatAgents.subagentTargetType");

    const targetSelects = result.container.querySelectorAll("select");
    expect(targetSelects).toHaveLength(2);
    expect(targetSelects[0].textContent).toContain("settings.deepchatAgents.subagentTargetSelf");
