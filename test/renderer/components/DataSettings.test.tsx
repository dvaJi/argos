...
const setup = async (
  ...
  const DataSettings = (await import('../../../src/renderer/settings/components/DataSettings'))
    .default
...
describe('DataSettings', () => {
  ...
  it('enables database encryption after matching password input', async () => {
    ...
    await fireEvent.click(
      findDatabaseEncryptionButton(container, 'settings.data.databaseEncryption.enableButton')
    )
    await act(async () => {})
  ...
  it('shows database encryption status as unknown when status loading fails', async () => {
    ...
    expect(
      buttons.some((button) =>
        button.textContent?.includes('settings.data.databaseEncryption.setPasswordButton')
      )
    ).toBe(false)
  })
