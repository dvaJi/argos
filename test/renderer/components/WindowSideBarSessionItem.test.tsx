...
describe('WindowSideBarSessionItem', () => {
  ...
  it('renders the correct pin action label for pinned and unpinned sessions', async () => {
    ...
    const unpinnedPinButton = unpinnedContainer.querySelector(
      '[aria-label="thread.actions.pin"]'
    ) as HTMLElement
    const pinnedPinButton = pinnedContainer.querySelector(
      '[aria-label="thread.actions.unpin"]'
    ) as HTMLElement
  ...
  it('renders shortcut badges independently from the delete action', async () => {
    ...
    expect(
      (badgeContainer.querySelector('.right-button') as HTMLElement).getAttribute(
        'data-shortcut-badge-visible'
      )
    ).toBe('true')
    ...
    expect(
      normalContainer.querySelector('[data-testid="sidebar-session-shortcut-badge"]')
    ).toBeFalsy()
    expect(normalContainer.querySelector('[aria-label="thread.actions.delete"]')).toBeTruthy()
    expect(
      (normalContainer.querySelector('.right-button') as HTMLElement).getAttribute(
        'data-shortcut-badge-visible'
      )
    ).toBeNull()
  }, 10000)
