...
describe("createBridge", () => {
  it("invokes typed routes through the shared IPC channel", async () => {
    ...
    expect(ipcRenderer.invoke).toHaveBeenCalledWith(DEEPCHAT_ROUTE_INVOKE_CHANNEL, "settings.getSnapshot", {
      keys: ["fontSizeLevel"],
    });
  ...
  it("validates typed event payloads before calling listeners", () => {
    ...
    expect(ipcRenderer.removeListener).toHaveBeenCalledWith(DEEPCHAT_EVENT_CHANNEL, expect.any(Function));
  });
