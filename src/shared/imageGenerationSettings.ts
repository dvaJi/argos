...
export type OpenAIImageGenerationBackground = (typeof OPENAI_IMAGE_GENERATION_BACKGROUND_VALUES)[number];
...
const normalizeText = (value: unknown): string => (typeof value === "string" ? value.trim().toLowerCase() : "");
...
const isOpenAICompatibleProvider = (target: OpenAIImageGenerationSettingsTarget): boolean => {
  ...
  if (
    NON_OPENAI_IMAGE_PROVIDER_HINTS.some(
      (hint) => providerId.includes(hint) || providerApiType.includes(hint) || providerKind.includes(hint),
    )
  ) {
    return false;
  }
...
export const supportsOpenAIImageGenerationSettings = (target: OpenAIImageGenerationSettingsTarget): boolean =>
  isOpenAICompatibleProvider(target) && hasOpenAIImageGenerationRoute(target);

export const validateOpenAIImageGenerationSize = (size: string): OpenAIImageGenerationSizeValidationResult => {
  ...
  if (width % OPENAI_IMAGE_GENERATION_SIZE_MULTIPLE !== 0 || height % OPENAI_IMAGE_GENERATION_SIZE_MULTIPLE !== 0) {
...
export const normalizeImageGenerationOptions = (
  ...
  if (canUseCompression && typeof value.outputCompression === "number" && Number.isFinite(value.outputCompression)) {
