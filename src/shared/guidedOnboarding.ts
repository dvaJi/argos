...
export type GuidedOnboardingSettingsRouteName = "settings-provider" | "settings-mcp" | "settings-skills";
...
export const GUIDED_ONBOARDING_SKIPPABLE_STEP_IDS = ["provider-api-key", "provider-model", "mcp", "skills"] as const;
...
} as const satisfies Partial<Record<SharedGuidedOnboardingStepId, GuidedOnboardingSettingsRouteName>>;
...
export const isGuidedOnboardingStepId = (value: unknown): value is SharedGuidedOnboardingStepId =>
  typeof value === "string" && GUIDED_ONBOARDING_STEP_IDS.includes(value as SharedGuidedOnboardingStepId);

export const isLegacyGuidedOnboardingStepId = (value: unknown): value is LegacyGuidedOnboardingStepId =>
  typeof value === "string" && LEGACY_GUIDED_ONBOARDING_STEP_IDS.includes(value as LegacyGuidedOnboardingStepId);

export const isGuidedOnboardingRequiredStepId = (
  stepId: SharedGuidedOnboardingStepId | null | undefined,
): stepId is SharedGuidedOnboardingStepId => Boolean(stepId && GUIDED_ONBOARDING_REQUIRED_STEP_ID_SET.has(stepId));

export const isGuidedOnboardingSkippableStepId = (
  stepId: SharedGuidedOnboardingStepId | null | undefined,
): stepId is SharedGuidedOnboardingStepId => Boolean(stepId && GUIDED_ONBOARDING_SKIPPABLE_STEP_ID_SET.has(stepId));

export const isGuidedOnboardingChatStepId = (
  stepId: SharedGuidedOnboardingStepId | null | undefined,
): stepId is SharedGuidedOnboardingStepId => Boolean(stepId && GUIDED_ONBOARDING_CHAT_STEP_ID_SET.has(stepId));

export const isGuidedOnboardingSettingsStepId = (
  stepId: SharedGuidedOnboardingStepId | null | undefined,
): stepId is keyof typeof GUIDED_ONBOARDING_SETTINGS_ROUTE_NAMES =>
  Boolean(stepId && Object.prototype.hasOwnProperty.call(GUIDED_ONBOARDING_SETTINGS_ROUTE_NAMES, stepId));
...
export const getGuidedOnboardingStepIndex = (stepId: SharedGuidedOnboardingStepId | null | undefined): number => {
  if (!stepId) {
    return -1;
  }
...
export const getNextGuidedOnboardingStepId = (
  ...
  const index = getGuidedOnboardingStepIndex(stepId);
  return index >= 0 && index < GUIDED_ONBOARDING_STEP_IDS.length - 1 ? GUIDED_ONBOARDING_STEP_IDS[index + 1] : null;
};
