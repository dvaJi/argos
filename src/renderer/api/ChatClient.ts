...
export function createChatClient(bridge: DeepchatBridge = getDeepchatBridge()) {
  ...
  function onStreamUpdated(listener: (payload: DeepchatEventPayload<"chat.stream.updated">) => void) {
    return bridge.on(chatStreamUpdatedEvent.name, listener);
  }

  function onStreamCompleted(listener: (payload: DeepchatEventPayload<"chat.stream.completed">) => void) {
