...
export default function FloatingButton({ theme }: FloatingButtonProps) {
  ...
  const collapsedClass = snapshot.expanded
  ...
  const cursorClass = snapshot.expanded
    ? isDragging
      ? 'cursor-grabbing'
      : 'cursor-grab'
    : 'cursor-pointer'
            ...
            >
              <div className="status-orb-face status-orb-logo absolute inset-0 flex items-center justify-center">
                <img
                  src={logoSrc}
                  alt="Task Overview"
                  className="logo-orb-image status-orb-logo-image h-9 w-9"
                />
              </div>
            ...
            <div className="panel-header relative z-[1] flex items-center justify-between gap-3 px-1 pb-3 pt-1">
              <div className="flex min-w-0 items-center gap-3">
                ...
                <div className="min-w-0">
                  <p className="panel-title truncate text-[15px] font-semibold tracking-[0.01em]">
                    Task Overview
                  </p>
                ...
                <div className="flex flex-col space-y-2">
                  {snapshot.sessions.map((session, index) => (
                    <div
                      ...
                      style={{ '--session-index': Math.min(index, 6) } as React.CSSProperties}
                    >
                      <FloatingSessionItem
                        session={session}
                        theme={theme}
