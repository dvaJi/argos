...
import { uiSettingsStore, loadSettings as loadUiSettings } from '../src/stores/uiSettingsStore'
...
import { ollamaStore } from '@/stores/ollamaStore'
import {
...
export default function SettingsApp() {
  ...
  const providerImportConfirmDisabled = useMemo(() => {
    ...
    if (pendingProviderImportPreview.kind === 'builtin') {
      return !providerState.providers.some(
        (provider) => provider.id === pendingProviderImportPreview.id
      )
    }
  ...
  const ensureProviderStoreReady = useCallback(async () => {
    ...
    if (!providerStoreInitializePromise.current) {
      providerStoreInitializePromise.current = Promise.resolve(
  ...
  const ensureProviderRouteReady = useCallback(
    async (providerId?: string) => {
      ...
      await modelStore.state.ensureProviderModelsReady?.(providerId)

      if (provider.apiType === 'ollama') {
        await ollamaStore.ensureProviderReady?.(providerId)
      }
    },
    [ensureProviderStoreReady, ollamaStore]
  )
  ...
  const showDatabaseRepairSuggestedToast = useCallback(
  ...
  const confirmProviderImport = useCallback(async () => {
    const preview = useProviderDeeplinkImportStore.getState().preview
    ...
    try {
      if (preview.kind === 'builtin') {
        const targetProvider = providerStore.state.providers.find(
        ...
        await providerStore.state.updateProviderApi?.(preview.id, preview.apiKey, preview.baseUrl)
        if (!targetProvider.enable) {
          await providerStore.state.updateProviderStatus?.(preview.id, true)
        }

        await modelStore.state.refreshProviderModels?.(preview.id)
        await navigateToProviderSettings(preview.id)
      } else {
        ...
        await providerStore.state.addCustomProvider?.(newProvider)
        await modelStore.state.refreshProviderModels?.(providerId)
        await navigateToProviderSettings(providerId)
      }

      useProviderDeeplinkImportStore.getState().clearPreview()
      releaseProviderPreviewProcessing()
    } catch (error) {
  ...
  useEffect(() => {
    ...
    const providerId = (routerInstance.state.location.search as any)?.providerId as
      | string
      | undefined
  ...
  useEffect(() => {
    const newClass = uiSettingsState.fontSizeClass
  ...
  return (
    <div
      ...
      className={`w-full h-screen flex flex-col ${isWinMacOS ? '' : 'bg-background'}`}
    >
      <div
