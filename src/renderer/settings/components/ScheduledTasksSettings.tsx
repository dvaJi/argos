...
export default function ScheduledTasksSettings() {
  ...
  const refreshFormBuffers = useCallback(() => {
    setOnceInputValues(
      tasks.map((t) => (t.trigger.kind === 'once' ? formatDateTimeLocal(t.trigger.firesAt) : ''))
    )
              ...
              <div className="overflow-hidden rounded-xl border bg-card/30">
                {settings.tasks.map((task, index) => (
                  <Collapsible
                    ...
                    onOpenChange={(open) => {
                      setOpenTaskIds((prev) =>
                        open ? [...prev, task.id] : prev.filter((id) => id !== task.id)
                      )
                    }}
                  >
                    <div
                                      ...
                                      onChange={(e) => {
                                        ...
                                        if (Number.isFinite(ts)) {
                                          const next = {
                                            ...
                                            tasks: settings.tasks.map((t, i) =>
                                              i === index
                                                ? { ...t, trigger: { kind: 'once', firesAt: ts } }
                                                : t
                                            )
                                          }
                                          setSettings(next)
                                    ...
                                    {task.trigger.kind === 'weekly' && (
                                      <div className="space-y-1.5">
                                        ...
                                        >
                                          ...
                                          <SelectContent>
                                            {Object.entries(DAY_OF_WEEK_OPTIONS).map(
                                              ([value, label]) => (
                                  ...
                                  <>
                                    ...
                                    <div className="grid gap-3 sm:grid-cols-2">
                                      <div className="min-w-0 space-y-1.5">
                                        ...
                                        <Select
                                          value={
                                            (task.action as PromptAction).agentId ?? 'deepchat'
                                          }
                                          onValueChange={(value) => {
                                        ...
                                        >
                                          ...
                                          >
                                            <ModelSelect
                                              ...
                                              selectedModelId={
                                                (task.action as PromptAction).modelId ?? ''
                                              }
                                              onUpdateModel={(
