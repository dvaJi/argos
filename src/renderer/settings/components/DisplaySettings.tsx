...
export default function DisplaySettings() {
  ...
  const [notificationsEnabled, setNotificationsEnabled] = useState(
    () => uiSettingsStore.notificationsEnabled
  )
  ...
  const confirmContentProtectionChange = useCallback(() => {
    uiSettingsStore.setContentProtectionEnabled(newContentProtectionValue)
    setContentProtectionEnabled(newContentProtectionValue)
    setIsContentProtectionDialogOpen(false)
  }, [uiSettingsStore, newContentProtectionValue])
  ...
  useEffect(() => {
    const update = async () => {
      await languageStore.updateLanguage(selectedLanguage)
    }
  ...
  useEffect(() => {
    uiSettingsStore.updateFontSizeLevel(fontSizeLevel)
  }, [fontSizeLevel, uiSettingsStore])
  ...
  const handleFloatingButtonChange = useCallback(
    (value: boolean) => floatingButtonStore.setFloatingButtonEnabled(value),
    [floatingButtonStore]
  )
        ...
        <div className="flex w-full flex-col gap-1.5">
          <div className="flex flex-col gap-2 px-2 py-2">
            <div className="flex items-center gap-3">
              <span
                className="flex min-w-[220px] shrink-0 items-center gap-2 text-sm font-medium"
                dir={dir}
              >
          ...
          <div className="flex flex-col gap-2 px-2 py-2">
            ...
            <div className="text-xs text-muted-foreground">
              Show system notifications for messages and updates
            </div>
          </div>
          ...
          {FLOATING_BUTTON_AVAILABLE && (
            <div className="flex flex-col gap-2 px-2 py-2">
              ...
              <div className="text-xs text-muted-foreground">
                Show a floating button for quick access
              </div>
            </div>
          )}
        </div>
