...
export default function AcpDebugDialog({
  open,
  onOpenChange,
  agentId,
  agentName
    ...
    <div className="fixed inset-0 z-[200] bg-background text-foreground flex flex-col pt-8 min-h-0">
      <header className="flex items-center justify-between px-6 py-4 border-b gap-3">
        ...
        <div className="flex items-center gap-3">
          ...
          >
            <span
              className={`h-2 w-2 rounded-full ${processReady ? 'bg-emerald-500' : 'bg-muted-foreground/60'}`}
            />
          ...
          <div className="flex-1 min-h-0 flex flex-col gap-3">
            ...
            <div className="flex-1 overflow-y-auto p-3 space-y-2 bg-muted/40 text-xs min-h-0 rounded-md">
              ...
              {sortedEvents.map((event) => (
                <div
                  key={event.id}
                  className={`rounded-md border p-2 space-y-1 ${eventTone(event.kind)}`}
                >
                  ...
                  {event.message && (
                    <div className="text-[11px] text-destructive">{event.message}</div>
                  )}
