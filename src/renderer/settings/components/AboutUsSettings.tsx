...
export default function AboutUsSettings() {
  ...
  const handleExternalCheckUpdate = useCallback(async () => {
    if (upgrade.isChecking || upgrade.isDownloading || upgrade.isRestarting) return
    if (upgrade.updateState === 'available' || upgrade.isReadyToInstall) return
    await handlePrimaryAction()
  }, [upgrade, handlePrimaryAction])

  useEffect(() => {
    const handler = () => void handleExternalCheckUpdate()
    window.electron?.ipcRenderer?.on(SETTINGS_EVENTS.CHECK_FOR_UPDATES, handler)
    void (async () => {
    ...
    return () => {
      window.electron?.ipcRenderer?.removeListener(SETTINGS_EVENTS.CHECK_FOR_UPDATES, handler)
    }
        ...
        <div className="flex min-h-[520px] w-full flex-col items-center justify-center gap-2">
          ...
          <div className="flex flex-col items-center gap-2" dir={languageStore.dir}>
            ...
            <div className="flex gap-2">
              <a
                ...
                onClick={(e) => {
                  e.preventDefault()
                  openExternalLink('https://deepchat.thinkinai.xyz/')
                }}
              >
              ...
              <a
                ...
                onClick={(e) => {
                  e.preventDefault()
                  openExternalLink('https://github.com/ThinkInAIXYZ/deepchat')
                }}
              >
          ...
          {upgrade.showManualDownloadOptions && (
            <div className="mt-2 flex w-full max-w-xl flex-col items-center gap-1">
              <p className="text-center text-xs text-muted-foreground">
                Auto-update failed. Please download manually.
              </p>
              {upgrade.updateError && (
          ...
          <div className="mt-2 flex flex-wrap justify-center gap-2">
            <Button
              ...
              onClick={() =>
                openExternalLink('https://github.com/ThinkInAIXYZ/deepchat/discussions/1226')
              }
            >
              ...
              >
                <Icon
                  ...
                  className={`mr-1 h-3 w-3 ${
                    upgrade.isChecking || upgrade.isDownloading ? 'animate-spin' : ''
                  }`}
                />
