...
export default function ModelProviderSettingsDetail({
  ...
  const enabledModels = useMemo(() => {
    const enabledModelsList = [
      ...customModels.filter((m) => m.enabled),
      ...providerModels.filter((m) => m.enabled)
    ]
  ...
  const providerWebsites = useMemo<ProviderWebsites | undefined>(
    () =>
      providerStore.defaultProviders.find((p) => p.id === provider.id)?.websites as
        | ProviderWebsites
        | undefined,
  ...
  const geminiSafetyLevelsForChild = useMemo(
    () => ({ ...geminiSafetyLevels }),
    [geminiSafetyLevels]
  )
