...
export default function UsageNostalgiaCard({ dashboard }: UsageNostalgiaCardProps) {
  ...
  const nostalgiaCard = useMemo(() => {
    ...
    const days = getDaysWithDeepChat(dashboard.recordingStartedAt)
    ...
    return {
      ...
      details: [
        {
          id: 'days',
          content: days === null ? 'N/A' : `${formattedDays} days with DeepChat`
        },
        {
          id: 'sessions',
  ...
  return (
    ...
    >
      <CardHeader className="space-y-1 pb-1">
        <CardTitle className="wrap-break-word whitespace-normal text-base leading-tight">
          Your journey
        </CardTitle>
      </CardHeader>
      {nostalgiaCard ? (
        <CardContent className="flex flex-1 flex-col gap-3 pt-0 lg:grid lg:grid-cols-[minmax(0,13rem)_minmax(0,1fr)] lg:items-start lg:gap-4 xl:flex xl:flex-col">
          ...
          <div data-testid="nostalgia-details" className="space-y-2 lg:pt-0.5">
            {nostalgiaCard.details.map((item) => (
              ...
              >
                <p className="wrap-break-word whitespace-normal text-sm leading-6">
                  {item.content}
                </p>
              </div>
            ))}
