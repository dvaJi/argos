...
export default function DefaultModelSettingsSection() {
  ...
  const selectBySetting = (
    ...
    const matchedModel = providerEntry.models.find(
      (model) =>
        model.id === setting.modelId && (!predicate || predicate(model, setting.providerId))
    )
    if (!matchedModel) {
      return null
    }
        ...
        <div className="ml-auto flex items-center gap-2">
          <Popover open={assistantModelSelectOpen} onOpenChange={setAssistantModelSelectOpen}>
            <PopoverTrigger asChild>
              <Button
                variant="outline"
                className="h-8 w-[320px] justify-between text-sm border-border hover:bg-accent"
              >
                <div className="flex items-center gap-2 min-w-0">
                  {selectedAssistantModel && (
                    <ModelIcon
                      modelId={selectedAssistantModel.providerId}
                      className="h-4 w-4"
                      isDark={isDark}
                    />
                  )}
                  <span className="truncate">
                    {selectedAssistantModel?.model?.name || 'Select model'}
                  </span>
                </div>
          ...
          <Popover open={chatModelSelectOpen} onOpenChange={setChatModelSelectOpen}>
            <PopoverTrigger asChild>
              ...
              >
                <div className="flex items-center gap-2 min-w-0">
                  {selectedChatModel && (
                    <ModelIcon
                      modelId={selectedChatModel.providerId}
                      className="h-4 w-4"
                      isDark={isDark}
                    />
                  )}
                  <span className="truncate">
                    {selectedChatModel?.model?.name || 'Select model'}
                  </span>
                </div>
                <ChevronDown className="h-4 w-4 opacity-50" />
              </Button>
            ...
            <PopoverContent className="w-[320px] p-0" align="end">
              <ModelSelect
                excludeProviders={['acp']}
                respectChatMode={false}
                onUpdateModel={handleChatModelSelect}
              />
            </PopoverContent>
          </Popover>
        </div>
