...
export default function SystemPromptSettingsSection() {
  ...
  const saveCurrentSystemPrompt = useCallback(async () => {
    ...
    try {
      await systemPromptStore.updateSystemPrompt(currentSystemPrompt.id, {
      ...
      setSystemPrompts((prev) =>
        prev.map((p) =>
          p.id === currentSystemPrompt.id
            ? { ...p, content: currentSystemPrompt.content, updatedAt: Date.now() }
            : p
        )
      )
    ...
    try {
      ...
      if (currentSystemPrompt?.id === 'default') {
        setCurrentSystemPrompt((prev) =>
          prev ? { ...prev, content: DEFAULT_SYSTEM_PROMPT_CONTENT } : null
        )
      }
      setSystemPrompts((prev) =>
        prev.map((p) =>
          p.id === 'default'
            ? { ...p, content: DEFAULT_SYSTEM_PROMPT_CONTENT, updatedAt: Date.now() }
            : p
        )
      )
  ...
  const handleSaveSystemPrompt = useCallback(
    async ({ id, name, content }: { id?: string; name: string; content: string }) => {
      ...
      try {
        if (id) {
          await systemPromptStore.updateSystemPrompt(id, { name, content, updatedAt: timestamp })
        } else {
      ...
      <div className="flex flex-row items-center gap-2">
        <div className="flex-1">
          ...
          <p className="text-xs text-muted-foreground">
            Configure the default system prompt used for conversations.
          </p>
        </div>
      ...
      {currentSystemPrompt && (
        <div className="space-y-2">
          <Textarea
            ...
            onChange={(e) =>
              setCurrentSystemPrompt((prev) => (prev ? { ...prev, content: e.target.value } : null))
            }
            ...
            {currentSystemPrompt.id === 'default' ? (
              ...
              <AlertDialog>
                ...
                <AlertDialogContent>
                  <AlertDialogHeader>
                    <AlertDialogTitle>
                      Delete &quot;{currentSystemPrompt.name}&quot;?
                    </AlertDialogTitle>
                  ...
                  <AlertDialogFooter>
                    ...
                    <AlertDialogAction
                      onClick={() => void deleteSystemPrompt(currentSystemPrompt.id)}
                    >
                      Confirm
                    </AlertDialogAction>
                  </AlertDialogFooter>
