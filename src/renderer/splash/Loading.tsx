...
export default function Loading() {
  ...
  const [unlockReason, setUnlockReason] =
    useState<DatabaseUnlockRequestPayload['reason']>('manual-required')
  ...
  const handleUnlockRequest = useCallback(
    (_event: unknown, payload: DatabaseUnlockRequestPayload) => {
  ...
  const handleUnlockProgress = useCallback(
    (_event: unknown, payload: DatabaseUnlockProgressPayload) => {
      setUnlockSubmitting(false)
      if (payload.active) {
        setSafeStorageAvailable(payload.safeStorageAvailable)
    ...
    <div className="splash-shell">
      {mode === 'unlock' && (
        <div className="unlock-stage">
          ...
          >
            <div className="unlock-title">DeepChat</div>
          ...
          {activities.map((activity) => (
            <div key={activity.key} className="activity-item">
              ...
              {activity.status === 'failed' && (
                <span className="status-icon status-icon--failed">!</span>
              )}
