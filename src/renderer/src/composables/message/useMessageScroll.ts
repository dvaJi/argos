...
export function useMessageScroll(options?: UseMessageScrollOptions) {
  ...
  const bottomScrollRetryTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null)
  ...
  const updateScrollInfo = useCallback(() => {
    ...
    debounceTimerRef.current = setTimeout(updateScrollInfoImmediate, 16)
  ...
  const scrollToBottom = useCallback(
    (force = false) => scheduleScrollToBottom(force),
