...
export function ChatPage({ sessionId }: ChatPageProps) {
  ...
  const [scrollMode, setScrollMode] = useState<
    'initial-bottom' | 'auto-follow' | 'anchored-reading' | 'manual-jump'
  >('initial-bottom')
  ...
  const pendingAnchorRestoreRef = useRef<ViewportAnchor | null>(null)
  ...
  const sessionTitle = sessionState.activeSession?.title ?? 'New Chat'
  const sessionProject = sessionState.activeSession?.projectDir ?? ''
  const isReadOnlySession = sessionState.activeSession?.sessionKind === 'subagent'
  const isGenerating = sessionState.activeSession?.status === 'working' || messageStore.isStreaming
  ...
  const resolveChatInputBoxElement = useCallback(
    () =>
      (chatInputHeroHostRef.current?.querySelector(
    ...
    (fromUserScroll = false) => {
      ...
      scrollReadFrameRef.current = window.requestAnimationFrame(() => {
        ...
        if (userInitiated && scrollMode !== 'manual-jump') {
          ...
          setScrollMode(
            uiSettingsStore.autoScrollEnabled && nearBottom ? 'auto-follow' : 'anchored-reading'
          )
        }
      })
    },
  ...
  const hasInlineStreamingTarget = messageStore.currentStreamMessageId
    ? messageStore.messageCache.has(messageStore.currentStreamMessageId)
    : false
  ...
  const ephemeralRateLimitMessageId = useMemo(() => {
    ...
    if (
      ...
      return null
    return messageId
  }, [messageStore.isStreaming, messageStore.currentStreamMessageId])

  const ephemeralRateLimitBlock = useMemo<DisplayAssistantMessageBlock | null>(() => {
    if (!ephemeralRateLimitMessageId || messageStore.streamingBlocks.length === 0) return null
    ...
    if (
      ...
      return null
    return firstBlock
  }, [ephemeralRateLimitMessageId, messageStore.streamingBlocks])
  ...
  const latestPlanSnapshot = useMemo(() => {
    ...
    return snapshot
  }, [agentPlanStore.snapshots, sessionId])

  const isPlanFloatCollapsed = agentPlanStore.isCollapsed(sessionId)
  ...
  const isInputSubmitDisabled =
    ...
    Boolean(activePendingInteraction) ||
    isHandlingInteraction ||
    (isGenerating && pendingInputStore.isAtCapacity) ||
    !hasDraftInput
  const showResumePendingQueue =
    !isGenerating && !activePendingInteraction && pendingInputStore.queueItems.length > 0
  ...
  const handleManualCompactionCommand = useCallback(
    async (text: string): Promise<boolean> => {
      ...
      if (sessionState.activeSession?.providerId === 'acp') return false
    ...
    [sessionState.activeSession, isGenerating, sessionId, sessionClient, toast]
  )
  ...
  const onSubmit = useCallback(async () => {
    ...
    if (isGenerating) {
      await pendingInputStore.queueInput(sessionId, { text, files })
    } else {
      agentPlanStore.clear(sessionId)
  ...
  const onMessageFork = useCallback(
    async (messageId: string) => {
      ...
      try {
        ...
        await sessionStore.getState().fetchSessions()
        await sessionStore.getState().selectSession(forked.id)
      } catch (error) {
  ...
  const onPendingInputUpdate = useCallback(
    async (payload: { itemId: string; text: string }) => {
      ...
      if (!target) return
      await pendingInputStore.updateQueueInput(sessionId, payload.itemId, {
        text: payload.text,
        files: target.payload.files ?? []
      })
    },
    [isReadOnlySession, sessionId, pendingInputStore]
  )

  const onPendingInputMove = useCallback(
    async (payload: { itemId: string; toIndex: number }) => {
      if (isReadOnlySession) return
      await pendingInputStore.moveQueueInput(sessionId, payload.itemId, payload.toIndex)
    },
    [isReadOnlySession, sessionId, pendingInputStore]
  )
  ...
  const onPendingInputDelete = useCallback(
    async (itemId: string) => {
      if (isReadOnlySession) return
      await pendingInputStore.deleteInput(sessionId, itemId)
    },
    [isReadOnlySession, sessionId, pendingInputStore]
  )
  ...
  const onResumePendingQueue = useCallback(async () => {
    if (isReadOnlySession) return
    await pendingInputStore.resumeQueue(sessionId)
  }, [isReadOnlySession, sessionId, pendingInputStore])

  const onDismissPlanFloat = useCallback(() => {
    agentPlanStore.setCollapsed(sessionId, true)
    agentPlanStore.clear(sessionId)
    setPlanFloatReservedHeight(0)
  }, [agentPlanStore, sessionId])
  ...
  useEffect(() => {
    ...
    if (sessionId) {
      ...
      cancelSessionRestoreTaskRef.current = scheduleStartupDeferredTask(async () => {
        ...
        await pendingInputStore.loadPendingInputs(sessionId)
  ...
  const displayMessages = useMemo(() => {
    ...
    for (const rec of messageStore.messages) {
    ...
    if (
      ...
      msgs.push(
        toStreamingMessage(messageStore.streamingBlocks, messageStore.currentStreamMessageId)
      )
    }
    return msgs
  }, [
  ...
  function toDisplayMessage(record: ChatMessageRecord): DisplayMessage {
    ...
    const modelId = metadata.model || sessionState.activeSession?.modelId || ''
    const providerId = metadata.provider || sessionState.activeSession?.providerId || ''
  ...
  function toStreamingMessage(
    ...
    const modelId = sessionState.activeSession?.modelId ?? ''
    const now = Date.now()
    return {
      ...
      model_id: modelId,
      model_provider: sessionState.activeSession?.providerId ?? '',
      status: 'pending',
      error: '',
      usage: buildUsage({}),
  ...
  return (
    <TooltipProvider delayDuration={200}>
      ...
      >
            ...
            <div className="mx-auto flex w-full max-w-5xl justify-end">
              <ChatSearchBar
                ...
                onPrevious={() => {
                  ...
                  const next =
                    (activeChatSearchIndex - 1 + chatSearchMatches.length) %
        ...
        <div
          ref={messageSearchRootRef}
          className="min-h-[calc(100%-242px)]"
          style={messageSearchRootStyle}
        >
          {messageStore.isLoadingHistory && (
            <div className="pointer-events-none px-6 py-2 text-center text-xs text-muted-foreground">
              Loading...
            </div>
          )}
          <MessageList
          ...
          <div className="chat-capture-hide sticky bottom-0 z-10 w-full px-6 pb-3 pt-3">
            <div className="mx-auto flex w-full max-w-5xl min-w-0 flex-col items-center">
              <div className="relative w-full">
                <PendingInputLane
                  steerItems={pendingInputStore.steerItems}
                  queueItems={pendingInputStore.queueItems}
                  disableSteerAction={pendingInputStore.isAtCapacity}
                  showResumeQueue={showResumePendingQueue}
                  className="mx-auto mb-1.5 max-w-4xl"
                  onUpdateQueue={onPendingInputUpdate}
                  onMoveQueue={onPendingInputMove}
                      ...
                      {activePendingInteraction && latestPlanSnapshot && (
                        <div className="agent-question-panel pointer-events-auto mx-auto w-full max-w-2xl overflow-hidden rounded-[20px] text-foreground backdrop-blur-[26px]">
                          ...
                          <AgentProgressFloat
                            ...
                            onToggleCollapse={() => agentPlanStore.toggleCollapsed(sessionId)}
                          />
                  ...
                  {!activePendingInteraction && (
                    ...
                    >
                      <ChatInputBox
                        ...
                        files={attachedFiles}
                        sessionId={sessionId}
                        workspacePath={sessionState.activeSession?.projectDir ?? null}
                        isAcpSession={sessionState.activeSession?.providerId === 'acp'}
                        isGenerating={isGenerating}
...
type ViewportAnchor = { messageId: string; viewportOffset: number }
