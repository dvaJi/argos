...
import { pageRouterStore, goToNewThread as goToNewThreadAction } from '@/stores/ui/pageRouter'
...
export function WelcomePage() {
  ...
  const pageRouterState = useStore(pageRouterStore)
  ...
  const currentGuideStepId = useMemo<GuidedOnboardingStepId>(() => {
    ...
    return (
      onboardingState?.steps?.find((step) => step.status === 'pending')?.id ?? 'select-provider'
    )
  }, [onboardingState])

  const currentGuideStepTitle = useMemo(
    () => guideStepTitle(currentGuideStepId),
    [currentGuideStepId]
  )
  ...
  const guideStepIds = useMemo(
    () => onboardingState?.steps?.map((step) => step.id) ?? [],
    [onboardingState]
  )
  ...
  const guideStepIcon = (_stepId: GuidedOnboardingStepId, status: GuidedOnboardingStepStatus) => {
    ...
    if (status === 'in_progress') return 'lucide:circle-dot'
          ...
          >
            ...
            <div className="mt-3 grid grid-cols-3 gap-2">
              {requiredGuideSteps.map((step) => (
                <div
                  key={step.id}
                  className={`rounded-xl border px-3 py-2 ${guideStepClass(step.id, step.status)}`}
                >
                  <div className="flex items-center gap-2">
                    <span
                      className={`h-3.5 w-3.5 shrink-0 ${guideStepIconClass(step.id, step.status)}`}
                    >
