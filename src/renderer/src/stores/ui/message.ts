...
export function getAssistantMessageBlocks(
...
export async function loadMessages(
  ...
  const desiredCount =
    ...
    (messageStore.state.currentSessionId === sessionId
      ? Math.max(messageStore.state.messageIds.length, 100)
      : 100)
  ...
  try {
    ...
    messageStore.setState((prev) => ({
      ...
      lastPersistedRevision:
        incomingIds.length > 0 ? prev.lastPersistedRevision + 1 : prev.lastPersistedRevision
    }))
