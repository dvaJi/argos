...
const removeSessions = (sessionIds: string[]): void => {
  ...
  sessionStore.setState((prev) => {
    ...
    const nextBootstrap =
      prev.bootstrapActiveSession && targetIds.has(prev.bootstrapActiveSession.id)
        ? null
        : prev.bootstrapActiveSession
    const nextSummary =
      prev.activeSessionSummary && targetIds.has(prev.activeSessionSummary.id)
        ? null
...
export const getNewConversationTargetAgentId = (): string | null => {
  const selectedId =
    typeof agentStore.state.selectedAgentId === 'string'
      ? agentStore.state.selectedAgentId.trim()
      : ''
  if (selectedId) {
    return selectedId
  }
...
const syncSelectedAgentToSession = (
  ...
  const targetSession =
    ...
    (sessionStore.state.bootstrapActiveSession?.id === sessionId
      ? sessionStore.state.bootstrapActiveSession
      : null)
...
const loadSessionPage = async (options: {
  ...
  if (options.reset) {
    ...
    try {
      ...
      sessionStore.setState((prev) => ({
        ...prev,
        sessions: options.preserveExisting
          ? mergeSessions(prev.sessions, nextSessions)
...
export async function fetchSessions(): Promise<void> {
  await loadSessionPage({
    reset: true,
    prioritizeSessionId:
      sessionStore.state.activeSessionId ?? sessionStore.state.bootstrapActiveSession?.id ?? null
  })
}
...
export async function setSessionSubagentEnabled(
  sessionId: string,
  enabled: boolean
): Promise<void> {
  sessionStore.setState((prev) => ({ ...prev, error: null }))
...
export async function setSessionProjectDir(
  sessionId: string,
...
export async function toggleSessionPinned(sessionId: string, pinned: boolean): Promise<void> {
  ...
  try {
    ...
    sessionStore.setState((prev) => {
      const nextSessions = prev.sessions.map((s) =>
        s.id === sessionId ? { ...s, isPinned: pinned } : s
      )
      const nextBootstrap =
        prev.bootstrapActiveSession?.id === sessionId
...
export function getPinnedSessions(agentId: string | null): UISession[] {
  const pinned = sortSessions(
    sessionStore.state.sessions.filter(
...
export function getFilteredGroups(agentId: string | null): SessionGroup[] {
  ...
  const grouped =
    sessionStore.state.groupMode === 'time'
