...
export function BrowserPanel({ sessionId }: BrowserPanelProps) {
  ...
  const callBrowserAction = useCallback(
    async <T,>(action: string, run: () => Promise<T>): Promise<T | null> => {
      try {
        return await run()
      } catch (error) {
  ...
  const hideEmbedded = useCallback(
    async (sid: string = currentSessionId) => {
      ...
      const hiddenBounds = lastSyncedBounds.current ??
        captureContainerBounds() ?? { x: 0, y: 0, width: 0, height: 0 }
  ...
  const navigate = useCallback(
    async (e: React.FormEvent) => {
      ...
      const result = await callBrowserAction('loadUrl', () =>
        browserClient.loadUrl(currentSessionId, nextUrl)
      )
  ...
  const cleanupInactiveSession = useCallback(
    async (sid: string) => {
      if (!sid) return
      await hideEmbedded(sid)
      if (getSessionUiStatus(sid) === 'working') {
        pendingBrowserDestroySessionIds.current.add(sid)
        return
      }
      pendingBrowserDestroySessionIds.current.delete(sid)
      await callBrowserAction('destroy', () => browserClient.destroy(sid))
    },
    [hideEmbedded, getSessionUiStatus, browserClient, callBrowserAction]
  )
  ...
  useEffect(() => {
    ...
    stopOpenRequestedListener.current =
      browserClient.onOpenRequestedForCurrentWindow(handleOpenRequested)
  ...
  return (
    <div className="flex h-full min-w-0 flex-1 flex-col bg-background">
      ...
      <div ref={containerRef} className="relative min-h-0 flex-1 overflow-hidden">
        {showPlaceholder && <BrowserPlaceholder className="absolute inset-0" />}
      </div>
    </div>
  )
}
