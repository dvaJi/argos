...
export function WorkspacePreviewPane({
  ...
  const resolvedContent = useMemo(
    () => artifact?.content ?? filePreview?.content ?? '',
  ...
  const resolvedTitle = useMemo(
    () => artifact?.title ?? filePreview?.name ?? 'Preview',
    [artifact, filePreview]
  )
  const imageSrc = useMemo(
    () => filePreview?.content || filePreview?.thumbnail || '',
    [filePreview]
  )
  ...
  if (previewKind === 'image') {
    return (
      <div
        className="min-h-0 w-full flex-1 overflow-auto bg-muted/20"
  ...
  if (documentPreviewUrl) {
    return (
      <div
        ...
        data-testid={`workspace-preview-${previewKind}`}
      >
        <iframe
  ...
  if (previewKind === 'react' && artifactBlock) {
    return (
      <div className="min-h-0 w-full flex-1 overflow-hidden" data-testid="workspace-preview-react">
        <ReactArtifact block={artifactBlock} isPreview={true} className="h-full min-h-0 w-full" />
      </div>
    )
  }
  ...
  return (
    <div
      className="min-h-0 w-full flex-1 overflow-auto px-4 py-3"
