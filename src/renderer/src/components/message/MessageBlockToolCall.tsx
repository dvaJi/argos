...
export const MessageBlockToolCall: React.FC<MessageBlockToolCallProps> = ({ block }) => {
  ...
  const parsedParamsRecord = useMemo(
    () => (isRecord(parsedParams.value) ? parsedParams.value : null),
    [parsedParams]
  )

  const rawToolName = useMemo(
    () => block.tool_call?.name?.trim().toLowerCase() ?? '',
    [block.tool_call?.name]
  )
  const isSubagentOrchestrator = useMemo(
    () => rawToolName === 'subagent_orchestrator',
    [rawToolName]
  )
  ...
  const isExecTool = useMemo(() => {
    ...
    return (
      matchesToolContractName(toolName, 'exec') || matchesToolContractName(toolName, 'skill_run')
    )
  }, [rawToolName])

  const isProcessTool = useMemo(
    () => matchesToolContractName(rawToolName, 'process'),
    [rawToolName]
  )
  ...
  const summaryText = useMemo(() => {
    if (isSubagentOrchestrator) {
      const progress =
        parseSubagentProgress(block.extra?.subagentProgress) ??
        parseSubagentProgress(block.extra?.subagentFinal)
  ...
  const subagentTasks = useMemo<SubagentProgressTask[]>(() => {
    ...
    return (progress?.tasks ?? []).map((task, index) => {
      ...
      return {
        ...
        targetAgentId:
          typeof task.targetAgentId === 'string'
            ? task.targetAgentId
            : (task.targetAgentId ?? null),
        targetAgentName:
          normalizeOptionalText(task.targetAgentName) || displayName || 'Unnamed Agent',
  ...
  const diffLanguage = useMemo(
    () => diffData?.language || getLanguageFromFilename(paramsPath),
    [diffData, paramsPath]
  )
  ...
  const [prevStatus, setPrevStatus] = useState<DisplayAssistantMessageBlock['status'] | undefined>(
    undefined
  )
  useEffect(() => {
        ...
        <div className="tool-call-labels flex items-center gap-2 font-mono font-medium min-w-0">
          ...
          {summaryText && (
            <span
              ...
              className="tool-call-summary text-[11px]"
              title={summaryText}
            >
              {summaryText}
            </span>
                    ...
                    <>
                      ...
                      {diffData.replacements !== undefined && (
                        <div className="text-xs text-muted-foreground">
                          {diffData.replacements} replacement(s)
                        </div>
                      )}
                    </>
                  ) : (
