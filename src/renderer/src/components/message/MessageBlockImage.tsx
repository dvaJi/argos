...
export const MessageBlockImage: React.FC<MessageBlockImageProps> = ({ block }) => {
  ...
  const resolvedImageData = useMemo(() => {
    if (block.image_data?.data) {
      ...
      if (
        ...
        return { data: rawData, mimeType: 'deepchat/image-url' }
      }
    ...
    if (typeof content === 'string' && content.length > 0) {
      ...
      if (
        ...
        return { data: content, mimeType: 'deepchat/image-url' }
      }
  ...
  const resolvedImageSrc = useMemo(() => {
    ...
    return resolvedImageData.mimeType === 'deepchat/image-url'
