...
export const MessageItemAssistant = forwardRef<MessageItemAssistantRef, MessageItemAssistantProps>(
  (props, ref) => {
    const {
      message,
      isCapturingImage,
      ...
      >
        <div className="shrink-0 w-5 h-5 flex items-center justify-center">
          {currentMessage.model_provider === 'acp' ? (
            <ModelIcon
              modelId={currentMessage.model_id}
          ...
          (currentMessage?.status ?? message.status) === 'pending' ? (
            ...
            <div className="flex flex-col w-full gap-1.5" data-message-content="true">
              {currentRenderItems.map((item) => {
                ...
                if (block.type === 'action') {
                  return (
                    <MessageBlockAction
                      ...
                      onSwitchProvider={handleBlockSwitchProvider}
                    />
                  )
                }
                ...
                if (block.type === 'error')
                  return <MessageBlockError key={item.key} block={block} />
                return null
              })}
            </div>
          )}
        ...
        <ContextMenuContent className="w-56">
          {showSelectionMenu ? (
            <>
              ...
              {!isReadOnly && (
                <ContextMenuItem onSelect={handleSelectionAskAI}>Ask AI</ContextMenuItem>
              )}
            </>
          ) : (
            <>
              ...
              {!isReadOnly && (
                <ContextMenuItem onSelect={() => handleAction('retry')}>Retry</ContextMenuItem>
              )}
              {!isReadOnly && (
                <ContextMenuItem
              ...
              {!isReadOnly && (
                <ContextMenuItem onSelect={() => handleAction('delete')}>Delete</ContextMenuItem>
              )}
            </>
          )}
        </ContextMenuContent>
