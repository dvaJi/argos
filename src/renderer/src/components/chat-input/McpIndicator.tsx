...
const GROUP_ORDER = ["agent-filesystem", "agent-core", "agent-skills", "deepchat-settings", "yobrowser"];
...
export default function McpIndicator({
  ...
  const isDeepchatContext = useMemo(() => {
    if (getHasActiveSession()) {
      const sessionAgentId = getActiveSession()?.agentId ?? "deepchat";
    ...
    const agentType = selectedAgent?.type ?? (agentStore.selectedAgentId === "deepchat" ? "deepchat" : "acp");
    return agentType === "deepchat";
  }, [sessionStore, agentStore, availableAgents]);

  const deepchatSessionId = isDeepchatContext && getHasActiveSession() ? (getActiveSession()?.id ?? null) : null;
  ...
  const persistDisabledTools = useCallback(
    async (nextList: string[], affectedToolNames: string[]) => {
      ...
      try {
        const persisted = await sessionClient.updateSessionDisabledAgentTools(deepchatSessionId, nextList);
