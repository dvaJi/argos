...
export function useChatInputMentions(options: UseChatInputMentionsOptions) {
  ...
  const ensureWorkspaceRegistered = useCallback(async (): Promise<boolean> => {
    ...
    try {
      await workspaceClient.registerWorkspace(
        workspacePath,
        options.isAcpSession ? 'workdir' : 'workspace'
      )
  ...
  const visibleTools = useMemo(
    () => mcpTools.filter((tool) => isVisibleServerName(tool.server.name)),
    [mcpTools]
  )
  ...
  const pluginTools = useMemo(
    () => mcpTools.filter((tool) => isPluginOwnedServerName(tool.server.name)),
    [mcpTools]
  )
  ...
  useEffect(() => {
    ...
    unsubscribeAcpCommandsReadyRef.current =
      sessionClient.onAcpCommandsReady(handleAcpCommandsReady)
