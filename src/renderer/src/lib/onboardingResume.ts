...
export const GUIDED_ONBOARDING_RESUME_REQUESTED_EVENT = "deepchat:guided-onboarding-resume-requested";
...
const isGuidedOnboardingResumeTrigger = (value: unknown): value is GuidedOnboardingResumeTrigger =>
  typeof value === "string" && GUIDED_ONBOARDING_RESUME_TRIGGERS.includes(value as GuidedOnboardingResumeTrigger);
...
export function readGuidedOnboardingResumeIntent(): GuidedOnboardingResumeIntent | null {
  ...
  try {
    ...
    if (!isGuidedOnboardingStepId(parsed.stepId) || !isGuidedOnboardingResumeTrigger(parsed.trigger)) {
      return null;
    }
