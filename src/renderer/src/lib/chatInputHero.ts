...
export const prepareChatInputHeroFlight = (sourceElement: HTMLElement | null): boolean => {
  ...
  if (!sourceElement || typeof window === "undefined" || typeof document === "undefined" || prefersReducedMotion()) {
    return false;
  }
...
export const playChatInputHeroFlight = async (targetElement: HTMLElement | null): Promise<boolean> => {
  if (!pendingFlight) {
    return false;
  }
