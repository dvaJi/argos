...
export class LifecycleManager implements ILifecycleManager {
  ...
  private async executeHooksByPriority(
    ...
    for (const priority of sortedPriorities) {
          ...
          if (!hookResult.success) {
            ...
            if (hookResult.hook.critical) {
              if (isShutdownPhase) {
                ...
                throw hookResult.error || new Error(`Critical hook '${hookResult.hook.name}' failed`);
              }
            } else {
            ...
            if (isShutdownPhase && phase === LifecyclePhase.BEFORE_QUIT && hookResult.result === false) {
      ...
      if (!isShutdownPhase && this.splashManager) {
        ...
        const hookProgress =
          phaseProgress.start +
          ((phaseProgress.end - phaseProgress.start) * totalCompletedHooks) / Math.max(totalHooks, 1);
  ...
  private async executeShutdownPhase(phase: LifecyclePhase): Promise<boolean> {
    ...
    const shutdownAllowed = await this.executeHooksByPriority(phaseHooks, this.lifecycleContext, phase, true);
   ...
   */
  private async executeHook(hook: LifecycleHook, context: LifecycleContext): Promise<void | boolean> {
    ...
    try {
      ...
      if (is.dev) {
        const hookDelay = normalizeLifecycleHookDelayMs(import.meta.env.VITE_APP_LIFECYCLE_HOOK_DELAY);
