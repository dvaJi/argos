  ...
  execute: async (_context: LifecycleContext) => {
    ...
    protocol.handle("deepcdn", (request) => {
      try {
        ...
        const baseResourcesDir = candidates.find((p) => fs.existsSync(path.join(p, "cdn"))) || candidates[0];
    ...
    protocol.handle("imgcache", (request) => {
      try {
        ...
        // Check if file exists
        if (!fs.existsSync(fullPath)) {
          console.warn(`protocolRegistrationHook: imgcache handler: Image file not found: ${fullPath}`);
    ...
    protocol.handle(WORKSPACE_PREVIEW_PROTOCOL, (request) => {
      try {
        ...
        if (!fs.existsSync(fullPath) || fs.statSync(fullPath).isDirectory()) {
          console.warn(`protocolRegistrationHook: ${WORKSPACE_PREVIEW_PROTOCOL} handler: File not found: ${fullPath}`);
        ...
        console.error(`protocolRegistrationHook: Error handling ${WORKSPACE_PREVIEW_PROTOCOL} request:`, error);
        const errorMessage = error instanceof Error ? error.message : String(error);
        return new Response(`Server error: ${errorMessage}`, {
          status: 500,
          headers: { "Content-Type": "text/plain" },
