...
function toUsageEvent(usage: {
  ...
  return createStreamEvent.usage({
    ...
    ...(usage.inputTokenDetails?.cacheReadTokens ? { cached_tokens: usage.inputTokenDetails.cacheReadTokens } : {}),
...
export async function* adaptAiSdkStream(
  ...
  const emitLegacyTextBuffer = async function* (flushAll = false): AsyncGenerator<LLMCoreStreamEvent> {
    while (true) {
      const startIndex = bufferedLegacyText.indexOf(FUNCTION_CALL_TAG);

      if (startIndex === -1) {
        const safeLength = flushAll ? bufferedLegacyText.length : resolveSafeTextLength(bufferedLegacyText);
        if (safeLength > 0) {
  ...
  for await (const part of fullStream) {
    switch (part.type) {
      ...
      case "reasoning-delta":
        yield createStreamEvent.reasoning(part.text, toProviderOptions((part as any).providerMetadata));
        break;
      ...
      case "tool-input-delta":
        ...
        yield createStreamEvent.toolCallChunk(part.id, part.delta, toProviderOptions((part as any).providerMetadata));
        break;
      ...
      case "finish":
        ...
        yield createStreamEvent.stop(
          !options.supportsNativeTools && legacyToolUseDetected ? "tool_use" : mapFinishReason(part.finishReason),
        );
        break;

      case "error":
        yield createStreamEvent.error(part.error instanceof Error ? part.error.message : String(part.error));
