...
export class AgentBashHandler {
  ...
  async executeCommand(
    ...
    if (this.commandPermissionHandler) {
      const permissionCheck = this.commandPermissionHandler.checkPermission(options.conversationId, command);
    ...
    result = await this.runShellProcess(prepared.command, cwd, timeout ?? COMMAND_DEFAULT_TIMEOUT_MS, {
      ...options,
      env: prepared.env,
      yieldMs,
    });
    ...
    if (prepared.rewritten && !result.timedOut && result.exitCode !== null && result.exitCode !== 0 && fallbackReason) {
      logger.warn("[AgentBashHandler] Falling back to original command after RTK capability error", {
        command,
      ...
      result = await this.runShellProcess(prepared.originalCommand, cwd, timeout ?? COMMAND_DEFAULT_TIMEOUT_MS, {
        ...options,
        env: prepared.env,
        yieldMs,
      });
  ...
  private async runManagedShellProcess(
    ...
    await backgroundExecSessionManager.write(conversationId, session.sessionId, options.stdin ?? "", true);
    ...
    try {
      ...
      if (shouldCleanupSession) {
        await backgroundExecSessionManager.remove(conversationId, session.sessionId).catch((error) => {
  ...
  private async runDetachedShellProcess(
    ...
    return new Promise((resolve, reject) => {
      ...
      const appendOutput = (chunk: string) => {
        ...
        const shouldOffload = outputFilePath !== null && (offloaded || totalOutputLength > COMMAND_OFFLOAD_THRESHOLD);
      ...
      timeoutId = setTimeout(() => {
        ...
        void terminateProcessTree(child, { graceMs: COMMAND_KILL_GRACE_MS }).then((closed) => {
          ...
          outputDecoders.flush();
          const preview =
            offloaded && outputFilePath ? this.readLastCharsFromFile(outputFilePath, COMMAND_PREVIEW_CHARS) : output;
  ...
  private createOutputFilePath(conversationId?: string, outputPrefix: string = "exec"): string | null {
    if (!conversationId) {
      return null;
    }
  ...
  private async executeCommandBackground(
    ...
    if (options.stdin !== undefined) {
      await backgroundExecSessionManager.write(conversationId, result.sessionId, options.stdin, true);
    }
  ...
  private async prepareCommand(command: string, env?: Record<string, string>): Promise<PreparedCommand> {
    const baseEnv = env ?? {};
    ...
    const prepared = await rtkRuntimeService.prepareShellCommand(command, baseEnv, this.configPresenter);
    return {
      originalCommand: prepared.originalCommand,
