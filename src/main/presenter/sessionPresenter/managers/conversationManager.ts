...
export class ConversationManager {
  ...
  private isLegacyTableMissingError(error: unknown): boolean {
    return (
      error instanceof Error && /no such table:\s*(conversations|messages|message_attachments)/i.test(error.message)
    );
  }
  ...
  async createConversation(
    ...
    try {
      ...
      if (!options.forceNewAndActivate && latestConversation) {
        const { list: messages } = await this.messageManager.getMessageThread(latestConversation.id, 1, 1);
      ...
      if (defaultModelsSettings) {
        ...
        if (sanitizedSettings.thinkingBudget === undefined && defaultModelsSettings.thinkingBudget !== undefined) {
  ...
  async updateConversationSettings(conversationId: string, settings: Partial<CONVERSATION_SETTINGS>): Promise<void> {
    ...
    const modelChanged =
      ...
      (settings.providerId !== undefined && settings.providerId !== conversation.settings.providerId);

    if (modelChanged) {
      const modelConfig = this.configPresenter.getModelConfig(mergedSettings.modelId, mergedSettings.providerId);
      if (modelConfig) {
  ...
  async broadcastThreadListUpdate(): Promise<void> {
    ...
    try {
      ...
      if (this.isLegacyTableMissingError(error)) {
        console.info("[ConversationManager] Skip legacy thread list broadcast: legacy tables not found.");
        return;
      }
      throw error;
    }
    ...
    eventBus.sendToRenderer(CONVERSATION_EVENTS.LIST_UPDATED, SendTarget.ALL_WINDOWS, finalGroupedList);
  }
  ...
  async forkConversation(
    ...
    try {
      ...
      const { list: fullHistory } = await this.messageManager.getMessageThread(targetConversationId, 1, 99999);
      ...
      for (const { msg, orderSeq } of messagesToProcess) {
        ...
        const content = typeof finalMsg.content === "string" ? finalMsg.content : JSON.stringify(finalMsg.content);
