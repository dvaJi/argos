import type { DiscordInboundAttachment, DiscordInboundMessage, TelegramCommandPayload } from "../types";
...
const normalizeAttachments = (attachments: DiscordRawAttachment[] | undefined): DiscordInboundAttachment[] =>
  (attachments ?? [])
    .map((attachment) => ({
...
const buildAttachmentFallbackText = (attachments: DiscordInboundAttachment[]): string =>
  attachments.length === 0
    ? ""
    : ["Attachments:", ...attachments.map((attachment) => `${attachment.filename}: ${attachment.url}`)].join("\n");
...
export class DiscordParser {
  parseDispatch(input: DiscordGatewayDispatch, botUserId?: string | null): DiscordInboundMessage | null {
  ...
  private parseMessageCreate(payload: DiscordRawMessage, botUserId?: string | null): DiscordInboundMessage | null {
  ...
  private parseInteractionCreate(payload: DiscordRawInteraction): DiscordInboundMessage | null {
    ...
    const channelId = payload.channel_id === undefined ? "" : String(payload.channel_id).trim();
    const applicationId = payload.application_id === undefined ? "" : String(payload.application_id).trim();
    ...
    const args = payload.data?.options?.find((option) => option.name?.trim() === "args")?.value?.trim() ?? "";
