...
const escapeAttribute = (value: string): string => escapeHtml(value).replace(/"/g, "&quot;").replace(/\n/g, " ");
...
const extractFencedCodeBlocks = (text: string, store: Array<{ lang: string; body: string }>): string => {
  ...
  if (dangling) {
    ...
    const index = store.push({ lang: lang.trim(), body }) - 1;
    result = result.slice(0, dangling.index ?? 0) + `${prefix}${PLACEHOLDER_PREFIX}${index}${PLACEHOLDER_SUFFIX}`;
  }
...
const renderLine = (line: string): { content: string; isBlockquote: boolean } => {
  ...
  escaped = escaped.replace(/(^|[\s([{"'>])_([^\s_][^_\n]*?[^\s_]|[^\s_])_(?=[\s).,;:!?\]}"'<]|$)/g, "$1<i>$2</i>");
...
const restoreCodeBlocks = (text: string, blocks: Array<{ lang: string; body: string }>, inlines: string[]): string => {
