...
): TelegramInlineKeyboardMarkup | undefined => (replyMarkup === null ? { inline_keyboard: [] } : replyMarkup);
...
export class TelegramClient {
  ...
  async sendPhoto(
    ...
    const payload = (await response.json().catch(() => ({}))) as TelegramApiResponse<TelegramSentMessage>;
    if (!response.ok || !payload.ok || payload.result === undefined) {
      const description = payload.description?.trim() || "Telegram sendPhoto failed.";
      throw new TelegramApiRequestError(description, payload.error_code, payload.parameters?.retry_after);
    }
    return payload.result.message_id;
  }

  async sendMessageDraft(target: TelegramTransportTarget, draftId: number, text: string): Promise<void> {
  ...
  async sendChatAction(target: TelegramTransportTarget, action: "typing" = "typing"): Promise<void> {
  ...
  async deleteMessage(params: { target: TelegramTransportTarget; messageId: number }): Promise<void> {
  ...
  async answerCallbackQuery(params: { callbackQueryId: string; text?: string; showAlert?: boolean }): Promise<void> {
  ...
  async setMessageReaction(params: { chatId: number; messageId: number; emoji?: string | null }): Promise<void> {
  ...
  private async request<T>(
    ...
    if (!response.ok || !payload.ok || payload.result === undefined) {
      ...
      const retrySuffix = typeof retryAfter === "number" && retryAfter > 0 ? ` (retry_after=${retryAfter})` : "";
      throw new TelegramApiRequestError(`${description}${retrySuffix}`, payload.error_code, retryAfter);
    }
