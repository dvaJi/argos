import type { ChannelAdapterConfig, ChannelFactory, ChannelStatusSnapshot, IChannelAdapter } from "./types/channel";
...
export class ChannelManager {
  ...
  async createAdapter(config: ChannelAdapterConfig): Promise<IChannelAdapter> {
    ...
    if (!factory) {
      throw new Error(`No channel factory is registered for ${config.source ?? "builtin"}:${config.channelType}.`);
    }
  ...
  listAdapters(channelType?: string): Array<{ channelType: string; channelId: string; adapter: IChannelAdapter }> {
  ...
  async disconnectAll(): Promise<void> {
    await Promise.all([...this.adapters.values()].map(async (adapter) => await adapter.disconnect()));
  }
  ...
  async dispatchTextUpdate(channelType: string, channelId: string, chatId: string, fullText: string): Promise<void> {
  ...
  async dispatchStreamError(channelType: string, channelId: string, chatId: string, error: string): Promise<void> {
    const adapter = this.getAdapter(channelType, channelId);
    if (!adapter) {
      return;
    }
