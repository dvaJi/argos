...
export class QQBotClient {
  ...
  async getAccessToken(forceRefresh: boolean = false): Promise<string> {
    if (!forceRefresh && this.accessToken && Date.now() < this.accessTokenExpiresAt - QQBOT_TOKEN_REFRESH_WINDOW_MS) {
    ...
    this.accessTokenExpiresAt = Date.now() + (Number.isFinite(expiresIn) && expiresIn > 0 ? expiresIn * 1000 : 0);
  ...
  async sendGroupMessage(target: {
    ...
    const response = await this.request(`/v2/groups/${encodeURIComponent(target.groupOpenId)}/messages`, {
      method: "POST",
      body: JSON.stringify({
        content: target.content,
        msg_type: 0,
  ...
  private async uploadGroupFile(groupOpenId: string, filePath: string): Promise<QQBotFileUploadResponse> {
    const fileData = await import("node:fs/promises").then(async (fs) =>
      (await fs.readFile(filePath)).toString("base64"),
    );
