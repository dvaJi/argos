...
import { buildFeishuPendingInteractionCard, buildFeishuPendingInteractionText } from "./feishuInteractionPrompt";
...
export class FeishuRuntime {
  ...
  private async resolveMessageAttachments(message: FeishuInboundMessage): Promise<FeishuInboundMessage> {
    ...
    return {
      ...message,
      attachments,
      allAttachmentsFailed: attachments.length > 0 && attachments.every((attachment) => attachment.failedDownload),
    };
  }
    ...
    return {
      ...
      segments: state.segments.map((segment) => ({
        ...
        messageIds: segment.messageIds.filter(
          (messageId): messageId is string | null => typeof messageId === "string" || messageId === null,
        ),
  ...
  private rememberDeliveryState(endpointKey: string, state: FeishuRemoteDeliveryState): FeishuRemoteDeliveryState {
  ...
  private async sendGeneratedImages(
    ...
    for (const asset of snapshot.generatedImages ?? []) {
      try {
        ...
        await this.deps.client.sendText(target, "[Image] Delivery failed - see local copy in the app.");
      }
    }
  }
  ...
  private isDeliveryStateCompatible(state: FeishuRemoteDeliveryState, segments: RemoteDeliverySegment[]): boolean {
  ...
  private async syncDeliverySegment(
    ...
    const previousChunks = existing.lastText ? chunkFeishuText(optimizeMarkdownForFeishu(existing.lastText)) : [];
    if (
  ...
  private async dispatchOutboundActions(
    ...
    for (const action of actions) {
      ...
      try {
        ...
        console.warn("[FeishuRuntime] Failed to send interactive card, falling back to text:", error);
