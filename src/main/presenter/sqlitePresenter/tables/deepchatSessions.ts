...
export class DeepChatSessionsTable extends BaseTable {
  ...
  private getRecoveryMigrationStatements(): string[] {
    ...
    if (!this.hasColumn("summary_cursor_order_seq")) {
      statements.push("ALTER TABLE deepchat_sessions ADD COLUMN summary_cursor_order_seq INTEGER NOT NULL DEFAULT 1;");
    }
    ...
    if (!this.hasColumn("force_interleaved_thinking_compat")) {
      statements.push("ALTER TABLE deepchat_sessions ADD COLUMN force_interleaved_thinking_compat INTEGER;");
    }
    ...
    if (!this.hasColumn("image_generation_options_json")) {
      statements.push("ALTER TABLE deepchat_sessions ADD COLUMN image_generation_options_json TEXT;");
    }
    if (!this.hasColumn("video_generation_options_json")) {
      statements.push("ALTER TABLE deepchat_sessions ADD COLUMN video_generation_options_json TEXT;");
    }
  ...
  get(id: string): DeepChatSessionRow | undefined {
    return this.db.prepare("SELECT * FROM deepchat_sessions WHERE id = ?").get(id) as DeepChatSessionRow | undefined;
  }
  ...
  updateGenerationSettings(id: string, settings: Partial<DeepChatSessionGenerationSettings>): void {
    ...
    params.push(id);
    this.db.prepare(`UPDATE deepchat_sessions SET ${updates.join(", ")} WHERE id = ?`).run(...params);
  }

  getSummaryState(id: string): DeepChatSessionSummaryRow | null {
    const row = this.db
      .prepare("SELECT summary_text, summary_cursor_order_seq, summary_updated_at FROM deepchat_sessions WHERE id = ?")
