...
export class DataImporter {
    ...
    const importTransaction = this.targetDb.transaction(() => {
      for (const table of tables) {
        try {
          ...
          throw new Error(`Failed to import table ${table}: ${error instanceof Error ? error.message : String(error)}`);
        }
      }
    });
  ...
  private importTable(tableName: string): number {
    ...
    const selectColumnsSql = commonColumns.map((column) => this.wrapIdentifier(column.name)).join(", ");
    const rows = this.sourceDb.prepare(`SELECT ${selectColumnsSql} FROM ${wrappedTableName}`).all() as Record<
      string,
      unknown
    >[];
