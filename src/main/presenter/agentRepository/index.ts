...
import { createDefaultDeepChatSubagentSlots, normalizeDeepChatSubagentConfig } from "@shared/lib/deepchatSubagents";
...
const mergeDeepChatConfig = (
  ...
  normalizeDeepChatSubagentConfig({
    ...
    subagents: overrideConfig.subagents ?? baseConfig.subagents ?? createDefaultDeepChatSubagentSlots(),
...
export class AgentRepository {
  ...
  private toAgent(row: AgentRow): Agent {
    return {
      ...
      config: row.agent_type === "deepchat" ? (parseJson<DeepChatAgentConfig>(row.config_json) ?? null) : null,
      installState: this.getAgentInstallState(row.id),
    };
  }
}
