...
import type { DeepChatSessionStore, ReconstructionAnchorPromptState, SessionSummaryState } from "./sessionStore";
import {
  buildHistoryTurns,
...
export function appendSummarySection(systemPrompt: string, summaryText: string | null | undefined): string {
...
function visibleReconstructionState(anchorName: string, state: Record<string, unknown>): Record<string, unknown> {
...
export class CompactionService {
  constructor(
    ...
    private readonly resolveSessionConfig: (sessionId: string) => Promise<DeepChatAgentConfig> = async () => ({}),
  ) {}
  ...
  async prepareForNextUserTurn(params: {
    ...
    const historyRecords = (params.historyRecords ?? this.messageStore.getMessages(params.sessionId))
    ...
    return this.prepareCompaction({
      ...
      projectedMessages: [
        createUserChatMessage(params.newUserContent, params.supportsVision, params.supportsAudioInput === true),
      ],
  ...
  async applyCompaction(intent: CompactionIntent, signal?: AbortSignal): Promise<CompactionExecutionResult> {
    try {
  ...
  private prepareCompaction(params: {
    ...
    const scopedRecords = params.records.filter((record) => record.orderSeq >= summaryState.summaryCursorOrderSeq);
    ...
    const systemPromptWithSummary = appendSummarySection(params.systemPrompt, summaryState.summaryText);
    const projectedHistory = turns.flatMap((turn) => turn.messages);
    const projectedPrompt = [
      ...(systemPromptWithSummary ? [{ role: "system" as const, content: systemPromptWithSummary }] : []),
      ...projectedHistory,
      ...params.projectedMessages,
    ];
    if (!params.force) {
      const requestBudget = Math.floor(
        (params.contextLength - params.reserveTokens - (params.extraReserveTokens ?? 0)) / SAFETY_MARGIN,
      );
    ...
    return {
      ...
      summaryBlocks,
      currentModel: this.getCurrentModelSpec(params.providerId, params.modelId, params.contextLength),
  ...
  private getCurrentModelSpec(providerId: string, modelId: string, fallbackContextLength: number): ModelSpec {
  ...
  private async getAssistantModelSpec(sessionId: string, currentModel: ModelSpec): Promise<ModelSpec | null> {
    ...
    try {
      ...
      return {
        providerId,
        modelId,
        contextLength: resolveModelContextLength(assistantConfig?.contextLength, currentModel.contextLength),
      };
    } catch (error) {
  ...
  private getSummarizationInputBudget(contextLength: number, reserveTokens: number): number {
    ...
    return Math.max(
      1024,
      Math.floor(contextLength / SAFETY_MARGIN) - summaryOutputTokens - SUMMARIZATION_OVERHEAD_TOKENS,
    );
  }
  ...
  private getRemainingSpanTokenBudget(
    ...
    return this.getSummarizationInputBudget(contextLength, reserveTokens) - approximateTokenSize(previousSummary || "");
  }
  ...
  private getMaxChunkTokens(totalTokens: number, contextWindowTokens: number): number {
    ...
    return Math.max(2048, Math.floor(contextWindowTokens * adaptiveRatio) - SUMMARIZATION_OVERHEAD_TOKENS);
  }
  ...
  private async generateRollingSummary(params: {
    ...
    const blockTokens = params.summaryBlocks.reduce((total, block) => total + approximateTokenSize(block), 0);
    const fullPayloadTokens = previousSummaryTokens + blockTokens;
    const preferredModel =
      assistantModel &&
      fullPayloadTokens <= this.getSummarizationInputBudget(assistantModel.contextLength, params.reserveTokens)
        ? assistantModel
        : currentModel;
  ...
  private async summarizeBlocks(
    ...
    const inputBudget = this.getSummarizationInputBudget(options.model.contextLength, options.reserveTokens);
    ...
    if (chunkedBlocks.length === 1 && chunkedBlocks[0].length === normalizedBlocks.length) {
      const splitBlocks = normalizedBlocks.flatMap((block) => this.splitLargeBlock(block, chunkTokens));
      if (splitBlocks.length === normalizedBlocks.length) {
        ...
        const strictChunkTokens = Math.max(256, Math.min(chunkTokens, Math.max(1, remainingSpanBudget)));
  ...
  private async summarizeChunkGroups(
    ...
    for (const chunk of chunkGroups) {
      throwIfAbortRequested(options.signal);
      chunkSummaries.push(
        await this.generateSummaryText(options.model, options.reserveTokens, null, chunk.join("\n\n"), options.signal),
      );
    }
