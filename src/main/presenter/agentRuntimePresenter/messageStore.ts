...
export class DeepChatMessageStore {
  ...
  setMessageError(messageId: string, blocks: AssistantMessageBlock[], metadata?: string): void {
    ...
    if (metadata === undefined) {
      this.sqlitePresenter.deepchatMessagesTable.updateContentAndStatus(messageId, serializedBlocks, "error");
      this.upsertAssistantSearchDocument(messageId, blocks);
      this.appendLiveTapeFacts(messageId);
      return;
    }
    this.sqlitePresenter.deepchatMessagesTable.updateContentAndStatus(messageId, serializedBlocks, "error", metadata);
    this.upsertAssistantSearchDocument(messageId, blocks);
    this.persistUsageStats(messageId, metadata, "live");
    this.appendLiveTapeFacts(messageId);
  }
  ...
  getMessagesUpToOrderSeq(sessionId: string, maxOrderSeq: number): ChatMessageRecord[] {
    const rows = this.sqlitePresenter.deepchatMessagesTable.getBySessionUpToOrderSeq(sessionId, maxOrderSeq);
    return this.toRecords(rows);
  }
  ...
  getLastUserMessageBeforeOrAt(sessionId: string, orderSeq: number): ChatMessageRecord | null {
    const row = this.sqlitePresenter.deepchatMessagesTable.getLastUserMessageBeforeOrAtOrderSeq(sessionId, orderSeq);
    if (!row) return null;
    return this.toRecord(row);
  }
  ...
  updateMessageContent(messageId: string, content: string): void {
    ...
    const updated = this.getMessage(messageId);
    if (updated) {
      appendMessageReplacementToTape(this.sqlitePresenter.deepchatTapeEntriesTable, updated, "message_content_updated");
    }
  }
  ...
  deleteMessage(messageId: string): void {
    this.runInDatabaseTransaction(() => {
      ...
      if (record) {
        appendMessageRetractionToTape(this.sqlitePresenter.deepchatTapeEntriesTable, record, "message_deleted");
      }
  ...
  private materializeContent(row: DeepChatMessageRow, maps?: StructuredMessageMaps): string {
    if (row.role === "user") {
      const userRow = maps?.userRows.get(row.id) ?? this.sqlitePresenter.deepchatUserMessagesTable.get(row.id);
      if (!userRow) {
        return row.content;
      }

      const fileRows =
        maps?.fileRows.get(row.id) ?? this.sqlitePresenter.deepchatUserMessageFilesTable.listByMessageIds([row.id]);
      const linkRows =
        maps?.linkRows.get(row.id) ?? this.sqlitePresenter.deepchatUserMessageLinksTable.listByMessageIds([row.id]);
    ...
    const assistantRows =
      maps?.assistantRows.get(row.id) ?? this.sqlitePresenter.deepchatAssistantBlocksTable.listByMessageId(row.id);
  ...
  private loadStructuredMaps(messageIds: string[]): StructuredMessageMaps {
    ...
    const assistantRows = this.sqlitePresenter.deepchatAssistantBlocksTable.listByMessageIds(messageIds);
