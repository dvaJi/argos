...
const DEFAULT_INMEMORY_SERVERS: Record<string, Omit<MCPServerConfig, "enabled">> = {
  ...
  "deepchat-inmemory/deep-research-server": {
    args: [],
    descriptions: "DeepChat内置深度研究服务，使用博查搜索(注意该服务需要较长的上下文模型，请勿在短上下文的模型中使用)",
    icons: "🔬",
    autoApprove: ["all"],
    type: "inmemory" as MCPServerType,
...
export class McpConfHelper {
  ...
  public onUpgrade(oldVersion: string | undefined): void {
    ...
    try {
      ...
      // 检查是否需要添加平台特有服务
      if (isMacOS() && !mcpServers["deepchat/apple-server"] && !removedBuiltInServers.has("deepchat/apple-server")) {
