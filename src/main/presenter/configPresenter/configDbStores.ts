...
export class AppSettingsDbBackedStore implements StoreLike<Record<string, unknown>> {
  ...
  get store(): Record<string, unknown> {
    ...
    return {
      ...
      ...(Object.keys(providerTimestamps).length > 0 || !useLegacyFallback ? { providerTimestamps } : {}),
      ...modelStatusEntries,
    };
  }
  ...
  set(keyOrValues: string | Record<string, unknown>, value?: unknown): void {
    ...
    if (key === "providerOrder" && Array.isArray(value)) {
      this.configTables.setProviderOrder(value.filter((item): item is string => typeof item === "string"));
      return;
    }
  ...
  has(key: string): boolean {
    if (key === "providers") {
      return this.configTables.listProviders().length > 0 || (this.shouldUseLegacyFallback() && this.hasLegacyKey(key));
    }
    if (key === "providerOrder") {
      return (
        this.configTables.getProviderOrder().length > 0 || (this.shouldUseLegacyFallback() && this.hasLegacyKey(key))
      );
    }
    if (key === "providerTimestamps") {
      return (
    ...
    if (this.isModelStatusKey(key)) {
      return this.configTables.hasModelStatus(key) || (this.shouldUseLegacyFallback() && this.hasLegacyKey(key));
    }
    if (this.isSensitiveAppSettingKey(key)) {
      return (
        this.configTables.hasAppSetting(key) || (this.shouldUseSensitiveLegacyFallback() && this.hasLegacyKey(key))
      );
    }
...
export class McpDbStore implements StoreLike<Record<string, unknown>> {
  ...
  has(key: string): boolean {
    ...
    return (
      this.configTables.getMcpSetting(key) !== undefined || (this.shouldUseLegacyFallback() && this.hasLegacyKey(key))
    );
  }
...
export class AcpDbStore implements StoreLike<Record<string, unknown>> {
  ...
  get store(): Record<string, unknown> {
    ...
    const sharedMcpSelections = this.configTables.getAgentMcpSelections(SHARED_AGENT_MCP_SELECTION_ID);
    return {
      ...this.getLegacyStoreSnapshot(),
  ...
  set(keyOrValues: string | Record<string, unknown>, value?: unknown): void {
    ...
    if (keyOrValues === "sharedMcpSelections" && Array.isArray(value)) {
      this.configTables.setAgentMcpSelections(value.filter((item): item is string => typeof item === "string"));
      return;
    }
