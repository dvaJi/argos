...
const getProviderCapabilityModelLookupKeys = (value: string | undefined): string[] => {
  ...
  return Array.from(
    new Set(
      [exactModelId, slashUnprefixedModelId, dottedUnprefixedModelId, canonicalModelId].filter((key) => key.length > 0),
    ),
  );
};

const matchesModelFamily = (modelId: string, families: string[]): boolean =>
  families.some((family) => modelId === family || modelId.startsWith(`${family}-`) || modelId.startsWith(`${family}.`));

const hasReasoningPortrait = (portrait: ReasoningPortrait | undefined | null): portrait is ReasoningPortrait =>
  Boolean(
    portrait &&
    Object.values(portrait).some((value) => (Array.isArray(value) ? value.length > 0 : value !== undefined)),
  );

const normalizeEffortOptions = (options: ReasoningEffort[] | undefined): ReasoningEffort[] | undefined => {
...
const usesExtendedEffortDefaultWithoutOptions = (portrait: ReasoningPortrait | undefined): boolean => {
...
const supportsVerbosityControls = (portrait: ReasoningPortrait | undefined | null): boolean => {
  ...
  return Boolean(
    (portrait.verbosityOptions && portrait.verbosityOptions.length > 0) || typeof portrait.verbosity === "string",
  );
};
...
const portraitFromLegacyReasoning = (reasoning: ProviderModel["reasoning"]): ReasoningPortrait | undefined => {
  if (!reasoning) {
    return undefined;
  }
...
export class ModelCapabilities {
  ...
  private getCanonicalProviderModelMatch(providerId: string, modelId: string): CapabilityModelMatch | undefined {
  ...
  private getProviderModelMatch(providerId: string, modelId: string): CapabilityModelMatch | undefined {
  ...
  private getRegistryPortrait(providerId: string, modelId: string): ReasoningPortrait | undefined {
    ...
    const selected = [...entries].sort((left, right) => {
      const leftSameProvider = normalizeCapabilityProviderId(left.providerId) === normalizedProviderId ? 0 : 1;
      const rightSameProvider = normalizeCapabilityProviderId(right.providerId) === normalizedProviderId ? 0 : 1;
  ...
  private getFallbackReasoningPortrait(providerId: string, modelId: string): ReasoningPortrait | undefined {
    const normalizedProviderId = normalizeCapabilityProviderId(providerId);
    ...
    const allowsOpenAIFallback =
      OPENAI_REASONING_FALLBACK_PROVIDERS.has(normalizedProviderId) || normalizedRawModelId.startsWith("openai/");

    if (allowsOpenAIFallback && matchesModelFamily(normalizedModelId, OPENAI_REASONING_EFFORT_MODEL_FAMILIES)) {
      return {
        supported: true,
        defaultEnabled: true,
        mode: "effort",
  ...
  getCapabilityModelMatch(providerId: string, modelId: string): CapabilityModelMatch | undefined {
    return this.getProviderModelMatch(providerId, modelId) ?? this.getModelMatch(providerId, modelId);
  }

  findCapabilityModelMatch(modelId: string, preferredProviderIds: string[] = []): CapabilityModelMatch | undefined {
    const resolvedPreferredProviderIds = Array.from(
      new Set(
        preferredProviderIds
          .map((providerId) => this.resolveProviderId(providerId)?.toLowerCase())
          .filter((providerId): providerId is string => Boolean(providerId && this.index.has(providerId))),
      ),
    );
  ...
  getReasoningPortrait(providerId: string, modelId: string): ReasoningPortrait | null {
    ...
    const extraCapabilitiesPortrait = portraitFromExtraCapabilities(exactModel?.extra_capabilities?.reasoning);
    ...
    const explicitPortrait = mergeReasoningPortraits(legacyPortrait, registryPortrait, extraCapabilitiesPortrait);
