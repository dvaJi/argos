...
export class AgentSessionPresenter {
  ...
  async createSession(input: CreateSessionInput, webContentsId: number): Promise<SessionWithState> {
    ...
    const deepChatAgentConfig = agentType === "deepchat" ? await this.resolveDeepChatAgentConfigCompat(agentId) : null;
    const projectDir =
    ...
    const disabledAgentTools =
      agentType === "deepchat"
        ? this.normalizeDisabledAgentTools(input.disabledAgentTools ?? deepChatAgentConfig?.disabledAgentTools)
        : [];
    ...
    const providerId =
      input.providerId ?? deepChatAgentConfig?.defaultModelPreset?.providerId ?? defaultModel?.providerId ?? "";
    const modelId = input.modelId ?? deepChatAgentConfig?.defaultModelPreset?.modelId ?? defaultModel?.modelId ?? "";
  ...
  async searchHistory(query: string, options?: HistorySearchOptions): Promise<HistorySearchHit[]> {
    ...
    const candidateSearchRows =
      searchDocumentRows.length > 0
        ? searchDocumentRows
        : this.sqlitePresenter.deepchatSearchDocumentsTable.searchLike(normalizedQuery, searchDocumentLimit);
  ...
  async getSearchResults(messageId: string, searchId?: string): Promise<SearchResult[]> {
    ...
    const rows = this.sqlitePresenter.deepchatMessageSearchResultsTable.listByMessageId(normalizedMessageId);
  ...
  async listMessageTraces(messageId: string): Promise<MessageTraceRecord[]> {
    ...
    return this.sqlitePresenter.deepchatMessageTracesTable.listByMessageId(messageId).map((row) => ({
      id: row.id,
      messageId: row.message_id,
      sessionId: row.session_id,
