...
export class YoBrowserToolHandler {
  ...
  async callTool(toolName: string, args: Record<string, unknown>, conversationId?: string): Promise<string> {
    try {
      ...
      switch (toolName) {
        ...
        case "load_url": {
          ...
          return JSON.stringify(await this.presenter.loadUrl(sessionId, url, undefined, undefined, "agent"));
        }
        case "cdp_send": {
          ...
          if (!status.initialized || !status.visible || !page || page.status === BrowserPageStatus.Closed) {
  ...
  private async createUnavailableError(
    ...
    return this.presenter
      ...
      .then(
        (status) =>
          new YoBrowserUnavailableError(buildYoBrowserUnavailablePayload(sessionId, method, status), originalError),
      );
  }
