...
export class DownloadManager {
  ...
  async downloadFile(url: string, savePath?: string, webContents?: WebContents): Promise<DownloadInfo> {
  ...
  private ensureSessionListener(session: Session): void {
    ...
    const handler = (_event: Electron.Event, item: DownloadItem) => this.handleWillDownload(session, item);
  ...
  private handleWillDownload(session: Session, item: DownloadItem): void {
    ...
    const fallbackIndex =
      exactMatchIndex !== -1 ? exactMatchIndex : queue.findIndex((pending) => now - pending.requestedAt < 15_000);
