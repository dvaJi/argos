...
export class ScrollCaptureManager {
   ...
   */
  async captureScrollableArea(rect: ScrollCaptureRect, options: ScrollCaptureOptions = {}): Promise<Buffer[]> {
    ...
    try {
      ...
      const maxSegmentHeight = Math.floor(pageInfo.viewportHeight * (options.maxSegmentHeight || 0.8));
