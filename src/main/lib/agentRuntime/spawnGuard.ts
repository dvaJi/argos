...
export function resolveUsableSpawnCwd(cwd: string): string {
  ...
  if (!fs.existsSync(normalizedCwd)) {
    throw new Error(`Working directory does not exist or is not accessible: ${normalizedCwd}. ${cwdErrorHint}`);
  }
...
export function describeSpawnFailure(
  ...
  const code =
    typeof (error as NodeJS.ErrnoException).code === "string" ? ` ${(error as NodeJS.ErrnoException).code}` : "";
