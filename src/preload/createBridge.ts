...
export function createBridge(ipcRenderer: IpcRendererLike): DeepchatBridge {
  return {
    async invoke<T extends DeepchatRouteName>(
      ...
      const output = await ipcRenderer.invoke(DEEPCHAT_ROUTE_INVOKE_CHANNEL, routeName, normalizedInput);
      return contract.output.parse(output);
    },

    on<T extends DeepchatEventName>(eventName: T, listener: (payload: DeepchatEventEnvelope<T>["payload"]) => void) {
