...
- [ ] Resolve remaining Open Questions in `spec.md` before any follow-up implementation SDD leaves spec.
- [ ] Add security review checklist (below) and have it reviewed.
...
- [ ] Introduce the `AuthContext` type and a single auth gate before HTTP route dispatch, WS upgrade, and protected static serving.
- [ ] Issue `desktop-bootstrap` in Electron main (CSPRNG); transmit to the daemon out-of-band (never argv, disk, or renderer); desktop-managed client authenticates via it.
- [ ] Delete `--token` / `--with-token` / `ARGOS_TOKEN` handling from the daemon (`apps/daemon/src/lifecycle.ts:27-51`).
- [ ] Replace the bearer checker and `WS ?token=` auth with the new gate; remove the raw shared-secret path (`apps/daemon/src/transport/auth.ts`, `apps/daemon/src/index.ts:89-110`).
- [ ] Delete `WorkspaceEntry.authToken`, the remote-workspace token UI, and the WS adapter token query (`packages/shared/src/workspaceConfig.ts:8`, `apps/desktop/src/renderer/src/components/workspace/RemoteWorkspaceSetup.tsx`, `apps/desktop/src/preload/hybridBridge.ts:206`).
- [ ] Generalize the existing rate-limiter (`apps/daemon/src/transport/auth.ts:1-32`) to cover bootstrap/session/pairing verification, not just bearer.
...
- [ ] Implement `one-time-token` pairing and `browser-session` (HTTP-only same-site cookie) issuance.
- [ ] Implement `bearer-session` issuance for non-browser clients (CLI, mobile).
...
- [ ] Add exposure-mode selection UI; require explicit confirmation for `network-accessible`.
- [ ] Enforce fail-closed bind-host validation (non-loopback host without opt-in must not silently downgrade).
...
- [ ] Reference this model in follow-up SDDs before their implementation begins: `daemon-transport-runtime`, `headless-backend-kernel`, local-api-facade, pairing/session-auth, `argos-connect-relay`.
- [ ] Security review checklist (complete before Phase 1 implementation):
