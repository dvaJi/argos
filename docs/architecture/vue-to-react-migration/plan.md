...
This document is the authoritative reference for the DeepChat renderer migration from Vue 3 to React.
...
- [ ] No changes to `src/preload/**` IPC contracts — `window.electron`, `window.api`, `window.deepchat` APIs remain identical
...
---
 ...
 - [x] `dev:trace` script: remove the `DEEPCHAT_VUE_DEVTOOLS_OVERLAY` env var reference
