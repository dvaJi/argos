...
Extract the DeepChat backend runtime from the Electron main process into a standalone
...
- Renderer connects via `DeepchatBridge` over HTTP+WS (no IPC)
...
**As a** developer,
**I want** to swap the `DeepchatBridge` transport from IPC to WebSocket,
**So that** the renderer works with both embedded and remote daemons.

**Acceptance Criteria:**

- `WebSocketBridge` implements `DeepchatBridge` interface
