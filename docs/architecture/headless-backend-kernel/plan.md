...
│  └─────┬──────┘  └──────────────────────────────┘    │
│        │                                            │
│        │  DeepchatBridge (HTTP + WS transport)       │
│        │                                            │
└────────┼────────────────────────────────────────────┘
         │
...
│   │   └── package.json
│   │
│   ├── client-sdk/        # DeepchatBridge transports
│   │   ├── src/
...
Current (Electron IPC):
  Presenter → EventBus.sendToRenderer() → ipcMain → webContents.send() → preload → DeepchatBridge.on()
...
- `dispatchDeepchatRoute` receives a runtime interface, not the Presenter class

### `dispatchDeepchatRoute` monolith (2600 lines)
...
- Implement `WebSocketBridge` for `DeepchatBridge`
