...
When Argos's Telegram remote control bot delivers AI replies, command output, and other generated text, users should see properly rendered formatting (bold, italic, inline code, fenced code blocks, links, lists, blockquotes) instead of raw Markdown symbols (`**bold**`, `# heading`, ` ``` `).
...
- If Telegram rejects converted HTML with an entity-parse error, Argos retries the same outbound chunk as plain text.
