...
- [ ] Add external session catalog sync that updates link metadata without creating duplicate Argos conversations.
- [ ] Add import path that creates or reuses a Argos conversation for a remote session.
- [ ] Add `session/load` import path gated by top-level `loadSession`.
- [ ] Stage replayed remote updates before converting them to Argos messages.
...
- [ ] Send Argos system prompt only once when a local conversation first binds to ACP runtime.
...
- [ ] Ensure session title update does not override user-edited Argos titles.
...
- [ ] Route only `session/request_permission` into Argos permission overlay.
