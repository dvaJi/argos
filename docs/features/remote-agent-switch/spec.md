...
5. Switching to an ACP agent (`Agent.type === 'acp'`) when the channel has no default workdir fails with `Cannot switch to ACP agent: this channel has no default workdir set. Configure the channel default workdir in Argos first.`
...
- `/agent` does NOT modify an existing session's `agentId` (that's not a supported operation in Argos); switching always rolls a new session.
