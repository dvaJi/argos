...
## Summary
Wire the existing `AgentTransferDialog` component into `DeepChatAgentsSettings.tsx` so that deleting a DeepChat agent shows impact assessment, offers session move-or-delete choice, and handles blocked sessions — matching the original Vue implementation.
...
## Scope
- `src/renderer/settings/components/DeepChatAgentsSettings.tsx` — replace bare `handleDelete` with transfer-dialog flow
