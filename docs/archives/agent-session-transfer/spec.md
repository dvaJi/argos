...
[Issue #1705](https://github.com/dvaJi/argos/issues/1705) reports that deleting an
...
- Custom Argos agent deletion asks for a simple browser confirm, then reassigns
  `new_sessions.agent_id` to the built-in `argos`.
...
move/import related conversations to another eligible Argos agent, or delete those conversations
together with the agent.

## User Need

Users need to understand and control what happens to chats owned by an agent before deleting that
agent. They also need a normal chat-level way to move an idle conversation to another Argos agent
when the conversation history is useful but the next turns should use a different agent.

## Goals

- Show a structured delete-agent dialog whenever a deletable Argos, manual ACP, or installed
  registry ACP agent has related sessions.
- Offer two clear outcomes in that dialog:
  - Move related sessions to another enabled Argos agent, then delete the source agent.
  - Delete related sessions, then delete the source agent.
- Allow regular idle sessions to move from their current agent to a Argos agent outside the delete
  flow.
...
- Support Argos-to-Argos and ACP-to-Argos moves for idle sessions by preserving
  conversation history and reinitializing the target Argos agent runtime for future turns.
- Prevent moves into ACP agents. Argos history must not be moved to ACP, and ACP-to-ACP moves are
...
- No moving any conversation history into ACP agents in the first increment. ACP sessions may only
  move out to a Argos agent.

## Terminology

- **Move/import**: keep the same Argos session id and stored messages, change the owning
  `agent_id`, and make future turns use the target Argos agent.
...
1. As a user deleting a custom Argos agent, I can see how many chats will be affected before I
   confirm deletion.
2. As a user deleting a manual ACP agent, I can move that agent's finished chats to a Argos agent
...
4. As a user who does not want to keep related chats, I can explicitly delete those chats together
   with the agent.
5. As a user viewing an idle regular conversation, I can move it to a different Argos agent from
...
1. Deleting a custom Argos, manual ACP, or installed registry ACP agent with related non-empty
...
3. The primary safe action is "Move chats to..." and requires selecting an enabled Argos target
...
6. Moving to a Argos agent applies the target agent's runtime defaults for future turns while
...
14. Tests cover impact summary, Argos deletion with move, manual/registry ACP deletion with move
    to a Argos target, explicit delete of related sessions, chat-level move, active-session
...
| Argos should handle them before the agent is deleted.       |
|                                                                |
| Impact                                                         |
|   Regular chats        12                                      |
|   Subagent chats        3                                      |
...
|                                                                |
| (o) Move chats to another Argos Agent                       |
|     Target Agent                                               |
|     [ Argos                                        v ]       |
|                                                                |
...
| Future replies will use the target Argos Agent.          |
| ACP agents are not listed as targets. ACP chats can move    |
| out to Argos, but chats cannot move into ACP.            |
+------------------------------------------------------------+
|                                      [ Cancel ] [ Move ]    |
+------------------------------------------------------------+
```
...
- Clearing stale ACP provider bindings must happen only after the target Argos context and
