...
Follow Argos's existing architectural patterns:
...
For new renderer-main capabilities, the default path has shifted from `useLegacyPresenter()` to
typed route / typed event + `renderer/api/*Client`. `useLegacyPresenter()` is retained only for
compatibility paths and should no longer be copied as a pattern for new code.
...
- New renderer-main capabilities should define `shared/contracts/*` and `renderer/api/*Client` first
- `useLegacyPresenter()` is no longer the recommended pattern
- If a legacy transport must be temporarily retained, funnel it into `src/renderer/api/legacy/**` rather than letting it enter business modules directly
- Creating a second quarantine directory to absorb renderer-main legacy transport is not allowed
