...
async function prepareSigningKeychain({ cwd, env }) {
  ...
  const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'argos-cua-codesign-'))
  const keychainFile = path.join(tempRoot, 'argos-cua.keychain')
...
export async function signMacHelperForRelease({
  ...
  try {
    const identity = await findDeveloperIdIdentity({
      ...
      qualifier: env.ARGOS_MAC_CODESIGN_IDENTITY ?? env.CSC_NAME
    })
    await signHelperApp({
      appPath,
      entitlementsPath,
